...
*     .. External Subroutines ..
     ...
     $                   SGELSD, SGELSS, SGELSX, SGELSY, SGELSZ, SGEMM,
     $                   SLACPY, SLARNV, SLASRT, SQRT13, SQRT15, SQRT16,
     $                   SSCAL, XLAENV
*     ..
*     .. Intrinsic Functions ..
...
*                       Set LWLSY to the adequate value.
*
                        LWLSY = MAX( 1, 3*MNMIN+3*N+1, 2*MNMIN+NRHS )
*
                        CALL SLACPY( 'Full', M, N, COPYA, LDA, A, LDA )
                        CALL SLACPY( 'Full', M, NRHS, COPYB, LDB, B,
     $                               LDB )
*
                        SRNAMT = 'SGELSZ'
                        CALL SGELSZ( M, N, NRHS, A, LDA, B, LDB, IWORK,
     $                               RCOND, CRANK, WORK, LWLSY, INFO )
                        IF( INFO.NE.0 )
     $                     CALL ALAERH( PATH, 'SGELSZ', INFO, 0, ' ', M,
     $                                  N, NRHS, -1, NB, ITYPE, NFAIL,
     $                                  NERRS, NOUT )
*
...
*                                workspace: M*N + 4*MIN(M,N) + MAX(M,N)
*
                        RESULT( 11 ) = SQRT12( CRANK, CRANK, A, LDA,
     $                                 COPYS, WORK, LWORK )
*
...
*
     ...
     $                                  NERRS, NOUT )
*
*                       Test 19:  Compute relative error in svd
*
                        IF( RANK.GT.0 ) THEN
                           CALL SAXPY( MNMIN, -ONE, COPYS, 1, S, 1 )
                           RESULT( 19 ) = SASUM( MNMIN, S, 1 ) /
     $                                    SASUM( MNMIN, COPYS, 1 ) /
     $                                    ( EPS*REAL( MNMIN ) )
                        ELSE
                           RESULT( 19 ) = ZERO
                        END IF
*
*                       Test 20:  Compute error in solution
*
                        CALL SLACPY( 'Full', M, NRHS, COPYB, LDB, WORK,
     $                               LDWORK )
                        CALL SQRT16( 'No transpose', M, N, NRHS, COPYA,
     $                               LDA, B, LDB, WORK, LDWORK,
     $                               WORK( M*NRHS+1 ), RESULT( 20 ) )
*
*                       Test 21:  Check norm of r'*A
*
                        RESULT( 21 ) = ZERO
                        IF( M.GT.CRANK )
     $                     RESULT( 21 ) = SQRT17( 'No transpose', 1, M,
     $                                    N, NRHS, COPYA, LDA, B, LDB,
     $                                    COPYB, LDB, C, WORK, LWORK )
*
*                       Test 22:  Check if x is in the rowspace of A
*
                        RESULT( 22 ) = ZERO
                        IF( N.GT.CRANK )
     $                     RESULT( 22 ) = SQRT14( 'No transpose', M, N,
     $                                    NRHS, COPYA, LDA, B, LDB,
     $                                    WORK, LWORK )
*
*                       Print information about the tests that did not
