...
*     03/05/2004
*
*     instrumented version of SGELSZ to count ops
*
*     .. Scalar Arguments ..
...
*     .. External Functions ..
      INTEGER            ILAENV
      REAL               SECOND, SLAMCH, SLANGE, SOPBL3, SOPLA
      EXTERNAL           ILAENV, SECOND, SLAMCH, SLANGE, SOPBL3, SOPLA
*     ..
...
*
         OPCNT( GELSZ ) = OPCNT( GELSZ ) + REAL( M*N )
...
*        Scale matrix norm up to SMLNUM
*
         OPCNT( GELSZ ) = OPCNT( GELSZ ) + REAL( M*NRHS )
...
*
      OPCNT( GELSZ ) = OPCNT( GELSZ ) + REAL( RANK*RANK-1 ) + OPS
*
      WSIZE = MN + WORK( MN+1 )
*
...
*
      OPCNT( TRSM ) = OPCNT( TRSM ) + SOPBL3( 'STRSM ', RANK, NRHS, 0 )
      T1 = SECOND( )
...
*
      IF( IASCL.EQ.1 ) THEN
         OPCNT( GELSZ ) = OPCNT( GELSZ ) + REAL( N*NRHS+RANK*RANK )
     ...
     $                INFO )
      END IF
      IF( IBSCL.EQ.1 ) THEN
         OPCNT( GELSZ ) = OPCNT( GELSZ ) + REAL( N*NRHS )
