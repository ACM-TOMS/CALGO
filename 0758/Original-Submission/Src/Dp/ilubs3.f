...
Ccc PARAMETER SPECIFICATION:
      ...
      DOUBLE PRECISION A(N,NPDE,NPDE,-4:4)
C
Ccc PURPOSE:
...
CFPP$ UNROLL
            DO 161 LC = 1, NPDE
CFPP$ UNROLL
      A(K,IC,1,1) = A(K,IC,1,1) - A(K,IC,LC,-3)*A(LLDG(K,-3),LC,1,4) - A
     1   (K,IC,LC,-2)*A(LLDG(K,-2),LC,1,3)
      A(K,IC,1,2) = A(K,IC,1,2) - A(K,IC,LC,-1)*A(K-1,LC,1,3)
      A(K,IC,1,3) = A(K,IC,1,3) - A(K,IC,LC,-1)*A(K-1,LC,1,4)
      A(K,IC,2,1) = A(K,IC,2,1) - A(K,IC,LC,-3)*A(LLDG(K,-3),LC,2,4) - A
     1   (K,IC,LC,-2)*A(LLDG(K,-2),LC,2,3)
      A(K,IC,2,2) = A(K,IC,2,2) - A(K,IC,LC,-1)*A(K-1,LC,2,3)
      A(K,IC,2,3) = A(K,IC,2,3) - A(K,IC,LC,-1)*A(K-1,LC,2,4)
      A(K,IC,3,1) = A(K,IC,3,1) - A(K,IC,LC,-3)*A(LLDG(K,-3),LC,3,4) - A
     1   (K,IC,LC,-2)*A(LLDG(K,-2),LC,3,3)
      A(K,IC,3,2) = A(K,IC,3,2) - A(K,IC,LC,-1)*A(K-1,LC,3,3)
      A(K,IC,3,3) = A(K,IC,3,3) - A(K,IC,LC,-1)*A(K-1,LC,3,4)
  161       CONTINUE
   60       CONTINUE
            DO 163 LC = 1, IC-1
CDIR$ IVDEP
...
Ccc PARAMETER SPECIFICATION:
     ...
     +   LSL(*), LLSL(0:*), LSU(*), LLSU(0:*)
      DOUBLE PRECISION A(N,NPDE,NPDE,-4:4), B(N,NPDE)
C
Ccc PURPOSE:
C Solve LUx = b
C    A stems from a 9-point stencil on a grid with irregular row lengths
