...
C==========================
      ...
      LOGICAL LWK(LENLWK)
      REAL T, TOUT, DT, XL, YL, XR, YU, DX, DY, TOLS, TOLT, RINFO(*),
     +   RWK(LENRWK)
C
...
Ccc PARAMETER SPECIFICATION:
C     INTEGER NPTS, NPDE
C     REAL T, X(NPTS), Y(NPTS), U(NPTS,NPDE)
C
Ccc PURPOSE:
...
C     REAL T, DT, DTNEW, XL, YL, DXB, DYB, SOL(*)
C
Ccc PURPOSE:
...
C     REAL T, X(NPTS), Y(NPTS), U(NPTS,NPDE), SPCMON(NPTS), TOL
C
Ccc PURPOSE:
...
Ccc PARAMETER SPECIFICATION:
C     INTEGER NPTS, NPDE, LLBND(0:*), ILBND(*), LBND(*)
C     REAL F(NPTS,NPDE), T, X(NPTS), Y(NPTS), U(NPTS,NPDE),
C    +   A0, DT, DX, DY, UIB(*),
C    +   UT(NPTS,NPDE), UX(NPTS,NPDE), UY(NPTS,NPDE),
C    +   UXX(NPTS,NPDE), UXY(NPTS,NPDE), UYY(NPTS,NPDE),
...
C     REAL T, XL, YL, DXB, DYB, SOL(*), UNIFRM(0:NX,0:NY,NPDE)
C
Ccc PURPOSE:
...
C of the BLAS library. If I1MACH and R1MACH of the file blas.f are used,
C the functions should be altered for the particular machine used (cf.
C comment in I1MACH and R1MACH).
C
Ccc CMNCMMACH
C
...
Ccc PARAMETER SPECIFICATION:
     ...
     +   LENRWK, IWK(LENIWK), LENLWK, IERR
      LOGICAL LWK(LENLWK)
      REAL TN, TE, DT, DTMIN, DTMAX, XL, YL, XR, YU, DX, DY,
...
C
     ...
     +   LENPRE, LENLSW, LU, LUO
      ...
      REAL DTNEW, DTRAT, TOLWGT, SPCMON, TIMMON
C
      IF (IERR .EQ. 0) THEN
C
...
Ccc PARAMETER SPECIFICATION:
      ...
      LOGICAL REFFLG(0:*)
      REAL T, U(0:*), X(*), Y(*), SPCTOL(NPDE), TOLWGT, WORK(*), SPCMON
C
Ccc PURPOSE:
...
Ccc PARAMETER SPECIFICATION:
     ...
     +   LLBND(0:*), ILBND(*), LBND(*), LBELOW(NPTS), LABOVE(NPTS)
      ...
      REAL T, U(0:NPTS*NPDE), X(NPTS), Y(NPTS),
...
Ccc PARAMETER SPECIFICATION:
      ...
      LOGICAL LEVO
      REAL UIC(0:*), UC(0:*), UO(0:*), U(0:*), UIB(*), RWORK(*)
C
Ccc PURPOSE:
...
C PARAMETER SPECIFICATION:
     ...
     +   LBELOW(*), LABOVE(*)
      REAL T, X(*), Y(*), UNP1(0:*), UN(0:*), UNM1(0:*), DT, DTRAT,
...
Ccc PARAMETER SPECIFICATION:
      INTEGER NPTS, NPDE
      REAL UNP1(NPTS,NPDE), UN(NPTS,NPDE), UNM1(NPTS,NPDE), DT, DTRAT,
     +   UT(NPTS,NPDE)
C
Ccc PURPOSE:
...
Ccc PARAMETER SPECIFICATION:
     ...
     +   LBELOW(NPTS), LABOVE(NPTS)
      REAL U(0:NPTS*NPDE), DX, DY, UX(NPTS*NPDE), UY(NPTS*NPDE),
...
C
      ...
      REAL FACX, FACY, FACXX, FACXY, FACYY
C
...
Ccc PARAMETER SPECIFICATION:
      ...
      REAL RWK(*), TIMWGT(NPDE), RELTOL(NPDE), ABSTOL(NPDE),
     +   WORK(*), DT, DTNEW, TIMON
C
Ccc PURPOSE:
...
C       restrict time step variance
      ...
      RETURN
      END
      REAL FUNCTION TIMMON (U, UO, NPTS, NPDE, LBND, NBIPTS, TIMWGT,
     +   RELTOL, ABSTOL, DTUT)
C
C-----------------------------------------------------------------------
C
Ccc PARAMETER SPECIFICATION:
      ...
      REAL U(NPTS,NPDE), UO(NPTS,NPDE), TIMWGT(NPDE),
...
Ccc PARAMETER SPECIFICATION:
      ...
      REAL X(*), Y(*), UIB(*), UNP1(0:*), UN(0:*), UNM1(0:*),
...
Ccc EXTERNALS USED:
      REAL MAXNRM, WSNRM2
      EXTERNAL ERRWGT, BICGST, JAC, JACPB, MAXNRM, RESID, WSNRM2
C
C
Ccc   INCLUDE 'PARNEWTON'
C
...
C
     ...
     +   NJAC, NRES, I, NIT, ITER
      LOGICAL NEWJAC
      REAL ERR, CORNRM, OLDNRM, RATE, TOL, UNRM
C
      IERR = 0
C
      IF (LUNNLS .NE. 0) THEN
...
Ccc Compute derivatives and residual
     ...
     +      MAXNRM(LENU, F), WSNRM2 (LENU, F, WT)
      ENDIF
C
...
Ccc PARAMETER SPECIFICATION:
      INTEGER NPTS, NPDE, LLBND(0:*), ILBND(*), LBND(*)
      REAL F(*), T, X(*), Y(*), DT, DTRAT, DX, DY, U(*), UIB(*),
...
Ccc PARAMETER SPECIFICATION:
      ...
      REAL U(NPTS,NPDE), A0, DT, UT(NPTS,NPDE), TOL, DEL(NPTS),
...
Ccc PARAMETER SPECIFICATION:
      INTEGER ICPTB, NPTS, NPDE
      REAL U(NPTS,NPDE), TOL, DEL(NPTS), UBAR(NPTS,NPDE)
C
Ccc PURPOSE:
C Perturb the ICPTB-th component of U. Return perturbance in DEL and
C perturbed U in UBAR.
...
Ccc PARAMETER SPECIFICATION:
     ...
     +   LUN, ITER, IERR
      REAL A(*), X(NPTS*NPDE), B(NPTS*NPDE), WT(NPTS*NPDE), TOL, ALU(*),
...
Ccc EXTERNALS USED:
      REAL WSNRM2, SDOT
      EXTERNAL MVDIAG, BCKSLV, RCOPY, WSNRM2, SDOT, SAXPY
C
C
Ccc   INCLUDE 'CMNCMMACH'
C
...
C
      INTEGER I, N
      REAL ALPHA, BETA, OMEGA, RHOIM1, RHOI, SXMIN, TNRM2
C
      N = NPTS*NPDE
      ITER = 0
      IERR = 0
...
Ccc PARAMETER SPECIFICATION:
      ...
      REAL AD(NPTS,NPDE,NPDE,-4:4), X(NPTS,NPDE), Y(NPTS,NPDE)
C
Ccc PURPOSE:
...
Ccc EXTERNALS USED:
      REAL MAXNRM, WSNRM2
      EXTERNAL ERRWGT, GGCRO, JAC, JACBD, MAXNRM, RESID, WSNRM2
C
C
Ccc   INCLUDE 'PARNEWTON'
C
...
Ccc PARAMETER SPECIFICATION:
      ...
      REAL G(NPTS*NPDE,NPDE,-4:4), PREC(NPTS*NPDE,*)
C
Ccc PURPOSE:
...
Ccc PARAMETER SPECIFICATION:
      INTEGER NPTS, NPDE
      REAL A(NPTS,NPDE,NPDE), X(NPTS,NPDE), B(NPTS,NPDE)
C
Ccc PURPOSE:
C Solve LUx = b
...
Ccc PARAMETER SPECIFICATION:
     ...
     +   ITER, IERR
      LOGICAL BDPREC
      REAL G(*), XV(NPTS*NPDE), F(NPTS*NPDE), WT(NPTS*NPDE), TOL,
     +   PREC(NPTS*NPDE),
...
Ccc EXTERNALS USED:
      REAL SDOT, SNRM2
      EXTERNAL BCKBDI, GGMRSO, MVDIAG, RCOPY, SAXPY, SDOT, SNRM2, ZERO
C
C
Ccc   INCLUDE 'CMNCMMACH'
C
...
C Compute ZW[0:k-1,k] = C_k^T.v
         DO 300 I = 0, K-1
            ZW(I,K) = SDOT(N, C(1,I),1, WORK(LV),1)
  300    CONTINUE
C
         GRITER = GRITER + 1
C
...
C Compute x = x + U_k.Z_k^(-1).1
  ...
  320          CONTINUE
               CALL SAXPY (N, WORK(LV+I), U(1,I), 1, XV, 1)
  310       CONTINUE
            ITER = GRITER + PRITER
            GOTO 900
         ENDIF
...
Ccc PARAMETER SPECIFICATION:
     ...
     +   ITER, IERR
      LOGICAL BDPREC
      REAL G(*), XV(NPTS*NPDE), BV(NPTS*NPDE), WT(NPTS*NPDE),
     +   PREC(NPTS*NPDE), CO(NPTS*NPDE,0:M), TOL,
...
Ccc EXTERNALS USED:
      REAL SDOT, SNRM2
      EXTERNAL BCKBDI, MVDIAG, RCOPY, SAXPY, SDOT, SNRM2, ZERO
C
C-----------------------------------------------------------------------
C
      INTEGER I, J, K, N
      REAL C, CNM2, R0NRM, PROD, RHO, S, TEM, T1, T2, T, VNRM
C
      N = NPTS*NPDE
      IERR = 0
      ITER = 0
...
C modified Gram-Schmidt
         DO 801 I = 0, M-1
            TEM = SDOT (N, CO(1,I), 1, V(1,K+1), 1) /
     +            SDOT (N, CO(1,I), 1, CO(1,I), 1)
            CALL SAXPY (N, -TEM, CO(1,I), 1, V(1,K+1), 1)
  801    CONTINUE
C
C Orthogonalize V(*,K+1) against the previous V using
C modified Gram-Schmidt
         DO 81 I = 1, K
            HES(I,K) = SDOT (N, V(1,I), 1, V(1,K+1), 1)
            CALL SAXPY (N, -HES(I,K), V(1,I), 1, V(1,K+1), 1)
   81    CONTINUE
         VNRM = SNRM2(N, V(1,K+1), 1)
         HES(K+1,K) = VNRM
C
...
C Solve R.y = Q.beta.e1
      DO 85 I = 1, K
         ...
         CALL SAXPY (J-1, -WORK(J), HES(1,J),1, WORK,1)
   85 CONTINUE
C
C X = X + Vk.y
      DO 120 I = 1,K
         CALL SAXPY(N, WORK(I), V(1,I), 1, XV, 1)
 120  CONTINUE
C
...
C Compute c_m = (b - r) / <c_m,c_m>
  ...
  130 CONTINUE
      CNM2 = 1 / SDOT (N, CO(1,M), 1, CO(1,M), 1)
      DO 140 J = 1, N
         CO(J,M) = CO(J,M) * CNM2
  140 CONTINUE
C
...
Ccc EXTERNALS USED:
      REAL MAXNRM, WSNRM2
      EXTERNAL ERRWGT, GCRO, PINIT, MAXNRM, RESID, WSNRM2
C
C
Ccc   INCLUDE 'PARNEWTON'
C
...
C
     ...
     +   NPRE, NRES, I, NIT, ITER
      ...
      REAL A0, ERR, CORNRM, OLDNRM, RATE, TOL, UNRM
C
      IERR = 0
C
      A0 = (1+2*DTRAT) / ((1+DTRAT)*DT)
...
C
      ...
      REAL FACX, FACY, FACXX, FACYY, TOL
...
Ccc PARAMETER SPECIFICATION:
      INTEGER NPTS, NPDE
      REAL U(NPTS,NPDE), A0, DT, UT(NPTS,NPDE), ABSTOL(NPDE),
...
Ccc PARAMETER SPECIFICATION:
      INTEGER NPTS, NPDE
      REAL U(NPTS,NPDE), ABSTOL(NPDE), FAC, DEL(NPTS,NPDE),
     +   UBAR(NPTS,NPDE)
C
Ccc PURPOSE:
C Perturb U. Return perturbance in DEL and perturbed U in UBAR.
...
C
      INTEGER I, LENU
      REAL DG, EPS, FACX, FACY, FACXX, FACYY
C
      EPS = SQRT(UROUND)
C
      LENU  = NPTS*NPDE
...
Ccc EXTERNALS USED:
      REAL SDOT, SNRM2
      EXTERNAL BCKBDI, GMRESO, MVDIFF, RCOPY, SAXPY, SDOT, SNRM2, ZERO
C
C
Ccc   INCLUDE 'CMNCMMACH'
C
...
Ccc PARAMETER SPECIFICATION:
     ...
     +   ITER, IERR
      LOGICAL BDPREC
      REAL XV(N), BV(N), WT(N), PREC(N), CO(N,0:M), TOL,
     +   F(*), X(*), Y(*), UIB(*), UNP1(0:*),
     +   TNP1, A0, DX, DY, RWORK(*),
...
Ccc EXTERNALS USED:
      REAL SDOT, SNRM2
      EXTERNAL BCKBDI, MVDIFF, RCOPY, SAXPY, SDOT, SNRM2, ZERO
C
C-----------------------------------------------------------------------
C
      INTEGER I, J, K
...
Ccc PARAMETER SPECIFICATION:
     ...
     +   NPDE, NXB, NYB, LGRID(0:*), ISTRUC(*), LSOL(*), NX, NY
      REAL T, XL, YL, DXB, DYB, SOL(*), UNIFRM(0:NX,0:NY,NPDE)
C
Ccc PURPOSE:
C Write (interpolated) solution values at grid level UNILEV to file
C LUNS.
...
C-----------------------------------------------------------------------
      INTEGER NPTS, NPDE
      REAL U(NPTS,NPDE), RELTOL(NPDE), ABSTOL(NPDE), WT(NPTS,NPDE)
C-----------------------------------------------------------------------

      INTEGER IC, IPT
      REAL SN

      SN = 1.0/SQRT(REAL(NPTS*NPDE))
      DO 10 IC = 1, NPDE
         DO 20 IPT = 1, NPTS
...
C-----------------------------------------------------------------------
   ...
   10 CONTINUE
      WMXNRM = WMXNRM*SQRT(REAL(N))

      RETURN
      END
      REAL FUNCTION WSNRM2 (N, V, W)
C-----------------------------------------------------------------------
      INTEGER N
      REAL V(N), W(N)
C-----------------------------------------------------------------------
