...
C
      ...
      LOGICAL LWK(LENLWK)
      REAL T, TOUT, DT, XL, YL, XR, YU, DX, DY,
...
Ccc PARAMETER SPECIFICATION:
      ...
      REAL F(NPTS,NPDE), T, X(NPTS), Y(NPTS), U(NPTS,NPDE),
     +   A0, DT, DX, DY, UIB(*),
...
C Local arrays:
      ...
      REAL X(MAXPTS), Y(MAXPTS), UEX(MAXPTS*NPDE)
C
C-----------------------------------------------------------------------
C
...
C Compute ||err||_max
   ...
   10 CONTINUE
      ...
      REAL T, X(NPTS), Y(NPTS), U(NPTS,NPDE), UEX(NPTS,NPDE)
