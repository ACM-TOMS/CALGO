...
C*
*
        subroutine drive_dfgmres(n,nloc,m,lwork,work,
     &                         irc,icntl,cntl,info,rinfo)
*
*  Purpose
*  =======
*    drive_dfgmres is the driver routine for solving the linear system 
...
* Authors :  Valerie Fraysse, Luc Giraud, Serge Gratton
*             Parallel Algorithms - CERFACS
* Comments: Totally rewritten from the dfgmres implementation
*
...
*
* cntl     (input) double precision array, length 3
...
* setup some pointers on the workspace
       ...
       Zptr     = lwork - (m*nloc)+1
*
       call dfgmres(nloc,m,lwork,work(bptr),work(xptr),work(dotptr),
...
*
        subroutine dfgmres(n,m,lwork,b,x,dot,H,w,r0,V,Z,yCurrent,
     &                xCurrent,rotSin,rotCos,irc,icntl,cntl,info,rinfo)
*
*
*  Purpose
*  =======
*  dfgmres solves the linear system Ax = b using the
...
*
* Z         (workspace)  double precision/double precision
...
*
* rotCos    (workspace) double precision/double precision
...
* ----------------------
       integer  irc(7)
       double precision x(n), dot(m), H(m+1,m+1), w(n)
       double precision r0(n), V(n,m+1), Z(n,m)
       double precision xCurrent(n), rotSin(m), yCurrent(m)   
       double precision rotCos(m)
*
* Local variables
* ---------------
       ...
       double precision    beta, bn, sA, sb, bea, be, temp
...
*
        ...
        irc(7) = irc(7) - n 
*
*       Most outer loop : dfgmres iteration
*
*       REPEAT
 7      continue
*
...
*
          call dgemv('N',n,jH,ONE,Z(1,m-jH+1),n,
...
*
*
        subroutine init_dfgmres(icntl,cntl)
*
*  Purpose
*  =======
