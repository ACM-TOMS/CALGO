*************************************************************************
**                 TEST PROGRAMME FOR THE cfgmres CODE
*************************************************************************
      program validation
*
...
*
      if (me.eq.0) then
         ...
         write(*,*)  'fort.31', ': log file of FGMRES iterations '
         write(*,*)  'fort.131',' : log file of inner
     & cgmres iterations '
         write(*,*) 'and ','sol_cTest','  : output of the computation.'
         write(*,*) '***********************************************'
         write(*,*)
...
*
*************************
*c Tune some parameters for cfgmres
*************************
*
...
*
*************************
*c Tune some parameters for cgmres
*************************
*
* Tolerance
      cntl2(1) = 1.d-2
* warning output stream
      icntl2(2) = 0
      if (me.eq.0) then
* Save the convergence history in file 'fort.130'
        icntl2(3) = 131
      else
        icntl2(3) = 0
      endif
...
*****************************************
*
10     call drive_cfgmres(n,nloc,m,lwork,work,
...
* perform the right preconditioning for the cfgmres iteration
*
* Check if there is enough space left in the workspace to perfrom
* few steps of cgmres as right preconditioner
         rn = float(n)
         rx         = rn + 5.0
...
*
         if (m2.gt.0) then
* Perform at most two restarts in the inner cgmres
           icntl2(7) = 2*m2
* copy colx in the workspace (right hand side location) of the inner
* gmres iteration 
           call ccopy(nloc,work(colx),1,work(irc(6)+nloc),1)
 20        call drive_cgmres(n,nloc,m2,irc(7),
...
* Perform the matrix vector product for the inner cgmres iteration
             call ccopy(nloc,work(colx2),1,aux(istart),1)
* Send the entry of aux required to perform the parallel tridiagonal matrix-vector
* product
             if (me.ne.(nproc-1)) then
*  send the last entry of y local vector to the next processor that
...
* perform the preconditioning for the inner cgmres iteration
             do i =0,nloc-1
...
*      perform the scalar product for the inner cgmres iteration
*      work(colz) <-- work(colx) work(coly)
             call cgemv('C',nloc,nbscal2,ONE,work(colx2),nloc,
     &               work(coly2),1,ZERO,aux,1)
             call MPI_ALLREDUCE(aux,work(colz2),nbscal2,
     &          MPI_COMPLEX, MPI_SUM,comm_inner,infompi)
             goto 20
           endif
           call ccopy(nloc,work(irc(6)),1,work(colz),1)
           goto 10
         else
* (m2.le.0)
