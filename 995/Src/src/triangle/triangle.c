...
{
  if(uniform == 1)
    return isotropic_area < area;

  REAL center_x = (triorg[0]+tridest[0]+triapex[0]) / 3.0;
  REAL center_y = (triorg[1]+tridest[1]+triapex[1]) / 3.0;

  REAL distance;

  for(int c=0; c<components; c++) {
    REAL component_distance = sqrt(pow(center_x-center_points[c*2], 2) + pow(center_y-center_points[c*2 + 1], 2));

    if(c == 0) 
      distance = component_distance;
    else if(component_distance < distance)
      distance = component_distance;
  }

  if(distance < 10) 
    if((isotropic_area*pow(1.0+distance, 4)) < area)
      return 1;
    else return 0;
  else if((pow(distance, 2)*isotropic_area*pow(11, 2)) < area)
      return 1;
  else return 0;
}
...
void triangulate(char *triswitches, struct triangulateio *in,
                 ...
                 REAL iso_area, int u, int comps, REAL *centers)
#else /* not ANSI_DECLARATORS */
void triangulate(triswitches, in, out, vorout, iso_area, u, comps, centers)
char *triswitches;
