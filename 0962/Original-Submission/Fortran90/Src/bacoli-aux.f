c bacoli-aux.f (supporting routines for BACOLI-3, the source code
c contained in the file bacoli.f.)

c This file contains:

c (i) The COLROW software package: J. C. Diaz, G. Fairweather and
...
c www.netlib.org/slatec/src/aaaaaa.f
c Many of these routines have been modified for BACOLI's purposes,
c particularly those of the DAE solver DASSL. The DASSL routines
c were modified to use COLROW for linear algebra, and to pass
...
c     COLROW, and the dimension of atol and rtol (if a vector is used)
c     is modified from neq to npde to save the storage.
c
c     The modification to the DASSL package to accommodate the ABD
c     linear algebra is limited to the routines:
c
c                        DASSL, DAJAC, and DASLV.
c
c     The modification to atol and rtol is limited to the routines:
c
c                        DASSL, DAWTS.
c
c     The modification to iteration matrix for overcoming the
c     condition number problem in a index-1 DAE is limited to the
c     routines:
c
c                        DAINI, DASTP, DASLV (because the calling
c                                             sequence of DASLV is
c                                             modified)
c
c     This is a result of the design of DASSL which allows the user to
c     specify his or her own linear algebra factorization and solve
c     routines.
c
c     However, due to undeclared variables (using implicit versus
c     explicit declarations), the following routines have been changed:
c
c                   XERMSG, XERPRN, XGETUA, AND XSETUA.
c
c     Modifications will be separated by comment lines with a "c"
c     followed by 71 "-"'s.
c
c     Further modifications were done to make the interface to BACOLI
c     more flexible, ie, user subroutines specifying the PDE system
c     are now passed down to CALRES and CALJAC.
c-----------------------------------------------------------------------
c
c Modified by Rong Wang, August 13, 2001.
c Last modified by Jack Pew, May 16, 2013.
c
c-----------------------------------------------------------------------
...
C                       G(T,Y,YPRIME) = 0.)
c-----------------------------------------------------------------------
c        INFO(15) --DDASSL normally requires either a full or banded
c               Jacobian matrix. If the structure of the Jacobian
c               matrix is almost block diagonal (ABD) as defined by the
c               package COLROW, the user may chose this option to use
c               the linear algebra routines in order to save
c               storage requirements (and decrease the execution time).
c               In the case of an ABD Jacobian matrix, the user MUST
c               supply the routine JAC to define the analytic ABD
c               Jacobian matrix, since a finite difference ABD Jacobian
c               approximation routine has not been developed yet. Thus
c               INFO(5) will be ignored if the ABD linear algebra option
c               is chosen.
c          ****   Is the Jacobian matrix full or banded?
c                 Yes - Set INFO(15) = 0
c                  No - Set INFO(15) = 1
c                       and define the ABD structure by setting
c                       IWORK(17) = npde, IWORK(18) = kcol,
c                       IWORK(19) = nint.
c-----------------------------------------------------------------------
C
...
c----------------------------------------------------------------------
c               For the case of a user-defined ABD Jacobian
c               (when INFO(15) = 1), we require that
c
c                     lrw = 40 + (IWORK(LMXORD) + 4) * NEQ +
c                           npde * npde * (nconti +
c                           nint * kcol * (kcol + nconti) + nconti)
c                           npde * npde * (nconti +
c                           nint * (kcol + 1) * (kcol + 1 + nconti)
c                           + nconti)
c
c----------------------------------------------------------------------
C
...
C
     ...
     *   RPAR(*)
      EXTERNAL  RES, JAC
c-----------------------------------------------------------------------
      external f, derivf, bndxa, difbxa, bndxb, difbxb
c-----------------------------------------------------------------------
C
C     Declare externals.
C
...
C
      INTEGER  I, ITEMP, LALPHA, LBETA, LCJ, LCJOLD, LDELTA,
     *   LENIW, LENPD, LENRW, LE, LGAMMA, LH, LHMAX, LHOLD,
...
C     SET POINTERS INTO IWORK
     ...
     *  LNRE=12, LNJE=13, LNPD=16,
     *  LJCALC=5, LPHASE=6, LK=7, LKOLD=8,
     *  LNS=9, LNSTL=10, LIWM=1)
c-----------------------------------------------------------------------
c     Define the ABD related offsets.
      parameter (lnpde = 17)
      parameter (lkcol = 18)
      parameter (lnint = 19)
c-----------------------------------------------------------------------
...
c     upload the parameters related to an ABD Jacobian matrix.
         npde   = iwork(lnpde)
         kcol   = iwork(lkcol)
         nint   = iwork(lnint)
c-----------------------------------------------------------------------
...
10       CONTINUE
c-----------------------------------------------------------------------
c     Check to see whether info(15), for abd matrix, is valid.
c
      if(info(15).ne.0.and.info(15).ne.1) go to 701
c-----------------------------------------------------------------------
C
...
c-----------------------------------------------------------------------
60    continue
c     Override linear algebra choices - only ABD is accepted.
c     Should the Jacobian be constructed analytically or via finite
c     differences?
      if (info(5) .eq. 1) then
         iwork(lmtype) = 3
      else
...
c     Compute lenpd and lenrw with responding to an ABD Jacobian matrix.
      lenpd  = npde * npde * (nconti + nint * kcol * (kcol + nconti)
     &         + nconti)
...
c-----------------------------------------------------------------------
      CALL DDAWTS(INFO(2),RTOL,ATOL,Y,RWORK(LWT),RPAR,IPAR)
c-----------------------------------------------------------------------
      DO 305 I = 1,NEQ
...
C     UPDATE WT
c-----------------------------------------------------------------------
510   CALL DDAWTS(INFO(2),RTOL,ATOL,RWORK(LPHI),
     *  RWORK(LWT),RPAR,IPAR)
c-----------------------------------------------------------------------
      DO 520 I=1,NEQ
...
c-----------------------------------------------------------------------
c
c Modified by Rong Wang, September 4, 2001.
...
c-----------------------------------------------------------------------
      ...
      integer iework
      integer siztop, sizbot, sizblk
      integer hgttop, hgtbot, hgtblk
      integer wdhtop, wdhbot, wdhblk
...
c     Dimensions of blocks in the ABD matrix.
      hgttop = npde
      hgtbot = hgttop
      hgtblk = npde * kcol
      wdhtop = npde * nconti
      wdhbot = wdhtop
      wdhblk = npde * (kcol + nconti)
      siztop = hgttop * wdhtop
      sizbot = siztop
      sizblk = hgtblk * wdhblk

c     Pointers to sections of the ABD matrix in the work space.
      npdbk1 = npd + siztop
      npdbt1 = npdbk1 + nint * sizblk

      GO TO (100,200,300,400,500,350),MTYPE
c-----------------------------------------------------------------------
C
C
...
c-----------------------------------------------------------------------
  300 continue
c     Construct analytic ABD Jacobian matrix (MTYPE 3.)
      call jac(x, y, yprime, wm(npd), cj, rpar, ipar,
     &         derivf, bndxa, difbxa, bndxb, difbxb)
      goto 390

  350 continue
c     Construct finite difference ABD Jacobian matrix (MTYPE 6.)
c     Each subinterval must use the same number of collocation points.
      ires = 0
      squr = sqrt(uround)

c     y and yprime values before perturbation are saved in rpar(iework).
      msave = nint
      isave = ipar(iework)-1
      ipsave = isave + msave

c     There are three sets of calls to res.
      do l = 1, 3
         if (l .eq. 1) then
c           The first set includes the top block and every other
c           overlapping section between blocks, starting from the 2nd.
c           The bottom block is included if nint is even.
            i1 = 1
            i2 = wdhtop
            step = 2 * kcol * npde
         elseif (l .eq. 2) then
c           The second set accounts for the nonoverlapping parts of each
c           of the nint middle blocks.
            i1 = wdhtop + 1
            i2 = wdhblk - wdhbot
            step = kcol * npde
         else
c           The final set accounts for every other overlapping section
c           between blocks, starting from the 1st.
c           The bottom block is included if nint is odd.
            i1 = wdhblk - wdhbot + 1
            i2 = wdhblk
            step = 2 * kcol * npde
         endif
         do i = i1, i2
            ...
            enddo
            call res(x, y, yprime, e, ires, rpar, ipar, f, bndxa, bndxb)
            if (ires .lt. 0) return
            do n = i, neq, step
               k = (n - i) / step + 1
               y(n) = rpar(isave+k)
               yprime(n) = rpar(ipsave+k)
               del = squr * max(abs(y(n)), abs(h*yprime(n)), abs(wt(n)))
               del = sign(del, h*yprime(n))
               del = (y(n) + del) - y(n)
               delinv = 1d0/del
...
c              In the following:
c              n is the current column number.
c              jj is the offset for the current column subsection in wm.
c              jn is the offset for the current column subsection in e.
               if (l .ne. 2) then
c                 The first and last sections have a different pattern.
                  if (n .le. wdhtop) then
c                    Handle (a column of) the top block.
                     jj = (n-1) * hgttop
c                    jn = 0
                     do j = 1, hgttop
                        wm(jj+j) = (e(j) - delta(j)) * delinv
                     enddo
c                    Then the left side of the first middle block.
                     jj = siztop + (n-1) * hgtblk
                     jn = hgttop
                     do j = 1, hgtblk
                        wm(jj+j) = (e(jn+j) - delta(jn+j)) * delinv
                     enddo
                  elseif (n .ge. nint * hgtblk + 1) then
c                    Then the right side of the last middle block.
                     jj = siztop + (nint-1) * sizblk
     &                  + hgtblk * hgtblk + (mod(n,hgtblk)-1) * hgtblk
                     jn = hgttop + (nint-1) * hgtblk
                     do j = 1, hgtblk
                        wm(jj+j) = (e(jn+j) - delta(jn+j)) * delinv
                     enddo
c                    Then the bottom block.
                     jj = siztop + nint * sizblk
     &                  + (n-nint*hgtblk-1) * hgtbot
                     jn = hgttop + nint * hgtblk
                     do j = 1, hgtbot
                        wm(jj+j) = (e(jn+j) - delta(jn+j)) * delinv
                     enddo
                  else
c                    This column has two overlapping blocks.
                     jj = siztop + (n/hgtblk-1) * sizblk
     &                  + hgtblk * hgtblk + (mod(n,hgtblk)-1) * hgtblk
                     jn = hgttop + (n/hgtblk-1) * hgtblk
                     do j = 1, hgtblk
                        wm(jj+j) = (e(jn+j) - delta(jn+j)) * delinv
                     enddo
                     jj = siztop + (n/hgtblk) * sizblk
     &                  + (mod(n,hgtblk)-1) * hgtblk
                     jn = jn + hgtblk
                     do j = 1, hgtblk
                        wm(jj+j) = (e(jn+j) - delta(jn+j)) * delinv
                     enddo
...
c                 This is for the middle columns of each block.
                  jj = siztop + ((n-1)/hgtblk) * sizblk
     &               + mod(n-1,hgtblk) * hgtblk
                  jn = hgttop + ((n-1)/hgtblk) * hgtblk
                  do j = 1, hgtblk
                     wm(jj+j) = (e(jn+j) - delta(jn+j)) * delinv
                  enddo
               endif
...
C***FIRST EXECUTABLE STATEMENT  DDASLV
      MTYPE=IWM(LMTYPE)
c-----------------------------------------------------------------------
      GO TO(100,100,300,400,400,300),MTYPE
c-----------------------------------------------------------------------
C
C     DENSE MATRIX
...
C------END OF SUBROUTINE DDATRP------
      END
c-----------------------------------------------------------------------
      SUBROUTINE DDAWTS (IWT, RTOL, ATOL, Y, WT, RPAR, IPAR)
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
c     Explicit declarations made for variables declared by the implicit
c     Fortran 77 conventions.
c-----------------------------------------------------------------------
...
C***END PROLOGUE  XERMSG
      ...
      CHARACTER*72  TEMP
c-----------------------------------------------------------------------
      integer i, level, lkntrl, ltemp, mkntrl, nerr
c-----------------------------------------------------------------------
...
C
      IF (LEVEL.LE.0 .OR. (LEVEL.EQ.1 .AND. MKNTRL.LE.1)) RETURN
C
...
c--- EISPACK routines
c    (called by GAULEG to find Gauss-Legendre points for the
c     collocation and quadrature rules BACOLI employs)
...
c--- GAULEG routine from Pat Keast
...
C
C        IMQTL2: EISPACK ROUTINE TO COMPUTE THE EIGENVECTORS AND
...
c--- BLAS routines
c    (may be removed in favor of an optimised implementation)
