      SUBROUTINE DOSGSB( TRANQ1, TRANQ2, M, N, K, Q1, LDQ1, Q2, LDQ2,
     $                   CS, TAU, DWORK, LDWORK, INFO )
      IMPLICIT NONE
C
C     PURPOSE
C
C     Generates a matrix Q with orthogonal columns (spanning an
C     isotropic subspace), which is defined as the first k columns
...
C
C     as returned by DGESQR. The matrix Q is returned in terms of its
C     first 2*M rows
C
C                      [  op( Q1 )   op( Q2 ) ]
C                  Q = [                      ].
C                      [ -op( Q2 )   op( Q1 ) ]
C
C     Blocked version of routine DOSGSQ.
C
C     ARGUMENTS
C
C     Mode Parameters
...
C             which defines the elementary reflector F(i), as returned
C             by DGESQR.
...
C             diagonal, the scalar factor of H(i), as returned by
C             DGESQR.
C             On entry with TRANQ1 = 'C' or TRANQ1 = 'T', the leading
...
C             the optimal block size determined by the function ILAHAP.
...
C     .. External Subroutines ..
      EXTERNAL          DLAESB, DLAEST, DOSGSQ, XERBLA
C     .. Intrinsic Functions ..
      INTRINSIC         DBLE, INT, MIN, MAX, SQRT
C
...
C
      ...
      NB = ILAHAP( 1, 'DOSGSB', TRANQ1 // TRANQ2, M, N, K )
C
...
C
      ...
      ELSE IF ( K.LT.0 .OR. K.GT.N ) THEN
         INFO = -5
      ELSE IF ( ( LTRQ1.AND.LDQ1.LT.MAX( 1, M ) ).OR.
     $          ( .NOT.LTRQ1.AND.LDQ1.LT.MAX( 1, N ) ) ) THEN
         INFO = -7
      ELSE IF ( ( LTRQ2.AND.LDQ2.LT.MAX( 1, M ) ).OR.
     $          ( .NOT.LTRQ2.AND.LDQ2.LT.MAX( 1, N ) ) ) THEN
         INFO = -9
...
C
         NX = MAX( 0, ILAHAP( 3, 'DOSGSB', TRANQ1 // TRANQ2, M, N, K ) )
         IF ( NX.LT.K ) THEN
C
...
C
     ...
     $                     - DBLE( 4*N ) ) / 15.0D0 )
               NBMIN = MAX( 2, ILAHAP( 2, 'DOSGSB', TRANQ1 // TRANQ2, M,
     $                                 N, K ) )
            END IF
         END IF
      END IF
...
C
      IF ( KK.LT.N )
     $   CALL DOSGSQ( TRANQ1, TRANQ2, M-KK, N-KK, K-KK, Q1(KK+1,KK+1),
...
C
               CALL DOSGSQ( 'Transpose', 'Transpose', M-I+1, IB, IB,
...
C                 reflector SH.
C
                  CALL DLAEST( 'Forward', 'Rowwise', 'Columnwise',
...
C
                  CALL DLAESB( 'Zero Structure', 'No Transpose',
...
C
               CALL DOSGSQ( 'Transpose', 'No Transpose', M-I+1, IB, IB,
     $                      Q1(I,I), LDQ1, Q2(I,I), LDQ2, CS(2*I-1),
     $                      TAU(I), DWORK, LDWORK, IERR )
   20       CONTINUE
C
...
C                 reflector SH.
C
                  CALL DLAEST( 'Forward', 'Columnwise', 'Rowwise',
     $                         M-I+1, IB, Q1(I,I), LDQ1, Q2(I,I), LDQ2,
     $                         DWORK(PDRS), NB,  DWORK(PDT), NB,
     $                         CS(2*I-1), TAU(I), DWORK(PDW) )
C
...
C              Apply SH to columns/rows i:m of the current block.
C
               CALL DOSGSQ( 'No Transpose', 'Transpose', M-I+1, IB, IB,
     $                      Q1(I,I), LDQ1, Q2(I,I), LDQ2, CS(2*I-1),
     $                      TAU(I), DWORK, LDWORK, IERR )
   30       CONTINUE
C
...
C
               CALL DOSGSQ( 'No Transpose', 'No Transpose', M-I+1, IB,
