      SUBROUTINE DHAESU( BALANC, JOB, JOBU, JOBV, N, A, LDA, QG, LDQG,
...
C     conditioning of eigenvalues is computed (see DHABAL). In this
C     case, the matrix H in decomposition (2) must be replaced by the
C     balanced matrix.
C
C     The HAPACK routine DHASUB can be used to compute invariant
...
C             = 'S': compute matrices A and B of (2);
C             = 'G': compute matrices A, B and G of (2).
C
...
C             On exit, the leading N-by-N this array is overwritten.
C             If JOB = 'S' or JOB = 'G', this array contains the upper
C             triangular matrix T of decomposition (2). Otherwise, this
...
C             the (1,1) block of the orthogonal symplectic matrix U of
C             decomposition (2).
C
C     LDU1    INTEGER
...
C             On exit, the leading N-by-N part of this array contains
C             the (2,1) block of the orthogonal symplectic matrix U of
C             decomposition (2).
C
C     LDU2    INTEGER
...
C             On exit, the leading N-by-N part of this array contains
C             the (1,1) block of the orthogonal symplectic matrix V of
C             decomposition (2).
C
C     LDV1    INTEGER
...
C             On exit, the leading N-by-N part of this array contains
C             the (2,1) block of the orthogonal symplectic matrix V of
C             decomposition (2).
C
C     LDV2    INTEGER
...
C             factors applied when balancing H, see DHABAL.
...
C             value of LDWORK.
C             On exit, if  INFO = -29,  DWORK(1)  returns the minimum
C             value of LDWORK.
C
...
C     REFERENCES
C
C     [1] P. BENNER, V. MEHRMANN and H. XU:
C     A numerically stable, structure preserving method for computing
C     the eigenvalues of real Hamiltonian or symplectic pencils.
C     Numer. Math., Vol 78(3), pp. 329--358, 1998.
C
C     [2] P. BENNER, V. MEHRMANN and H. XU:
C     A new method for computing the stable invariant subspace of a real
C     Hamiltonian matrix,  J. Comput. Appl. Math., vol. 86, 1997,
C     pp 17-43.
C
C     CONTRIBUTORS
C
...
C     .. Local Scalars ..
     ...
     $                   WANTV
      INTEGER            I, IERR, J, K, L, PBETA, PCSL, PCSR, PDW, PQ,
     $                   PTAUL, PTAUR, PZ, WRKMIN, WRKOPT
      DOUBLE PRECISION   BIGNUM, CSCALE, EPS, HNRM, SMLNUM, TEMP, TEMPI,
     $                   TEMPR
C     .. External Functions ..
      LOGICAL            LSAME
      DOUBLE PRECISION   DLAMCH, DLANHA
      EXTERNAL           LSAME, DLAMCH, DLANHA
C     .. External Subroutines ..
      EXTERNAL           DCOPY, DCROOT, DGEMM, DGESUB, DHGPQR, DHABAL,
     $                   DLABAD, DLACPY, DLASCL, DLASET, DOSMSB, DSCAL,
     $                   XERBLA
...
C
      HNRM = DLANHA( 'Hamiltonian', 'MaxElement', N, A, LDA, QG, LDQG,
     $               DWORK )
      SCALEH = .FALSE.
...
C
      CALL DHABAL( BALANC, N, A, LDA, QG, LDQG, ILO, SCALE, IERR )
C
...
C
      IF ( .NOT.WANTU .AND. .NOT.WANTV  ) THEN
         CALL DGESUB( 'Not Transposed', 'Transposed', N, ILO, T, LDT, A,
...
C
         ...
         ELSE
            VCHAR = 'N'
         END IF
         CALL DHGPQR( JOB, VCHAR, UCHAR, N, ILO, N, A, LDA, T, LDT, V1,
...
C
         ...
         PZ    = PQ + N*N
            PDW   = PZ + N*N
         CALL DHGPQR( 'Schur', 'Init', 'Init', N, ILO, N, A, LDA, T,
...
C
      DO 910 I = INFO + 1, N         
         ...
         IF ( TEMPI.EQ.ZERO ) THEN
            ...
            END IF
         ELSE
            CALL DCROOT( TEMPR, TEMPI, WR(I), WI(I) )
            WR(I) = -WR(I) * SQRT( TEMP )
            IF ( TEMP.GT.0 ) THEN
               WI(I) = WI(I) * SQRT( TEMP )
            ELSE
...
C
         ...
         CALL DOSMSB( 'No Transpose', 'No Transpose', 'No Transpose',
...
C
     ...
     $                MAX(0,N-ILO), QG(ILO+1,ILO), LDQG, QG(ILO,ILO+1),
     $                LDQG, V1(ILO+1,1), LDV1, V2(ILO+1,1),
     $                LDV2, DWORK(PCSR+2*ILO-2), DWORK(PTAUR+ILO-1),
     $                DWORK(PDW), LDWORK-PDW-1, IERR )
         WRKOPT = MAX( WRKOPT, INT( DWORK(PDW) ) + PDW - 1 )
      ELSE IF ( WANTU .AND. WANTV .AND. .NOT.WANTG ) THEN
C
...
C
         ...
         CALL DOSMSB( 'No Transpose', 'No Transpose', 'No Transpose',
     $                'Columnwise', 'Rowwise', MAX(0,N-ILO), N,
     $                MAX(0,N-ILO), QG(ILO+1,ILO), LDQG, U2(ILO,ILO+1),
     $                LDU2, V1(ILO+1,1), LDV1, V2(ILO+1,1),
     $                LDV2, DWORK(PCSR+2*ILO-2), DWORK(PTAUR+ILO-1),
...
C
         ...
         CALL DOSMSB( 'No Transpose', 'No Transpose', 'No Transpose',
     $                'Columnwise', 'Rowwise', MAX(0,N-ILO), N,
     $                MAX(0,N-ILO), A(ILO+1,ILO), LDA,
     $                DWORK(PQ+ILO*N+ILO-1), N, V1(ILO+1,1), LDV1,
     $                V2(ILO+1,1), LDV2, DWORK(PCSR+2*ILO-2),
...
C
         ...
         CALL DOSMSB( 'No Transpose', 'No Transpose', 'No Transpose',
     $                'Columnwise', 'Rowwise', MAX(0,N-ILO), N,
     $                MAX(0,N-ILO), A(ILO+1,ILO), LDA, U2(ILO,ILO+1),
     $                LDU2, V1(ILO+1,1), LDV1, V2(ILO+1,1),
     $                LDV2, DWORK(PCSR+2*ILO-2), DWORK(PTAUR+ILO-1),
     $                DWORK(PDW), LDWORK-PDW-1, IERR )
         WRKOPT = MAX( WRKOPT, INT( DWORK(PDW) ) + PDW - 1 )
