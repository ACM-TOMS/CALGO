      SUBROUTINE DLABMX( COMPC, COMPU, COMPV, COMPW, WHICH, SELECT, N,
...
C             If COMPC = 'N' or SELECT = 'A', this array is not
C             referenced.
C
C     LDC     INTEGER
...
C             LDU2 >= N,  if COMPU = 'U' and SELECT = 'S'.
C
C     V1      (input/output) DOUBLE PRECISION array, dimension (LDV1,N)
C             On entry, if COMPV = 'U' and SELECT = 'S', the leading
C             N-by-N part of this array must contain V1, the (1,1)
C             block of an orthogonal symplectic matrix
C             V = [ V1, V2; -V2, V1 ].
C             On exit, if COMPV = 'U' and SELECT = 'S', the leading
C             N-by-N part of this array contains V1*Vr.
C             If COMPV = 'N' or SELECT = 'A', this array is not
C             referenced.
C
C     LDV1    INTEGER
C             The leading dimension of the array V1.  LDV1 >= 1.
C             LDV1 >= N,  if COMPV = 'U' and SELECT = 'S'.
C
...
C             LDV2 >= N,  if COMPV = 'U' and SELECT = 'S'.
C
...
C             On exit, if COMPW = 'I' the leading 2*M-by-2*M part of
C             this array is post-multiplied by the transformation
C             matrix that produced (2).
C             If COMPW = 'N', this array is not referenced. 
C
C     LDW     INTEGER
C             The leading dimension of the array W.  LDW >= 1.
C             LDW >= N,  if COMPW = 'I' or COMPW = 'V'.
C
...
C
C     [2] B. KAGSTROM and P. POROMAA:
C     Computing Eigenspaces with Specified Eigenvalues of a Regular
C     Matrix Pair (A, B) and Condition Estimation: Theory, Algorithms
C     and Software, Numer. Algorithms, 1996, vol. 12, pp 369-407.
C
C     [3] P. BENNER, V. MEHRMANN, and H. XU:
C     A new method for computing the stable invariant subspace of a real
C     Hamiltonian matrix,  J. Comput. Appl. Math., 86:17--43, 1997.
C
C     CONTRIBUTORS
C
...
C     .. Scalar Arguments ..
      CHARACTER*1       COMPC, COMPU, COMPV, COMPW, WHICH
...
C     .. Array Arguments ..
     ...
     $                  W(LDW,*), WI(*), WR(*)
C     .. Local Scalars ..
      LOGICAL           CMPALL, INITW, PAIR, SWAP, WANTC, WANTU,
     $                  WANTV, WANTW
...
C     .. Local Arrays ..
      LOGICAL           SELNEW(4), LDUM(1)
      DOUBLE PRECISION  DW12(12), Q(LDQZ,LDQZ), T(LDQZ,LDQZ), WINEW(4),
     $                  WRNEW(4), Z(LDQZ,LDQZ)
      INTEGER           IDUM(1)
C     .. External Functions ..
      LOGICAL           LSAME,LFDUM
      EXTERNAL          LSAME,LFDUM
C     .. External Subroutines ..
      EXTERNAL          DCOPY, DGEMM, DGEES, DLACPY, DLASET, DSCAL,
     $                  DTGPX2, DTRSEN, XERBLA
...
C
                  CALL DTGPX2( .TRUE., .TRUE., NBNEXT, NBF, A(POS,POS),
...
C
     ...
     $                  NBNEXT = 2
                  ...
                  CALL DLASET( 'All', NB, NB, ZERO, ONE, Z, LDQZ )
C
                  CALL DTGPX2( .TRUE., .TRUE., NBNEXT, 1, A(POS,POS),
     $                         LDA, B(POS,POS), LDB, Q, LDQZ, Z, LDQZ,
     $                         IERR )
C
                  IF ( IERR.NE.0 ) THEN
...
C
                        ...
                        CALL DLASET( 'All', NB, NB, ZERO, ONE, Z, LDQZ )
C
                        CALL DTGPX2( .TRUE., .TRUE., 2, 1, A(POS,POS),
     $                               LDA, B(POS,POS), LDB, Q, LDQZ, Z,
     $                               LDQZ, IERR )
C
                        IF ( IERR.NE.0 ) THEN
...
C
     ...
     $                         LDQZ )
                  CALL DLACPY( 'All', NBL, NBL, A(L,L), LDA, T(1,1),
     $                         LDQZ )
...
C
               IF ( K.NE.L ) THEN
                  CALL DLACPY( 'All', NBL, NB+NBL, Q(1,1), LDQZ,
     $                         Z(NB+1,1), LDQZ )
...
C
     ...
     $                        NB, ONE, B(1,K), LDB, Q(1,1), LDQZ, ZERO,
     $                        DWORK(PW), M )
...
C
     ...
     $                        ONE, Q(1,1), LDQZ, DWORK(PWDL+2*NBL), 2,
     $                        ZERO, DWORK(PW), 2 )
...
C
               ...
               IF ( LEN.GT.0 ) THEN
                  CALL DGEMM( 'Transpose', 'No Transpose', NB, LEN, NB,
     $                        ONE, Q(1,1), LDQZ, DWORK(PWCK+2*NB), 2,
     $                        ZERO, DWORK(PW), 2 )
                  CALL DGEMM( 'Transpose', 'No Transpose', NBL, LEN, NB,
...
C
                  CALL DGEMM( 'No Transpose', 'No Transpose', LEN, NB,
     $                        NB, ONE, W(M+POS,K), LDW, Q(1,1), LDQZ,
     $                        ZERO, DWORK(PW), M )
                  CALL DGEMM( 'No Transpose', 'No Transpose', LEN, NBL,
