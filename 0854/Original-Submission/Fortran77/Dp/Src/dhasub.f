      SUBROUTINE DHASUB( WHICH, METH, STAB, BALANC, ORTBAL, SELECT, N,
     $                   ILO, SCALE, S, LDS, T, LDT, G, LDG, U1, LDU1,
     $                   U2, LDU2, V1, LDV1, V2, LDV2, WR, WI, US, LDUS,
     $                   UU, LDUU, MM, M, LWORK, IWORK, DWORK, LDWORK,
     $                   INFO )
      IMPLICIT NONE
C
C     PURPOSE
C
C     Given the output of the routine DHAESU this routine computes
C     the stable and unstable invariant subspaces for a Hamiltonian
C     matrix with no eigenvalues on the imaginary axis.
C
C     ARGUMENTS
C
C     Mode Parameters
...
C             may result in more accurately computed invariant subspaces,
C             see [1].
C
C     STAB    CHARACTER*1
C             Specifies the type of invariant subspaces to be computed:
C             = 'S':   compute the stable invariant subspaces, i.e., the
C                      invariant subspace belonging to those selected
C                      eigenvalues that have negative real part;
C             = 'U':   compute the unstable invariant subspaces, i.e.,
...
C             DHAESU. Note that if the data is further post-processed,
...
C             factors, as returned by DHAESU.
...
C
C     U2      (input/output) DOUBLE PRECISION array, dimension (LDU1,N)
C             On entry, the leading N-by-N part of this array must
...
C             eigenvalues computed by DHAESU.
C
...
C                LDWORK >= 4*M*M + MAX( 8*M, 4*N, 1 ).
C             If WHICH = 'A' and METH = 'L' and
C                ( STAB = 'U' or STAB = 'S' ):
C                LDWORK >= MAX( 2*N*N + 2*N, 8*N+1 ).
...
C             = 2:  reordering of the product S*T in routine DLABMX
...
C                   DHAORD failed because some eigenvalues are too close
C                   to separate;
C
C     METHOD
C
...
C     .. Local Scalars ..
      ...
      INTEGER           I, IERR, J, K, PW, PDW, WRKMIN, WRKOPT
      DOUBLE PRECISION  TEMP
C     .. External Functions ..
      LOGICAL           LSAME
      EXTERNAL          LSAME
C     .. External Subroutines ..
      EXTERNAL          DAXPY, DGEMM, DGEQP3, DGEQRF, DHABAK, DHAORD,
     $                  DLABMX, DLACPY, DLASCL, DLASET, DORGQR, DSCAL,
     $                  DTRQML, XERBLA
...
C
         ...
         ELSE
            ...
            ELSE
               WRKOPT = MAX( WRKOPT, 2*N*N + 2*N, 8*N + 1 )
            END IF
         END IF
C
         IF ( N.LT.0 ) THEN
...
C
         PW  = 1
         PDW = PW + 4*M*M
         CALL DLABMX( 'No Update', 'Update', 'Update', 'Init', WHICH,
...
C
         PDW = PW + 2*M*M
         CALL DTRQML( 'Right', 'Upper', 'No Transpose', N, M, ONE,
...
C
         IF ( LBAL .AND. LBEF ) THEN
            IF ( LUS )
     $         CALL DHABAK( BALANC, 'Positive', N, ILO, SCALE, M, US,
     $                      LDUS, US(N+1,1), LDUS, IERR )
            IF ( LUU )
     $         CALL DHABAK( BALANC, 'Positive', N, ILO, SCALE, M, UU,
...
C
C           Workspace requirements: MAX( 2*N*N + 2*N, 8*N+1 )
C
            CALL DLABMX( 'Update', 'Update', 'Update', 'Init', WHICH,
     ...
     $         GO TO 250
C
            CALL DTRQML( 'Left', 'Lower', 'Transpose', N, N, ONE,
     $                   US(N+1,N+1), LDUS, G, LDG, DWORK, LDWORK,
     $                   IERR )
            WRKOPT = MAX( WRKOPT, INT( DWORK(1) ) )
C
            CALL DTRQML( 'Right', 'Lower', 'No Transpose', N, N, ONE,
...
C
            ...
            CALL DTRQML( 'Right', 'Upper', 'No Transpose', 2*N, N, -ONE,
...
C
            ...
            CALL DTRQML( 'Right', 'Upper', 'No Transpose', N, N, ONE,
...
C
  ...
  110            CONTINUE
C
            CALL DHAORD( 'Hamiltonian', 'Update', LWORK, LWORK(N+1), N,
...
C
         IF ( LBAL .AND. LBEF ) THEN
            IF ( LUS )
     $         CALL DHABAK( BALANC, 'Positive', N, ILO, SCALE, N, US,
     $                      LDUS, US(N+1,1), LDUS, IERR )
            IF ( LUU )
     $         CALL DHABAK( BALANC, 'Positive', N, ILO, SCALE, N, UU,
     $                      LDUU, UU(N+1,1), LDUU, IERR )
         END IF
C
