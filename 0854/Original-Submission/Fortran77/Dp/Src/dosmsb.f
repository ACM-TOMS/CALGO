...
C
      ...
      LTRQ = LSAME( TRANQ, 'T' )
      NB = ILAHAP( 1, 'DOSMSB', TRANC // TRAND // TRANQ, M, N, K )
C
...
C
         NX = MAX( 0, ILAHAP( 3, 'DOSMSB', TRANC // TRAND // TRANQ, M,
     $                        N, K ) )
         IF ( NX.LT.K ) THEN
C
...
C
     ...
     $                     - DBLE( 9*N ) ) / 30.0D0 )
               NBMIN = MAX( 2, ILAHAP( 2, 'DOSMSB', TRANC // TRAND //
     $                                 TRANQ, M, N, K ) )
            END IF
         END IF
      END IF
...
C              reflector SH.
C
               CALL DLAEST( 'Forward', STOREV, STOREW, M-I+1, IB,
