      SUBROUTINE DGESUB( TRANA, TRANB, N, ILO, A, LDA, B, LDB, G, LDG,
...
C     PURPOSE
C
C     Computes a symplectic URV (SURV) decomposition of a real 2N-by-2N
C     matrix H,
C
C            [ op(A)   G   ]                 [ op(R11)   R12   ]    
C        H = [             ] = U R V'  = U * [                 ] * V' ,
...
C     Hessenberg matrix and U, V are 2N-by-2N are orthogonal symplectic
...
C             to DHABAL; otherwise it should be set to 1.
...
C             the triangular matrix R11, and in the zero parts
...
C             On exit, the leading N-by-N part of this array contains
C             the Hessenberg matrix R22, and in the zero parts
C             information about the elementary reflectors used to
C             compute the SURV decomposition.
C
C     LDB     INTEGER
...
C     .. Local Scalars ..
     ...
     $                  PYA, PYB, PYG, PYQ, PXA, PXB, PXG, PXQ, WRKOPT
C     .. External Functions ..
      LOGICAL           LSAME
      INTEGER           ILAHAP
      EXTERNAL          ILAHAP, LSAME
C     .. External Subroutines ..
      EXTERNAL          DGEMM, DGESUV, DLASUB, XERBLA
...
C
      NB = ILAHAP( 1, 'DGESUB', TRANA // TRANB, N, ILO, -1 )
      NBMIN = 2
      WRKOPT = N
...
C
         NX = MAX( NB, ILAHAP( 3, 'DGESUB', TRANA//TRANB, N, ILO, -1 ) )
         IF ( NX.LT.NH ) THEN
C
...
C
               NBMIN = MAX( 2, ILAHAP( 2, 'DGESUB', TRANA // TRANB, N,
     $                                 ILO, -1 ) )
               NB = LDWORK / ( 16*N + 5 )
            END IF
         END IF
...
C
            CALL DLASUB( LTRA, LTRB, N-I+1, I-1, IB, A(I,1), LDA,
...
C           are needed to update the unreduced parts of the matrices.
C
            CALL DLASUB( LTRA, LTRB, N-I+1, I-1, IB, A(1,I), LDA,
     $                   B(1,I), LDB, G, LDG, Q(I,I), LDQ, DWORK(PXA),
     $                   N, DWORK(PXB), N, DWORK(PXG), N, DWORK(PXQ), N,
     $                   DWORK(PYA), N, DWORK(PYB), N, DWORK(PYG), N,
     $                   DWORK(PYQ), N, CSL(2*I-1), CSR(2*I-1), TAUL(I),
...
C
      CALL DGESUV( TRANA, TRANB, N, I, A, LDA, B, LDB, G, LDG, Q, LDQ,
     $             CSL, CSR, TAUL, TAUR, DWORK, LDWORK, IERR )
C
      DWORK(1) = DBLE( WRKOPT )
C
      RETURN
C *** Last line of DGESUB ***
      END
