      SUBROUTINE DLADFB( WANTT, WANTQ, WANTZ, N, ILO, IHI, ILOQ, IHIQ,
     $                   POS, A, LDA, B, LDB, Q, LDQ, Z, LDZ, INFO )
C
C     PURPOSE
C
C     DLADFB is an auxiliary routine called by DGEPQR and DLAPQR to deal
C     with zero elements on the diagonal of the triangular matrix B.
C
C     ARGUMENTS
C
C     Mode Parameters
...
C     METHOD
C
C     Illustrated by Wilkinson diagrams for N = 5, POS = 3:
C
...
C           [ o o o x x ]       [ o o o o x ]
C
C     Secondly, an RQ factorization is applied to A(4:6,4:6) and the
C     resulting nonzero in the updated matrix B is immediately
C     annihilated by a Givens rotation acting on rows 5 and 6:
C
C           [ x x x x x ]       [ x x x x x ]
C           [ x x x x x ]       [ o x x x x ]
C       A = [ o o x x x ],  B = [ o o o x x ]
...
C
      ...
      END IF
C
C     Apply an zero-shifted QR step.
C
      DO 10  J = ILO, POS-1
         TEMP = A(J,J)
   ...
   20 CONTINUE
C
C     Apply an zero-shifted RQ step.
C
      DO 30  J = IHI, POS+1, -1
         TEMP = A(J,J)
