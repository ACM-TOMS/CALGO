      SUBROUTINE DTGPY2( M, N, A, LDA, B, LDB, C, LDC, D, LDD, E, LDE,
...
C     A Schur approach for solving some periodic matrix equations, in
C     U. Helmke et al (eds), Systems and Networks : Mathematical Theory
C     and Applications, Akademie Verlag, Berlin, 1994, vol. 77,
C     pp 339-362.
C
C     CONTRIBUTORS
C
...
C
     ...
     $                       C(IS,JE+1), LDC )
                  CALL DGER( MB, N-JE, ONE, RHS(1), 1, E(JS,JE+1), LDB,
     $                       F(IS,JE+1), LDF )
               END IF
C
