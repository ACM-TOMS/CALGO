      SUBROUTINE DOSGSU( JOB, TRANS, N, ILO, Q1, LDQ1, Q2, LDQ2, CS,
     $                   TAU, DWORK, LDWORK, INFO )
      IMPLICIT NONE
C
C     PURPOSE
C
C     Generates orthogonal symplectic matrices U or V, defined as
...
C
C     as returned by DGESUV or DGESUB. The matrices U and V are returned
C     in terms of their first N/2 rows:
C
C                 [  U1   U2 ]           [  V1   V2 ]
C             U = [          ],      V = [          ].
C                 [ -U2   U1 ]           [ -V2   V1 ]
...
C             the argument TRANA in the previous call of DGESUV or
C             DGESUB.
C             If  JOB = 'V'  then TRANS must have the same value as
C             the argument TRANB in the previous call of DGESUV or
C             DGESUB.
C
C     N       (input) INTEGER
...
C             DGESUV or DGESUB. U and V are equal to the unit matrix
...
C
         IF ( NH.GT.0 ) THEN
            CALL DOSGSB( 'Transpose', 'No Transpose', NH, NH, NH,
     $                   Q1(ILO,ILO), LDQ1, Q2(ILO,ILO), LDQ2, CS(ILO),
     $                   TAU(ILO), DWORK, LDWORK, IERR )
         END IF
...
C
         NH = N - ILO
         IF ( NH.GT.0 ) THEN
            CALL DOSGSB( 'Transpose', 'Transpose', NH, NH, NH,
...
C
         IF ( NH.GT.0 ) THEN
            CALL DOSGSB( 'No Transpose', 'Transpose', NH, NH, NH,
     $                   Q1(ILO+1,ILO+1), LDQ1, Q2(ILO+1,ILO+1), LDQ2,
     $                   CS(ILO), TAU(ILO), DWORK, LDWORK, IERR )
         END IF
      END IF
      RETURN
C *** Last line of DOSGSU ***
      END
