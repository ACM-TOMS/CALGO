      SUBROUTINE DLASUB( LTRA, LTRB, N, K, NB, A, LDA, B, LDB, G, LDG,
...
C     PURPOSE
C
C     Reduces 2*NB columns and rows of a real (k+2n)-by-(k+2n) matrix H:
C
C             [ op(A)   G   ]
C         H = [             ],
C             [  Q    op(B) ]
...
C     transformations UU'*H*VV and matrices U, V, XA, YB, YG, YQ, XA,
C     XB, XG, and XQ are returned so that
C
C                    [ op(Aout)+U*XA'+XA*V'     G+U*YG'+XG*V'    ]
C         UU' H VV = [                                           ].
C                    [   Qout+U*YQ'+XQ*V'   op(Bout)+U*YB'+XB*V' ]
C
C     This is an auxiliary routine called by DGESUB.
C
C     ARGUMENTS
C
C     Mode Parameters
...
C             subdiagonal of in the first NB columns of op(A) are
...
C                     (LDA,N)     if LTRA = .F.
C                     (LDA,K+N)   if LTRA = .T.
C             On entry with LTRA = .F., the leading (K+N)-by-N part of
C             this array must contain the matrix A.
C             On entry with LTRA = .T., the leading N-by-(K+N) part of
C             this array must contain the matrix A.
C             On exit with LTRA = .F., the leading (K+N)-by-N part of
...
C             On exit with LTRA = .T., the leading N-by-(K+N) part of
C             this array contains the matrix Aout and in the zero parts
C             information about the elementary reflectors.
C
C     LDA     INTEGER
C             The leading dimension of the array A.
C             LDA >= MAX(1,K+N),  if LTRA = .F.;
C             LDA >= MAX(1,N),    if LTRA = .T..
C
C     B       (input/output) DOUBLE PRECISION array, dimension
C                     (LDB,K+N)   if LTRB = .F.
C                     (LDB,N)     if LTRB = .T.
C             On entry with LTRB = .F., the leading N-by-(K+N) part of
C             this array must contain the matrix B.
C             On entry with LTRB = .T., the leading (K+N)-by-N part of
C             this array must contain the matrix B.
C             On exit with LTRB = .F., the leading N-by-(K+N) part of
C             this array contains the matrix Bout and in the zero parts
C             information about the elementary reflectors used to
C             compute the reduction.
C             On exit with LTRB = .T., the leading (K+N)-by-N part of
C             this array contains the matrix Bout and in the zero parts
C             information about the elementary reflectors.
C
C     LDB     INTEGER
C             The leading dimension of the array B.
C             LDB >= MAX(1,K+N),  if LTRB = .F.;
C             LDB >= MAX(1,N),    if LTRB = .T..
C
...
C     Workspace
C
C     DWORK   DOUBLE PRECISION array, dimension (5*NB)C
C
C     METHOD
C
...
C     TAUL and TAUR see the description of DGESUB.
C
C     The contents of A, B, G and Q on exit are illustrated by the
C     following example with op(A) = A, op(B) = B, n = 5, k = 2 and
C     nb = 2:
C
C          ( a  r  r  a  a  )       ( g  g  g  r  r  g  g  g  )
C          ( a  r  r  a  a  )       ( g  g  g  r  r  g  g  g  )
C          ( r  r  r  r  r  )       ( r  r  r  r  r  r  r  r  )
C      A = ( u2 r  r  r  r  ),  G = ( r  r  r  r  r  r  r  r  ),
C          ( u2 u2 r  a  a  )       ( g  g  g  r  r  g  g  g  )
C          ( u2 u2 r  a  a  )       ( g  g  g  r  r  g  g  g  )
C          ( u2 u2 r  a  a  )       ( g  g  g  r  r  g  g  g  )
C
C          ( t  t  v1 v1 v1 )       ( r  r  r  r  r  v2 v2 v2 )
C          ( u1 t  t  v1 v1 )       ( r  r  r  r  r  r  v2 v2 )
C      Q = ( u1 u1 r  q  q  ),  B = ( b  b  b  r  r  b  b  b  ).
C          ( u1 u1 r  q  q  )       ( b  b  b  r  r  b  b  b  )
C          ( u1 u1 r  q  q  )       ( b  b  b  r  r  b  b  b  )
C
...
C     .. Scalar Arguments ..
     ...
     $                  LDYA, LDYB, LDYG, LDYQ, N, NB, NB1, NB2, NB3
C     .. Array Arguments ..
...
C
      IF ( LTRA.AND.LTRB ) THEN
         DO 90  I = 1, NB
C
C           Transform first row/column of A and Q. See routine DGESUV.
C
            ALPHA = Q(I,I)
            CALL DLARFG( N-I+1, ALPHA, Q(I+1,I), 1, Q(I,I) )
            TAUQ = Q(I,I)
            Q(I,I) = ONE
...
C
      ELSE IF ( LTRB ) THEN
         DO 270  I = 1, NB
C
C           Transform first columns of A and Q. See routine DGESUV.
C
            ALPHA = Q(I,I)
            CALL DLARFG( N-I+1, ALPHA, Q(I+1,I), 1, Q(I,I) )
            TAUQ = Q(I,I)
...
C
      ELSE
         DO 360  I = 1, NB
C
C           Transform first columns of A and B. See routine DGESUV.
C
            ALPHA = Q(I,I)
            CALL DLARFG( N-I+1, ALPHA, Q(I+1,I), 1, Q(I,I) )
            TAUQ = Q(I,I)
