      SUBROUTINE DHGPQR( JOB, COMPQ, COMPZ, N, ILO, IHI, A, LDA, B, LDB,
...
C     a product of matrices, H = A*B, with A upper Hessenberg and B upper
C     triangular without evaluating any part of the product.
...
C             part of this array contains the transformation matrices
...
C                   value;
C             > 0:  if INFO = i, then DHGPQR failed to compute the Schur
...
C     REFERENCES
C
C     [1] A. W. BOJANCZYK, G. H. GOLUB, and P. VAN DOOREN:
...
C     .. Local Scalars ..
      ...
      INTEGER            I, I1, I2, IERR, ITEMP, ITN, ITS, J, K, KK, L,
     $                   MAXB, NH, NR, NS, NV, PV2, PV3, DUM
...
C     .. External Functions ..
      LOGICAL            LSAME
      INTEGER            IDAMAX, ILAHAP
      DOUBLE PRECISION   DLAMCH, DLANHS
      EXTERNAL           DLAMCH, DLANHS, IDAMAX, ILAHAP, LSAME
C     .. External Subroutines ..
      EXTERNAL           DAXPY, DCOPY, DGEMV, DLABAD, DLACPY, DLADFB,
     $                   DLAPQR, DLARFG, DLARFX, DLARNV, DLASET, DSCAL,
     $                   DTRMV
...
C
      IF ( WANTQ ) THEN
         NS =   ILAHAP( 4, 'DHGPQR', JOB // COMPQ, N, ILO, IHI )
         MAXB = ILAHAP( 8, 'DHGPQR', JOB // COMPQ, N, ILO, IHI )
      ELSE
         NS =   ILAHAP( 4, 'DHGPQR', JOB // COMPZ, N, ILO, IHI )
         MAXB = ILAHAP( 8, 'DHGPQR', JOB // COMPZ, N, ILO, IHI )
      END IF
C
...
C
         CALL DLAPQR( WANTT, WANTQ, WANTZ, N, ILO, IHI, ILO, IHI, A,
...
C
            B(KK,KK) = ZERO
            CALL DLADFB( WANTT, WANTQ, WANTZ, N, K, I, ILO, IHI, KK,
...
C
     ...
     $                   LDBS )
            CALL DLAPQR( .FALSE., .FALSE., .FALSE., NS, 1, NS, 1, NS,
...
C
      CALL DLAPQR( WANTT, WANTQ, WANTZ, N, L, I, ILO, IHI, A, LDA, B,
