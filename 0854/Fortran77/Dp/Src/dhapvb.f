      SUBROUTINE DHAPVB( N, ILO, A, LDA, QG, LDQG, CS, TAU, DWORK,
     $                   LDWORK, INFO )
      IMPLICIT NONE
C
C     PURPOSE
...
C             previous call to DHABAL; otherwise it should be set to 1.
...
C     .. External Subroutines ..
      EXTERNAL          DGEMM, DHAPVL, DLAPVB, DSYR2K, XERBLA
...
C
         NX = MAX( NB, ILAHAP( 3, 'DHAPVB', ' ', N, ILO, -1 ) )
         IF ( NX.LT.NH ) THEN
C
...
C
               NBMIN = MAX( 2, ILAHAP( 2, 'DHAPVB', ' ', N, ILO, -1 ) )
               NB = LDWORK / ( 8*N + 3 )
            END IF
         END IF
      END IF
...
C
            CALL DLAPVB( .TRUE., N-I+1, I-1, IB, A(1,I), LDA, QG(1,I),
...
C
      CALL DHAPVL( N, I, A, LDA, QG, LDQG, CS, TAU, DWORK, LDWORK,
     $             IERR )
C
      DWORK( 1 ) = DBLE( WRKOPT )
C
      RETURN
C *** Last line of DHAPVB ***
      END
