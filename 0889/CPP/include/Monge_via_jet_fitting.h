...
// All rights reserved.
//
// This file has been extracted from CGAL and adapted as follows for the CALGO
// collection of algorithms: this file features a stand-alone C++
// template class, with all dependencies upon CGAL removed. See the
// accompanying ACM TOMS paper to perform an instantiation of class
// Monge_via_jet_fitting
//
//This file may be redistributed according to the CGAL license, which
//is the the Q Public License version 1.0.  See the file LICENSE.QPL
//distributed with CGAL.
//
// Licensees holding a valid CGAL commercial license may use this file in
...
//
// To retrieve the CGAL corresponding file:
// $URL: svn+ssh://fcazals@scm.gforge.inria.fr/svn/mp-papers/cvs05-diffProps_ridges_CGAL/code_jet_toms/include/Monge_via_jet_fitting.h $
// $Id: Monge_via_jet_fitting.h 375 2008-04-22 13:29:22Z fcazals $
//
...
template < class DataKernel, class LocalKernel, class KernelConverters, class SvdTraits>  
  class Monge_via_jet_fitting {
 public:
...
//continue main class Monge_via_jet_fitting ////////
 ...
 typedef  KernelConverters Kernel_converters;
 ...
 typedef typename Local_kernel::LKMatrix LKMatrix;
 typedef typename Local_kernel::Aff_transformation Aff_transformation;
 
 typedef typename Data_kernel::FT       DFT;
 typedef typename DataKernel::Vector_3  DVector_3;
 ...
 //use the orthonormal basis (vx,vy,vz) instead of PCA
 template <class InputIterator>
 Monge_form operator()(InputIterator begin, InputIterator end,  
		       size_t d, size_t dprime,
		       DVector_3 vx, DVector_3 vy, DVector_3 vz);
 ...
 template <class InputIterator>
  ...
  void switch_to_direct_orientation(Vector_3& v1, const Vector_3& v2,
    ...
    operator<<(std::ostream& out_stream, 
	       const typename Monge_via_jet_fitting<DataKernel, LocalKernel, KernelConverters, SvdTraits>::Monge_form& monge){
...
template < class DataKernel, class LocalKernel, class KernelConverters, class SvdTraits>  
  void Monge_via_jet_fitting<DataKernel, LocalKernel, KernelConverters, SvdTraits>::
  Monge_form::
  set_up(int degree) {
...
template < class DataKernel, class LocalKernel, class KernelConverters, class SvdTraits>  
void Monge_via_jet_fitting<DataKernel, LocalKernel, KernelConverters, SvdTraits>::Monge_form::
...
template < class DataKernel, class LocalKernel, class KernelConverters, class SvdTraits>  
  Monge_via_jet_fitting<DataKernel, LocalKernel, KernelConverters, SvdTraits>::
  Monge_via_jet_fitting()
{
...
template < class DataKernel, class LocalKernel, class KernelConverters, class SvdTraits> 
template <class InputIterator>
 typename  Monge_via_jet_fitting<DataKernel, LocalKernel, KernelConverters, SvdTraits>::Monge_form
  Monge_via_jet_fitting<DataKernel, LocalKernel, KernelConverters, SvdTraits>::
	     ...
	     size_t d, size_t dprime)
{
  ...
  assert( nb_input_pts >= nb_d_jet_coeff );
	     ...
	     DVector_3 vx, DVector_3 vy, DVector_3 vz)
{
  ...
  //Store the change of basis W->F, with the given coordinate system
  Aff_transformation 
    change_basis (vx.x(), vx.y(), vx.z(),
		  vy.x(), vy.y(), vy.z(),
		  vz.x(), vz.y(), vz.z());
...
compute_PCA(InputIterator begin, InputIterator end)
{
  ...
  for (; begin != end; begin++)
    {
      Point_3 lp = Kc.D2L_converter(*begin);
      x = lp.x();
      y = lp.y();
      z = lp.z();   
      sumX += x / n;
  ...
  LK.eigen_symmetric(covariance,3,eigen_vectors,eigen_values);
	    ...
	    int d, LAMatrix &M, LAVector& Z)
{
  ...
  Point_3 point0 = Kc.D2L_converter(*begin);
  ...
  Aff_transformation transl(1,0,0,v_point0_orig.x(), 
			    0,1,0,v_point0_orig.y(),
			    0,0,1,v_point0_orig.z());
  ...
  for (; begin != end; begin++) {
    Point_3 cur_pt = transf_points(Kc.D2L_converter(*begin));
  ...
  for (; itb != ite; itb++) precond += std::fabs(itb->x()) + std::fabs(itb->y());
...
compute_Monge_basis(const FT* A, Monge_form& monge_form)
{
  ...
  if ( this->deg_monge == 1 ) {  
    ...
    normal = normal / LK.Lsqrt(norm2);
    monge_form.origin() = Kc.L2D_converter(
      (this->translate_p0.inverse() * 
       this->change_world2fitting.inverse()) (orig_monge) );
    monge_form.normal_direction() = Kc.L2D_converter(this->change_world2fitting.inverse()(normal));
  }
  ...
  FT e = 1+A[1]*A[1], f = A[1]*A[2], g = 1+A[2]*A[2],
    l = A[3], m = A[4], n = A[5];
  LKMatrix  weingarten(2);
  typename LKMatrix::iterator Mit= weingarten.begin();
  *Mit = (g*l-f*m)/ (LK.Lsqrt(norm2)*norm2);
  *++Mit = (g*m-f*n)/ (LK.Lsqrt(norm2)*norm2);
  *++Mit = (e*m-f*l)/ (LK.Lsqrt(norm2)*norm2);
  *++Mit = (e*n-f*m)/ (LK.Lsqrt(norm2)*norm2);
  // Y, Z are normalized GramSchmidt of Xu, Xv
  // Xu->Y=Xu/||Xu||;
  // Xv->Z=Xv-(Xu.Xv)Xu/||Xu||^2;
  // Z-> Z/||Z||
  Vector_3 Y, Z;
  FT normXu = LK.Lsqrt( Xu*Xu );
  Y = Xu / normXu;
  FT XudotXv = Xu * Xv;
  Z = Xv - XudotXv * Xu / (normXu*normXu);
  FT normZ = LK.Lsqrt( Z*Z );
  Z = Z / normZ;
  LKMatrix change_XuXv2YZ(2);
  Mit= change_XuXv2YZ.begin();
  *Mit = 1 / normXu;
  *++Mit = -XudotXv / (normXu * normXu * normZ);
  *++Mit = 0;
  *++Mit = 1 / normZ;
  FT det = 0.;
  LKMatrix inv = LK.inverse ( change_XuXv2YZ, det );
  ...
  LK.eigen_symmetric(W,2,evec,eval);
  ...
  monge_form.origin() = Kc.L2D_converter(
    (this->translate_p0.inverse() * 
     this->change_world2fitting.inverse()) (orig_monge ));
  monge_form.maximal_principal_direction() = Kc.L2D_converter(this->change_world2fitting.inverse()(d_max));
  monge_form.minimal_principal_direction() = Kc.L2D_converter(this->change_world2fitting.inverse()(d_min));
  monge_form.normal_direction()  = Kc.L2D_converter(this->change_world2fitting.inverse()(normal));
  monge_form.coefficients()[0] = Kc.L2D_converter(eval[0]);
  monge_form.coefficients()[1] = Kc.L2D_converter(eval[1]);
  }
  //end else
}
...
/* 	    << A[10] << " "<< A[11] << " "<< A[12] << " "<< A[13]<< " " << A[14] << std::endl; */
  ...
  monge_form.coefficients()[2] = Kc.L2D_converter(b0);
  monge_form.coefficients()[3] = Kc.L2D_converter(b1);
  monge_form.coefficients()[4] = Kc.L2D_converter(b2);
  monge_form.coefficients()[5] = Kc.L2D_converter(b3);
  ...
  if ( dprime == 4 )
	...
	-1/(f3*f3*f3)*(f2222*(f3*f3)+3*f33*f22*f22-6*f223*f3*f22-4*f23*f3*f222+12*f23*f23*f22) ; 
      ...
      monge_form.coefficients()[6] = Kc.L2D_converter(c0);
      monge_form.coefficients()[7] = Kc.L2D_converter(c1);
      monge_form.coefficients()[8] = Kc.L2D_converter(c2);
      monge_form.coefficients()[9] = Kc.L2D_converter(c3);
      monge_form.coefficients()[10] = Kc.L2D_converter(c4);
    }
}
...
{
  LKMatrix M(3);
  typename LKMatrix::iterator Mit= M.begin();
  *Mit=v1[0];
  *++Mit=v2[0];
  *++Mit=v3[0];
  *++Mit=v1[1];
  ...
  *++Mit=v3[2];
  ...
  int orientation = LK.sign_of_determinant(M);
  if (orientation < 0) v1 = -v1;
}
