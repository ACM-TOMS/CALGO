...
*
*  T must be in Schur form (as returned by PDHSEQR), that is, block
...
*          Block parameters (some should be replaced by calls
*          to PILAENVX and others by meaningful default values
*          in future releases):
...
*  Modified by Meiyue Shao, July 2013.
*
*  Keywords
*  ========
*
...
*
      IF( INFO.EQ.0 ) THEN
         CALL CHK1MAT( N, 4, N, 4, IT, JT, DESCT, 8, INFO )
      END IF
      IF( INFO.EQ.0 ) THEN
         CALL CHK1MAT( N, 4, N, 4, IQ, JQ, DESCQ, 12, INFO )
      END IF
*
*     Check the blocking sizes for alignment requirements.
*
      IF( INFO.EQ.0 ) THEN
         IF( DESCT( MB_ ).NE.DESCT( NB_ ) ) INFO = -(800 + MB_)
      END IF
      IF( INFO.EQ.0 ) THEN
         IF( DESCQ( MB_ ).NE.DESCQ( NB_ ) ) INFO = -(1200 + MB_)
      END IF
      IF( INFO.EQ.0 ) THEN
         IF( DESCT( MB_ ).NE.DESCQ( MB_ ) ) INFO = -(1200 + MB_)
      END IF
*
...
*
      IF( INFO.EQ.0 ) THEN
         CALL PCHK1MAT( N, 4, N, 4, IT, JT, DESCT, 8, 0, IDUM1,
     $        IDUM2, INFO )
      END IF
      IF( INFO.EQ.0 ) THEN
         CALL PCHK1MAT( N, 4, N, 4, IQ, JQ, DESCQ, 12, 0, IDUM1,
     $        IDUM2, INFO )
      END IF
      IF( INFO.EQ.0 ) THEN
         CALL PCHK2MAT( N, 4, N, 4, IT, JT, DESCT, 8, N, 4, N, 4,
     $        IQ, JQ, DESCQ, 12, 0, IDUM1, IDUM2, INFO )
      END IF
*
...
*
            ...
            NWIN = 2*NB-1
            LWMIN = NB + NWIN*( 5 + 2*NWIN + TROWS + TCOLS )
     $        + MAX( 2, NWIN*MAX( TROWS, TCOLS ) )
            IF( WANTQ ) LWMIN = LWMIN + TROWS*NWIN
...
*                                prepare to leave in a clean fashion.
*                                If IERR.EQ.2, we must update SELECT to
...
*
     ...
     $                    NWIN )
                     IF( RSRC1.NE.RSRC4 .OR. CSRC1.NE.CSRC4 ) THEN
*                       Proc#1 <==> Proc#4
c                        CALL DGESD2D( ICTXT, DIM1, DIM1,
c     $                       WORK(IPW2), NWIN, RSRC4, CSRC4 )
...
*                       Proc#4 <==> Proc#1
     ...
     $                       CSRC1 )
c                        CALL DGERV2D( ICTXT, DIM1, DIM1,
c     $                       WORK(IPW2), NWIN, RSRC1, CSRC1 )
                     END IF
                  END IF
                  IF( MYROW.EQ.RSRC2 .AND. MYCOL.EQ.CSRC2 ) THEN
                     ILOC = INDXG2L( I, NB, MYROW, DESCT( RSRC_ ),
...
*                       Proc#2 ==> Proc#1
     ...
     $                       WORK(IPW2+DIM1*NWIN), NWIN, RSRC1, CSRC1 )
                     END IF
                  END IF
c                  IF( MYROW.EQ.RSRC2 .AND. MYCOL.EQ.CSRC2 ) THEN
c                     IF( RSRC2.NE.RSRC4 .OR. CSRC2.NE.CSRC4 ) THEN
c*                       Proc#2 ==> Proc#4
c                        CALL DGESD2D( ICTXT, DIM1, DIM4,
c     $                       WORK(IPW2+DIM1*NWIN), NWIN, RSRC4, CSRC4 )
c                     END IF
c                  END IF
                  IF( MYROW.EQ.RSRC3 .AND. MYCOL.EQ.CSRC3 ) THEN
                     ILOC = INDXG2L( I+DIM1, NB, MYROW, DESCT( RSRC_ ),
...
*                       Proc#3 ==> Proc#1
     ...
     $                       RSRC1, CSRC1 )
                     END IF
                  END IF
c                  IF( MYROW.EQ.RSRC3 .AND. MYCOL.EQ.CSRC3 ) THEN
c                     IF( RSRC3.NE.RSRC4 .OR. CSRC3.NE.CSRC4 ) THEN
c*                       Proc#3 ==> Proc#4
c                        CALL DGESD2D( ICTXT, 1, 1,
c     $                       WORK(IPW2+(DIM1-1)*NWIN+DIM1), NWIN,
c     $                       RSRC4, CSRC4 )
c                     END IF
c                  END IF
                  IF( MYROW.EQ.RSRC1 .AND. MYCOL.EQ.CSRC1 ) THEN
...
*                       Proc#1 <== Proc#3
     ...
     $                       RSRC3, CSRC3 )
                     END IF
                  END IF
c                  IF( MYROW.EQ.RSRC4 .AND. MYCOL.EQ.CSRC4 ) THEN
c                     IF( RSRC4.NE.RSRC2 .OR. CSRC4.NE.CSRC2 ) THEN
c*                       Proc#4 <== Proc#2
c                        CALL DGERV2D( ICTXT, DIM1, DIM4,
c     $                       WORK(IPW2+DIM1*NWIN), NWIN, RSRC2,
c     $                       CSRC2 )
c                     END IF
c                     IF( RSRC4.NE.RSRC3 .OR. CSRC4.NE.CSRC3 ) THEN
c*                       Proc#4 <== Proc#3
c                        CALL DGERV2D( ICTXT, 1, 1,
c     $                       WORK(IPW2+(DIM1-1)*NWIN+DIM1), NWIN,
c     $                       RSRC3, CSRC3 )
c                     END IF
c                  END IF
...
*
 ...
 330                 CONTINUE
*
*                    Make sure that Proc#1 and Proc#4 have exactly the
*                    same data.
*
                     IBUFF( 1 ) = I
                     IBUFF( 2 ) = NWIN
                     ...
                     IF(RSRC1.NE.RSRC4 .OR. CSRC1.NE.CSRC4) THEN
                        IF(MYROW.EQ.RSRC1 .AND. MYCOL.EQ.CSRC1) THEN
                           CALL DGESD2D( ICTXT, NWIN, NWIN,
     $                          WORK(IPW2), NWIN, RSRC4, CSRC4 )
                           CALL IGESD2D( ICTXT, NWIN, 1,
     $                          SELECT(I), NWIN, RSRC4, CSRC4 )
                           CALL IGESD2D( ICTXT, 8, 1,
     $                          IBUFF, 8, RSRC4, CSRC4 )
                           IF(ILEN.GT.0)
     $                          CALL IGESD2D( ICTXT, ILEN, 1,
     $                               IWORK(IPIW), ILEN, RSRC4, CSRC4 )
                           IF(DLEN.GT.0)
     $                          CALL DGESD2D( ICTXT, DLEN, 1,
     $                               WORK(IPW3), DLEN, RSRC4, CSRC4 )
                        END IF
                        IF(MYROW.EQ.RSRC4 .AND. MYCOL.EQ.CSRC4) THEN
                           CALL DGERV2D( ICTXT, NWIN, NWIN,
     $                          WORK(IPW2), NWIN, RSRC1, CSRC1 )
                           CALL IGERV2D( ICTXT, NWIN, 1,
     $                          SELECT(I), NWIN, RSRC1, CSRC1 )
                           CALL IGERV2D( ICTXT, 8, 1,
     $                          IBUFF, 8, RSRC1, CSRC1 )
                           PITRAF = IBUFF( 3 )
                           KS = IBUFF( 4 )
                           PDTRAF = IBUFF( 5 )
                           ...
                           PDTRAF = IPW3 + DLEN - 1
                           IF(ILEN.GT.0)
     $                          CALL IGERV2D( ICTXT, ILEN, 1,
     $                               IWORK(IPIW), ILEN, RSRC1, CSRC1 )
                           IF(DLEN.GT.0)
     $                          CALL DGERV2D( ICTXT, DLEN, 1,
     $                               WORK(IPW3), DLEN, RSRC1, CSRC1 )
                        END IF
                     END IF
                  END IF
 360              CONTINUE
