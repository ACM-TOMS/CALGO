...
*
*       R. Granat, B. Kagstrom, D. Kressner, and M. Shao,
*       Parallel Library Software for the Multishift QR Algorithm
*       with Aggressive Early Deflation.
*       ACM Trans. Math. Software, 2014 (to appear).
*
*     ================================================================
*
*     .. Parameters ..
...
*     .. Local Scalars ..
     ...
     $                   ELEM1, ELEM2, ELEM3
      INTEGER            I, J, IERR, IT, ITMAX, K, KACC22, KBOT, KS,
     $                   KT, KTOP, KV, KWTOP, KWV, LD, LS,
     ...
     $                   WCOLS, HRSRC, HCSRC, NB, NPROCS, IROFFH,
     $                   ICOFFH, NWIN, JW, LWK1, LWK2, LWK3, LIWKOPT,
...
*     .. External Subroutines ..
     ...
     $                   PDELGET, DLAQR0, DLASET, PDGEMR2D, BLACS_GET,
     $                   BLACS_GRIDINFO, BLACS_GRIDMAP, BLACS_GRIDEXIT,
     $                   IGAMX2D
*     ..
*     .. Intrinsic Functions ..
...
*        Estimate optimal workspace
*
*        Workspace query call to PDLAQR0/PDLAQR1
*
         CALL PDLAQR1( WANTT, WANTZ, N, ILO, IHI, H, DESCH, WR, WI,
     $        ILOZ, IHIZ, Z, DESCZ, WORK, -1, IWORK, -1, INFO )
         LWK1 = INT( WORK( 1 ) )
         LIWKOPT = IWORK( 1 )
         IF( RECLEVEL.LT.RECMAX ) THEN
            CALL PDLAQR0( WANTT, WANTZ, N, ILO, IHI, H, DESCH, WR, WI,
     $           ILOZ, IHIZ, Z, DESCZ, WORK, -1, IWORK, -1, INFO,
     $           RECLEVEL+1 )
            LWK1 = MAX( LWK1, INT( WORK( 1 ) ) )
            LIWKOPT = MAX( LIWKOPT, IWORK( 1 ) )
         END IF
*
*        Extra workspace for Aggressive Early Deflation
*
         LWK2 = 3*ICEIL(NWR,NPROW)*ICEIL(NWR,NPCOL)
*
...
*
     ...
     $        DESCH, ILOZ, IHIZ, Z, DESCZ, WORK, -1, IWORK, -1 )
*
*        Optimal workspace = MAX(PDLAQR3, PDLAQR5)
*
         LWK3 = MAX( LWK3, INT( WORK( 1 ) ) )
         LWKOPT = MAX( LWK1, LWK2+LWK3 )
         LIWKOPT = MAX( LIWKOPT, IWORK( 1 ) )
*
...
*
     ...
     $                          LWORK-IPWRK+1, IWORK, LIWORK, IERR )
                        END IF
                     ELSE
*
...
*
 ...
 50                     CONTINUE
     ...
     $                       MYROW_NEW, MYCOL_NEW )
c                        IF( MYROW.GE.NPMIN .OR. MYCOL.GE.NPMIN )
c     $                     ICTXT_NEW = -1
*
...
*
c
c   This part is still not perfect due to restrictions in BLACS.
c   Either PDLAQR0 or PDLAQR1 can work, but not both.
c   The inactive branch will be restored once more flexibility
c   regarding the processor grid is introduced in future BLACS
c   implementations.
c
...
c                           END IF
 ...
 60                        CONTINUE
                        END IF
                        CALL IGAMX2D( ICTXT, 'All', '1-Tree', 1, 1,
     $                       IERR, 1, -1, -1, -1, -1, -1 )
...
*              If there aren't NS shifts available, then
*              use them all, possibly dropping one to make
*              the number of shifts even.
*
               NS = MIN( NS, KBOT-KS+1 )
               NS = NS - MOD( NS, 2 )
               KS = KBOT - NS + 1
