...
*     .. Local Scalars ..
     ...
     $                   HRSRC4, HCSRC4, LWKOPT, LIWKOPT, IERR
...
*
     ...
     $     ILO, IHI, Z, DESCZ, WORK, -1, IWORK, -1, IERR )
      LWKOPT = INT( WORK(1) )
      LIWKOPT = IWORK(1)
      CALL PDLAQR0( WANTT, WANTZ, N, ILO, IHI, H, DESCH, WR, WI,
     $     ILO, IHI, Z, DESCZ, WORK, -1, IWORK, -1, IERR, 0 )
      IF( N.LT.NL ) THEN
      ...
      IWORK(1) = LIWKOPT
*
      IF( .NOT.LQUERY .AND. LWORK.LT.LWKOPT ) THEN
         INFO = -13
...
*
     ...
     $                       CS, SN, WORK, LWORK, IERR )
                     CALL PDROT( I-1, H, 1, I, DESCH, 1, H, 1, I+1,
     $                    DESCH, 1, CS, SN, WORK, LWORK, IERR )
                  END IF
                  IF( WANTZ ) THEN
                     CALL PDROT( IHI-ILO+1, Z, ILO, I, DESCZ, 1,
     $                    Z, ILO, I+1, DESCZ, 1, CS, SN, WORK, LWORK,
     $                    IERR )
                  END IF
...
*
         ...
         DO 50 K = ILO, IHI
            IF( .NOT. PAIR ) THEN
               BORDER = ( K.NE.IHI .AND. MOD( K, NB ).EQ.0 ) .OR.
     $              ( K.NE.ILO .AND. MOD( K, NB ).EQ.1 )
               IF( .NOT. BORDER ) THEN
