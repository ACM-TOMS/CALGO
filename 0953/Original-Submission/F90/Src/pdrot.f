...
*
      LEFT  = INCX.GE.DESCX(M_) .AND. INCY.GE.DESCY(M_)
...
*
     ...
     $                    X((XLOC2-1)*LLDX+XLOC1), LLDX, RSRC2, CSRC )
     ...
     $                    MYROW, MYCOL, YLOC1, YLOC2, RSRC, CSRC )
                     CALL DROT( XYCOLS, X((XLOC2-1)*LLDX+XLOC1),
     $                    LLDX, Y((YLOC2-1)*LLDY+YLOC1), LLDY, CS, SN )
                  END IF
               END IF
            END IF
            IF( MYROW.EQ.RSRC2 .AND. RSRC1.NE.RSRC2 .AND. XYCOLS.GT.0 )
     $           THEN
               IF( INDX.EQ.1 ) THEN
                  JYY = JY
               ELSE
     ...
     $              MYCOL, YLOC1, YLOC2, RSRC, CSRC )
               IF( MYROW.EQ.RSRC .AND. MYCOL.EQ.CSRC ) THEN
                  CALL DGESD2D( ICTXT, 1, XYCOLS,
     $                 Y((YLOC2-1)*LLDY+YLOC1), LLDY, RSRC1, CSRC )
                  CALL DGERV2D( ICTXT, 1, XYCOLS, WORK, 1, RSRC1, CSRC )
 ...
 10      CONTINUE
     ...
     $                    X((XLOC2-1)*LLDX+XLOC1), LLDX, RSRC, CSRC2 )
     ...
     $                    MYROW, MYCOL, YLOC1, YLOC2, RSRC, CSRC )
                     CALL DROT( XYROWS, X((XLOC2-1)*LLDX+XLOC1),
     $                    1, Y((YLOC2-1)*LLDY+YLOC1), 1, CS, SN )
                  END IF
               END IF
            END IF
            IF( MYCOL.EQ.CSRC2 .AND. CSRC1.NE.CSRC2 .AND. XYROWS.GT.0)
     $           THEN
               IF( INDX.EQ.1 ) THEN
                  IYY = IY
               ELSE
     ...
     $              MYCOL, YLOC1, YLOC2, RSRC, CSRC )
               IF( MYROW.EQ.RSRC .AND. MYCOL.EQ.CSRC ) THEN
                  CALL DGESD2D( ICTXT, XYROWS, 1,
     $                 Y((YLOC2-1)*LLDY+YLOC1), LLDY, RSRC, CSRC1 )
                  CALL DGERV2D( ICTXT, XYROWS, 1, WORK, XYROWS,
     $                 RSRC, CSRC1 )
