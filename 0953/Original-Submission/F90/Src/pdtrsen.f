...
*                                                                     *
*     pdtrsen.f:                                                      *
*         Computational routine in the package PDHSEQR.               *
*                                                                     *
...
*  library is needed for condition estimation. This functionality will
*  be activated in future releases combining SCASY and PDHSEQR.
*
*  T must be in Schur form (as returned by PDHSEQR/PDLAHQR), that is,
...
*  PDHSEQR/PDLAHQR does not compute Schur forms of submatrices anyway.
*
*  References
*  ==========
*
...
*     .. Local Scalars ..
     ...
     $                   NB, NWIN, NOEXSY, NPCOL, NPROCS, NPROW, SPACE,
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, CHK1MAT, DESCINIT, IGAMX2D,
     $                   IGAMN2D, INFOG2L, PDLACPY, PDTRORD, PXERBLA,
     $                   PCHK1MAT, PCHK2MAT
*     $                   , PGESYCTD, PSYCTCON
*     ..
*     .. Intrinsic Functions ..
...
*
      IF( INFO.EQ.0 ) THEN
         IF( DESCT( MB_ ).NE.DESCT( NB_ ) ) INFO = -(900 + MB_)
      END IF
      IF( INFO.EQ.0 ) THEN
         IF( DESCQ( MB_ ).NE.DESCQ( NB_ ) ) INFO = -(1300 + MB_)
      END IF
      IF( INFO.EQ.0 ) THEN
         IF( DESCT( MB_ ).NE.DESCQ( MB_ ) ) INFO = -(1300 + MB_)
      END IF
*
...
*
            ...
            NWIN = 2*NB-1
            WRK3 = NB + NWIN*( 5 + 2*NWIN + TROWS + TCOLS )
     $        + MAX( 2, NWIN*MAX( TROWS, TCOLS ) )
            IF( WANTQ ) WRK3 = WRK3 + TROWS*NWIN
