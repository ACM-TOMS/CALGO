...
*        Algorithm.
*        In Applied Parallel and Scientific Computing (PARA 2010),
*        K. Jonasson, Editor, LNCS-7133, Springer-Verlag, 1--10, 2012.
*
*  ================================================================
*     .. Parameters ..
...
*     .. Local Scalars ..
     ...
     $                   INFO, II, IROW, IROW1, ITMP1, ITMP2, JAFIRST,
     $                   JJ, L, LDA, LDZ, LLDTMP, MYCOL, MYROW, NODE,
     $                   NPCOL, NPROW, DBLK, HSTEP, VSTEP, KKROW, KKCOL,
     $                   KLN, LTOP, LEFT, RIGHT, UP, DOWN, D1, D2
*     ..
*     .. Local Arrays ..
...
*
     ...
     $        DBLK, WORK( 3*DBLK*DBLK+1 ), LWORK-3*DBLK*DBLK )
         IF( NPROW*NPCOL.GT.1 ) THEN
            ...
            CALL DGEBS2D( CONTXT, 'All', ' ', DBLK, 1, WR, DBLK )
            CALL DGEBS2D( CONTXT, 'All', ' ', DBLK, 1, WI, DBLK )
            CALL IGEBS2D( CONTXT, 'All', ' ', 1, 1, ND, 1 )
            CALL IGEBS2D( CONTXT, 'All', ' ', 1, 1, NS, 1 )
         END IF
      ELSE
         CALL DGEBR2D( CONTXT, 'All', ' ', DBLK, DBLK, V, LDV, II, JJ )
         CALL DGEBR2D( CONTXT, 'All', ' ', DBLK, 1, WR, DBLK, II, JJ )
         CALL DGEBR2D( CONTXT, 'All', ' ', DBLK, 1, WI, DBLK, II, JJ )
         CALL IGEBR2D( CONTXT, 'All', ' ', 1, 1, ND, 1, II, JJ )
         CALL IGEBR2D( CONTXT, 'All', ' ', 1, 1, NS, 1, II, JJ )
      END IF
*
      IF( ND .GT. 0 ) THEN
*
