...
*  =======
*
*  dORQLx generates an M-by-N real matrix Q with orthonormal columns,
...
*  =======
*
*  dORQRx generates an M-by-N real matrix Q with orthonormal columns,
...
*  =======
*
*  dORGTx generates a real orthogonal matrix Q which is defined as the
...
*
*  A       (input/output) dPreicision array, dimension (LDA,N)
C*  A       (input/output) DOUBLE PRECISION array, dimension (LDA,N)
...
*
*  TAU     (input) dPreicision array, dimension (N-1)
...
C      SUBROUTINE DSBTRD( VECT, UPLO, N, KD, AB, LDAB, D, E, Q, LDQ,
      SUBROUTINE DSBTRX( VECT, UPLO, N, KD, AB, LDAB, D, E, Q, LDQ,
     $                   WORK, INFO )
*
*  -- LAPACK routine (version 3.0) --
*     Univ. of Tennessee, Univ. of California Berkeley, NAG Ltd.,
...
*  =======
*
*  dSYTRx reduces a real symmetric matrix A to real symmetric
...
*
     ...
     >                          SZU = NMAX * NMAX ,
     >                          SZWORK = NMAX * NMAX  )
        DOUBLE PRECISION      A( SZA ), ACPY( SZACPY ), U( SZU ),
...
*          --- I/O control ---
*
        INTEGER               IUNIT, OUNIT, OUNITF, I1MACH
...
*
*   sytrd   tests for dsytrd (full -> tridiagonal, LAPACK)
*   orgtr   tests for dorgtr (backward accumulation, LAPACK)
*   sbtrd   tests for dsbtrd (banded -> tridiagonal, LAPACK)
*   syrdb   tests for dsyrdb (full -> banded, SBR)
*   sygtr   tests for dsygtr (backward accumulation, SBR)
*   sbrdb   tests for dsbrdb (banded -> banded, SBR)
*   sbrdt   tests for dsbrdt (banded -> tridiagonal, SBR)
*   sy2bc   tests for dsy2bc (full -> banded copy, SBR)
*   sy2bi   tests for dsy2bi (full -> banded repacking, SBR)
*   sb2bc   tests for dsb2bc (banded -> banded copy, SBR)
*   sb2bi   tests for dsb2bi (banded -> banded repacking, SBR)
*   syrdd   tests for dsyrdd (reduction driver for full matrices, SBR)
*   sbrdd   tests for dsbrdd (reduction driver for banded matrices, SBR)
*   total   all tests
...
*   nx1     cross-over point for LAPACK tridiagonalization dsytrd
*   nx2     cross-over point for LAPACK backward accumulation dorgtr
...
*
        CHARACTER             CHECK
        DOUBLE PRECISION      ORTHU, SORTHU, TORTHU, RES, SRES, TRES,
     >                        DEVLS, SDEVLS, TDEVLS
...
*
        ...
        EXTERNAL              SECOND, D1MACH, LSAME, LXSAME
*
*   dsecnd  timer (LAPACK)
...
*
        EXTERNAL              DLACPY, DORGTX, DSB2BC, DSB2BI, DSBRDB,
     >                        DSBRDD, DSBRDT, DSBTRX, DSY2BC, DSY2BI,
     >                        DSYGTR, DSYINI, DSYNCK, DSYRDB, DSYRDD,
     >                        DSYTRX
*
*   dlacpy  copy matrix (LAPACK)
*   dorgtx  backward accumulation of transformations (LAPACK, modified)
*   dsb2bc  banded -> banded copy (SBR)
*   dsb2bi  banded -> banded repacking (SBR)
*   dsbrdb  banded -> banded reduction (SBR)
*   dsbrdd  reduction driver for banded matrices (SBR)
*   dsbrdt  banded -> tridiagonal reduction (SBR)
*   dsbtrx  banded -> tridiagonal reduction (LAPACK, modified)
*   dsy2bc  full -> banded copy (SBR)
*   dsy2bi  full -> banded repacking (SBR)
*   dsygtr  backward accumulation of transformations (SBR)
*   dsyini  initialize symmetric (full or banded) matrix
*   dsynck  numerical checks for the reduction
*   dsyrdb  full -> banded reduction (SBR)
*   dsyrdd  reduction driver for full matrices (SBR)
*   dsytrx  full -> tridiagonal reduction (LAPACK, modified)
*
        INTRINSIC             MAX, MIN
*
...
*
*       EPS = DLAMCH( 'Epsilon' )
*       NOTUSD = ONE / DLAMCH( 'SafeMinimum' )
        EPS = D1MACH(4)
	NOTUSD = ONE /(D1MACH(1)*(ONE+EPS))
*
...
*
          INFO = 0
          BTIME = SECOND()
          CALL DSYINI( MTYPE, EVLS, NMAX, DIAM,
...
*
          ...
          IF ( PRBLM .EQ. SYTRD ) THEN
            CALL DSYTRX( NB( 1 ), NX1, UPLO, N, A, LDA1, D, E,
     >                   TAU, WORK, LWORK1, INFO )
          ELSEIF ( PRBLM .EQ. SBTRD ) THEN
            CALL DSBTRX( JOBU, UPLO, N, B1, A, LDA1, D, E,
     >                   U, LDU, WORK, INFO )
          ELSEIF ( PRBLM .EQ. SYRDB ) THEN
            CALL DSYRDB( UPLO, JOBU, N, B2, A, LDA1, DRPTOL,
     >                   U, LDU, NB( 1 ), TAU, WORK, LWORK1, INFO )
          ELSEIF ( PRBLM .EQ. SBRDB ) THEN
            CALL DSBRDB( JOBU, N, B1, B2, A, LDA1, DRPTOL,
     >                   U, LDU, NB( 1 ), WORK, LWORK1, INFO )
          ELSEIF ( PRBLM .EQ. SBRDT ) THEN
            CALL DSBRDT( JOBU, N, B1, A, LDA1, DRPTOL, D, E,
     >                   U, LDU, NB( 1 ), WORK, LWORK1, INFO )
          ELSEIF ( PRBLM .EQ. SY2BC ) THEN
            CALL DSY2BC( UPLO, N, B1, A, LDA1, ACPY, LDA2, INFO )
          ELSEIF ( PRBLM .EQ. SY2BI ) THEN
            CALL DSY2BI( UPLO, N, B1, A, LDA1, LDA2, INFO )
          ELSEIF ( PRBLM .EQ. SB2BC ) THEN
            CALL DSB2BC( UPLO, N, B1, A, LDA1, ACPY, LDA2, INFO )
          ELSEIF ( PRBLM .EQ. SB2BI ) THEN
            CALL DSB2BI( UPLO, N, B1, A, LDA1, LDA2, INFO )
          ELSEIF ( PRBLM .EQ. SYRDD ) THEN
            CALL DSYRDD( JOBU, UPLO, N, B2, A, LDA1, DRPTOL,
     >                   ACPY, LDA2, D, E, NSTEPS, B, NB,
     >                   WORK, LWORK1, STEP, INFO )
          ELSEIF ( PRBLM .EQ. SBRDD ) THEN
            CALL DSBRDD( JOBU, UPLO, N, B1, B2, A, LDA1, DRPTOL,
...
*
            CALL DLACPY( UPLO, N, N, A, LDA1, U, LDU )
*
...
*
            ...
            INFO = 0
            BTIME = SECOND()
            CALL DORGTX( NB( 2 ), NX2, UPLO, N, U, LDU, TAU,
     >                   WORK, LWORK2, INFO )
            CTIME = SECOND() - BTIME
            RTIME = RTIME + CTIME
*
...
*
            IF ( INFO .NE. XINFO2 ) THEN
              ...
              WRITE( OUNIT, 9901 ) LINE0, 'dorgtr', INFO, XINFO2
              GOTO 3000
...
*
            ...
            INFO = 0
            BTIME = SECOND()
            CALL DSYGTR( UPLO, N, B2, U, LDU, TAU, WORK, LWORK2, INFO )
            CTIME = SECOND() - BTIME
            RTIME = RTIME + CTIME
*
*              --- protocol output, if required ---
...
*
            IF ( INFO .NE. XINFO2 ) THEN
              ...
              WRITE( OUNIT, 9901 ) LINE0, 'dsygtr', INFO, XINFO2
              GOTO 3000
...
*
            IF ( PRBLM .EQ. SYRDB ) THEN
              INFO = 0
              BTIME = SECOND()
              CALL DSY2BC( UPLO, N, B2, A, LDA1, ACPY, LDA2, INFO )
              CTIME = SECOND() - BTIME
              IF ( OLEVEL .GE. OUTDET ) THEN
     ...
     >               ( ( PRBLM .EQ. SBRDD ) .AND. ( B2 .GT. 1 ) ) ) THEN
              INFO = 0
              BTIME = SECOND()
              CALL DSB2BC( 'Lower', N, B2, A, LDA1, ACPY, LDA2, INFO )
              CTIME = SECOND() - BTIME
              IF ( OLEVEL .GE. OUTDET ) THEN
     ...
     >      THEN
              INFO = 0
              BTIME = SECOND()
              CALL DSB2BC( 'Lower', N, B2, A, LDA2, ACPY, LDA2, INFO )
              CTIME = SECOND() - BTIME
              IF ( OLEVEL .GE. OUTDET ) THEN
...
*
              CALL DLACPY( 'Full', N, N, A, LDA1, U, LDU )
            ENDIF
*
...
*
            INFO = 0
            BTIME = SECOND()
            CALL DSYNCK( EVLS, FRMT1, UPLO, N, B1, A, LDA1, SZA,
...
*
 9500   FORMAT( 1X, '  dsyini: mtype=', A1, ', szevls=', I5,
...
*
 9510   FORMAT( 1X, '  dsytrd: uplo=', A1, ', n=', I5, ', lda=', I5,
     >          ', nb0=', I5, ', nx0=', I5, ', lwork=', I8, ',' )
 9515   FORMAT( 1X, 'dsytrd: uplo=', A1, ', n=', I5, ', nb=', I3,
     >          ', info=', I3, ', time=', F9.3 )
*
 9520   FORMAT( 1X, '  dsbtrd: vect=', A1, ', uplo=', A1, ', n=', I5,
     >          ', kd=', I5, ', ldab=', I5, ', ldu=', I5, ',' )
 9525   FORMAT( 1X, 'dsbtrd: uplo=', A1, ', jobu=', A1, ', n=', I5,
     >          ', b1=', I3, ', info=', I3, ', time=', F9.3 )
*
 9530   FORMAT( 1X, '  dsyrdb: uplo=', A1, ', job=', A1, ', n=', I5,
     >          ', b=', I5, ', lda=', I5, ', drptol=', E11.4, ',' )
 9531   FORMAT( 1X, '          ldu=', I5, ', nb=', I5, ', lwork=', I8,
     >          ',' )
 9535   FORMAT( 1X, 'dsyrdb: uplo=', A1, ', jobu=', A1, ', n=', I5,
     >          ', b2=', I3, ', nb=', I3, ', info=', I3,
     >          ', time=', F9.3 )
*
 9540   FORMAT( 1X, '  dsbrdb: job=', A1, ', n=', I5, ', b1=', I5,
     >          ', b2=', I5, ', lda=', I5, ', drptol=', E11.4, ',' )
 9545   FORMAT( 1X, 'dsbrdb: jobu=', A1, ', n=', I5, ', b1=', I3,
     >          ', b2=', I3, ', nb=',I3, ', info=', I3,
     >          ', time=', F9.3 )
*
 9550   FORMAT( 1X, '  dsbrdt: job=', A1, ', n=', I5, ', b=', I5,
     >          ', lda=', I5, ', drptol=', E11.4, ',' )
 9555   FORMAT( 1X, 'dsbrdt: jobu=', A1, ', n=', I5, ', b1=', I3,
     >          ', nb=', I3, ', info=', I3, ', time=', F9.3 )
*
 9560   FORMAT( 1X, '  dsy2bc: uplo=', A1, ', n=', I5, ', b=', I5,
     >          ', ldfull=', I5, ', ldband=', I5, ',' )
*
 9570   FORMAT( 1X, '  dsy2bi: uplo=', A1, ', n=', I5, ', b=', I5,
     >          ', ldfull=', I5, ', ldband=', I5, ',' )
*
 9580   FORMAT( 1X, '  dsb2bc: uplo=', A1, ', n=', I5, ', b=', I5,
     >          ', ldsrc=', I5, ', lddst=', I5, ',' )
*
 9590   FORMAT( 1X, '  dsb2bi: uplo=', A1, ', n=', I5, ', b=', I5,
     >          ', ldsrc=', I5, ', lddst=', I5, ',' )
*
 9600   FORMAT( 1X, '  dsyrdd: job=', A1, ', uplo=', A1, ', n=', I5,
     >          ', b2=', I5, ', lda=', I5, ',' )
 9601   FORMAT( 1X, '          drptol=', E11.4, ', ldband=', I5,
     >          ', nsteps=', I5, ', lwork=', I8, ',' )
 9605   FORMAT( 1X, 'dsyrdd: uplo=', A1, ', job=', A1, ', n=', I5,
     >          ', b2=', I3, ', nsteps=', I2, ', step=', I2,
     >          ', info=', I3, ', time=', F9.3 )
*
 9610   FORMAT( 1X, '  dsbrdd: jobu=', A1, ', uplo=', A1, ', n=', I5,
     >          ', b1=', I5, ', b2=', I5, ', lda=', I5, ',' )
 9611   FORMAT( 1X, '          drptol=', E11.4, ', ldu=', I5,
     >          ', nsteps=', I5, ', lwork=', I8, ',' )
 9615   FORMAT( 1X, 'dsbrdd: uplo=', A1, ', jobu=', A1, ', n=', I5,
     >          ', b1=', I3, ', b2=', I3, ', nsteps=', I2,
     >          ', step=', I2, ', info=', I3, ', time=', F9.3 )
*
 9620   FORMAT( 1X, '  dorgtr: uplo=', A1, ', n=', I5, ', lda=', I5,
     >          ', nb=', I5, ', nx=', I5, ', lwork=', I8, ',' )
*
 9630   FORMAT( 1X, '  dsygtr: uplo=', A1, ', n=', I5, ', b=', I5,
     >          ', lda=', I5, ', lwork=', I8, ',' )
*
 9640   FORMAT( 1X, '  dsynck: frmt1=', A1, ', uplo1=', A1, ', n=', I5,
...
* **********************************************************************
*
        SUBROUTINE DSYINI( MTYPE, EVLS, SZEVLS, DIAM,
...
*
*   Note: In contrast to the LAPACK routine dlagsy, this routine
...
*
     ...
     >                        INFO
        DOUBLE PRECISION      DIAM, NOTUSD
        DOUBLE PRECISION      A( LDA, * ), U( LDU, * ), EVLS( * ),
     >                        WORK( * )
*
...
*                         around 0 and 1.
*
*   evls    (in OR out) double precision array, dimension ( szevls )
...
*           0 <= b <= max( n-1, 0 ).
*
*   a       (out) double precision array, dimension ( lda, n )
...
*
        INTEGER               D, I, J, K, Q
        DOUBLE PRECISION      C, S, FILL, A11, A12, A21, A22, TMP
*
...
*   lsame   case-insensitive character matching (BLAS)
*
        DOUBLE PRECISION      D1MACH, DLARAN
        EXTERNAL              DCOPY, DLAGSY, D1MACH, DLARAN, DLARTG,
     >                        DLASET
*
*   dcopy   vector copy (BLAS)
*   dlagsy  generate symmetric matrix (LAPACK TMG)
*   dlamch  inquire machine parameters (LAPACK)
*   dlaran  random number generator (LAPACK TMG)
*   dlartg  generate rotation (LAPACK)
*   dlaset  initialize diagonal matrix (LAPACK)
*
        INTRINSIC             MAX, MIN
*
* ----------------------------------------------------------------------
...
*
          ...
          ISEED( 4 ) = 1001
*
          SDIAM = TWO * DIAM * D1MACH(4)
          DO 1000 I = 1, N
            IF ( ISDA ) THEN
              EVLS( I ) = SDIAM * ( DLARAN( ISEED ) - HALF )
              IF ( DLARAN( ISEED ) .LT. HALF ) THEN
                EVLS( I ) = EVLS( I ) + ONE
              ENDIF
            ELSE
              EVLS( I ) = TWO * ( DLARAN( ISEED ) - HALF )
            ENDIF
 1000     CONTINUE
        ENDIF
*
...
*
          CALL DLAGSY( N, B, EVLS, A, LDA, ISEED, WORK, INFO )
*
          IF ( UPPER ) THEN
*
...
*
              DO 3200 K = I+D, N-1, D
*
                CALL DLARTG( A( D+1, K-D ), FILL, C, S, TMP )
                A( D+1, K-D ) = TMP
*
                DO 3110 Q = K-D+1, K-1
...
*
            DO 3700 I = 0, B/2
              CALL DCOPY( N-B+I, A( B+1-I, 1 ), LDA, WORK, 1 )
              DO 3610 J = 1, N-I
                A( B+1-I, I+J ) = A( I+1, J )
 3610         CONTINUE
              DO 3620 K = 1, I
                A( B+1-I, K ) = NOTUSD
 3620         CONTINUE
              CALL DCOPY( N-B+I, WORK, 1, A( I+1, B+1-I ), LDA )
              DO 3630 K = 1, B-I
                A( I+1, K ) = NOTUSD
 3630         CONTINUE
 3700       CONTINUE
...
*
        IF ( NEEDU ) THEN
          CALL DLASET( 'All', N, N, ZERO, ONE, U, LDU )
        ENDIF
*
 9999   RETURN
        END
...
* **********************************************************************
*
        SUBROUTINE DSYNCK( EVLS, FRMT1, UPLO1, N, B1, A1, LDA1, SZA1,
...
*                 T
*      A --> B = U  * A * U.
*   A is rebuilt by calling dsyini, B may be given in banded or
*   tridiagonal format.
...
*
     ...
     >                        SZWORK, INFO
        DOUBLE PRECISION      ORTHU, SORTHU, RES, SRES, DEVLS, SDEVLS,
     >                        NOTUSD
        DOUBLE PRECISION      EVLS( * ), A1( LDA1, * ), A2( LDA2, * ),
     >                        D( * ), E( * ), U( LDU, * ), WORK( * )
*
*   evls    (in) double precision array, dimension ( n )
*           The eigenvalues of the matrix A, as returned from dsyini.
*
...
*           0 <= b1 <= max( n-1, 0 ).
*
*   a1      (out) double precision array, dimension ( lda1, n )
...
*           0 <= b2 <= max( n-1, 0 ).
*
*   a2      (in) double precision array, dimension ( lda2, n )
...
*
*   e       (in) double precision array, dimension ( n-1 )
...
*                = 'N' : No.
*
*   u       (in) double precision array, dimension ( ldu, n )
...
*   lsame   case-insensitive character matching (BLAS)
*
        DOUBLE PRECISION      D1MACH, DLANGE, DNRM2
        EXTERNAL              DAXPY, DCOPY, DGEMM, DGEMV, D1MACH,
     >                        DLANGE, DLASRT, DNRM2, DSBEV, DSBMV,
     >                        DSCAL, DSTEV, DSYINI, DSYMM
*
*   daxpy   add multiple of a vector to another one (BLAS)
*   dcopy   vector copy (BLAS)
*   dgemm   matrix-matrix product (BLAS)
*   dgemv   matrix-vector product (BLAS)
*   dlamch  determine machine parameters (LAPACK)
*   dlange  matrix norm (LAPACK)
*   dlasrt  sort a vector (LAPACK)
*   dnrm2   2-norm of a vector (BLAS)
*   dsbev   compute eigensystem of symmetric banded matrix (LAPACK)
*   dsbmv   banded symmetri matrix-vector product (BLAS)
*   dscal   scale a vector (BLAS)
*   dstev   compute eigensystem of symmetric tridiagonal matrix (LAPACK)
*   dsyini  initialize symmetric matrix (this file)
...
*
            CALL DGEMM( 'Transpose', 'NoTranspose', N, COLS, N,
...
*
 ...
 1010       CONTINUE
            ORTHU = ORTHU + DLANGE( 'Frobenius', N, COLS, WORK, N,
     >                              WORK )**2
 1100     CONTINUE
          ORTHU = SQRT( ORTHU )
          SORTHU = ORTHU / ( N * EPS )
...
*
          CALL DSYINI( 'Spectrum', EVLS, N, ZERO,
...
*
            IF ( LSAME( FRMT2, 'Tridiagonal' ) ) THEN
              DO 2010 J = J0, J0+COLS-1
                IX = ( J - J0 ) * N + 1
                CALL DCOPY( N, U( 1, J ), 1, WORK( IX ), 1 )
                CALL DSCAL( N, D( J ), WORK( IX ), 1 )
                IF ( J .GT. 1 ) THEN
                  CALL DAXPY( N, E( J-1 ), U( 1, J-1 ), 1,
     >                        WORK( IX ), 1 )
                ENDIF
                IF ( J .LT. N ) THEN
                  CALL DAXPY( N, E( J ), U( 1, J+1 ), 1, WORK( IX ), 1 )
                ENDIF
 2010         CONTINUE
            ELSE
              DO 2020 J = J0, J0+COLS-1
                IX = ( J - J0 ) * N + 1
                CALL DGEMV( 'NoTranspose', N, MIN( B2+1, N-J+1 ),
     >                      ONE, U( 1, J ), LDU, A2( 1, J ), 1,
     >                      ZERO, WORK( IX ), 1 )
                IF ( B2 .GT. 0 ) THEN
                  IF ( J .GT. B2 ) THEN
                    CALL DGEMV( 'NoTranspose', N, B2,
     >                          ONE, U( 1, J-B2 ), LDU,
     >                          A2( B2+1, J-B2 ), LDA2 - 1,
     >                          ONE, WORK( IX ), 1 )
                  ELSEIF ( J .GT. 1 ) THEN
                    CALL DGEMV( 'NoTranspose', N, J - 1,
     >                          ONE, U( 1, 1 ), LDU,
     >                          A2( J, 1 ), LDA2 - 1,
     >                          ONE, WORK( IX ), 1 )
                  ENDIF
...
*
            IF ( LSAME( FRMT1, 'Full' ) ) THEN
              CALL DSYMM( 'Left', UPLO1, N, COLS,
     >                   -ONE, A1, LDA1, U( 1, J0 ), LDU,
     >                   ONE, WORK, N )
            ELSE
              DO 2110 J = J0, J0+COLS-1
                CALL DSBMV( UPLO1, N, B1, -ONE, A1, LDA1, U( 1, J ), 1,
     >                      ONE, WORK( (J-J0)*N+1 ), 1 )
 2110         CONTINUE
            ENDIF
*
            RES = RES + DLANGE( 'Frobenius', N, COLS, WORK, N, WORK )**2
 2000     CONTINUE
*
          RES = SQRT( RES )
...
*
          IF ( LSAME( FRMT2, 'Tridiagonal' ) ) THEN
            CALL DSTEV( 'NoEigenvectors', N, D, E, WORK, N, WORK, INFO )
          ELSE
            CALL DSBEV( 'NoEigenvectors', 'Lower', N, B2, A2, LDA2,
     >                  D, WORK, N, WORK, INFO )
          ENDIF
*
...
*
          IF ( INFO .EQ. 0 ) THEN
            CALL DLASRT( 'Increasing', N, EVLS, INFO )
            CALL DAXPY( N, -ONE, EVLS, 1, D, 1 )
            DEVLS = DNRM2( N, D, 1 )
