...
% 
%   VARMA MODELS:
%     X = VARMA_SIM(A,B,Cov,n), where Cov is an rÚr matrix uses a VARMA(p,q)
...
%     rÚr matrices. A and B should contain A = [A1 A2...Ap] (an r Ú r¸p matrix)
%     and B = [B1 B2 ... Bq] (an r Ú r¸q matrix). The rÚn matrix X returns x(t)
...
% 
%     X = VARMA_SIM(...,mu,M) returns M sequences in an rÚnÚM multidimensional X
...
function [x, eps] = varma_sim(A, B, Sig, n, mu, M, x0)
  ...
  elseif r==1      %  several ARMA sequences:
    x = reshape(x,n,M) + mu;
    eps = reshape(eps,n,M);
  else             %   one or more VARMA sequences in rÚnÚM array:
...
function x = randnm(n,Sig,mu)
  ...
  %    r-vector. X will be NÚr. X = RANDNM(N,S) uses mean 0.
  %
