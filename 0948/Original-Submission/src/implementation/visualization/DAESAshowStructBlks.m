function DAESAshowStructBlks(sadata, varargin)
% function showStructBlks(sadata)
% function showStructBlks(sadata, rc)
%
% Input:
...
if isa(sadata, 'SAdata')
    ...
    switch exitflag
        case -3 % No visualization
            fprintf('%s is structurally ill posed. Coarse blocks cannot be displayed.\n', func2str(fcn));
            return;
            
        case {-2,-1} %
            switch nargin
                case 1
                    DAESAshowStructUOblocks(sadata); return
                case 3
                    if varargin{1}==1
                        DAESAshowSMUO(sadata, varargin{2}); return;
                    elseif varargin{1}==2
                        DAESAshowBSMUO(sadata, varargin{2}); return;
                    end
                otherwise
    ...
    elseif nargin == 3
        if varargin{1}==1
            DAESAshowSMBlks(sadata, varargin{2}); return;
        elseif varargin{1}==2
            DAESAshowBSMBlks(sadata, varargin{2}); return;
        end
    end
...
figure(gcf);
set(gcf, 'Name', [fcn ' (coarse)'] , 'Position', figureSize);
clf, hold on;
...
title({['\bf\fontsize{12}' upper(fcn) ': Coarse BTF'], ...
...
if n < 41
    ...
    text(ypos, repmat(m + Z, size(ypos)), num2str(d(:)), ...
...
plot(y, x, ':b');
% Vertical alignment lines from (s(k)-1, 0) to (s(k)-1, n-1) for each k.
x = [r-1; r-1; NaN(siz)]; x = x(:);
%siz = siz-1;
y = [zeros(siz); repmat(n,siz); NaN(siz)]; y = y(:);
plot(x, y, ':b');
