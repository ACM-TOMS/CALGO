function DAESAshowBSMBlks(sadata, rc)
...
if length(rc)==4
    % checkRC(n, rc);
    rows = rC(rc(1)):(rC(rc(2)+1)-1);
    cols = rC(rc(3)):(rC(rc(4)+1)-1);
    r = rC(rc(1):rc(2)) - rC(rc(1)) + 1;
    s = rC(rc(3):rc(4)) - rC(rc(3)) + 1;
elseif length(rc)==2
    rows = rC(rc(1)):(rC(rc(2)+1)-1); cols = rows;
    rc(3) = rc(1); rc(4) = rc(2);
    r = rC(rc(1):rc(2)) - rC(rc(1)) + 1;
    s = r;
else
...
title({['\bf\fontsize{12}' upper(fcn) ': Coarse Blocks \Sigma(' num2str(rc(1)) ':' num2str(rc(2)) ',' ...
    num2str(rc(3)) ':' num2str(rc(4)) ')']...
