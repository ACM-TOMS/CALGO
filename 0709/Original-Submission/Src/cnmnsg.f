     ...
     -                         TRACES,TUN,NTR )
      ...
      REAL              FUNCNM, X(N), F, G(N), ACC, W(MDIM)
C!!!! DOUBLE PRECISION  FUNCNM, X(N), F, G(N), ACC, W(MDIM)
...
C                    END
C
      REAL              FP,FMIN,ALPHA,AT,AP,GSQ,DG,DG1
C!!!! DOUBLE PRECISION  FP,FMIN,ALPHA,AT,AP,GSQ,DG,DG1
      REAL              DP,STEP,DAL,U1,U2,U3,U4,EPS, ZZMPAR
C!!!! DOUBLE PRECISION  DP,STEP,DAL,U1,U2,U3,U4,EPS, ZZMPAR
      REAL              RTST,  SQRT, AMIN1, AMAX1,  ABS
C!!!! DOUBLE PRECISION  RTST, DSQRT, DMIN1, DMAX1, DABS
C
      REAL RW(1)
...
C
   ...
   30 CONTINUE
      ...
      CALL ZZSCAL ( N, -1.E0, W, 1 )
C!!!! CALL ZZSCAL ( N, -1.D0, W, 1 )
C
...
C!!!! IF(F.GT.(FMIN+.0001*ALPHA*DG).OR.DABS(DAL/DG)
     1.GT.(.9))GO TO 130
C
C IF THEY HAVE BEEN MET, TEST IF TWO POINTS HAVE BEEN TRIED
C IF NMETH=0 AND IF THE TRUE LINE MINIMUM HAS NOT BEEN FOUND.
C
      IF((IFUN-NCALLS).LE.1.AND. ABS(DAL/DG).GT.ACC.AND.
C!!!! IF((IFUN-NCALLS).LE.1.AND.DABS(DAL/DG).GT.ACC.AND.
...
C
      IF(AT.LT.(1.01*AMIN1(ALPHA,AP)).OR.AT.GT.(.99*AMAX1
C!!!! IF(AT.LT.(1.01*DMIN1(ALPHA,AP)).OR.AT.GT.(.99*DMAX1
...
C
140   CONTINUE
      IF(DAL .GT.0.0.AND.0.0.LT.AT.AND.AT.LT.(.99*AMIN1(AP,ALPHA)))
C!!!! IF(DAL .GT.0.0.AND.0.0.LT.AT.AND.AT.LT.(.99*DMIN1(AP,ALPHA)))
     1GO TO 150
C
...
C!!!! IF(DABS(RTST/GSQ).GT.0.2)NRST=N
C
...
C
      ...
      IF(TRACES(1))
     -    WRITE(TUN,*)'RESTART, ABS(RTST/GSQ),N ', ABS(RTST/GSQ),N
C!!!!-    WRITE(TUN,*)'RESTART,DABS(RTST/GSQ),N ',DABS(RTST/GSQ),N
      W(NCONS+1)=0.
      W(NCONS+2)=0.
      DO 210 I=1,N
        NRDPI=NRD+I
