...
C----- TRACE VALUES, MISSING PARAMETERS AND USER PARAMETERS.
      ...
      REAL                         DFLT,        DUSVAL
C!!!! DOUBLE PRECISION             DFLT,        DUSVAL
      LOGICAL          DTRVAL
...
C----- PROBLEM DEFAULTS
      ...
      INTEGER                         DARGNO
      REAL              DFARG
C!!!! DOUBLE PRECISION  DFARG
      PARAMETER       ( DFARG = DFLT, DARGNO = 0 )

      INTEGER           DIX0
      REAL                        DX1,        DX2
C!!!! DOUBLE PRECISION            DX1,        DX2
...
C-----ARRAYS AND I/O STRINGS, EXCEPT FOR DICTIONARIES.
      ...
      REAL              G(MAXN), RWORK(LR), X(MAXN), SOLNS(MXSOLN)
C!!!! DOUBLE PRECISION  G(MAXN), RWORK(LR), X(MAXN), SOLNS(MXSOLN)

      REAL              PARS(MXPARS),  USER(NU), USERD(NU), X0(MAXN)
C!!!! DOUBLE PRECISION  PARS(MXPARS),  USER(NU), USERD(NU), X0(MAXN)
      ...
      REAL               ILOOPC(ILPCSZ), OLOOPC(OLPCSZ)
C!!!! DOUBLE PRECISION   ILOOPC(ILPCSZ), OLOOPC(OLPCSZ)
...
C-----VARIABLE DECLARATIONS OF A MISCELLANEOUS TYPE.
      ...
      REAL              PRTIME, TM, TMIN, GSQ, PAR1, FNC, TFUNC, TIMEIT
C!!!! DOUBLE PRECISION  PRTIME, TM, TMIN, GSQ, PAR1, FNC, TFUNC, TIMEIT

      REAL              VALUE, MING, MAXG, DF, DX, FSTAR
C!!!! DOUBLE PRECISION  VALUE, MING, MAXG, DF, DX, FSTAR
...
C-----VARIABLES FOR ZZEVAL.
      ...
      REAL              MXERR, DIGS
C!!!! DOUBLE PRECISION  MXERR, DIGS

C-----VARIABLES FOR ZZFNS.

      REAL              FARG(FNO)
C!!!! DOUBLE PRECISION  FARG(FNO)
...
C----- DECLARE PROBLEM STORAGE VARIABLES FOR GETG, GETP, PUTG, PUTP
      ...
      REAL               LPDATA(ILPCSZ)
C!!!! DOUBLE PRECISION   LPDATA(ILPCSZ)
...
C                                    DISPLAY NUMERIC CONTROL VALUES
     ...
     -                              (OLOOPC(I), I = J+2, K )
            DO 87100 II = 1, NU
               ...
               S2 = CMDS( JJ-7 : JJ )
               IF    (  S2 .NE. BLANK ) THEN
                 WRITE(TMPUN,98500) II, S2, N, ( OLOOPC(I), I= J+2, K)
C!!!!            WRITE(TMPUN,98450) II, S2, N, ( OLOOPC(I), I= J+2, K)
               ENDIF
87100       CONTINUE
         ENDIF
