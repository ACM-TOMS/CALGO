C     PROGRAM TVARL
C
C     TEST PROGRAM FOR THE SUBROUTINE PVARL
C
C      CALL TYTIM1(ITIME)
C
...
C     ..
C     .. Local Arrays ..
      DOUBLE PRECISION CF(20),CG(400),CL(20),CU(20),RA(2000),RPAR(7),
     +                 X(40),XL(40),XU(40)
      INTEGER IA(100),IC(20),IPAR(7),IX(40)
C     ..
C     .. External Subroutines ..
      EXTERNAL PVARL,TILD22
C     ..
C     .. Common blocks ..
...
C
   ...
   20     CONTINUE
          ...
          IF (IPAR(1).EQ.1) THEN
              RPAR(5) = 0.25D0
              IF (NEXT.EQ.3) RPAR(5) = 0.0D0
              IF (NEXT.EQ.5) RPAR(5) = 0.5D0
              IF (NEXT.EQ.6) RPAR(5) = 0.5D0
              IF (NEXT.EQ.7) RPAR(5) = 0.1D0
              IF (NEXT.EQ.9) RPAR(5) = 1.0D-3
              IF (NEXT.EQ.10) RPAR(5) = 0.5D0
              IF (NEXT.EQ.11) RPAR(5) = 1.0D0
              IF (NEXT.EQ.12) RPAR(5) = 1.0D-2
              IF (NEXT.EQ.13) RPAR(5) = 1.0D-3
              IF (NEXT.EQ.14) RPAR(5) = 0.1D0
          END IF
C
C     PROBLEM DIMENSION
C
...
C
          CALL TILD22(NF,NAA,NB,NC,X,IX,XL,XU,IC,CL,CU,CG,FMIN,RPAR(7),
     +                NEXT,IEXT,IERR)
          IF (NEXT.EQ.3) RPAR(7) = 1.0D0
          IF (NEXT.EQ.7) RPAR(7) = 0.5D-1
          IF (NEXT.EQ.9) RPAR(7) = 1.0D0
          IF (NEXT.EQ.10) RPAR(7) = 1.0D2
          IF (NEXT.EQ.11) RPAR(7) = 1.0D0
          IF (NEXT.EQ.13) RPAR(7) = 1.0D0
          IF (IERR.NE.0) GO TO 30
C
C     SOLUTION
C
          CALL PVARL(NF,NA,NB,NC,X,IX,XL,XU,CF,IC,CL,CU,CG,IA,RA,IPAR,
     +               RPAR,F,GMAX,ITERM)
   30 CONTINUE
C      CALL TYTIM2(ITIME)
      STOP
