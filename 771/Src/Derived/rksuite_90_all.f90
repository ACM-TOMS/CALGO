...
!starta!
!starta!
public :: rk_comm_complex_0d
type rk_comm_complex_0d
...
real(kind=wp), pointer :: thresh, weights, ymax        !shp-dep!

complex(kind=wp), pointer :: scratch, y, yp, y_new        !dep!
complex(kind=wp), pointer :: y_old, yp_old, v0, v1        !dep!
complex(kind=wp), pointer :: err_estimates, v2, v3        !dep!
complex(kind=wp), pointer ::  vtemp                       !dep!
complex(kind=wp), dimension(:), pointer :: stages                     !dep!
...
integer :: ge_f_count
real(kind=wp), pointer :: ge_assess                    !shp-dep!

complex(kind=wp), pointer :: ge_y, ge_yp, ge_y_new        !dep!
complex(kind=wp), pointer :: ge_err_estimates             !dep!
complex(kind=wp), dimension(:), pointer :: ge_stages                  !dep!
...
complex(kind=wp), pointer :: xstage, ytemp                !dep!
complex(kind=wp), dimension(:), pointer :: p                          !dep!
...
end type rk_comm_complex_0d
!enda!
!starta!
public :: rk_comm_complex_1d
type rk_comm_complex_1d
...
complex(kind=wp), dimension(:), pointer :: scratch, y, yp, y_new        !dep!
complex(kind=wp), dimension(:), pointer :: y_old, yp_old, v0, v1        !dep!
complex(kind=wp), dimension(:), pointer :: err_estimates, v2, v3        !dep!
complex(kind=wp), dimension(:), pointer ::  vtemp                       !dep!
complex(kind=wp), dimension(:,:), pointer :: stages                     !dep!
...
complex(kind=wp), dimension(:), pointer :: ge_y, ge_yp, ge_y_new        !dep!
complex(kind=wp), dimension(:), pointer :: ge_err_estimates             !dep!
complex(kind=wp), dimension(:,:), pointer :: ge_stages                  !dep!
...
complex(kind=wp), dimension(:), pointer :: xstage, ytemp                !dep!
complex(kind=wp), dimension(:,:), pointer :: p                          !dep!
...
end type rk_comm_complex_1d
!enda!
!starta!
public :: rk_comm_complex_2d
type rk_comm_complex_2d
...
end type rk_comm_complex_2d
!enda!
!starta!
public :: rk_comm_real_0d
type rk_comm_real_0d
...
real(kind=wp), pointer :: scratch, y, yp, y_new        !dep!
real(kind=wp), pointer :: y_old, yp_old, v0, v1        !dep!
real(kind=wp), pointer :: err_estimates, v2, v3        !dep!
real(kind=wp), pointer ::  vtemp                       !dep!
...
real(kind=wp), pointer :: ge_y, ge_yp, ge_y_new        !dep!
real(kind=wp), pointer :: ge_err_estimates             !dep!
...
real(kind=wp), pointer :: xstage, ytemp                !dep!
...
!enda!
!starta!
public :: rk_comm_real_2d
type rk_comm_real_2d
...
!endb!
!startc!
subroutine setup_c0(comm,t_start,y_start,t_end,tolerance,thresholds, &
...
complex(kind=wp), intent(in) :: y_start                   !dep!
real(kind=wp), intent(in) :: thresholds                !shp-dep!
...
" ** range of permitted values is (",comm%round_off,",0.01)."
   ...
   if ((thresholds) < comm%sqtiny) then                        !spec-ar!
...
!
   allocate(comm%thresh, &                          !alloc!
            comm%err_estimates, &                   !alloc!
            comm%weights, &                         !alloc!
            comm%y_old, &                           !alloc!
            comm%scratch, &                         !alloc!
            comm%y, &                               !alloc!
            comm%yp, &                              !alloc!
            comm%stages(tr_dim_of_stages), &         !alloc!
            comm%ymax,stat=ier)                     !alloc!
   if (ier /= 0) then
      ier = fatal; nrec = 1 ; write (comm%rec,"(a)") &
   ...
   if (comm%use_range) then
      if (comm%intrp_able) then
         if (comm%rk_method==1) then
            ...
            allocate(comm%p(5), &                    !alloc!
                     comm%ytemp, &                  !alloc!
                     comm%xstage,stat=ier)          !alloc!
            if (ier /= 0) then
               ier = fatal; nrec = 1 ; write (comm%rec,"(a)") &
" ** Not enough storage available to create workspace required internally."
               exit body
...
!  respectively).
      if (.not.comm%intrp_needs_stages) then
         ...
         allocate(comm%ge_stages(tr_dim_of_stages),stat=ier) !alloc!
         if (ier /= 0) then
            ier = fatal; nrec = 1 ; write (comm%rec,"(a)") &
" ** Not enough storage available to create workspace required internally."
            exit body
      ...
      allocate(comm%ge_y, &                         !alloc!
               comm%ge_yp, &                        !alloc!
               comm%ge_err_estimates, &             !alloc!
               comm%ge_assess, &                    !alloc!
               comm%ge_y_new,stat=ier)              !alloc!
      if (ier /= 0) then
         ier = fatal; nrec = 1 ; write (comm%rec,"(a)") &
" ** Not enough storage available to create workspace required internally."
         exit body
...
!
call rkmsg_c0(ier,srname,nrec,comm)
!
contains
...
end subroutine setup_c0


subroutine collect_garbage_c0(comm)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
!
...
!
end subroutine collect_garbage_c0

recursive subroutine range_integrate_c0(comm,f,t_want,t_got,y_got,yderiv_got, &
                                        flag)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
...
complex(kind=wp), intent(out), optional :: y_got, yderiv_got !dep!
integer, intent(out), optional :: flag
type(rk_comm_complex_0d), intent(inout) :: comm
!
interface 
   function f(t,y)
      use rksuite_90_prec, only:wp
      real(kind=wp), intent(in) :: t                                 !indep!
      complex(kind=wp), intent(in) :: y                   !dep!
      complex(kind=wp) :: f                       !dep!
   end function f
end interface
!
...
!
   state = get_saved_state_c0("SETUP",comm%save_states)
   if (state==fatal) then
...
" ** were going to use STEP_INTEGRATE. This is not permitted."
   ...
   state = get_saved_state_c0(srname,comm%save_states)
...
" ** SETUP to start another problem."
   ...
   state = usable
   call set_saved_state_c0(srname,state,comm)
!
...
!
      if (.not.comm%at_t_start) then
         ...
         baderr = get_saved_fatal_c0(comm)
         if (baderr) exit body
      end if
   end if
!
...
!
         ...
         if (goback) then
            call interpolate(comm,f,t_want,y_want=y_got,yderiv_want=yderiv_got)
            baderr = get_saved_fatal_c0(comm)
            if (baderr) exit body
            t_got = t_want
         end if
...
!  from inside COMM.
      ...
      call step_integrate(comm,f,t_now,y_now=y_got,yderiv_now=yderiv_got, & 
                          flag=step_flag)
      ier = step_flag 
!  
...
!  what happened
!
call rkmsg_c0(ier,srname,nrec,comm,flag)
!
end subroutine range_integrate_c0

recursive subroutine step_integrate_c0(comm,f,t_now,y_now,yderiv_now,flag)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
!
...
integer, intent(out), optional :: flag
type(rk_comm_complex_0d), intent(inout) :: comm
complex(kind=wp), intent(out), optional :: y_now, yderiv_now !dep!
interface
   function f(t,y)
      use rksuite_90_prec, only:wp
      real(kind=wp), intent(in) :: t                                 !indep!
...
logical :: failed, phase1, phase2, phase3, toomch, sure_stiff
!
integer, parameter :: not_ready=-1, usable=-2, fatal=911, catastrophe=912, &
...
" ** You have not called SETUP, so you cannot use STEP_INTEGRATE."
   ...
   if (comm%use_range) then
      if (get_saved_state_c0("RANGE_INTEGRATE",comm%save_states)/=usable) then
         ier = fatal; nrec = 2; write (comm%rec,"(a/a)") &
...
!  stiffness.
!
         call stiff_c0(comm,f,toomch,sure_stiff)
         if (sure_stiff) then
!
...
!
      htry = comm%h
      phase2 = comm%phase2
      call step_c0(comm,f,comm%t,comm%y,comm%yp,comm%tol,htry, &
                   err,hmin,comm%phase2)
      comm%phase2 = phase2
      comm%h = htry
!
...
if (ier < fatal) then
   ...
   if (present(y_now))y_now = comm%y
   if (present(yderiv_now)) yderiv_now = comm%yp
   comm%ymax = max(abs(comm%y),comm%ymax)
   if (ier==just_fine) then
      state = usable; call set_saved_state_c0("INTERPOLATE",state,comm)
   end if
end if
!
...
call rkmsg_c0(ier,srname,nrec,comm,flag)
!
end subroutine step_integrate_c0


recursive subroutine truerr_c0(comm,f,ier)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
!
...
!          see main doc for contact details
!
type(rk_comm_complex_0d), intent(inout), target :: comm
...
complex(kind=wp), dimension(:), pointer :: ge_stages                  !dep!
complex(kind=wp), pointer :: ge_y, ge_yp, ge_y_new        !dep!
complex(kind=wp), pointer :: y                            !dep!
real(kind=wp), pointer :: ge_assess, weights           !shp-dep!
!
...
!
!  Take a step.
      call step_c0(comm,f,tsec,ge_y,ge_yp,ge_test1,hsec, &
         ge_err)
!
...
!
   ...
   diff = (abs(ge_y-y)/weights)                                !spec-ar!
   if (diff>errmax) then
...
!
end subroutine truerr_c0

recursive subroutine step_c0(comm,f,tnow,y,yp,tol,htry,    &
                     err,hmin,phase_2)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
...
!
complex(kind=wp), intent(in) :: y, yp                     !dep!
!
interface
   function f(t,y)
      use rksuite_90_prec, only:wp
...
!
complex(kind=wp), pointer :: errest, y_new                !dep!
complex(kind=wp), dimension(:), pointer :: stages                     !dep!
real(kind=wp), pointer :: weights, thresh              !shp-dep!
...
if (main) then
   stages => comm%stages
   errest => comm%err_estimates
   y_new => comm%y_new
else
   stages => comm%ge_stages
   errest => comm%ge_err_estimates
   y_new => comm%ge_y_new
end if
!
attempt_step: do
!
   if (main) then
      if (phase_2) weights = max(thresh,abs(y))
   end if
!
...
!
                  if (abs(htry)<=hmin) then
                     htry = sign(hmin,htry); phase_2 = .false.
                  end if
                  cycle attempt_step
               end if
            end if
...
!
   err = (abs(errest/weights))                                 !spec-ar!
!
   exit attempt_step
!
end do attempt_step
...
!
   real(kind=wp), intent(inout) :: htry                              !indep!
   complex(kind=wp), intent(in) :: ypnew                  !dep!
...
!
   ...
   ynrm = (abs(y)/weights)                                     !spec-ar!
   ystgnm = (abs(y_new)/weights)                               !spec-ar!
   argdif = (abs(y_new-y)/weights)                             !spec-ar!
   fdiff = (abs(ypnew-yp)/weights)                             !spec-ar!
!
...
!
   cutbak = argdif > comm%round_off*max(ynrm,ystgnm)
   if (cutbak) then
      cutbak = (abs(htry)*fdiff) > (comm%stability_radius*argdif)
      if (cutbak) then
...
!
   if (main) then
      err = ( abs( e(1)*yp + e(3)*stages(ptr(3)) + &         !spec-ar!
...
!
end subroutine step_c0


recursive subroutine stiff_c0(comm,f,toomch,sure_stiff)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
!
...
!
complex(kind=wp) :: alpha1, alpha2, beta1, beta2                        !dep!
complex(kind=wp) :: rold, v1v0, v2v0, v2v1, v3v1, v3v2                  !dep!
...
complex(kind=wp), pointer :: v0, v1, v2, v3, y, y_old     !dep!
real(kind=wp), pointer :: weights, thresh              !shp-dep!
!
...
! All non-procedure arguments to these contained procedures
! should have intent(in) but IBM RS/600 AIX xlf v3.2.1.x
! produces rubbish so intent attribute has been dropped

function approx_jacobian(f,v,vdotv)
!
real(kind=wp) :: vdotv
complex(kind=wp) :: v                         !dep!
complex(kind=wp) :: approx_jacobian               !dep!
!
interface
   function f(t,y)
      use rksuite_90_prec, only:wp
...
!
complex(kind=wp) :: alpha, beta                             !dep!
...
!
real(kind=wp) :: v0v0, v1v1
complex(kind=wp) :: v1v0                                    !dep!
logical :: dominant_eigenvalue
!
complex(kind=wp) :: ratio                                               !dep!
...
!
if (big) then
   root_pair(bigr) = cmplx(ratio,kind=wp)
   root_pair(smlr) = cmplx(zero,kind=wp)
end if
!
rold = ratio
...
!
complex(kind=wp) :: vec_1, vec_2              !dep!
complex(kind=wp) :: wt_inner_prod                                       !dep!
!
!
wt_inner_prod =  ( (vec_1/weights) * (conjg(vec_2)/weights) )            !spec-ar!
!
end function wt_inner_prod

function rev_wt_inner_prod(value)
!
complex(kind=wp) :: value                                   !dep!
complex(kind=wp) :: rev_wt_inner_prod                                   !dep!
!
...
!
rev_wt_inner_prod = conjg(value)                                            !spec-line!
!
end function rev_wt_inner_prod

end subroutine stiff_c0

subroutine statistics_c0(comm,total_f_calls,step_cost,waste,num_succ_steps,&
                         h_next,y_maxvals)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
...
real(kind=wp), optional, intent(out) :: y_maxvals      !shp-dep!
...
" ** reported from the integrator. You cannot call STATISTICS again."
   ...
   state = get_saved_state_c0("STEP_INTEGRATE",comm%save_states)
   if (state==not_ready) then
      ier = fatal; nrec = 1
      if (comm%use_range) then
         write (comm%rec,"(a)") &
...
call rkmsg_c0(ier,srname,nrec,comm)
!
end subroutine statistics_c0

subroutine global_error_c0(comm,rms_error,max_error,t_max_error)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
!
...
real(kind=wp), optional, intent(out) :: rms_error      !shp-dep!
!
...
call rkmsg_c0(ier,srname,nrec,comm)
!
end subroutine global_error_c0

subroutine reset_t_end_c0(comm,t_end_new)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
!
...
call rkmsg_c0(ier,srname,nrec,comm)
!
end subroutine reset_t_end_c0

recursive subroutine interpolate_c0(comm,f,t_want,y_want,yderiv_want)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
!
...
real(kind=wp), intent(in) :: t_want                                  !indep!
type(rk_comm_complex_0d), intent(inout), target :: comm
complex(kind=wp), intent(out), optional :: y_want         !dep!
complex(kind=wp), intent(out), optional :: yderiv_want    !dep!
!
interface
   function f(t,y)
      use rksuite_90_prec, only:wp
...
!
   ...
   case(2)
      ...
      if (.not.associated(comm%xstage)) then
         allocate(comm%xstage,stat=jer)              !alloc!
      end if
      if (.not.associated(comm%ytemp)) then
         allocate(comm%ytemp,stat=jer)               !alloc!
      end if
      if (.not.associated(comm%p)) then
         allocate(comm%p(5),stat=jer)                 !alloc!
      end if
      npcls = 5
      if (jer /= 0) then
         ier = fatal; nrec = 1 ; write (comm%rec,"(a)") &
...
!  step
!
   intrp_initialised = get_saved_state_c0(srname,comm%save_states) /= usable
!
...
contains
!
recursive subroutine form_intrp(f,p)
!
complex(kind=wp), intent(out), dimension(:) :: p                      !dep!
!
interface
   function f(t,y)
      use rksuite_90_prec, only:wp
...
complex(kind=wp), dimension(:), pointer :: stages                     !dep!
complex(kind=wp), pointer :: y, yp, y_old, yp_old         !dep!
complex(kind=wp), pointer :: xstage                       !dep!
!
stages => comm%stages
r => comm%r
y => comm%y
...
!  METHOD = 'M'.
!
   if (.not.intrp_initialised) call extra_stages(f,comm%ytemp,xstage)
!
...
!
complex(kind=wp), optional, intent(out) :: y_want         !dep!
complex(kind=wp), optional, intent(out) :: yderiv_want    !dep!
complex(kind=wp), dimension(:), intent(in) :: p                       !dep!
!
real :: sigma
integer :: i
!
...
recursive subroutine extra_stages(f,ytemp,xstage)
!
complex(kind=wp), intent(out) :: ytemp, xstage            !dep!
!
interface 
   function f(t,y)
      use rksuite_90_prec, only:wp
...
!
complex(kind=wp), dimension(:), pointer :: stages                     !dep!
complex(kind=wp), pointer :: yp, y_old, yp_old            !dep!
!
real(kind=wp), dimension(:,:), pointer :: a                          !real!
...
end subroutine interpolate_c0

subroutine rkmsg_c0(ier,srname,nrec,comm,flag)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
!
...
!
call set_saved_state_c0(srname,ier,comm)
!
!  Indicate that a catastrophic error has been detected
!
!call set_saved_fatal_c0(comm,ier >= catastrophe)
!
end subroutine rkmsg_c0

subroutine set_saved_state_c0(srname,state,comm)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
!
...
!
end subroutine set_saved_state_c0

function get_saved_state_c0(srname,save_states)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
!
...
if (any(save_states(1:7)==fatal)) then
   get_saved_state_c0 = fatal
else
   get_saved_state_c0 = save_states(name)
end if
!
end function get_saved_state_c0

function get_saved_fatal_c0(comm)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
!
...
!          see main doc for contact details
!
type(rk_comm_complex_0d), intent(in) :: comm
logical :: get_saved_fatal_c0
!
get_saved_fatal_c0 = comm%saved_fatal_err
!
end function get_saved_fatal_c0

subroutine set_stop_on_fatal_c0(comm,action)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
!
...
!
end subroutine set_stop_on_fatal_c0

function get_stop_on_fatal_c0(comm)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
!
...
!
type(rk_comm_complex_0d), intent(in) :: comm
logical get_stop_on_fatal_c0
!
get_stop_on_fatal_c0 = comm%stop_on_fatal
!
end function get_stop_on_fatal_c0

!endc!
!startc!
subroutine setup_c1(comm,t_start,y_start,t_end,tolerance,thresholds, &
                    method,task,error_assess,h_start,message)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
...
real(kind=wp), intent(in) :: t_end, t_start                          !indep!
real(kind=wp), intent(in) :: tolerance
complex(kind=wp), dimension(:), intent(in) :: y_start                   !dep!
...
!
call rkmsg_c1(ier,srname,nrec,comm)
!
contains
...
subroutine collect_garbage_c1(comm)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
!
...
!
end subroutine collect_garbage_c1

recursive subroutine range_integrate_c1(comm,f,t_want,t_got,y_got,yderiv_got, &
                                        flag)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
...
real(kind=wp), intent(in) :: t_want                                  !indep!
real(kind=wp), intent(out) :: t_got                                  !indep!
complex(kind=wp), dimension(:), intent(out), optional :: y_got, yderiv_got !dep!
integer, intent(out), optional :: flag
type(rk_comm_complex_1d), intent(inout) :: comm
!
interface 
   function f(t,y)
      use rksuite_90_prec, only:wp
      real(kind=wp), intent(in) :: t                                 !indep!
      complex(kind=wp), dimension(:), intent(in) :: y                   !dep!
      complex(kind=wp), dimension(size(y,1)) :: f                       !dep!
   end function f
end interface
!
character(len=*), parameter :: srname="RANGE_INTEGRATE"
...
!  Is it permissible to call RANGE_INTEGRATE?
!
   state = get_saved_state_c1("SETUP",comm%save_states)
   if (state==fatal) then
      ier = catastrophe; nrec = 1; write (comm%rec,"(a)") &
" ** A catastrophic error has already been detected elsewhere."
      exit body
...
" ** were going to use STEP_INTEGRATE. This is not permitted."
   ...
   state = get_saved_state_c1(srname,comm%save_states)
   if (state==5 .or. state==6) then
      ier = fatal; nrec = 1; write (comm%rec,"(a/a)") &
" ** This routine has already returned with a hard failure. You must call",&
" ** SETUP to start another problem."
   ...
   state = usable
   call set_saved_state_c1(srname,state,comm)
!
   if (comm%at_t_start) then
!
!  First call.
...
!
      if (.not.comm%at_t_start) then
         call reset_t_end(comm,t_want)
         baderr = get_saved_fatal_c1(comm)
         if (baderr) exit body
      end if
   end if
!
...
!  what happened
!
call rkmsg_c1(ier,srname,nrec,comm,flag)
!
end subroutine range_integrate_c1

recursive subroutine step_integrate_c1(comm,f,t_now,y_now,yderiv_now,flag)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
!
...
integer, intent(out), optional :: flag
type(rk_comm_complex_1d), intent(inout) :: comm
complex(kind=wp), dimension(:), intent(out), optional :: y_now, yderiv_now !dep!
interface
   function f(t,y)
      use rksuite_90_prec, only:wp
      real(kind=wp), intent(in) :: t                                 !indep!
...
" ** You have not called SETUP, so you cannot use STEP_INTEGRATE."
   ...
   if (comm%use_range) then
      if (get_saved_state_c1("RANGE_INTEGRATE",comm%save_states)/=usable) then
         ier = fatal; nrec = 2; write (comm%rec,"(a/a)") &
" ** You have called STEP_INTEGRATE after you specified in SETUP that you", &
" ** were going to use RANGE_INTEGRATE. This is not permitted."
         comm%use_range = .false.
...
!  stiffness.
!
         call stiff_c1(comm,f,toomch,sure_stiff)
         if (sure_stiff) then
!
!  Predict how much extra work will be needed to reach TND.
            extra_wk = (comm%cost*abs((comm%t_end-comm%t)/comm%h_average)) / &
...
!
      htry = comm%h
      phase2 = comm%phase2
      call step_c1(comm,f,comm%t,comm%y,comm%yp,comm%tol,htry, &
                   err,hmin,comm%phase2)
      comm%phase2 = phase2
      comm%h = htry
!
...
if (ier < fatal) then
   ...
   comm%ymax = max(abs(comm%y),comm%ymax)
   if (ier==just_fine) then
      state = usable; call set_saved_state_c1("INTERPOLATE",state,comm)
   end if
end if
!
!  Call RKMSG_R1 to report what happened and set FLAG.
...
call rkmsg_c1(ier,srname,nrec,comm,flag)
!
end subroutine step_integrate_c1


recursive subroutine truerr_c1(comm,f,ier)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
!
...
!          see main doc for contact details
!
type(rk_comm_complex_1d), intent(inout), target :: comm
integer, intent(inout) :: ier
!
interface
   function f(t,y)
...
complex(kind=wp), dimension(:,:), pointer :: ge_stages                  !dep!
complex(kind=wp), dimension(:), pointer :: ge_y, ge_yp, ge_y_new        !dep!
complex(kind=wp), dimension(:), pointer :: y                            !dep!
...
!
!  Take a step.
      call step_c1(comm,f,tsec,ge_y,ge_yp,ge_test1,hsec, &
         ge_err)
!
!  The primary integration is using a step size of H_OLD and the
!  secondary integration is using the smaller step size 
...
!
end subroutine truerr_c1

recursive subroutine step_c1(comm,f,tnow,y,yp,tol,htry,    &
                     err,hmin,phase_2)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
...
logical, intent(inout), optional :: phase_2
!
complex(kind=wp), dimension(:), intent(in) :: y, yp                     !dep!
!
interface
   function f(t,y)
      use rksuite_90_prec, only:wp
...
intrinsic         abs, max, sign
!
complex(kind=wp), dimension(:), pointer :: errest, y_new                !dep!
complex(kind=wp), dimension(:,:), pointer :: stages                     !dep!
...
!
   real(kind=wp), intent(inout) :: htry                              !indep!
   complex(kind=wp), dimension(:), intent(in) :: ypnew                  !dep!
   logical, intent(out) :: cutbak
!
   real(kind=wp) :: argdif, fdiff, scl, tdiff, twt, ynrm, ystgnm
!
...
!
end subroutine step_c1


recursive subroutine stiff_c1(comm,f,toomch,sure_stiff)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
!
...
complex(kind=wp), dimension(2) :: root_pair, prv_root_pair
integer, parameter :: bigr=1, smlr=2
complex(kind=wp), dimension(:), pointer :: v0, v1, v2, v3, y, y_old     !dep!
real(kind=wp), dimension(:), pointer :: weights, thresh              !shp-dep!
!
integer, parameter :: max_f_count=5000
real(kind=wp),parameter :: zero=0.0_wp, pt001=0.001_wp, pt9=0.9_wp, &
...
!
real(kind=wp) :: vdotv
complex(kind=wp), dimension(:) :: v                         !dep!
complex(kind=wp), dimension(size(v,1)) :: approx_jacobian               !dep!
!
interface
   function f(t,y)
      use rksuite_90_prec, only:wp
...
function wt_inner_prod(vec_1,vec_2)
!
complex(kind=wp), dimension(:) :: vec_1, vec_2              !dep!
complex(kind=wp) :: wt_inner_prod                                       !dep!
!
!
wt_inner_prod = sum ( (vec_1/weights) * (conjg(vec_2)/weights) )            !spec-ar!
!
end function wt_inner_prod
...
end subroutine stiff_c1

subroutine statistics_c1(comm,total_f_calls,step_cost,waste,num_succ_steps,&
                         h_next,y_maxvals)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
...
" ** reported from the integrator. You cannot call STATISTICS again."
   ...
   state = get_saved_state_c1("STEP_INTEGRATE",comm%save_states)
   if (state==not_ready) then
      ier = fatal; nrec = 1
      if (comm%use_range) then
         write (comm%rec,"(a)") &
...
call rkmsg_c1(ier,srname,nrec,comm)
!
end subroutine statistics_c1

subroutine global_error_c1(comm,rms_error,max_error,t_max_error)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
!
...
call rkmsg_c1(ier,srname,nrec,comm)
!
end subroutine global_error_c1

subroutine reset_t_end_c1(comm,t_end_new)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
!
...
call rkmsg_c1(ier,srname,nrec,comm)
!
end subroutine reset_t_end_c1

recursive subroutine interpolate_c1(comm,f,t_want,y_want,yderiv_want)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
!
...
real(kind=wp), intent(in) :: t_want                                  !indep!
type(rk_comm_complex_1d), intent(inout), target :: comm
complex(kind=wp), dimension(:), intent(out), optional :: y_want         !dep!
complex(kind=wp), dimension(:), intent(out), optional :: yderiv_want    !dep!
!
interface
   function f(t,y)
      use rksuite_90_prec, only:wp
...
!  step
!
   intrp_initialised = get_saved_state_c1(srname,comm%save_states) /= usable
!
!  Some initialization must be done before interpolation is possible.
!
   if (.not.intrp_initialised) call form_intrp(f,comm%p)
...
real(kind=wp), dimension(:,:), pointer :: r                          !real!
complex(kind=wp), dimension(:,:), pointer :: stages                     !dep!
complex(kind=wp), dimension(:), pointer :: y, yp, y_old, yp_old         !dep!
complex(kind=wp), dimension(:), pointer :: xstage                       !dep!
!
stages => comm%stages
r => comm%r
y => comm%y
...
subroutine evaluate_intrp(p,y_want,yderiv_want)
!
complex(kind=wp), dimension(:), optional, intent(out) :: y_want         !dep!
complex(kind=wp), dimension(:), optional, intent(out) :: yderiv_want    !dep!
complex(kind=wp), dimension(:,:), intent(in) :: p                       !dep!
!
real :: sigma
integer :: i
...
recursive subroutine extra_stages(f,ytemp,xstage)
!
complex(kind=wp), dimension(:), intent(out) :: ytemp, xstage            !dep!
!
interface 
   function f(t,y)
      use rksuite_90_prec, only:wp
...
!
complex(kind=wp), dimension(:,:), pointer :: stages                     !dep!
complex(kind=wp), dimension(:), pointer :: yp, y_old, yp_old            !dep!
!
real(kind=wp), dimension(:,:), pointer :: a                          !real!
real(kind=wp), dimension(:), pointer :: c                            !real!
real(kind=wp), pointer :: h_old, t_old                               !indep!
...
end subroutine interpolate_c1

subroutine rkmsg_c1(ier,srname,nrec,comm,flag)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
!
...
!  Save the status of the routine associated with SRNAME
!
call set_saved_state_c1(srname,ier,comm)
!
!  Indicate that a catastrophic error has been detected
!
!call set_saved_fatal_c1(comm,ier >= catastrophe)
!
end subroutine rkmsg_c1

subroutine set_saved_state_c1(srname,state,comm)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
!
...
comm%saved_fatal_err = state >= fatal
!
end subroutine set_saved_state_c1

function get_saved_state_c1(srname,save_states)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
!
...
!
if (any(save_states(1:7)==fatal)) then
   get_saved_state_c1 = fatal
else
   get_saved_state_c1 = save_states(name)
end if
!
end function get_saved_state_c1

function get_saved_fatal_c1(comm)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
!
...
!          see main doc for contact details
!
type(rk_comm_complex_1d), intent(in) :: comm
logical :: get_saved_fatal_c1
!
get_saved_fatal_c1 = comm%saved_fatal_err
!
end function get_saved_fatal_c1

subroutine set_stop_on_fatal_c1(comm,action)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
!
...
comm%stop_on_fatal = action
!
end subroutine set_stop_on_fatal_c1

function get_stop_on_fatal_c1(comm)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
!
...
!
type(rk_comm_complex_1d), intent(in) :: comm
logical get_stop_on_fatal_c1
!
get_stop_on_fatal_c1 = comm%stop_on_fatal
!
end function get_stop_on_fatal_c1

!endc!
!startc!
subroutine setup_c2(comm,t_start,y_start,t_end,tolerance,thresholds, &
                    method,task,error_assess,h_start,message)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
...
!  Allocate a number of arrays using pointers.
!
   allocate(comm%thresh(size(y_start,1),size(y_start,2)), &                          !alloc!
            comm%err_estimates(size(y_start,1),size(y_start,2)), &                   !alloc!
            comm%weights(size(y_start,1),size(y_start,2)), &                         !alloc!
            comm%y_old(size(y_start,1),size(y_start,2)), &                           !alloc!
            comm%scratch(size(y_start,1),size(y_start,2)), &                         !alloc!
            comm%y(size(y_start,1),size(y_start,2)), &                               !alloc!
            comm%yp(size(y_start,1),size(y_start,2)), &                              !alloc!
            comm%stages(size(y_start,1),size(y_start,2),tr_dim_of_stages), &         !alloc!
            comm%ymax(size(y_start,1),size(y_start,2)),stat=ier)                     !alloc!
   if (ier /= 0) then
      ier = fatal; nrec = 1 ; write (comm%rec,"(a)") &
" ** Not enough storage available to create workspace required internally."
      exit body
   ...
   if (comm%use_range) then
      if (comm%intrp_able) then
         if (comm%rk_method==1) then
            ...
            allocate(comm%p(size(y_start,1),size(y_start,2),5), &                    !alloc!
                     comm%ytemp(size(y_start,1),size(y_start,2)), &                  !alloc!
                     comm%xstage(size(y_start,1),size(y_start,2)),stat=ier)          !alloc!
            if (ier /= 0) then
               ier = fatal; nrec = 1 ; write (comm%rec,"(a)") &
" ** Not enough storage available to create workspace required internally."
               exit body
...
!  respectively).
      if (.not.comm%intrp_needs_stages) then
         ...
         allocate(comm%ge_stages(size(y_start,1),size(y_start,2),tr_dim_of_stages),stat=ier) !alloc!
         if (ier /= 0) then
            ier = fatal; nrec = 1 ; write (comm%rec,"(a)") &
" ** Not enough storage available to create workspace required internally."
            exit body
      ...
      allocate(comm%ge_y(size(y_start,1),size(y_start,2)), &                         !alloc!
               comm%ge_yp(size(y_start,1),size(y_start,2)), &                        !alloc!
               comm%ge_err_estimates(size(y_start,1),size(y_start,2)), &             !alloc!
               comm%ge_assess(size(y_start,1),size(y_start,2)), &                    !alloc!
               comm%ge_y_new(size(y_start,1),size(y_start,2)),stat=ier)              !alloc!
      if (ier /= 0) then
         ier = fatal; nrec = 1 ; write (comm%rec,"(a)") &
" ** Not enough storage available to create workspace required internally."
         exit body
...
!
call rkmsg_c2(ier,srname,nrec,comm)
!
contains
...
subroutine collect_garbage_c2(comm)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
!
...
!
end subroutine collect_garbage_c2

recursive subroutine range_integrate_c2(comm,f,t_want,t_got,y_got,yderiv_got, &
                                        flag)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
...
complex(kind=wp), dimension(:,:), intent(out), optional :: y_got, yderiv_got !dep!
integer, intent(out), optional :: flag
type(rk_comm_complex_2d), intent(inout) :: comm
!
interface 
   function f(t,y)
      use rksuite_90_prec, only:wp
      real(kind=wp), intent(in) :: t                                 !indep!
      complex(kind=wp), dimension(:,:), intent(in) :: y                   !dep!
      complex(kind=wp), dimension(size(y,1),size(y,2)) :: f                       !dep!
   end function f
end interface
!
character(len=*), parameter :: srname="RANGE_INTEGRATE"
...
!  Is it permissible to call RANGE_INTEGRATE?
!
   state = get_saved_state_c2("SETUP",comm%save_states)
   if (state==fatal) then
      ier = catastrophe; nrec = 1; write (comm%rec,"(a)") &
" ** A catastrophic error has already been detected elsewhere."
      exit body
...
" ** were going to use STEP_INTEGRATE. This is not permitted."
   ...
   state = get_saved_state_c2(srname,comm%save_states)
   if (state==5 .or. state==6) then
      ier = fatal; nrec = 1; write (comm%rec,"(a/a)") &
" ** This routine has already returned with a hard failure. You must call",&
" ** SETUP to start another problem."
   ...
   state = usable
   call set_saved_state_c2(srname,state,comm)
!
   if (comm%at_t_start) then
!
!  First call.
...
!
      if (.not.comm%at_t_start) then
         call reset_t_end(comm,t_want)
         baderr = get_saved_fatal_c2(comm)
         if (baderr) exit body
      end if
   end if
!
...
!  what happened
!
call rkmsg_c2(ier,srname,nrec,comm,flag)
!
end subroutine range_integrate_c2

recursive subroutine step_integrate_c2(comm,f,t_now,y_now,yderiv_now,flag)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
!
...
" ** You have not called SETUP, so you cannot use STEP_INTEGRATE."
   ...
   if (comm%use_range) then
      if (get_saved_state_c2("RANGE_INTEGRATE",comm%save_states)/=usable) then
         ier = fatal; nrec = 2; write (comm%rec,"(a/a)") &
" ** You have called STEP_INTEGRATE after you specified in SETUP that you", &
" ** were going to use RANGE_INTEGRATE. This is not permitted."
         comm%use_range = .false.
...
!  stiffness.
!
         call stiff_c2(comm,f,toomch,sure_stiff)
         if (sure_stiff) then
!
!  Predict how much extra work will be needed to reach TND.
            extra_wk = (comm%cost*abs((comm%t_end-comm%t)/comm%h_average)) / &
...
!
      htry = comm%h
      phase2 = comm%phase2
      call step_c2(comm,f,comm%t,comm%y,comm%yp,comm%tol,htry, &
                   err,hmin,comm%phase2)
      comm%phase2 = phase2
      comm%h = htry
!
...
if (ier < fatal) then
   ...
   comm%ymax = max(abs(comm%y),comm%ymax)
   if (ier==just_fine) then
      state = usable; call set_saved_state_c2("INTERPOLATE",state,comm)
   end if
end if
!
!  Call RKMSG_R1 to report what happened and set FLAG.
...
call rkmsg_c2(ier,srname,nrec,comm,flag)
!
end subroutine step_integrate_c2


recursive subroutine truerr_c2(comm,f,ier)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
!
...
!          see main doc for contact details
!
type(rk_comm_complex_2d), intent(inout), target :: comm
integer, intent(inout) :: ier
!
interface
   function f(t,y)
...
!
!  Take a step.
      call step_c2(comm,f,tsec,ge_y,ge_yp,ge_test1,hsec, &
         ge_err)
!
!  The primary integration is using a step size of H_OLD and the
!  secondary integration is using the smaller step size 
...
!
end subroutine truerr_c2

recursive subroutine step_c2(comm,f,tnow,y,yp,tol,htry,    &
                     err,hmin,phase_2)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
...
!
end subroutine step_c2


recursive subroutine stiff_c2(comm,f,toomch,sure_stiff)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
!
...
real(kind=wp) :: vdotv
complex(kind=wp), dimension(:,:) :: v                         !dep!
complex(kind=wp), dimension(size(v,1),size(v,2)) :: approx_jacobian               !dep!
!
interface
   function f(t,y)
      use rksuite_90_prec, only:wp
...
end subroutine stiff_c2

subroutine statistics_c2(comm,total_f_calls,step_cost,waste,num_succ_steps,&
                         h_next,y_maxvals)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
...
" ** reported from the integrator. You cannot call STATISTICS again."
   ...
   state = get_saved_state_c2("STEP_INTEGRATE",comm%save_states)
   if (state==not_ready) then
      ier = fatal; nrec = 1
      if (comm%use_range) then
         write (comm%rec,"(a)") &
...
call rkmsg_c2(ier,srname,nrec,comm)
!
end subroutine statistics_c2

subroutine global_error_c2(comm,rms_error,max_error,t_max_error)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
!
...
call rkmsg_c2(ier,srname,nrec,comm)
!
end subroutine global_error_c2

subroutine reset_t_end_c2(comm,t_end_new)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
!
...
call rkmsg_c2(ier,srname,nrec,comm)
!
end subroutine reset_t_end_c2

recursive subroutine interpolate_c2(comm,f,t_want,y_want,yderiv_want)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
!
...
!
   ...
   case(2)
      jer = 0
      if (.not.associated(comm%xstage)) then
         allocate(comm%xstage(size(comm%y,1),size(comm%y,2)),stat=jer)              !alloc!
      end if
      if (.not.associated(comm%ytemp)) then
         allocate(comm%ytemp(size(comm%y,1),size(comm%y,2)),stat=jer)               !alloc!
      end if
      if (.not.associated(comm%p)) then
         allocate(comm%p(size(comm%y,1),size(comm%y,2),5),stat=jer)                 !alloc!
      end if
      npcls = 5
      if (jer /= 0) then
         ier = fatal; nrec = 1 ; write (comm%rec,"(a)") &
...
!  step
!
   intrp_initialised = get_saved_state_c2(srname,comm%save_states) /= usable
!
!  Some initialization must be done before interpolation is possible.
!
   if (.not.intrp_initialised) call form_intrp(f,comm%p)
...
end subroutine interpolate_c2

subroutine rkmsg_c2(ier,srname,nrec,comm,flag)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
!
...
!  Save the status of the routine associated with SRNAME
!
call set_saved_state_c2(srname,ier,comm)
!
!  Indicate that a catastrophic error has been detected
!
!call set_saved_fatal_c2(comm,ier >= catastrophe)
!
end subroutine rkmsg_c2

subroutine set_saved_state_c2(srname,state,comm)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
!
...
comm%saved_fatal_err = state >= fatal
!
end subroutine set_saved_state_c2

function get_saved_state_c2(srname,save_states)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
!
...
!
if (any(save_states(1:7)==fatal)) then
   get_saved_state_c2 = fatal
else
   get_saved_state_c2 = save_states(name)
end if
!
end function get_saved_state_c2

function get_saved_fatal_c2(comm)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
!
...
!          see main doc for contact details
!
type(rk_comm_complex_2d), intent(in) :: comm
logical :: get_saved_fatal_c2
!
get_saved_fatal_c2 = comm%saved_fatal_err
!
end function get_saved_fatal_c2

subroutine set_stop_on_fatal_c2(comm,action)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
!
...
comm%stop_on_fatal = action
!
end subroutine set_stop_on_fatal_c2

function get_stop_on_fatal_c2(comm)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
!
...
!
type(rk_comm_complex_2d), intent(in) :: comm
logical get_stop_on_fatal_c2
!
get_stop_on_fatal_c2 = comm%stop_on_fatal
!
end function get_stop_on_fatal_c2

!endc!
!startc!
subroutine setup_r0(comm,t_start,y_start,t_end,tolerance,thresholds, &
                    method,task,error_assess,h_start,message)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
...
real(kind=wp), intent(in) :: t_end, t_start                          !indep!
real(kind=wp), intent(in) :: tolerance
real(kind=wp), intent(in) :: y_start                   !dep!
real(kind=wp), intent(in) :: thresholds                !shp-dep!
...
!
call rkmsg_r0(ier,srname,nrec,comm)
!
contains
...
end subroutine setup_r0


subroutine collect_garbage_r0(comm)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
!
...
!
end subroutine collect_garbage_r0

recursive subroutine range_integrate_r0(comm,f,t_want,t_got,y_got,yderiv_got, &
                                        flag)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
...
real(kind=wp), intent(in) :: t_want                                  !indep!
real(kind=wp), intent(out) :: t_got                                  !indep!
real(kind=wp), intent(out), optional :: y_got, yderiv_got !dep!
integer, intent(out), optional :: flag
type(rk_comm_real_0d), intent(inout) :: comm
!
interface 
   function f(t,y)
      use rksuite_90_prec, only:wp
      real(kind=wp), intent(in) :: t                                 !indep!
      real(kind=wp), intent(in) :: y                   !dep!
      real(kind=wp) :: f                       !dep!
   end function f
end interface
!
...
!  Is it permissible to call RANGE_INTEGRATE?
!
   state = get_saved_state_r0("SETUP",comm%save_states)
   if (state==fatal) then
      ier = catastrophe; nrec = 1; write (comm%rec,"(a)") &
" ** A catastrophic error has already been detected elsewhere."
      exit body
...
" ** were going to use STEP_INTEGRATE. This is not permitted."
   ...
   state = get_saved_state_r0(srname,comm%save_states)
   if (state==5 .or. state==6) then
      ier = fatal; nrec = 1; write (comm%rec,"(a/a)") &
" ** This routine has already returned with a hard failure. You must call",&
" ** SETUP to start another problem."
   ...
   state = usable
   call set_saved_state_r0(srname,state,comm)
!
   if (comm%at_t_start) then
!
!  First call.
...
!
      if (.not.comm%at_t_start) then
         call reset_t_end(comm,t_want)
         baderr = get_saved_fatal_r0(comm)
         if (baderr) exit body
      end if
   end if
!
...
!  what happened
!
call rkmsg_r0(ier,srname,nrec,comm,flag)
!
end subroutine range_integrate_r0

recursive subroutine step_integrate_r0(comm,f,t_now,y_now,yderiv_now,flag)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
!
...
integer, intent(out), optional :: flag
type(rk_comm_real_0d), intent(inout) :: comm
real(kind=wp), intent(out), optional :: y_now, yderiv_now !dep!
interface
   function f(t,y)
      use rksuite_90_prec, only:wp
      real(kind=wp), intent(in) :: t                                 !indep!
...
" ** You have not called SETUP, so you cannot use STEP_INTEGRATE."
   ...
   if (comm%use_range) then
      if (get_saved_state_r0("RANGE_INTEGRATE",comm%save_states)/=usable) then
         ier = fatal; nrec = 2; write (comm%rec,"(a/a)") &
" ** You have called STEP_INTEGRATE after you specified in SETUP that you", &
" ** were going to use RANGE_INTEGRATE. This is not permitted."
         comm%use_range = .false.
...
!  stiffness.
!
         call stiff_r0(comm,f,toomch,sure_stiff)
         if (sure_stiff) then
!
!  Predict how much extra work will be needed to reach TND.
            extra_wk = (comm%cost*abs((comm%t_end-comm%t)/comm%h_average)) / &
...
!
      htry = comm%h
      phase2 = comm%phase2
      call step_r0(comm,f,comm%t,comm%y,comm%yp,comm%tol,htry, &
                   err,hmin,comm%phase2)
      comm%phase2 = phase2
      comm%h = htry
!
...
if (ier < fatal) then
   ...
   comm%ymax = max(abs(comm%y),comm%ymax)
   if (ier==just_fine) then
      state = usable; call set_saved_state_r0("INTERPOLATE",state,comm)
   end if
end if
!
!  Call RKMSG_R1 to report what happened and set FLAG.
...
call rkmsg_r0(ier,srname,nrec,comm,flag)
!
end subroutine step_integrate_r0


recursive subroutine truerr_r0(comm,f,ier)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
!
...
!          see main doc for contact details
!
type(rk_comm_real_0d), intent(inout), target :: comm
integer, intent(inout) :: ier
!
interface
   function f(t,y)
...
!
!  Take a step.
      call step_r0(comm,f,tsec,ge_y,ge_yp,ge_test1,hsec, &
         ge_err)
!
!  The primary integration is using a step size of H_OLD and the
!  secondary integration is using the smaller step size 
...
!
end subroutine truerr_r0

recursive subroutine step_r0(comm,f,tnow,y,yp,tol,htry,    &
                     err,hmin,phase_2)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
...
logical, intent(inout), optional :: phase_2
!
real(kind=wp), intent(in) :: y, yp                     !dep!
!
interface
   function f(t,y)
      use rksuite_90_prec, only:wp
...
intrinsic         abs, max, sign
!
real(kind=wp), pointer :: errest, y_new                !dep!
real(kind=wp), dimension(:), pointer :: stages                     !dep!
real(kind=wp), pointer :: weights, thresh              !shp-dep!
real(kind=wp), dimension(:,:), pointer :: a                          !real!
real(kind=wp), dimension(:), pointer :: b, bhat, c                   !real!
...
!
   real(kind=wp), intent(inout) :: htry                              !indep!
   real(kind=wp), intent(in) :: ypnew                  !dep!
   logical, intent(out) :: cutbak
!
   real(kind=wp) :: argdif, fdiff, scl, tdiff, twt, ynrm, ystgnm
!
...
!
end subroutine step_r0


recursive subroutine stiff_r0(comm,f,toomch,sure_stiff)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
!
...
complex(kind=wp), dimension(2) :: root_pair, prv_root_pair
integer, parameter :: bigr=1, smlr=2
real(kind=wp), pointer :: v0, v1, v2, v3, y, y_old     !dep!
real(kind=wp), pointer :: weights, thresh              !shp-dep!
!
integer, parameter :: max_f_count=5000
real(kind=wp),parameter :: zero=0.0_wp, pt001=0.001_wp, pt9=0.9_wp, &
...
real(kind=wp) :: vdotv
real(kind=wp) :: v                         !dep!
real(kind=wp) :: approx_jacobian               !dep!
!
interface
   function f(t,y)
      use rksuite_90_prec, only:wp
...
function wt_inner_prod(vec_1,vec_2)
!
real(kind=wp) :: vec_1, vec_2              !dep!
real(kind=wp) :: wt_inner_prod                                       !dep!
!
!
wt_inner_prod =  ( (vec_1/weights) * (vec_2/weights) )            !spec-ar!
!
end function wt_inner_prod

function rev_wt_inner_prod(value)
!
real(kind=wp) :: value                                   !dep!
...
end subroutine stiff_r0

subroutine statistics_r0(comm,total_f_calls,step_cost,waste,num_succ_steps,&
                         h_next,y_maxvals)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
...
" ** reported from the integrator. You cannot call STATISTICS again."
   ...
   state = get_saved_state_r0("STEP_INTEGRATE",comm%save_states)
   if (state==not_ready) then
      ier = fatal; nrec = 1
      if (comm%use_range) then
         write (comm%rec,"(a)") &
...
call rkmsg_r0(ier,srname,nrec,comm)
!
end subroutine statistics_r0

subroutine global_error_r0(comm,rms_error,max_error,t_max_error)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
!
...
call rkmsg_r0(ier,srname,nrec,comm)
!
end subroutine global_error_r0

subroutine reset_t_end_r0(comm,t_end_new)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
!
...
call rkmsg_r0(ier,srname,nrec,comm)
!
end subroutine reset_t_end_r0

recursive subroutine interpolate_r0(comm,f,t_want,y_want,yderiv_want)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
!
...
real(kind=wp), intent(in) :: t_want                                  !indep!
type(rk_comm_real_0d), intent(inout), target :: comm
real(kind=wp), intent(out), optional :: y_want         !dep!
real(kind=wp), intent(out), optional :: yderiv_want    !dep!
!
interface
   function f(t,y)
      use rksuite_90_prec, only:wp
...
!  step
!
   intrp_initialised = get_saved_state_r0(srname,comm%save_states) /= usable
!
!  Some initialization must be done before interpolation is possible.
!
   if (.not.intrp_initialised) call form_intrp(f,comm%p)
...
real(kind=wp), dimension(:,:), pointer :: r                          !real!
real(kind=wp), dimension(:), pointer :: stages                     !dep!
real(kind=wp), pointer :: y, yp, y_old, yp_old         !dep!
real(kind=wp), pointer :: xstage                       !dep!
!
stages => comm%stages
r => comm%r
y => comm%y
...
subroutine evaluate_intrp(p,y_want,yderiv_want)
!
real(kind=wp), optional, intent(out) :: y_want         !dep!
real(kind=wp), optional, intent(out) :: yderiv_want    !dep!
...
recursive subroutine extra_stages(f,ytemp,xstage)
!
real(kind=wp), intent(out) :: ytemp, xstage            !dep!
!
interface 
   function f(t,y)
      use rksuite_90_prec, only:wp
...
!
real(kind=wp), dimension(:), pointer :: stages                     !dep!
real(kind=wp), pointer :: yp, y_old, yp_old            !dep!
!
real(kind=wp), dimension(:,:), pointer :: a                          !real!
real(kind=wp), dimension(:), pointer :: c                            !real!
real(kind=wp), pointer :: h_old, t_old                               !indep!
...
end subroutine interpolate_r0

subroutine rkmsg_r0(ier,srname,nrec,comm,flag)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
!
...
!  Save the status of the routine associated with SRNAME
!
call set_saved_state_r0(srname,ier,comm)
!
!  Indicate that a catastrophic error has been detected
!
!call set_saved_fatal_r0(comm,ier >= catastrophe)
!
end subroutine rkmsg_r0

subroutine set_saved_state_r0(srname,state,comm)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
!
...
comm%saved_fatal_err = state >= fatal
!
end subroutine set_saved_state_r0

function get_saved_state_r0(srname,save_states)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
!
...
!
if (any(save_states(1:7)==fatal)) then
   get_saved_state_r0 = fatal
else
   get_saved_state_r0 = save_states(name)
end if
!
end function get_saved_state_r0

function get_saved_fatal_r0(comm)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
!
...
!          see main doc for contact details
!
type(rk_comm_real_0d), intent(in) :: comm
logical :: get_saved_fatal_r0
!
get_saved_fatal_r0 = comm%saved_fatal_err
!
end function get_saved_fatal_r0

subroutine set_stop_on_fatal_r0(comm,action)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
!
...
comm%stop_on_fatal = action
!
end subroutine set_stop_on_fatal_r0

function get_stop_on_fatal_r0(comm)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
!
...
!
type(rk_comm_real_0d), intent(in) :: comm
logical get_stop_on_fatal_r0
!
get_stop_on_fatal_r0 = comm%stop_on_fatal
!
end function get_stop_on_fatal_r0
...
real(kind=wp), intent(in) :: t_want                                  !indep!
real(kind=wp), intent(out) :: t_got                                  !indep!
real(kind=wp), dimension(:), intent(out), optional :: y_got, yderiv_got !dep!
integer, intent(out), optional :: flag
...
integer, intent(out), optional :: flag
type(rk_comm_real_1d), intent(inout) :: comm
real(kind=wp), dimension(:), intent(out), optional :: y_now, yderiv_now !dep!
interface
   function f(t,y)
      use rksuite_90_prec, only:wp
      real(kind=wp), intent(in) :: t                                 !indep!
...
!
      htry = comm%h
      phase2 = comm%phase2
      call step_r1(comm,f,comm%t,comm%y,comm%yp,comm%tol,htry, &
                   err,hmin,comm%phase2)
      comm%phase2 = phase2
      comm%h = htry
!
...
recursive subroutine truerr_r1(comm,f,ier)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
!
...
real(kind=wp), dimension(:,:), pointer :: ge_stages                  !dep!
real(kind=wp), dimension(:), pointer :: ge_y, ge_yp, ge_y_new        !dep!
real(kind=wp), dimension(:), pointer :: y                            !dep!
real(kind=wp), dimension(:), pointer :: ge_assess, weights           !shp-dep!
!
ge_stages => comm%ge_stages
ge_y => comm%ge_y
...
!
!  Take a step.
      call step_r1(comm,f,tsec,ge_y,ge_yp,ge_test1,hsec, &
         ge_err)
!
!  The primary integration is using a step size of H_OLD and the
!  secondary integration is using the smaller step size 
...
recursive subroutine step_r1(comm,f,tnow,y,yp,tol,htry,    &
                     err,hmin,phase_2)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
...
intrinsic         abs, max, sign
!
real(kind=wp), dimension(:), pointer :: errest, y_new                !dep!
real(kind=wp), dimension(:,:), pointer :: stages                     !dep!
real(kind=wp), dimension(:), pointer :: weights, thresh              !shp-dep!
real(kind=wp), dimension(:,:), pointer :: a                          !real!
real(kind=wp), dimension(:), pointer :: b, bhat, c                   !real!
...
recursive subroutine stiff_r1(comm,f,toomch,sure_stiff)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
!
...
function wt_inner_prod(vec_1,vec_2)
!
real(kind=wp), dimension(:) :: vec_1, vec_2              !dep!
real(kind=wp) :: wt_inner_prod                                       !dep!
!
!
...
recursive subroutine interpolate_r1(comm,f,t_want,y_want,yderiv_want)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
!
...
!endc!
!startc!
subroutine setup_r2(comm,t_start,y_start,t_end,tolerance,thresholds, &
                    method,task,error_assess,h_start,message)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
...
!
call rkmsg_r2(ier,srname,nrec,comm)
!
contains
...
end subroutine setup_r2


subroutine collect_garbage_r2(comm)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
!
...
!
end subroutine collect_garbage_r2

recursive subroutine range_integrate_r2(comm,f,t_want,t_got,y_got,yderiv_got, &
                                        flag)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
...
real(kind=wp), dimension(:,:), intent(out), optional :: y_got, yderiv_got !dep!
integer, intent(out), optional :: flag
type(rk_comm_real_2d), intent(inout) :: comm
!
interface 
   function f(t,y)
      use rksuite_90_prec, only:wp
      real(kind=wp), intent(in) :: t                                 !indep!
      real(kind=wp), dimension(:,:), intent(in) :: y                   !dep!
      real(kind=wp), dimension(size(y,1),size(y,2)) :: f                       !dep!
   end function f
end interface
!
character(len=*), parameter :: srname="RANGE_INTEGRATE"
...
!  Is it permissible to call RANGE_INTEGRATE?
!
   state = get_saved_state_r2("SETUP",comm%save_states)
   if (state==fatal) then
      ier = catastrophe; nrec = 1; write (comm%rec,"(a)") &
" ** A catastrophic error has already been detected elsewhere."
      exit body
...
" ** were going to use STEP_INTEGRATE. This is not permitted."
   ...
   state = get_saved_state_r2(srname,comm%save_states)
   if (state==5 .or. state==6) then
      ier = fatal; nrec = 1; write (comm%rec,"(a/a)") &
" ** This routine has already returned with a hard failure. You must call",&
" ** SETUP to start another problem."
   ...
   state = usable
   call set_saved_state_r2(srname,state,comm)
!
   if (comm%at_t_start) then
!
!  First call.
...
!
      if (.not.comm%at_t_start) then
         call reset_t_end(comm,t_want)
         baderr = get_saved_fatal_r2(comm)
         if (baderr) exit body
      end if
   end if
!
...
!  what happened
!
call rkmsg_r2(ier,srname,nrec,comm,flag)
!
end subroutine range_integrate_r2

recursive subroutine step_integrate_r2(comm,f,t_now,y_now,yderiv_now,flag)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
!
...
" ** You have not called SETUP, so you cannot use STEP_INTEGRATE."
   ...
   if (comm%use_range) then
      if (get_saved_state_r2("RANGE_INTEGRATE",comm%save_states)/=usable) then
         ier = fatal; nrec = 2; write (comm%rec,"(a/a)") &
" ** You have called STEP_INTEGRATE after you specified in SETUP that you", &
" ** were going to use RANGE_INTEGRATE. This is not permitted."
         comm%use_range = .false.
...
!  stiffness.
!
         call stiff_r2(comm,f,toomch,sure_stiff)
         if (sure_stiff) then
!
!  Predict how much extra work will be needed to reach TND.
            extra_wk = (comm%cost*abs((comm%t_end-comm%t)/comm%h_average)) / &
...
!
      htry = comm%h
      phase2 = comm%phase2
      call step_r2(comm,f,comm%t,comm%y,comm%yp,comm%tol,htry, &
                   err,hmin,comm%phase2)
      comm%phase2 = phase2
      comm%h = htry
!
...
if (ier < fatal) then
   ...
   comm%ymax = max(abs(comm%y),comm%ymax)
   if (ier==just_fine) then
      state = usable; call set_saved_state_r2("INTERPOLATE",state,comm)
   end if
end if
!
!  Call RKMSG_R1 to report what happened and set FLAG.
...
call rkmsg_r2(ier,srname,nrec,comm,flag)
!
end subroutine step_integrate_r2


recursive subroutine truerr_r2(comm,f,ier)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
!
...
!          see main doc for contact details
!
type(rk_comm_real_2d), intent(inout), target :: comm
integer, intent(inout) :: ier
!
interface
   function f(t,y)
...
!
!  Take a step.
      call step_r2(comm,f,tsec,ge_y,ge_yp,ge_test1,hsec, &
         ge_err)
!
!  The primary integration is using a step size of H_OLD and the
!  secondary integration is using the smaller step size 
...
!
end subroutine truerr_r2

recursive subroutine step_r2(comm,f,tnow,y,yp,tol,htry,    &
                     err,hmin,phase_2)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
...
!
end subroutine step_r2


recursive subroutine stiff_r2(comm,f,toomch,sure_stiff)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
!
...
real(kind=wp) :: vdotv
real(kind=wp), dimension(:,:) :: v                         !dep!
real(kind=wp), dimension(size(v,1),size(v,2)) :: approx_jacobian               !dep!
!
interface
   function f(t,y)
      use rksuite_90_prec, only:wp
...
end subroutine stiff_r2

subroutine statistics_r2(comm,total_f_calls,step_cost,waste,num_succ_steps,&
                         h_next,y_maxvals)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
...
" ** reported from the integrator. You cannot call STATISTICS again."
   ...
   state = get_saved_state_r2("STEP_INTEGRATE",comm%save_states)
   if (state==not_ready) then
      ier = fatal; nrec = 1
      if (comm%use_range) then
         write (comm%rec,"(a)") &
...
call rkmsg_r2(ier,srname,nrec,comm)
!
end subroutine statistics_r2

subroutine global_error_r2(comm,rms_error,max_error,t_max_error)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
!
...
call rkmsg_r2(ier,srname,nrec,comm)
!
end subroutine global_error_r2

subroutine reset_t_end_r2(comm,t_end_new)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
!
...
call rkmsg_r2(ier,srname,nrec,comm)
!
end subroutine reset_t_end_r2

recursive subroutine interpolate_r2(comm,f,t_want,y_want,yderiv_want)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
!
...
!  step
!
   intrp_initialised = get_saved_state_r2(srname,comm%save_states) /= usable
!
!  Some initialization must be done before interpolation is possible.
!
   if (.not.intrp_initialised) call form_intrp(f,comm%p)
...
end subroutine interpolate_r2

subroutine rkmsg_r2(ier,srname,nrec,comm,flag)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
!
...
!  Save the status of the routine associated with SRNAME
!
call set_saved_state_r2(srname,ier,comm)
!
!  Indicate that a catastrophic error has been detected
!
!call set_saved_fatal_r2(comm,ier >= catastrophe)
!
end subroutine rkmsg_r2

subroutine set_saved_state_r2(srname,state,comm)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
!
...
comm%saved_fatal_err = state >= fatal
!
end subroutine set_saved_state_r2

function get_saved_state_r2(srname,save_states)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
!
...
!
if (any(save_states(1:7)==fatal)) then
   get_saved_state_r2 = fatal
else
   get_saved_state_r2 = save_states(name)
end if
!
end function get_saved_state_r2

function get_saved_fatal_r2(comm)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
!
...
!          see main doc for contact details
!
type(rk_comm_real_2d), intent(in) :: comm
logical :: get_saved_fatal_r2
!
get_saved_fatal_r2 = comm%saved_fatal_err
!
end function get_saved_fatal_r2

subroutine set_stop_on_fatal_r2(comm,action)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
!
...
comm%stop_on_fatal = action
!
end subroutine set_stop_on_fatal_r2

function get_stop_on_fatal_r2(comm)
!
! Part of rksuite_90 v1.2 (December 1995)
!         software for initial value problems in ODEs
!
...
!
type(rk_comm_real_2d), intent(in) :: comm
logical get_stop_on_fatal_r2
!
get_stop_on_fatal_r2 = comm%stop_on_fatal
!
end function get_stop_on_fatal_r2
