/*----------------------------------------------------------------------
  ...
  This next piece of code is a customized version of the GNU qsort for
  the data type we are sorting here. It will be used if the -DQSORTOPT
  flag is used in compliation. It should generally make the code
  faster, but may be incompatible with some machine types. To
  determine if this qsort will work for you, first compile without the
  flag, time and test the code. Then try the same thing again with the
  flag on. If you do not get the same answers or the code is slower,
  than you should not use this option.

  ----------------------------------------------------------------------*/

#ifdef QSORTOPT
#include <alloca.h>		/* used by our optimized qsort */

/* Slightly Modified so the compare operation is in-lined. -T. Kolda */
...
#define CMP(A,B) (((sdddouble_plus*)(B))->val - ((sdddouble_plus*)(A))->val)
#define size     (sizeof(sdddouble_plus))
...
#define BYTES_PER_WORD (sizeof(ulong))
#endif
...
int qsortopt (char *base_ptr, int total_elems)
{
      ...
      while (STACK_NOT_EMPTY)
        {
          ...
          if ((right_ptr - lo) <= max_thresh)
            {
              if ((hi - left_ptr) <= max_thresh) /* Ignore both small
						    partitions. */
                POP (lo, hi); 
              else              /* Ignore small left partition. */  
                lo = left_ptr;
            }
          else if ((hi - left_ptr) <= max_thresh) /* Ignore small
						     right partition. */
            hi = right_ptr;
          else if ((right_ptr - lo) > (hi - left_ptr)) /* Push larger
							  left
							  partition
							  indices. */
            {                   
