...
double complex *SEQ_LTsamples_blktrd (unsigned int Nrows, double XY[], unsigned int NOPTS, double complex S[], double tol)
/**************************************************************************
...
**************************************************************************/
{
    ...
    mxArray *plhs[1], *prhs[5];     /* pointers to left and right side MATLAB parameters */
    double  *vr, *vi;
    ...
    prhs[4] = mxCreateDoubleScalar(tol);                 /* 5th param.: tol */
    ...
    /*  CALL THE MATLAB FUNCTION LT_samples:  U = LTsamples (NXYval,XY,NOPTS,S,tol,thrds)
        ...
        mexCallMATLAB(  1,  plhs,   5,  prhs, "LT_samples");
    ...
    if ( mxIsComplex(plhs[0]) )
        FS = cplxArray_fromMATLABtoC (Nrows*NOPTS, 0, (const mxArray **)plhs);
    else
        FS = realArray_fromMATLABtoC (Nrows*NOPTS, 0, (const mxArray **)plhs);
