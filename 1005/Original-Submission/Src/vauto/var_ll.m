...
%   covariance Sig. Sig and the Ai's are r×r matrices. A should contain the r ×
...
%
%   The r×n logical array miss should be true in missing locations. If missing
...
function [ll, ok, varargout] = var_ll(X, A, Sig, varargin)
  ...
  [ko,ro,km] = find_missing_info(miss); %#ok<ASGLU>
  mubar = repmat(mu, n, 1);
  X = X(:);
