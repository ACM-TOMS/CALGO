PROGRAM TEST_STBSV_RMD
  use rmd_stesttools
  implicit none
  procedure(F_interf)     :: F
  procedure(F_rmd_interf) :: F_rmd
  real :: tol
  integer :: n, k, ul, tr, di, esa, incx, i, j, dimu
  type(fdata) :: testcase
  character :: uplo(2), trans(2), diag(2)
  real, allocatable :: A(:, :), u(:)
  !
  tol = rmd_stolerance
  uplo = ['u', 'l']
  trans = ['n', 't']
          ...
          do di = 1, 2
            do esa = 0, 3, 3
              ...
              do incx = 1, 3, 2
                ...
                do i = 1, k+1
                  do j = 1, n
                    A(i, j) = rmd_srandom()*4.0 - 2.0
                  end do
                end do
                if(uplo(ul) == 'u') then
                  do i = 1, n
                    A(k+1, i) = rmd_srandom() + 2
                    if(rmd_srandom() < 0.5) A(k+1, i) = -A(k+1, i)
                  end do
                else
                  do i = 1, n
                    A(1, i) = rmd_srandom() + 2
                    if(rmd_srandom() < 0.5) A(1, i) = -A(1, i)
                  end do
                end if
                !
                if(uplo(ul) == 'u') then
                  call rmd_sbandshape_m2v(n, n, 0, k, u, A, k+1+esa)
                else
                  call rmd_sbandshape_m2v(n, n, k, 0, u, A, k+1+esa)
                end if
                !
                ! x
                do i = 1, n
                  u(dimu - n + i) = rmd_srandom()*4.0 - 2.0
                end do
                !
                testcase = fdata([real::], [n, k, esa, incx], [uplo(ul), trans(tr), diag(di)])
                call rmd_stestf(F, F_rmd, u, dimu, n, tol, testcase)
                !        
              end do
              deallocate(A, u)
            end do
...
SUBROUTINE F(testcase, u, v)
  ...
  !
  call stbsv(uplo, trans, diag, n, k, A, k+1+esa, x, incx)
  !
...
SUBROUTINE F_RMD(testcase, u, v, ua, va)
  ...
  !
  call stbsv_rmd(uplo, trans, diag, n, k, A, k+1+esa, x, incx, Aa, xa, dummy, '11')
  !
  if(uplo == 'u' .or. uplo == 'U') then
