...
if (not os.path.exists(EXE_AT)) or (not os.path.exists(EXE_ST)) or (not os.path.exists(EXE_TT_SB)) or (not os.path.exists(EXE_TT_SF)):
    ...
    quit()
def print_error():
    print('run_all_tests [-nruns/-nr <number of runs for timing tests>|-nstat/-ns <number of samples for statistical tests|')
    print('-ntimings/-nt <number of samples for timing tests>]')
    quit()
