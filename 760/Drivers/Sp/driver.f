...
*     ..
* Data statements
      DATA             NMPR/'TPRG3P'/,NMWF/'WFRG3P'/,NMSR/'RGBI3P',
     +                 'RGSF3P'/,LBL/'Calculated ZI Values',
     +                 'Differences         '/
      ...
      DO 50 IYI = 1,NYI
          DO 40 IXI = 1,NXI
              ...
              ELSE
                  MD = 2
              END IF
              CALL RGBI3P(MD,NXD,NYD,XD,YD,ZD,1,XI(IXI),YI(IYI),
     +                    ZI(IXI,IYI),IER, WK)
              IF (IER.GT.0) STOP
...
* - Writes the calculated results.
      WRITE (6,FMT=9030) NEWPG,NMPR,NMSR(1),LBL(1)
      DO 70 ISEC = 1,2
          IF (ISEC.EQ.1) THEN
              IXIMN = 1
              IXIMX = 11
...
* - Writes the differences.
      WRITE (6,FMT=9030) NEWPG,NMPR,NMSR(1),LBL(2)
      DO 90 ISEC = 1,2
          IF (ISEC.EQ.1) THEN
              IXIMN = 1
              IXIMX = 11
...
* - Performs surface fitting and calculates the differences.
      MD = 1
      CALL RGSF3P(MD,NXD,NYD,XD,YD,ZD,NXI,XI,NYI,YI, ZI,IER, WK)
      IF (IER.GT.0) STOP
      DO 110 IYI = 1,NYI
          DO 100 IXI = 1,NXI
              DZI(IXI,IYI) = ZI(IXI,IYI) - ZIE(IXI,IYI)
  ...
  110 CONTINUE
* - Writes the calculated results.
      WRITE (6,FMT=9030) NEWPG,NMPR,NMSR(2),LBL(1)
      DO 130 ISEC = 1,2
          IF (ISEC.EQ.1) THEN
              IXIMN = 1
              IXIMX = 11
  ...
  130 CONTINUE
* - Writes the differences.
      WRITE (6,FMT=9030) NEWPG,NMPR,NMSR(2),LBL(2)
      DO 150 ISEC = 1,2
          IF (ISEC.EQ.1) THEN
              IXIMN = 1
              IXIMX = 11
  ...
  150 CONTINUE
      STOP
* Format statements
 9000 FORMAT (A6,10X,'Original Data',/,/,/,/,35X,'ZD(XD,YD)')
 9010 FORMAT (4X,'YD    XD=',/,7X,F8.1,2 (1X,3F6.1,F7.1),/)
 9020 FORMAT (1X,F6.1,F8.1,2 (1X,3F6.1,F7.1))
 9030 FORMAT (A1,A6,6X,'Program Check for ',A6,6X,A20)
 9040 FORMAT (1X,/,38X,'ZI(XI,YI)',/,2X,'YI',3X,'XI=',/,5X,3F7.2,2F6.2,
     +       2F7.2,2F6.2,2F7.2,/)
 9050 FORMAT (F5.2,3F7.2,2F6.2,2F7.2,2F6.2,2F7.2)
 9060 FORMAT (1X,/,38X,'DZI(XI,YI)',/,2X,'YI',3X,'XI=',/,5X,3F7.2,2F6.2,
     +       2F7.2,2F6.2,2F7.2,/)
      END
