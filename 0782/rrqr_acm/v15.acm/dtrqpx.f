      SUBROUTINE DTRQPX( JOB, M, N, K, A, LDA, C, LDC, JPVT, IRCOND,
...
*     ..
*     .. Array Arguments ..
      DOUBLE PRECISION   A( LDA, * ), C( LDC, * ), SVLUES( 4 )
...
*  =======
*
*  DTRQPX detects the right rank for upper triangular matrix A.
*  The algorithm used here is related to Chandrasekaran&Ipsen
*  algorithm Hybrid-III.
...
*          value of A, and SVLUES(4) wil be an estimate for the smallest
*          singular value of A.
...
*
*  LWORK   (input) INTEGER
*          The dimension of array WORK. LWORK >= MN+MAX(N,2*MN), where
*          MN = min(M,N).
*
*  INFO    (output) INTEGER
*          = 0: Successful exit.
...
*
      CALL DTRRNK( MN, A, LDA, RCOND, RANK, WORK, INFO )
*
*     ************************
*     * First call to xTRQXC *
*     ************************
*
*     Get tighter bounds for the value RANK.
*
      CALL DTRQXC( JOB, M, N, K, A, LDA, C, LDC, JPVT,
...
*
 10   CONTINUE
      IF( .NOT. RNKDTD ) THEN
*
*        Call to xTRQXC to get tighter bounds for the value RANK.
*
         CALL DTRQXC( JOB, M, N, K, A, LDA, C, LDC, JPVT,
     $               RANK, SVLUES, RCNR, RCNRP1, WORK, INFO )
         IF( INFO.NE.0 )
