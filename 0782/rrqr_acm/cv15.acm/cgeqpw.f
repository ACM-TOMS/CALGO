      SUBROUTINE CGEQPW( M, LWSIZE, NB, OFFSET, LACPTD, A, LDA, JPVT,
...
*     .. Scalar Arguments ..
      ...
      REAL               IRCOND, SMIN, MXNM
*     ..
*     .. Array Arguments ..
      INTEGER            JPVT( * )
      COMPLEX            A( LDA, * ), TAU( * ), X( * ), WORK( * )
      REAL               RWORK( * )
*
*
*  Purpose
*  =======
*
*  CGEQPW applies one block step of the Householder QR
...
*
*  Then CGEQP2 tries to identify NB columns in
...
*          This equals the desired blocksize in CGEQPB.
*
...
*
*  A       (input/output) COMPLEX array, dimension (LDA,OFFSET+LWSIZE)
...
*
*  X       (input/output) COMPLEX array, dimension (OFFSET+NB)
...
*
*  TAU     (output) COMPLEX array, dimension (OFFSET+LWSIZE)
*          On exit, TAU(1:OFFSET+LACPTD) contains details of
*          the matrix Q2.
*
*  WORK    (workspace) COMPLEX array, dimension (LWSIZE)
*
*  RWORK   (workspace) REAL array, dimension (2*LWSIZE)
*
*  ================================================================
*
*     .. Parameters ..
...
*     .. Local Scalars ..
      ...
      REAL               TEMP, TEMP2, SMAX
      COMPLEX            GAMMA, AKK
*     ..
*     .. External Subroutines ..
      EXTERNAL           SCNRM2, CSCAL, CSWAP, CLARFG,
     $                   CLARF, ISAMAX, CLAUC1, SLASMX
      INTEGER            ISAMAX
      REAL               SCNRM2, SLASMX
      LOGICAL            CLAUC1
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          ABS, MAX, MIN, SQRT, REAL, CMPLX, CONJG
*     ..
...
*
      DO 10 I = 1,LWSIZE
         RWORK( I ) = SCNRM2( M-OFFSET, A( OFFSET+1, OFFSET+I ), 1 )
...
*        =========================
     ...
     $            ISAMAX( LWSIZE-LACPTD, RWORK( LACPTD+1 ), 1 )
         K = OFFSET + LACPTD + 1
*
*        Exchange current column and pivot column.
*
         IF( PVTIDX.NE.K ) THEN
            CALL CSWAP( M, A( 1, PVTIDX ), 1, A( 1, K ), 1 )
            I1 = JPVT( PVTIDX )
            JPVT( PVTIDX ) = JPVT( K )
            JPVT( K ) = I1
...
*        =====================================
         IF( CLAUC1( K, X, SMIN, A( 1, K ), GAMMA, SMAX*IRCOND ) )
     $      THEN
*
...
*
            IF( K.LT.M ) THEN
               CALL CLARFG( M-K+1, A( K, K ), A( K+1, K ), 1,
     $                      TAU( K ) )
            ELSE
               CALL CLARFG( 1, A( M, K), A( M, K ), 1, TAU ( K ) )
            END IF
*
...
*
            IF( LACPTD.LT.LWSIZE ) THEN
               AKK = A( K, K )
               A( K, K ) = CMPLX( ONE )
               CALL CLARF( 'Left', M-K+1, LWSIZE-LACPTD,
     $                     A( K, K ), 1, CONJG( TAU( K ) ),
     $                     A( K, K+1 ), LDA, WORK )
               A( K, K ) = AKK
            END IF
