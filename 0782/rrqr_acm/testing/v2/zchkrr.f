      SUBROUTINE ZCHKRR( DOTYPE, NM, MVAL, NN, NVAL, NNB, NBVAL,
...
*     .. Array Arguments ..
     ...
     $                   NBVAL( * ), NXVAL( * )
      ...
      COMPLEX*16         A( * ), AQ( * ), COPYA( * ), WORK( * )
*     ..
*
*  Purpose
*  =======
*
*  ZCHKRR tests ZGEQPX and ZGEQPY.
*
*  Arguments
*  =========
*
...
*
*  AQ      (workspace) COMPLEX*16 array, dimension (MMAX*NMAX)
*          where MMAX is the maximum value of M in MVAL and NMAX is the
*          maximum value of N in NVAL.
*
...
*                      (min(MMAX,NMAX))
*
*  WORK    (workspace) COMPLEX*16 array, dimension (LNWK)
*
*  LNWK    (input) INTEGER
...
*     ..
*     .. External Functions ..
      DOUBLE PRECISION   DLAMCH, ZQRT12, ZRRT01,
     $                   ZRRT02
      EXTERNAL           DLAMCH, ZQRT12, ZRRT01,
     $                   ZRRT02
*     ..
*     .. External Subroutines ..
      EXTERNAL           ALAHD, ALASUM, DLAORD, ZERRRR,
     $                   ZGEQPX, ZGEQPY,
...
*
               IF( IMODE.EQ.1 ) THEN
                  CALL ZLASET( 'Full', M, N, CZERO, CZERO, COPYA,
     $                          LDA )
                  DO 30 I = 1, MNMIN
                     COPYS( I ) = ZERO
   30             CONTINUE
...
*
                  SRNAMT = 'ZGEQPX'
                  CALL ZGEQPX( 1, M, N, 0, A, LDA, AQ, LDA, IWORK,
...
*
                  CALL ZLASET( 'All', M, M, CZERO, CONE, AQ, LDA )
*
*                 Compute the Rank-Revealing QR factorization of A
*
                  IF ( NB.LT.3 ) THEN
                  ...
                  SRNAMT = 'ZGEQPX'
                  CALL ZGEQPX( 2, M, N, M, A, LDA, AQ, LDA, IWORK,
     $                         IRCOND, ORCOND, RANK, SVLUES, WORK, LW,
     $                         RWORK, INFO )
*
*                 Compute norm(svd(a) - svd(r))
*
                  RESULT( 2 ) = ZQRT12( M, N, A, LDA, COPYS, WORK,
     $                          LWORK, RWORK )
*
*                 Compute norm( A*P - Q*R )
*
                  RESULT( 3 ) = ZRRT01( 'Conjugate Transpose',
     $                          M, N, COPYA, LDA, IWORK,
     $                          AQ, LDA, A, LDA, WORK, LWORK )
*
*                 Compute norm( Q'*Q - Identity )
*
                  RESULT( 4 ) = ZRRT02( M, AQ, LDA, WORK, LWORK )
*
*                 *-*-*-*-*-*-*-*-*-*-*-*-*-*
...
*
                  ...
                  SRNAMT = 'ZGEQPX'
                  CALL ZGEQPX( 3, M, N, M, A, LDA, AQ, LDA, IWORK,
     $                         IRCOND, ORCOND, RANK, SVLUES, WORK, LW,
     $                         RWORK, INFO )
*
*                 Compute norm(svd(a) - svd(r))
*
                  RESULT( 5 ) = ZQRT12( M, N, A, LDA, COPYS, WORK,
     $                          LWORK, RWORK )
*
*                 Compute norm( A*P - Q*R )
*
                  RESULT( 6 ) = ZRRT01( 'No transpose', M, N,
     $                          COPYA, LDA, IWORK,
     $                          AQ, LDA, A, LDA, WORK, LWORK )
*
*                 Compute norm( Q'*Q - Identity )
*
                  RESULT( 7 ) = ZRRT02( M, AQ, LDA, WORK, LWORK )
*
*
*                 *-*-*-*-*-*-*-*-*-*-*
*                 * Printing results  *
...
*
     ...
     $                     CALL ALAHD( NOUT, PATH )
                        WRITE( NOUT, FMT = 9999 ) 'ZGEQPX', M, N,
     $                                            NB, IMODE, K,
     $                                            RESULT( K )
                        NFAIL = NFAIL + 1
                     END IF
...
*
                  SRNAMT = 'ZGEQPY'
                  CALL ZGEQPY( 1, M, N, 0, A, LDA, AQ, LDA, IWORK,
     $                         IRCOND, ORCOND, RANK, SVLUES, WORK, LW,
     $                         RWORK, INFO )
*
*                 Compute norm(svd(a) - svd(r))
*
                  RESULT( 8 ) = ZQRT12( M, N, A, LDA, COPYS, WORK,
     $                          LWORK, RWORK )
...
*
                  ...
                  SRNAMT = 'ZGEQPY'
                  CALL ZGEQPY( 2, M, N, M, A, LDA, AQ, LDA, IWORK,
     $                         IRCOND, ORCOND, RANK, SVLUES, WORK, LW,
     $                         RWORK, INFO )
*
*                 Compute norm(svd(a) - svd(r))
*
                  RESULT( 9 ) = ZQRT12( M, N, A, LDA, COPYS, WORK,
     $                          LWORK, RWORK )
*
*                 Compute norm( A*P - Q*R )
*
                  RESULT( 10 ) = ZRRT01( 'Conjugate Transpose',
     $                          M, N, COPYA, LDA, IWORK,
     $                          AQ, LDA, A, LDA, WORK, LWORK )
*
*                 Compute norm( Q'*Q - Identity )
*
                  RESULT( 11 ) = ZRRT02( M, AQ, LDA, WORK, LWORK )
*
*                 *-*-*-*-*-*-*-*-*-*-*-*-*-*
...
*
                  ...
                  SRNAMT = 'ZGEQPY'
                  CALL ZGEQPY( 3, M, N, M, A, LDA, AQ, LDA, IWORK,
     $                         IRCOND, ORCOND, RANK, SVLUES, WORK, LW,
     $                         RWORK, INFO )
*
*                 Compute norm(svd(a) - svd(r))
*
                  RESULT( 12 ) = ZQRT12( M, N, A, LDA, COPYS, WORK,
     $                          LWORK, RWORK )
*
*                 Compute norm( A*P - Q*R )
*
                  RESULT( 13 ) = ZRRT01( 'No transpose', M, N,
     $                          COPYA, LDA, IWORK,
     $                          AQ, LDA, A, LDA, WORK, LWORK )
*
*                 Compute norm( Q'*Q - Identity )
*
                  RESULT( 14 ) = ZRRT02( M, AQ, LDA, WORK, LWORK )
*
*
*                 *-*-*-*-*-*-*-*-*-*-*
*                 * Printing results  *
...
*
                  DO 110 K = 8, 14
                     IF( RESULT( K ).GE.THRESH ) THEN
                        IF( NFAIL.EQ.0 .AND. NERRS.EQ.0 )
     $                     CALL ALAHD( NOUT, PATH )
                        WRITE( NOUT, FMT = 9999 ) 'ZGEQPY', M, N,
     $                                            NB, IMODE, K,
     $                                            RESULT( K )
                        NFAIL = NFAIL + 1
                     END IF
