...
1000  FORMAT(/,1x,a,'(',i3,') = ',i6,' > bound = ',i6)
      END
*********************************************************************
      REAL             FUNCTION Sckqrf( m, n, qt, ldqt, r, ldr,
     $                                  a, lda, jpvt, work )
      INTEGER m,n,ldqt,ldr,lda
      REAL             qt(ldqt,m),r(ldr,n),a(lda,n),work(m)
      INTEGER jpvt(n)
*
...
*        Substract j-th column of Q'*A*P and j-th column of R.
         CALL Sgemv('No transpose',m,m,-ONE,qt,ldqt,a(1,jpvt(j)),1,
     $             ONE,work,1)
*        Accumulate the residuals.
         aux = aux + Snrm2(m,work,1) ** 2
 10   CONTINUE
      Sckqrf = sqrt(aux)
      RETURN
      END
*******************************************************************
      REAL             FUNCTION Sckort( m, n, q, ldq, work )
      INTEGER m,n,ldq
      REAL             q(ldq,n), work(n)
*
...
*        Compute work:= work - Q'*Q(:,j).
         CALL Sgemv('Transpose',m,n,-ONE,q,ldq,q(1,j),1,
     $              ONE,work,1)
*        Accumulate the residuals.
         aux = aux + Snrm2(n,work,1) ** 2
 10   CONTINUE
      Sckort = sqrt(aux)
      RETURN
      END
*******************************************************************
      REAL             FUNCTION Scksvd( m, n, a, lda, svlues,
     $                                   work1,work2)
      INTEGER m, n, lda
      REAL             a(lda,*), svlues(*),work1(m,*),
     $     work2(*)
*
...
*
      ...
      INTEGER i, j, info, mn
      REAL             dummy, nrmsvl
      EXTERNAL Sgebd2, Sbdsqr, Saxpy, Snrm2
      REAL             Snrm2
      mn = min( m, n )
      nrmsvl = Snrm2(mn,svlues,1)
...
*
      CALL Sgebd2(m,n,work1,m,work2(1),work2(mn+1),work2(2*mn+1),
     $            work2(3*mn+1),work2(4*mn+1),info)
      CALL Sbdsqr('upper',mn,0,0,0,work2(1),work2(mn+1),dummy,mn,
     $            dummy,1,dummy,mn,work1(1,1),info)
*
*     compare svlues and work1
*
      CALL Saxpy(mn,-ONE,svlues,1,work2,1)
      Scksvd = Snrm2(mn,work2,1)/nrmsvl
      RETURN
      END
*********************************************************************
      REAL             FUNCTION Sckpqr(m,n,k,qr,ldq,tau,a,lda,
     $                                     jpvt,work)
      INTEGER m,n,k,lda,ldq
      REAL             qr(ldq,n),tau(n),a(lda,n),work(*)
      INTEGER jpvt(n)
*
...
*
      ...
      INTEGER i, j, info
      REAL             aux
      EXTERNAL Sorm2r, Scopy, Snrm2
      REAL             Snrm2
      INTRINSIC sqrt
      aux = ZERO
      DO 10 j = 1, n
*        Compute j-th column of Q'*A*P and put into vector work(1:m).
         CALL Scopy( m,a(1,jpvt(j)),1,work(1),1)
         CALL Sorm2r( 'left','transpose', m, 1, k, qr, ldq, tau,
...
*
      END
*********************************************************************
      SUBROUTINE Ssort(n,x,incrx,job)
      INTEGER n, incrx
      CHARACTER*1 job
      REAL             x(*)
*
...
200   DO 30 i = n-1,1,-1
 ...
 30   CONTINUE
      RETURN
*
*     next line is last line of SUBROUTINE Ssort
      END
*********************************************************************
...
c
            ...
            IF( nrmxl .eq. ZERO) GOTO 180
               ...
               CALL Sscal(n-l+1,ONE/nrmxl,x(l,l),1)
               x(l,l) = ONE + x(l,l)
c
