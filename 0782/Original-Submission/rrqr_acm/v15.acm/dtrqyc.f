      SUBROUTINE DTRQYC( JOB, M, N, K, A, LDA, C, LDC, JPVT,
...
*  =======
*
*  DTRQYC carries out Pan-Tang Algorithm 3 for the stage RANK.
...
*
*  A       (input/output) DOUBLE PRECISION array (LDA,N)
*          Upper triangular m by n matrix.
*
*  LDA     (input) INTEGER
*          The leading dimension of array A. LDA >= MAX( 1, M ).
*
*  C       (input/output) DOUBLE PRECISION array (LDC,K)
...
*
*  WORK    (workspace) DOUBLE PRECISION array, dimension (N+3*MIN(M,N))
*
*  INFO    (output) INTEGER
*          = 0: Successful exit.
...
*
*     .. Parameters ..
      DOUBLE PRECISION   FP, SF, ONE
      PARAMETER          ( FP = 0.9D+0, SF = 1.0D+2, ONE = 1.0D+0 )
*
...
*     .. Local Scalars ..
      ...
      DOUBLE PRECISION   COSINE, DIAG, F, SMAX, SMAXPR, SMIN, SMINPR,
...
*     .. External Subroutines ..
      EXTERNAL           DSCAL, DLAIC1, DTRSV, DLARTG,
     $                   DGRET, DHESS, DSWAP, DCOPY
*     ..
*     .. External Functions ..
      EXTERNAL           IDAMAX, DNRM2, DLASMX, DLAMCH
      INTEGER            IDAMAX
      DOUBLE PRECISION   DNRM2, DLASMX, DLAMCH
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          ABS, SQRT, DBLE, MAX, MIN
*     ..
...
*
         JJ = MN - 1 + IDAMAX( N-MN+1, A( MN, MN ), LDA )
         IF( JJ.GT.MN ) THEN
            CALL DSWAP( M, A( 1, MN ), 1, A( 1, JJ ), 1 )
            ITEMP = JPVT( MN )
            JPVT( MN ) = JPVT( JJ )
            JPVT( JJ ) = ITEMP
         END IF
...
*
         ...
         WORK( MN+1 ) = ONE
         DO 10 J = 2, RANK
            CALL DLAIC1( 1, J-1, WORK( 1 ), SMAX, A( 1, J ),
     $                   A( J, J ), SMAXPR, SINE, COSINE )
            CALL DSCAL( J-1, SINE, WORK( 1 ), 1 )
            WORK( J ) = COSINE
            SMAX = SMAXPR
*
            CALL DLAIC1( 2, J-1, WORK( MN+1 ), SMIN, A( 1, J ),
...
*              RANK->RANK-1.
*
               CALL DCOPY( RANK, A( 1, JJ ), 1, WORK( 1 ), 1 )
               DO 20 J = JJ+1, RANK
                  CALL DCOPY( J, A( 1, J ), 1, A( 1, J-1 ), 1 )
20             CONTINUE
               CALL DCOPY( RANK, WORK( 1 ), 1, A( 1, RANK ), 1 )
*
...
*
         ...
         WORK( N+1 ) = ONE
         DO 110 J = 2, RANK
            CALL DLAIC1( 2, J-1, WORK( N+1 ), SMIN, A( 1, J ),
     $                   A( J , J ), SMINPR, SINE, COSINE )
            CALL DSCAL( J-1, SINE, WORK( N+1 ), 1 )
            WORK( N+J ) = COSINE
            SMIN = SMINPR
110      CONTINUE
*
...
*
            ...
            DO 120 I = MIN( MN, II )-1, RANK+1, -1
               CALL DLARTG( A( I, II ), DIAG, COSINE, SINE, TEMP )
               DIAG = TEMP
120         CONTINUE
*
            CALL DLAIC1( 2, RANK, WORK( N+1 ), SMIN, A( 1, II ),
...
*              ...,II-1->II,II->RANK+1.
*
               CALL DCOPY( MIN( MN, II ), A( 1, II ), 1,
     $                     WORK( N+MN+1 ), 1 )
               DO 130 J = II-1, RANK+1, -1
                  CALL DCOPY( MIN( MN, J+1 ), A( 1, J ), 1 ,
     $                        A( 1, J+1 ), 1 )
130            CONTINUE
               CALL DCOPY( MIN( MN, II ), WORK( N+MN+1 ), 1,
     $                     A( 1, RANK+1 ), 1 )
*
*              Exchange in the same way vector JPVT.
*
...
*
               IF( JOB.EQ.1 ) THEN
                  CALL DGRET( JOB, MIN( M, II )-RANK, N-RANK, K,
...
*              *-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*
*
               ITEMP = IDAMAX( RANK+1, WORK, 1 )
               SMXRP1 = DLASMX( RANK+1 )*WORK( ITEMP )
*
*              *-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*
...
*
                  CALL DCOPY( RANK, WORK( N+1 ), 1, WORK( N+MN+1 ), 1 )
                  CALL DSCAL( RANK, SINE, WORK( N+MN+1 ), 1 )
...
*
                  JJ = IDAMAX( RANK+1, WORK( N+MN+1 ), 1 )
*
*                 *-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*
...
*
                     CALL DCOPY( RANK+1, A( 1, JJ ), 1,
     $                           WORK( N+MN+1 ), 1 )
                     DO 160 J = JJ+1, RANK+1
                        CALL DCOPY( J, A( 1, J ), 1,
     $                              A( 1, J-1 ), 1 )
160                  CONTINUE
                     CALL DCOPY( RANK+1, WORK( N+MN+1 ), 1,
     $                           A( 1, RANK+1 ), 1 )
*
*                    Exchange in the same way vector JPVT.
*
...
*
                     IF( JOB.EQ.1 ) THEN
                        CALL DHESS( JOB, RANK-JJ+2, N-JJ+1, K,
     $                              A( JJ, JJ ), LDA, RDUMMY, 1,
     $                              WORK( N+MN+1 ), INFO )
                     ELSE IF( JOB.EQ.2 ) THEN
                        CALL DHESS( JOB, RANK-JJ+2, N-JJ+1, K,
...
*
                     ...
                     WORK( N+1 ) = ONE
                     DO 190 J = 2, RANK
                        CALL DLAIC1( 2, J-1, WORK( N+1 ), SMIN,
     $                               A( 1, J ), A( J , J ), SMINPR,
     $                               SINE, COSINE )
                        CALL DSCAL( J-1, SINE, WORK( N+1 ), 1 )
                        WORK( N+J ) = COSINE
...
*        position RANK+1.
*
         JJ = RANK+IDAMAX( N-RANK, WORK( RANK+1 ), 1 )
         IF( ( JJ.GT.( RANK+1 ) ).AND.
     $       ( F*ABS( WORK( JJ ) ).GT.ABS( WORK( RANK+1 ) ) ) ) THEN
*
*           Exchange column JJ to position RANK+1.
*
            CALL DCOPY( MIN( MN, JJ ), A( 1, JJ ), 1,
     $                  WORK( N+MN+1 ), 1 )
            DO 200 J = JJ-1, RANK+1, -1
               CALL DCOPY( MIN( MN, J+1 ), A( 1, J ), 1 ,
     $                     A( 1, J+1 ), 1 )
200         CONTINUE
            CALL DCOPY( MIN( MN, JJ ), WORK( N+MN+1 ), 1,
     $                  A( 1, RANK+1 ), 1 )
*
*           Exchange in the same way vector JPVT.
*
...
*
            IF( JOB.EQ.1 ) THEN
               CALL DGRET( JOB, MIN( M, JJ )-RANK, N-RANK, K,
     $                     A( RANK+1, RANK+1 ), LDA, RDUMMY, 1,
     $                     WORK( N+MN+1 ), INFO )
            ELSE IF( JOB.EQ.2 ) THEN
               CALL DGRET( JOB, MIN( M, JJ )-RANK, N-RANK, K,
...
*
     ...
     $                SINE, COSINE )
         CALL DLAIC1( 2, RANK, WORK( N+1 ), SMIN,
     $                A( 1, RANK+1 ), A( RANK+1, RANK+1 ), SMINPR,
     $                SINE, COSINE )
         CALL DSCAL( RANK, SINE, WORK( N+1 ), 1 )
