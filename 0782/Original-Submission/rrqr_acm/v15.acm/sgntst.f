      SUBROUTINE SGNTST( WHICH, M, N, RTHRESH, GAP, STRIP, ISEED,
     $                   RANK, S, A, LDA, WORK )
*
...
*  =======
*
*  SGNTST forms a test matrix for SGEQPF, the QR factorization with
...
*     .. Local Scalars ..
      ...
      REAL               DUMMY, RTEPS, EPS, TEMP
*     ..
*     .. External Subroutines ..
      EXTERNAL           SGEMM, SCOPY, SSCAL, SLARNV,
     $                   SGEBD2, SBDSQR
*     ..
*     .. External Functions ..
      EXTERNAL           SLARAN, SFRANK, SNRM2, SLAMCH
      INTEGER            SFRANK
      REAL               SLARAN, SLAMCH, SNRM2
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MIN, MAX, MOD, SQRT, REAL
*     ..
...
*
         CALL SLATMS( M, WIDTH2, 'Uniform Distribution', ISEED,
...
*
         IF( WIDTH1.GT.0 ) THEN
            CALL SLARNV(1,ISEED,WIDTH1*WIDTH2,WORK(1))
            CALL SGEMM( 'no transpose', 'no transpose', M, WIDTH1,
...
*
         IF( MN.LT.N ) THEN
           CALL SLARNV(1,ISEED,WIDTH2*(N-MN),WORK(1))
           CALL SGEMM( 'no transpose', 'no transpose', M, N-MN,
     $                WIDTH2, RTEPS, A( 1, WIDTH1+1 ), LDA, WORK,
...
*        compute SVD of A
*
         CALL SLACPY( 'full', M, N, A, LDA, WORK( MN+1 ), M )
...
*
         ...
         CALL SLATMS( M, MN-1, 'Uniform Distribution', ISEED,
...
*
         CALL SCOPY( M, A( 1, 2 ), 1, A( 1, 1 ), 1 )
         CALL SSCAL( M, SLARAN( ISEED ), A( 1, 1 ), 1 )
*
*        multiply A(:,2:MN) with a random
*        (MN-1)-by-(N-MN) matrix to generate A(:,MN+1:N).
*
         IF( MN.LT.N ) THEN
           CALL SLARNV( 1, ISEED, (MN-1)*(N-MN), WORK( 1 ) )
           CALL SGEMM( 'no transpose' , 'no transpose', M, N-MN,
...
*        distribution.
*
         CALL SLATMS( M, N, 'Uniform', ISEED, 'Nonsymmetric', S,
...
80       CONTINUE
 ...
 85            CONTINUE
            ...
            RETURN
         END IF
         CALL SLATMS( M, N-WIDTH1, 'Uniform', ISEED, 'Nonsymmetric',
...
*
         WIDTH1 = MIN( STRIP, MN )
         CALL SLATMS( M, WIDTH1, 'Uniform', ISEED, 'Nonsymmetric',
     $               S, ARITH, ONE/( GAP*RTHRESH ), ONE, M, WIDTH1,
     $               'No Packing', A, LDA, WORK, INFO )
        IF( N.GT.WIDTH1 ) THEN
           CALL SLARNV( 1, ISEED, WIDTH1*( N-WIDTH1 ), WORK( 1 ) )
           DO 110 J = WIDTH1+1, N
              CALL SLARNV( 1, ISEED, M, A( 1, J ) )
110        CONTINUE
           CALL SGEMM( 'no transpose', 'no transpose', M, N-WIDTH1,
...
160     CONTINUE
        DO 180 I = MAX( 2, MN-5 ), MN
           S( I ) = RTHRESH*GAP*( ONE+3.0*SLARAN( ISEED ) )
180     CONTINUE
        CALL SLATMS( M, N, 'Uniform', ISEED, 'Nonsymmetric', S, 0,
