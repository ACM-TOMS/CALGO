...
*     =========
*
      REAL               ZERO, ONE, HUNDRED, MILLION
      PARAMETER          ( ZERO = 0.0E+0, ONE = 1.0E+0,
     $                   HUNDRED = 1.0E+2, MILLION = 1.0E+6 )
...
*     ********************************
     ...
     $                   strip, iseed(4), job, k
      REAL               irthresh, orthresh, gap, timmin, scale
...
*     ===========================
      REAL               ttime, tircond, tdrcond, tismax,
     $                   tdsmax, tisbefor, tdsbefor, tisafter,
     $                   tdsafter, tismin, tdsmin
      INTEGER            taccptd
      REAL               lint(LINRTS,PERFMSR),
...
*     ARRAYS FOR MATRICES
*     ===================
      REAL               a(MMAX,NMAX), copya(MMAX,NMAX), s(MMAX),
...
*     WORK SPACE
*     ==========
      REAL               work(MMAX*NMAX+4*MMAX+NMAX),wk1(mmax,mmax)
...
*     ================
*
      EXTERNAL           SECOND, SGNTST, SQRMTX,
     $                   SLATMS, SNRM2, iscle, iarle, find,
     $                   SLAMCH, sfrank, flXGEQPF, flXGEQRF,
     $                   flXGEQR2, SLASMX
      REAL               SECOND, SNRM2, SLAMCH, SLASMX
...
*     set to zero in case we don't DO error check
      data               LINNAMES /'SQRDC ','SGEQPF'/
      data               ctopt /'SGNTST','SQRMTX','SLATMS'/
...
*
              IF( topt.EQ.GNTST ) THEN
                CALL SGNTST(mode,m,n,irthresh,gap,strip,iseed,
     $                      rank,s,a,lda,work)
              ELSEIF( topt.EQ.QRMTX ) THEN
                CALL SQRMTX('all',scale,m,n,irthresh*gap,strip,
...
*
*             subtract the time for the SLACPY calls
*
              CALL SLACPY('all',m,n,a,lda,wk1,m)
              t1 = SECOND()
              DO 20 j = 1,runs
                CALL SLACPY('all',m,n,copya,lda,a,lda)
20            CONTINUE
              ttime = (ttime - (SECOND() - t1))/runs
              IF( ttime.LE.ZERO ) ttime = ZERO
              CALL SLACPY('all',m,n,wk1,m,a,lda)
*
*             save test results
*
              lint(BL1,TIME) = ttime
              lint(BL1,FLOPS) = REAL(flXGEQR2(m,n))
              IF( ttime .EQ. ZERO ) THEN
                lint(BL1,MFLOPS) = ZERO
              ELSE
                lint(BL1,MFLOPS) = lint(BL1,FLOPS)/ttime/MILLION
              END IF
              lint(BL1,NORUNS) = REAL(runs)
*
*
*             ************************************
...
100           CONTINUE
                CALL izero(n,jpvt)
                CALL SLACPY('all',m,n,copya,lda,a,lda)
                CALL SGEQPF(m,n,a,lda,jpvt,qraux,work,info)
                t2 = SECOND()
                ttime = t2 - t1
                runs = runs + 1
...
*
              ...
              ELSE
                DO 120 i = 1,mn
                  mnrmpr = MAX(mnrm,SNRM2(i,a(1,i),1))
                  smaxpr = SLASMX(i)*mnrmpr
...
*
              ...
              END IF
              lint(BL2,NORUNS) = REAL(runs)
              lint(BL2,TRANK) = REAL(taccptd)
...
200             CONTINUE
                  CALL SLACPY('all',m,n,copya,lda,a,lda)
                  CALL SGEQRF(m,n,a,lda,qraux,work,n*nb,info)
                  t2 = SECOND()
                  ttime = t2 - t1
                  runs = runs + 1
...
*
                ...
                cwytnop(inb,STFLPS) = REAL(flXGEQR2(m,n))
                cwytnop(inb,RLFLPS) = REAL(flXGEQRF(m,n,nb))
                IF( ttime.EQ.ZERO ) THEN
...
*               ********************************
                job = 1
                k = 0
*               Length of work array for SGEQPB
                IF( job.EQ.1 ) THEN
                   IF( nb.EQ.1 ) THEN
                      ls = 2*min(m,n)+3*n
                   ELSE
...
300             CONTINUE
                  ...
                  flppst = 0
                  CALL SLACPY('all',m,n,copya,lda,a,lda)
                  CALL SGEQPB(job,m,n,k,a,lda,wk1,mmax,jpvt,
...
*
     ...
     $                    cwytqpb(inb,STFLPS)/ttime/MILLION
                END IF
                cwytqpb(inb,ICFLPS) = REAL(flpice)
                cwytqpb(inb,POFLPS) = REAL(flppst)
                cwytqpb(inb,RLFLPS) = REAL(flppre)+
     $                                REAL(flpice)+REAL(flppst)
                IF( ttime.EQ.ZERO ) THEN
     ...
     $                    cwytqpb(inb,RLFLPS)/ttime/MILLION
                END IF
                cwytqpb(inb,NORUNS) = REAL(runs)
                cwytqpb(inb,TRANK) = REAL(rank)
...
400             CONTINUE
                  ...
                  flppst = 0
                  CALL SLACPY('all',m,n,copya,lda,a,lda)
                  CALL SGEQPX(job,m,n,k,a,lda,wk1,mmax,jpvt,
     $                        irthresh,orthresh,rank,svlues,work,ls,
     $                        info)
                  t2 = SECOND()
                  ttime = t2 - t1
...
*
     ...
     $                    cwytqpx(inb,STFLPS)/ttime/MILLION
                END IF
                cwytqpx(inb,ICFLPS) = REAL(flpice)
                cwytqpx(inb,POFLPS) = REAL(flppst)
                cwytqpx(inb,RLFLPS) = REAL(flppre)+
     $                                 REAL(flpice)+REAL(flppst)
                IF( ttime.EQ.ZERO ) THEN
     ...
     $                    cwytqpx(inb,RLFLPS)/ttime/MILLION
                END IF
                cwytqpx(inb,NORUNS) = REAL(runs)
                cwytqpx(inb,TRANK) = REAL(rank)
...
500             CONTINUE
                  ...
                  flppst = 0
                  CALL SLACPY('all',m,n,copya,lda,a,lda)
                  CALL SGEQPY(job,m,n,k,a,lda,wk1,mmax,jpvt,
     $                        irthresh,orthresh,rank,svlues,work,ls,
     $                        info)
                  t2 = SECOND()
                  ttime = t2 - t1
...
*
     ...
     $                    cwytqpy(inb,STFLPS)/ttime/MILLION
                END IF
                cwytqpy(inb,ICFLPS) = REAL(flpice)
                cwytqpy(inb,POFLPS) = REAL(flppst)
                cwytqpy(inb,RLFLPS) = REAL(flppre)+
     $                                 REAL(flpice)+REAL(flppst)
                IF( ttime.EQ.ZERO ) THEN
     ...
     $                    cwytqpy(inb,RLFLPS)/ttime/MILLION
                END IF
                cwytqpy(inb,NORUNS) = REAL(runs)
                cwytqpy(inb,TRANK) = REAL(rank)
...
1240  FORMAT(1x,'SGEQRF',' nb:',i2,' |  ',e8.2,
     $       '  |   ',f7.2,'   |   ',f7.2,'    | ',1x,i4)
...
1260  FORMAT(1x,a6,' nb:',i2,' |  ',e8.2,'  |   ',
     ...
     $       ' | ',i4,' | ',e8.2)
*
1400  FORMAT(/,1x,'SQRDC: ',6x,'  svd:',e8.2)
1410  FORMAT(/,1x,'SGEQPF:',6x,'  svd:',e8.2,'  qrf:',e8.2,
     $       '  ort:',e8.2)
1420  FORMAT(/,1x,'SGEQRF: nb:',i2,'  svd:',e8.2,
     $       '  qrf:',e8.2,'  ort:',e8.2)
1430  FORMAT(1x,'SGEQPB: nb:',i2,
     $       '  svd:',e8.2,'  qrf:',e8.2,'  ort:',e8.2)
1440  FORMAT(1x,'SGEQPX: nb:',i2,
     $       '  svd:',e8.2,'  qrf:',e8.2,'  ort:',e8.2)
1450  FORMAT(1x,'SGEQPY: nb:',i2,
     $       '  svd:',e8.2,'  qrf:',e8.2,'  ort:',e8.2)
*
*
*
