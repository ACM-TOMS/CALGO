      SUBROUTINE SGEQPB( JOB, M, N, K, A, LDA, C, LDC, JPVT, IRCOND,
...
*  =======
*
*  SGEQPB computes a QR factorization
*       A*P = Q*[ R11 R12 ]
*               [  0  R22 ]
...
*
     ...
     $         ( ( JOB.EQ.3 ).AND.( LDC.LT.MAX( 1, K ) ) ) ) THEN
      ...
      ELSE IF( LWORK.LT.MAX( 1, WKMIN ) ) THEN
         INFO = -15
      END IF
      IF( ( INFO.EQ.0 ).OR.( INFO.EQ.-15 ) ) THEN
*
...
*
         ...
         ELSE
            NB = MAX( ILAENV( INB, 'SGEQRF', ' ', M, N, 0, 0 ),
     $                ILAENV( INB, 'SORMQR', ' ', M, N, 0, 0 ) )
            WSIZE = MAX( 2*MN + 2*N + MAX( N, K ),
     $                   2*MN + NB*NB + NB*MAX( N, K ) )
         END IF
         WORK( 1 ) = REAL( WSIZE )
      END IF
      IF( INFO.NE.0 ) THEN
         CALL XERBLA( 'SGEQPB', -INFO )
         RETURN
      END IF
*
...
*
      BLOCK = ( ( NB.GT.1 ).AND.
     $    ( NB.GE.ILAENV( INBMIN, 'SGEQRF', ' ', M, N, 0, 0 ) ).AND.
     $    ( MN.GE.ILAENV( IXOVER, 'SGEQRF', ' ', M, N, 0, 0 ) ) )
*
...
*     ***************************************************
*
      CALL SGEQPC( JOB, M, N, K, A, LDA, C, LDC, 1, 0,
     $             RCOND, LACPTD, JPVT, WORK( 1 ), WORK( MN+1 ),
     $             SVLUES, MXNM, WORK( 2*MN+1 ), LWORK-2*MN )
      IF( LACPTD.EQ.1 ) THEN
...
*
            ...
            CALL SGEQPW( M, LWSIZE, KB, KK-1, LACPTD, A, LDA,
     $                   JPVT, RCOND, WORK( MN+1 ), SMIN, MXNM,
     $                   WORK( 1 ), WORK( 2*MN+1 ) )
            IF( LACPTD.GT.0 ) THEN
*
*              Accumulate Householder vectors in a block reflector.
*
               CALL SLARFT( 'Forward', 'Columnwise', M-KK+1,
...
*
     ...
     $                         'Columnwise', M-KK+1, N-KK-LWSIZE+1,
     $                         LACPTD, A( KK, KK ), LDA,
     $                         WORK( 2*MN+1 ), LACPTD,
     $                         A( KK, KK+LWSIZE ), LDA,
...
*
     ...
     $                      MIN( KK+LWSIZE-1, STREJ-LWSIZE+LACPTD-1 )
                     MVIDX = MVIDX - 1
                     CALL SSWAP( M, A( 1, I ),1, A( 1, MVIDX ),1 )
                     ITEMP = JPVT( I )
                     JPVT( I ) = JPVT( MVIDX )
                     JPVT( MVIDX ) = ITEMP
 40               CONTINUE
...
*
            CALL SGEQPC( JOB, M, N, K, A, LDA, C, LDC, MN-KK+1, KK-1,
     $                   RCOND, LACPTD, JPVT, WORK( 1 ), WORK( MN+1 ),
     $                   SVLUES, MXNM, WORK( 2*MN+1 ), LWORK-2*MN )
            RANK = ACCPTD + LACPTD
         ELSE
...
*
         ACCPTD = 1
         CALL SGEQPC( JOB, M, N, K, A, LDA, C, LDC, MN-ACCPTD, ACCPTD,
     $                RCOND, LACPTD, JPVT, WORK( 1 ), WORK( MN+1 ),
     $                SVLUES, MXNM, WORK( 2*MN+1 ), LWORK-2*MN )
         RANK = ACCPTD+LACPTD
*
