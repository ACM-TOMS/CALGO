...
*
         ...
         WRITE( IOUNIT, FMT = '( '' Messages:'' )' )
*
      ELSE IF( LSAMEN( 2, P2, 'RR' ) ) THEN
*
*        Rank-Revealing QR decomposition
*
         WRITE( IOUNIT, FMT = 9920 )PATH
         WRITE( IOUNIT, FMT = 9929 )
         WRITE( IOUNIT, FMT = '( '' Test ratios:'' )' )
         WRITE( IOUNIT, FMT = 9928 )1
         WRITE( IOUNIT, FMT = 9927 )2
         WRITE( IOUNIT, FMT = 9926 )3
         WRITE( IOUNIT, FMT = 9925 )4
         WRITE( IOUNIT, FMT = 9924 )5
         WRITE( IOUNIT, FMT = 9923 )6
         WRITE( IOUNIT, FMT = 9922 )7
         WRITE( IOUNIT, FMT = 9921 )
         WRITE( IOUNIT, FMT = '( '' Messages:'' )' )
*
...
*
 ...
 9920 FORMAT( / 1X, A3, ':  Rank-Revealing QR factorization' )
...
*     RR matrix types
*
 9929 FORMAT( ' Matrix types (2-3 have condition 1/EPS):', / 4X,
     $      '1. Zero matrix', / 4X,
...
*
 9928 FORMAT( 3X, I2, ': xGEQPX w/ JOB=1 norm(svd(A) - svd(R)) / ',
     $      '( M * norm(svd(R)) * EPS )' )
 9927 FORMAT( 3X, I2, ': xGEQPX w/ JOB=2 norm(svd(A) - svd(R)) / ',
     $      '( M * norm(svd(R)) * EPS )' )
 9926 FORMAT( 3X, I2, ': xGEQPX w/ JOB=2 ',
     $      'norm( A*P - Q*R )     / ( M * norm(A) * EPS )' )
 9925 FORMAT( 3X, I2, ': xGEQPX w/ JOB=2 ',
     $      'norm( I - Q''*Q )      / ( M * EPS )' )
 9924 FORMAT( 3X, I2, ': xGEQPX w/ JOB=3 norm(svd(A) - svd(R)) / ',
     $      '( M * norm(svd(R)) * EPS )' )
 9923 FORMAT( 3X, I2, ': xGEQPX w/ JOB=3 ',
     $      'norm( A*P - Q*R )     / ( M * norm(A) * EPS )' )
 9922 FORMAT( 3X, I2, ': xGEQPX w/ JOB=3 ',
     $      'norm( I - Q''*Q )      / ( M * EPS )' )
 9921 FORMAT( 3X, ' 8-14: same as 1-7 for xGEQPY' )
*
      RETURN
*
*     End of ALAHD
