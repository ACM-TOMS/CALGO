      SUBROUTINE ZGEQPC( JOB, M, N, K, A, LDA, C, LDC, DSRD, OFFSET,
...
*     .. Local Scalars ..
      ...
      DOUBLE PRECISION   TEMP, TEMP2, SMIN, SMINPR, SMAX, SMAXPR
      COMPLEX*16         AII, SINE, COSINE
*     ..
*     .. External Subroutines ..
      EXTERNAL           ZLARFG, ZLARF, ZSWAP, ZSCAL,
     $                   ZLAIC1, ZGEQRF, ZUNMQR
*     ..
*     .. External Functions ..
      INTEGER            IDAMAX
      DOUBLE PRECISION   DZNRM2, DLASMX
      LOGICAL            ZLAUC1
      EXTERNAL           IDAMAX, DZNRM2, DLASMX, ZLAUC1
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          ABS, MAX, MIN, DCONJG
*     ..
...
*
      DO 10 I = OFFSET+1,N
         RWORK( I ) = DZNRM2( M-OFFSET, A( OFFSET+1, I ), 1 )
         RWORK( N+I ) = RWORK( I )
10    CONTINUE
*
...
*
         IF( I.LT.M ) THEN
            CALL ZLARFG( M-I+1, A( I, I ), A( I+1, I ), 1,
     $                   TAU( I ) )
         ELSE
            CALL ZLARFG( 1, A( M, M ), A( M, M ), 1, TAU( M ) )
         END IF
*
...
*
            CALL ZLARF( 'Left', M-I+1, K, A( I, I ), 1,
     $                   DCONJG( TAU( I ) ), C( I, 1 ), LDC, WORK )
         ELSE IF( ( JOB.EQ.3 ).AND.( K.GT.0 ) ) THEN
*
*           Apply the transpose of H(I) to C(1:K,I:M) from the right.
*
            CALL ZLARF( 'Right', K, M-I+1, A( I, I ), 1,
...
*
         IF( I.EQ.1 ) THEN
            ...
            END IF
         ELSE
            SMAXPR = DLASMX( I )*MXNM
            IF( ZLAUC1( I, X, SMIN, A( 1, I ), A( I, I ),
     $          SMAXPR*IRCOND ) ) THEN
*
*              Column accepted.
*
...
*
            CALL ZGEQRF( M-I, N-I, A( I+1, I+1 ), LDA, TAU( I+1 ),
     $                   WORK, LWORK, INFO )
*
...
*              right.
*
               CALL ZUNMQR( 'Right', 'No Transpose', K, M-I, MN-I,
     $                     A( I+1, I+1 ), LDA, TAU( I+1 ),
...
*
         DO 60 I = MAX( 2, OFFSET+LACPTD+1 ), MN
            CALL ZLAIC1( 2, I-1, X, SMIN, A( 1, I ), A( I, I ),
     $                  SMINPR, SINE, COSINE )
            CALL ZSCAL( I-1, SINE, X, 1 )
            X( I ) = COSINE
            SMIN = SMINPR
