      SUBROUTINE ZGEQPB( JOB, M, N, K, A, LDA, C, LDC, JPVT,
...
*  =======
*
*  ZGEQPB computes a QR factorization
*       A*P = Q*[ R11 R12 ]
*               [  0  R22 ]
...
*     .. Parameters ..
      ...
      COMPLEX*16         CZERO
      PARAMETER          ( ZERO = 0.0D+0, CZERO = ( 0.0D+0, 0.0D+0 ) )
...
*     .. Local Scalars ..
     ...
     $                   ACCPTD, NB, LWSIZE, NLLITY, KB, WSIZE, WKMIN
      DOUBLE PRECISION   SMIN, MXNM, RCOND
      LOGICAL            BLOCK
*     ..
*     .. External Subroutines ..
      EXTERNAL           XERBLA, ZGEQPC, ZGEQPW,
     $                   ZLARFT, ZLARFB
*     ..
*     .. External Functions ..
      INTEGER            ILAENV
      DOUBLE PRECISION   DLAMCH, DLASMX
      EXTERNAL           ILAENV, DLAMCH, DLASMX
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MAX, MIN, DBLE, INT
*     ..
...
*
         IF( JOB.EQ.1 ) THEN
            NB = ILAENV( INB, 'ZGEQRF', ' ', M, N, 0, 0 )
            WSIZE = 2*MN + MAX( 3*N, N*NB )
         ELSE
            NB = MAX( ILAENV( INB, 'ZGEQRF', ' ', M, N, 0, 0 ),
     $                ILAENV( INB, 'ZUNMQR', ' ', M, N, 0, 0 ) )
...
*
      BLOCK = ( ( NB.GT.1 ).AND.
     $   ( NB.GE.ILAENV( INBMIN, 'ZGEQRF', ' ', M, N, 0, 0 ) ).AND.
     $   ( MN.GE.ILAENV( IXOVER, 'ZGEQRF', ' ', M, N, 0, 0 ) ) )
*
...
*     ***************************************************
*
      CALL ZGEQPC( JOB, M, N, K, A, LDA, C, LDC, 1, 0,
...
*
            ...
            CALL ZGEQPW( M, LWSIZE, KB, KK-1, LACPTD, A, LDA, JPVT,
...
*
               CALL ZLARFT( 'Forward', 'Columnwise', M-KK+1,
...
*
     ...
     $                      MIN( KK+LWSIZE-1, STREJ-LWSIZE+LACPTD-1 )
                     MVIDX = MVIDX - 1
                     CALL ZSWAP( M, A( 1, I ),1, A( 1, MVIDX ),1 )
                     ITEMP = JPVT( I )
                     JPVT( I ) = JPVT( MVIDX )
                     JPVT( MVIDX ) = ITEMP
 40               CONTINUE
 ...
 20      CONTINUE
         ACCPTD = KK-1
         SVLUES( IMAX ) = DLASMX( ACCPTD )*MXNM
         SVLUES( IBEFOR ) = SMIN
         IF( ACCPTD.LT.MN ) THEN
*
*           Process rejected columns.
*
            CALL ZGEQPC( JOB, M, N, K, A, LDA, C, LDC, MN-KK+1,
...
*
         ACCPTD = 1
         CALL ZGEQPC( JOB, M, N, K, A, LDA, C, LDC, MN-ACCPTD,
