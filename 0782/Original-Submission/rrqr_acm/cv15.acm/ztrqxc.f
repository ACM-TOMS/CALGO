      SUBROUTINE ZTRQXC( JOB, M, N, K, A, LDA, C, LDC, JPVT,
...
*     .. Parameters ..
      DOUBLE PRECISION   F
      COMPLEX*16         CONE
      PARAMETER          (  F = 0.5D+0, CONE = ( 1.0D+0, 0.0D+0 ) )
*
...
*     ..
*     .. Local Scalars ..
      DOUBLE PRECISION   SMAX, SMAXPR, SMIN, SMINPR, SMXRP1
      COMPLEX*16         COSINE, SINE
      LOGICAL            PERMUT
...
*     .. External Functions ..
      INTEGER            IDAMAX
      DOUBLE PRECISION   DLASMX, DZNRM2
      EXTERNAL           IDAMAX, DLASMX, DZNRM2
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MAX, MIN
*     ..
...
*           easier.
*
            RWORK( RANK+1 ) = DZNRM2( RANK+1, A( 1, RANK+1 ), 1 )
*
*           Apply Chan-II for the stage RANK+1
*           on block A(1:RANK+1,1:RANK+1).
*
            CALL ZCNIIF( JOB, M, N, K, A, LDA, C, LDC, JPVT,
...
*
      DO 30 J = 2, RANK
         CALL ZLAIC1( 1, J-1, WORK( 1 ), SMAX,
     $                A( 1, J ), A( J, J ), SMAXPR, SINE, COSINE )
         CALL ZSCAL( J-1, SINE, WORK( 1 ), 1 )
         WORK( J ) = COSINE
         SMAX = SMAXPR
         CALL ZLAIC1( 2, J-1, WORK( MN+1 ), SMIN,
...
*
      IF( RANK.LT.MN ) THEN
         CALL ZLAIC1( 1, RANK, WORK( 1 ), SMAX,
     $                A( 1, RANK+1 ), A( RANK+1, RANK+1 ), SMAXPR,
     $                SINE, COSINE )
         SMAX = SMAXPR
         CALL ZLAIC1( 2, RANK, WORK( MN+1 ), SMIN,
...
*
      END
      SUBROUTINE ZGLBIF( JOB, M, N, K, A, LDA, C, LDC, JPVT,
...
*
         CALL ZCOPY( MIN( MN, JJ ), A( 1, JJ ), 1, WORK, 1 )
         DO 20 J = JJ-1, RANK, -1
            CALL ZCOPY( MIN( MN, J+1 ), A( 1, J ), 1,
     $                  A( 1, J+1 ), 1 )
 20      CONTINUE
         CALL ZCOPY( MIN( MN, JJ ), WORK, 1, A( 1, RANK ), 1 )
*
...
*
         IF( JOB.EQ.1 ) THEN
            CALL ZGRET( JOB, MIN( M, JJ )-RANK+1, N-RANK+1, K,
...
*
      END
      SUBROUTINE ZCNIIF( JOB, M, N, K, A, LDA, C, LDC, JPVT, VNORM,
     $                   F, RANK, PERMUT, WORK, RWORK, INFO )
*
*
*     .. Scalar Arguments ..
...
*     .. Local Scalars ..
      INTEGER            MN, JJ, J, ITEMP
      DOUBLE PRECISION   SMAX, SMIN, SMINPR, TEMP
...
*     ..
*     .. External Subroutines ..
      EXTERNAL           ZCOPY, ZTRSV, ZHESS
*     ..
*     .. External Functions ..
      INTEGER            IDAMAX, IZAMAX
      DOUBLE PRECISION   DZNRM2, DLAMCH, DLASMX
      EXTERNAL           IDAMAX, IZAMAX, DZNRM2,
     $                   DLAMCH, DLASMX
*     ..
*     .. Executable Statements ..
*
...
*
            CALL ZCOPY( RANK, A( 1, JJ ), 1, WORK, 1 )
            DO 20 J = JJ+1, RANK
               CALL ZCOPY( J, A( 1, J ), 1, A( 1, J-1 ), 1 )
 20         CONTINUE
            CALL ZCOPY( RANK, WORK, 1, A( 1, RANK ), 1 )
*
*           Exchange in the same way vector JPVT.
*
            ITEMP = JPVT( JJ )
...
*
      END
      SUBROUTINE ZGRET( JOB, M, N, K, A, LDA, C, LDC,
     $                       WORK, RWORK, INFO )
*
*
*     .. Scalar Arguments ..
...
*
         DO 20 J = 2, N, NB
            ...
            DO 30 I = MIN( M, J+JB-1 ), J, -1
               CALL ZROT( J+JB-I, A( I-1, I ), LDA, A( I, I ), LDA,
     $                    RWORK( I ), WORK( I ) )
 30         CONTINUE
            DO 40 I = MIN( M, J-1 ), 2, -1
               CALL ZROT( JB, A( I-1, J ), LDA, A( I, J ), LDA,
     $                    RWORK( I ), WORK( I ) )
 40         CONTINUE
 20      CONTINUE
*
...
*
            DO 50 J = 1, K, NB
               JB = MIN( NB, K-J+1 )
               DO 60 I = M, 2, -1
                  CALL ZROT( JB, C( I-1, J ), LDC, C( I, J ), LDC,
     $                      RWORK( I ), WORK( I ) )
 60            CONTINUE
 50         CONTINUE
...
*
            DO 70 I = M, 2, -1
               CALL ZROT( K, C( 1, I-1 ), 1, C( 1, I ), 1,
     $                   RWORK( I ), DCONJG( WORK( I ) ) )
 70         CONTINUE
         END IF
      ELSE
*
...
*
            CALL ZLARTG( A( ITEMP, 1 ), A( I , 1 ), COSINE, SINE, R )
            A( ITEMP, 1 ) = R
            A( I, 1 ) = CZERO
*
*           Update columns I:N of matrix A.
*
            CALL ZROT( N-I+1, A( ITEMP, I ), LDA, A( I, I ), LDA,
     $                 COSINE, SINE )
*
*           Update the corresponding part of matrix C.
*
...
*              Apply the previously computed rotations from the left.
*
               CALL ZROT( K, C( ITEMP, 1 ), LDC, C( I, 1 ), LDC,
     $                     COSINE, SINE )
            ELSE IF( ( JOB.EQ.3 ).AND.( K.GT.0 ) ) THEN
*
*              Apply the transpose of the previously computed rotations
*              from the right.
*
               CALL ZROT( K, C( 1, ITEMP ), 1, C( 1, I ), 1,
     $                     COSINE, DCONJG( SINE ) )
            END IF
 90      CONTINUE
      END IF
...
*
      END
      SUBROUTINE ZHESS( JOB, M, N, K, A, LDA, C, LDC,
     $                       WORK, RWORK, INFO )
*
*
*     .. Scalar Arguments ..
...
*
 ...
 20         CONTINUE
            DO 30 I = J+1, MIN( M, J+JB )
               ITEMP = I-1
               CALL ZLARTG( A( ITEMP, ITEMP ), A( I, ITEMP ),
     $                      RWORK( I ), WORK( I ), R )
               A( ITEMP, ITEMP ) = R
               A( I, ITEMP ) = CZERO
               CALL ZROT( J+JB-I, A( ITEMP, I ), LDA,
