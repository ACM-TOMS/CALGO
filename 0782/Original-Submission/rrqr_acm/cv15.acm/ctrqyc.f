      SUBROUTINE CTRQYC( JOB, M, N, K, A, LDA, C, LDC, JPVT, RANK,
...
*     .. Local Scalars ..
      ...
      REAL               F, RCOS, SMAX, SMAXPR, SMIN, SMINPR, SMXRP1,
     $                   SMNRP1, TEMP
      COMPLEX            COSINE, CTEMP, DIAG, SINE
      INTEGER            NS
*     .. Local Arrays ..
      COMPLEX            CDUMMY( 1, 1 )
*     ..
*     .. External Subroutines ..
      EXTERNAL           CCOPY, CGRET, CHESS, CLAIC1,
     $                   CLARTG, CSCAL, CSWAP, CTRSV
*     ..
*     .. External Functions ..
      EXTERNAL           ISAMAX, ICAMAX, SLAMCH,
     $                   SLASMX, SCNRM2
      INTEGER            ISAMAX, ICAMAX
      REAL               SLAMCH, SLASMX, SCNRM2
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          ABS, MAX, MIN, SQRT, REAL
*     ..
...
*
         JJ = MN - 1 + ICAMAX( N-MN+1, A( MN, MN ), LDA )
         IF( JJ.GT.MN ) THEN
            CALL CSWAP( M, A( 1, MN ), 1, A( 1, JJ ), 1 )
            ITEMP = JPVT( MN )
            JPVT( MN ) = JPVT( JJ )
            JPVT( JJ ) = ITEMP
         END IF
...
*
         ...
         WORK( MN+1 ) = CONE
         DO 10 J = 2, RANK
            CALL CLAIC1( 1, J-1, WORK( 1 ), SMAX, A( 1, J ),
     $                   A( J, J ), SMAXPR, SINE, COSINE )
            CALL CSCAL( J-1, SINE, WORK( 1 ), 1 )
            WORK( J ) = COSINE
            SMAX = SMAXPR
*
            CALL CLAIC1( 2, J-1, WORK( MN+1 ), SMIN, A( 1, J ),
...
*              RANK->RANK-1.
*
               CALL CCOPY( RANK, A( 1, JJ ), 1, WORK( 1 ), 1 )
               DO 20 J = JJ+1, RANK
                  CALL CCOPY( J, A( 1, J ), 1, A( 1, J-1 ), 1 )
20             CONTINUE
               CALL CCOPY( RANK, WORK( 1 ), 1, A( 1, RANK ), 1 )
*
...
*
         DO 100 J = 1, N
            RWORK( J ) = SCNRM2( MIN( M, J ), A( 1, J ), 1 )
100      CONTINUE
*
...
*
            ...
            DO 120 I = MIN( MN, II )-1, RANK+1, -1
               CALL CLARTG( A( I, II ), DIAG, RCOS, SINE, CTEMP )
               DIAG = CTEMP
120         CONTINUE
*
            CALL CLAIC1( 2, RANK, WORK( 1 ), SMIN, A( 1, II ),
...
*              ...,II-1->II,II->RANK+1.
*
               CALL CCOPY( MIN( MN, II ), A( 1, II ), 1,
     $                      WORK( MN+1 ), 1 )
               DO 130 J = II-1, RANK+1, -1
                  CALL CCOPY( MIN( MN, J+1 ), A( 1, J ), 1 ,
     $                         A( 1, J+1 ), 1 )
130            CONTINUE
               CALL CCOPY( MIN( MN, II ), WORK( MN+1 ), 1,
     $                      A( 1, RANK+1 ), 1 )
*
*              Exchange in the same way vector JPVT.
*
...
*
               IF( JOB.EQ.1 ) THEN
                  CALL CGRET( JOB, MIN( M, II )-RANK, N-RANK, K,
...
*
               ITEMP = ISAMAX( RANK+1, RWORK, 1 )
               SMXRP1 = SLASMX( RANK+1 )*RWORK( ITEMP )
*
*              *-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*
...
*
                  CALL CCOPY( RANK, WORK( 1 ), 1, WORK( MN+1 ), 1 )
...
*
                     CALL CCOPY( RANK+1, A( 1, JJ ), 1,
     $                            WORK( MN+1 ), 1 )
                     DO 160 J = JJ+1, RANK+1
                        CALL CCOPY( J, A( 1, J ), 1,
     $                               A( 1, J-1 ), 1 )
160                  CONTINUE
                     CALL CCOPY( RANK+1, WORK( MN+1 ), 1,
     $                            A( 1, RANK+1 ), 1 )
*
*                    Exchange in the same way vector JPVT.
*
...
*
                     IF( JOB.EQ.1 ) THEN
                        CALL CHESS( JOB, RANK-JJ+2, N-JJ+1, K,
     $                              A( JJ, JJ ), LDA, CDUMMY, 1,
     $                              WORK( MN+1 ), RWORK( N+1 ), INFO )
                     ELSE IF( JOB.EQ.2 ) THEN
                        CALL CHESS( JOB, RANK-JJ+2, N-JJ+1, K,
...
*
            IF( JOB.EQ.1 ) THEN
               CALL CGRET( JOB, MIN( M, JJ )-RANK, N-RANK, K,
     $                     A( RANK+1, RANK+1 ), LDA, CDUMMY, 1,
     $                     WORK( MN+1 ), RWORK( N+1 ), INFO )
            ELSE IF( JOB.EQ.2 ) THEN
               CALL CGRET( JOB, MIN( M, JJ )-RANK, N-RANK, K,
...
*
         DO 220 J = 2, RANK
            CALL CLAIC1( 1, J-1, WORK( MN+1 ), SMAX,
...
*
         CALL CLAIC1( 1, RANK, WORK( MN+1 ), SMAX,
     $                A( 1, RANK+1 ), A( RANK+1, RANK+1 ), SMXRP1,
     $                SINE, COSINE )
         CALL CLAIC1( 2, RANK, WORK( 1 ), SMIN,
     $                A( 1, RANK+1 ), A( RANK+1, RANK+1 ), SMINPR,
     $                SINE, COSINE )
         CALL CSCAL( RANK, SINE, WORK( 1 ), 1 )
         WORK( RANK+1 ) = COSINE
         SMIN = SMINPR
         SVLUES( IAFTER ) = SMIN
*
