...
*     Local variables
*
      DOUBLE PRECISION    alpha,       b,           c
      DOUBLE PRECISION    deltax(MAXPTS),           deltaw(MAXPTS)
      DOUBLE PRECISION    exact,       fn,          relavg,      relerr
      DOUBLE PRECISION    relmax,      relmin,      relulp,      result
      DOUBLE PRECISION    ulp,         v(2,MAXPTS), w(MAXPTS)
      DOUBLE PRECISION    x(MAXPTS),   xarg,        zinit(1)
*
      INTEGER             i,           ierr,        narg,        neval
      INTEGER             nqmax,       nqmin,       nquad,       nruns
      INTEGER             p,           pmax
*
      fprime(xarg,narg) = dfloat(narg) * xarg**(narg-1)
