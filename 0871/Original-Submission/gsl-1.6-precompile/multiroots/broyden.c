 ...
 broyden_iterate(void *vstate, gsl_multiroot_function * function, gsl_vector * x, gsl_vector * f, gsl_vector * dx)
{
  ...
  MpIeee phi0;MpIeee  phi1;MpIeee  t;MpIeee  lambda;
...
new_step:
  ...
  if (phi1 > phi0 && iter < MpIeee( "10" ) && t > MpIeee( "0.1" ))
    {
      /* full step goes uphill, take a reduced step instead */
      
      MpIeee theta=  phi1 / phi0;
      t *= (sqrt (MpIeee( "1.0" ) + MpIeee( "6.0" ) * theta) - MpIeee( "1.0" )) / (MpIeee( "3.0" ) * theta);
      goto new_step;
    }
  ...
  for (i = 0; i < n; i++)
    {
      MpIeee yi=  gsl_vector_get (fnew, i) - gsl_vector_get (f, i);
  ...
  for (i = 0; i < n; i++)
    {
      MpIeee vi=  gsl_vector_get (v, i) + t * gsl_vector_get (p, i);
      ...
      for (j = 0; j < n; j++)
        {
          ...
          MpIeee Hij=  gsl_matrix_get (H, i, j) - vi * wj / lambda;
