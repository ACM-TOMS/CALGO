...
int  FUNCTION(my, scheck) (MpIeee * x, size_t k, TYPE (gsl_vector) * data);
int  FUNCTION(my, lcheck) (MpIeee * x, size_t k, TYPE (gsl_vector) * data);
...
TYPE (test_sort_vector) (size_t N, size_t stride)
{
  ...
  MpIeee * large=  malloc(k * sizeof(MpIeee));
...
int
 FUNCTION(my, scheck) (MpIeee * x, size_t k, TYPE (gsl_vector) * data)
{
  size_t i;
...
int
 FUNCTION(my, lcheck) (MpIeee * x, size_t k, TYPE (gsl_vector) * data)
{
  size_t i;

  for (i = 0; i < k; i++)
    {
      if (x[i] != FUNCTION (gsl_vector, get) (data, data->size - i - MpIeee( "1" )))
        {
          return GSL_FAILURE;
        }
    }
