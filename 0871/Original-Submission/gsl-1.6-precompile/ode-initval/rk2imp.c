...
rk2imp_alloc (size_t dim)
{
  ...
  state->Y1 = (MpIeee*) malloc (dim * sizeof (MpIeee));
...
static int
 rk2imp_step(MpIeee *y, rk2imp_state_t *state, 
              ...
              MpIeee dydt_out[], const gsl_odeiv_system * sys)
{
 ...
 /* Then with two steps with half step length (save to y) */ 
  ...
  for (i = 0; i < dim; i++) 
    {
      yerr[i] = MpIeee( "4.0" ) * (y[i] - y_onestep[i]) / MpIeee( "3.0" );
    }
