...
typedef struct
{
  ...
  MpIeee t_primed;              /* system was primed for this value of t */
  MpIeee last_h;                /* last step size */
  gsl_odeiv_step *primer;       /* stepper to use for priming */
  MpIeee *yim1;                 /* y_{i-1}    */
  MpIeee *k;                    /* work space */
  MpIeee *y0;                   /* work space */
  MpIeee *y0_orig;
  MpIeee *y_onestep;
  int  stutter;
}
...
gear2_alloc (size_t dim)
{
  ...
  state->yim1 = (MpIeee*) malloc (dim * sizeof (MpIeee));
  ...
  state->k = (MpIeee*) malloc (dim * sizeof (MpIeee));
...
static int
 gear2_step(MpIeee *y, gear2_state_t * state,
  ...
  for (nu = 0; nu < iter_steps; nu++)
    {
      ...
      for (i = 0; i < dim; i++)
        {
          y[i] = ((MpIeee( "4.0" ) * y0[i] - yim1[i]) + MpIeee( "2.0" ) * h * k[i]) / MpIeee( "3.0" );
        }
    }
             ...
             MpIeee dydt_out[], const gsl_odeiv_system * sys)
{
  ...
  if (state->primed == 0 || t == state->t_primed || h != state->last_h)
  ...
  else
    {
      ...
      MpIeee *const k=  state->k;
      MpIeee *const y0=  state->y0;
      MpIeee *const y0_orig=  state->y0_orig;
      MpIeee *const yim1=  state->yim1;
      ...
      for (i = 0; i < dim; i++)
        {
          yerr[i] = MpIeee( "4.0" ) * (y[i] - y_onestep[i]);
          yim1[i] = y0[i];
        }
