...
static MpIeee ranlxd_get_double(void *vstate);
...
{
  return (ranlxd_get_double (vstate) * MpIeee("4294967296.0") ).toUnsignedLong();     /* 2^32 */
}
...
ranlxd_set_lux (void *vstate, unsigned long int s, unsigned int  luxury)
{
  ...
  int  ibit;int   jbit;int   i;int   k;int   l;int   xbit[31];
  MpIeee x;MpIeee  y;
  ...
  for (k = 0; k < 12; ++k)
    {
      ...
      for (l = 1; l <= 48; ++l)
        {
          y = (MpIeee) ((xbit[ibit] + MpIeee( "1" )) % MpIeee( "2" ));
          x += x + y;
