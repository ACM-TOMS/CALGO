...
inline
static MpIeee sup_norm(const gsl_matrix * A)
{
...
obtain_suggestion(const gsl_matrix * A, gsl_mode_t mode)
{
  ...
  const MpIeee norm_A=  sup_norm(A);
  ...
  else
  {
     ...
     * table entries
     */
    const MpIeee extra=  log(1.01*norm_A/1000.0) / M_LN2;
    const int extra_i = (unsigned int) MpIeee(ceil(extra)).toInt();
  ...
  )
{
  ...
  else
  {
    ...
    const MpIeee divisor=  exp(M_LN2 * sugg.j);
