      ...
      for (i = 0; i < N; i++)
        {
          ...
          MpIeee x=  gsl_blas_dnrm2 (&c.vector);
      ...
      for (i = 0; i < GSL_MIN (M, N); i++)
        {
          ...
          MpIeee max_norm=  gsl_vector_get(norm, i);
          size_t j, kmax = i;

          for (j = i + 1; j < N; j++)
            {
              MpIeee x=  gsl_vector_get (norm, j);
...

          {
            ...
            gsl_vector_view c = gsl_vector_subvector (&c_full.vector, 
                                                      i, M - i);
            MpIeee tau_i=  gsl_linalg_householder_transform (&c.vector);
              ...
              for (j = i + 1; j < N; j++)
                {
                  ...
                  if (x > MpIeee( "0.0" ))
                    {
                      MpIeee y=  MpIeee( "0" );
                      MpIeee temp=  gsl_matrix_get (A, i, j) / x;
                  
                      if (fabs (temp) >= 1)
                        y = MpIeee( "0.0" );
                      else
                        y = x * sqrt (MpIeee( "1" ) - temp * temp);
  ...
  if (Q->size1 != Q->size2 || R->size1 != R->size2)
  ...
  else
    {
      ...
      for (k = N - 1; k > 0; k--)
        {
          MpIeee c;MpIeee  s;
          MpIeee wk=  gsl_vector_get (w, k);
          MpIeee wkm1=  gsl_vector_get (w, k - 1);
      ...
      for (j = 0; j < N; j++)
        {
          MpIeee r0j=  gsl_matrix_get (R, 0, j);
          size_t p_j = gsl_permutation_get (p, j);
          MpIeee vj=  gsl_vector_get (v, p_j);
      ...
      for (k = 1; k < N; k++)
        {
          MpIeee c;MpIeee  s;
          MpIeee diag=  gsl_matrix_get (R, k - 1, k - 1);
          MpIeee offdiag=  gsl_matrix_get (R, k, k - 1);
