  ...
  if (n == 1)
    {
      ...
      MpIeee absa=  gsl_complex_abs (alpha);
      MpIeee beta_r=  - (GSL_REAL(alpha) >= MpIeee( "0" ) ? MpIeee( "1" ) : -MpIeee( "1" )) * absa ;

      gsl_complex tau;

      if (beta_r == MpIeee( "0.0" ))
        {
          GSL_REAL(tau) = 0.0;
          GSL_IMAG(tau) = 0.0;
        }
    ...
    { 
      ...
      gsl_complex alpha = gsl_vector_complex_get (v, 0) ;            
      MpIeee absa=  gsl_complex_abs (alpha);
      MpIeee xnorm=  gsl_blas_dznrm2 (&x.vector);
      
      if (xnorm == MpIeee( "0" ) && GSL_IMAG(alpha) == MpIeee( "0" )) 
        {
          gsl_complex zero = gsl_complex_rect(0.0, 0.0);
          return zero; /* tau = 0 */
        }
      
      beta_r = - (GSL_REAL(alpha) >= MpIeee( "0" ) ? MpIeee( "1" ) : -MpIeee( "1" )) * hypot(absa, xnorm) ;
