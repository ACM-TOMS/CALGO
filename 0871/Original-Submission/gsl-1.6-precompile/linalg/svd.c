                      ...
                      gsl_vector * work)
{
  ...
  if (N == 1)
    {
      ...
      MpIeee norm=  gsl_blas_dnrm2 (&column.vector);
  ...
  
  {
    ...
    while (b > 0)
      {
        MpIeee fbm1=  gsl_vector_get (&f.vector, b - 1);

        if (fbm1 == MpIeee( "0.0" ) || gsl_isnan (fbm1))
          {
            b--;
            continue;
          }
        ...
        while (a > 0)
          {
            MpIeee fam1=  gsl_vector_get (&f.vector, a - 1);

            if (fam1 == MpIeee( "0.0" ) || gsl_isnan (fam1))
              {
                break;
              }
  ...
  for (i = 0; i < K; i++)
    {
      MpIeee S_max=  gsl_vector_get (S, i);
      size_t i_max = i;
  ...
  for (i = 0; i < N; i++)
    {
      ...
      MpIeee tau_i=  gsl_linalg_householder_transform (&v.vector);
  ...
  for (i = 0; i < N; i++)
    {
      ...
      {
        MpIeee Aii=  gsl_matrix_get (A, i, i);
    ...
    for (i = 0; i < M; i++)
      {
        ...
        for (j = 0; j < N; j++)
          {
            MpIeee Lij=  gsl_vector_get (&L_i.vector, j);
      ...
      for (i = 0; i < N; i++)
        {
          ...
          MpIeee alpha=  gsl_vector_get (S, i);
          if (alpha != MpIeee( "0" ))
            alpha = MpIeee( "1.0" ) / alpha;
 ...
 gsl_linalg_SV_decomp_jacobi(gsl_matrix * A, gsl_matrix * Q, gsl_vector * S)
{
  if (Q->size1 < Q->size2)
  ...
  else
    {
      ...
      MpIeee tolerance=  MpIeee( "10" ) * GSL_DBL_EPSILON;
              ...
              for (k = j + 1; k < N; k++)
                {
                  ...
                  if (q < r)
                    ...
                    {
                      q -= r;
                      v = hypot (MpIeee( "2.0" ) * p, q);
                      cosine = sqrt ((v + q) / (MpIeee( "2.0" ) * v));
                      sine = p / (v * cosine);
                    }
                  ...
                  for (i = 0; i < M; i++)
                    {
                      const MpIeee Aik=  gsl_matrix_get (A, i, k);
                      const MpIeee Aij=  gsl_matrix_get (A, i, j);
                  ...
                  for (i = 0; i < N; i++)
                    {
                      const MpIeee Qij=  gsl_matrix_get (Q, i, j);
                      const MpIeee Qik=  gsl_matrix_get (Q, i, k);
        ...
        for (j = 0; j < N; j++)
          {
            ...
            if (norm == MpIeee( "0.0" ) || prev_norm == MpIeee( "0.0" ) 
                || (j > MpIeee( "0" ) && norm <= tolerance * prev_norm))
              {
                gsl_vector_set (S, j, 0.0);     /* singular */
                gsl_vector_set_zero (&column.vector);   /* annihilate column */

                prev_norm = MpIeee( "0.0" );
              }
            else
              {
