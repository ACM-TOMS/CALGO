...
static int
 vector_bfgs_set(void *vstate, gsl_multimin_function_fdf * fdf,
                 const gsl_vector * x, MpIeee *f, gsl_vector * gradient,
                 MpIeee step_size, MpIeee tol)
{
  vector_bfgs_state_t *state = (vector_bfgs_state_t *) vstate;
                     ...
                     gsl_vector * gradient, gsl_vector * dx)
{
  ...
  MpIeee pnorm=  state->pnorm;
  MpIeee g0norm=  state->g0norm;

  MpIeee fa=  *f;MpIeee  fb;MpIeee  fc;
  MpIeee dir;
  MpIeee stepa=  MpIeee( "0.0" );MpIeee  stepb;MpIeee  stepc=  state->step;MpIeee  tol=  state->tol;

  MpIeee g1norm;
  MpIeee pg;

  if (pnorm == MpIeee( "0.0" ) || g0norm == MpIeee( "0.0" ))
    {
  ...
  dir = (pg >= MpIeee( "0.0" )) ? MpIeee( "1.0" ) : -MpIeee( "1.0" );
...
#ifdef DEBUG
  {cout<<"got stepc = "<<setiosflags((ios::floatfield))<< stepc;
cout.precision(6);cout.fill(' ');cout.width(0);cout.setf(ios::dec,ios::basefield);
cout<<resetiosflags((ios::floatfield))<<" fc = "<<setiosflags((ios::floatfield))<< fc;
cout.precision(6);cout.fill(' ');cout.width(0);cout.setf(ios::dec,ios::basefield);
cout<<resetiosflags((ios::floatfield))<<"\n";}
  ...
  if (state->iter == 0)
  ...
  else
    {
      ...
      MpIeee dxg;MpIeee  dgg;MpIeee  dxdg;MpIeee  dgnorm;MpIeee  A;MpIeee  B;
      ...
      B = dxg / dxdg;
      A = -(MpIeee( "1.0" ) + dgnorm * dgnorm / dxdg) * B + dgg / dxdg;
...
#ifdef DEBUG
  {cout<<"updated conjugate directions\n";}
  {cout<<"p: ";}
