...
static int  qr_companion(MpIeee *h, size_t nc, gsl_complex_packed_ptr z);

static int
 qr_companion(MpIeee *h, size_t nc, gsl_complex_packed_ptr zroot)
{
  MpIeee t=  MpIeee( "0.0" );

  size_t iterations, e, i, j, k, m;

  MpIeee w;MpIeee  x;MpIeee  y;MpIeee  s;MpIeee  z;

  MpIeee p=  MpIeee( "0" );MpIeee  q=  MpIeee( "0" );MpIeee  r=  MpIeee( "0" ); 
...
next_iteration:
  ...
  for (e = n; e >= 2; e--)
    {
      MpIeee a1=  fabs (FMAT (h, e, e - 1, nc));
      MpIeee a2=  fabs (FMAT (h, e - 1, e - 1, nc));
      MpIeee a3=  fabs (FMAT (h, e, e, nc));
      ...
      if (k != m)
        {
          p = FMAT (h, k, k - 1, nc);
          q = FMAT (h, k + 1, k - 1, nc);
          r = notlast ? FMAT (h, k + 2, k - 1, nc) : MpIeee( "0.0" );
