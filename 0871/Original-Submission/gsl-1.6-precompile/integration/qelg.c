...
//TODO, CHANGE THIS!!!!, JUST MADE TO COMPILE HERE... NOT CORRECT!
#undef GSL_DBL_EPSILON
#undef GSL_DBL_MAX
#define GSL_DBL_EPSILON myEpsilon()
#define GSL_DBL_MAX  myMax()
...
static void
  append_table (struct extrapolation_table *table, MpIeee y);
...
#ifdef JUNK
static void
initialise_table (struct extrapolation_table *table, MpIeee y)
{
  table->n = 0;
  table->rlist2[0] = y;
  table->nres = 0;
...
static inline void
  qelg (struct extrapolation_table *table, MpIeee *result, MpIeee *abserr);

static inline void
qelg (struct extrapolation_table *table, MpIeee *result, MpIeee *abserr)
{
  MpIeee *epstab=  table->rlist2;
  MpIeee *res3la=  table->res3la;
  const size_t n = table->n - 1;

  const MpIeee current=  epstab[n];

  MpIeee absolute=  MpIeee(GSL_DBL_MAX);
  MpIeee relative=  MpIeee( "5" ) * GSL_DBL_EPSILON * fabs (current);
  ...
  *abserr = GSL_DBL_MAX;
  ...
  for (i = 0; i < newelm; i++)
    {
      MpIeee res=  epstab[n - 2 * i + 2];
      MpIeee e0=  epstab[n - 2 * i - 2];
      MpIeee e1=  epstab[n - 2 * i - 1];
      MpIeee e2=  res;

      MpIeee e1abs=  fabs (e1);
      MpIeee delta2=  e2 - e1;
      MpIeee err2=  fabs (delta2);
      MpIeee tol2=  GSL_MAX_DBL (fabs (e2), e1abs) * GSL_DBL_EPSILON;
      MpIeee delta3=  e1 - e0;
      MpIeee err3=  fabs (delta3);
      MpIeee tol3=  GSL_MAX_DBL (e1abs, fabs (e0)) * GSL_DBL_EPSILON;

      MpIeee e3;MpIeee  delta1;MpIeee  err1;MpIeee  tol1;MpIeee  ss;
      ...
      if (err2 <= tol2 && err3 <= tol3)
        {
          ...
          *result = res;
          absolute = err2 + err3;
          relative = MpIeee( "5" ) * GSL_DBL_EPSILON * fabs (res);
          *abserr = GSL_MAX_DBL (absolute, relative);
          return;
        }
      ...
      ss = (MpIeee( "1" ) / delta1 + MpIeee( "1" ) / delta2) - MpIeee( "1" ) / delta3;
      ...
      {
        const MpIeee error=  err2 + fabs (res - e2) + err3;
  ...
  *abserr = GSL_MAX_DBL (*abserr, MpIeee( "5" ) * GSL_DBL_EPSILON * fabs (*result));
