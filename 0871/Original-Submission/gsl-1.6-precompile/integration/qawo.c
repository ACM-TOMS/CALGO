                      ...
                      MpIeee *result, MpIeee *abserr)
{
  ...
  MpIeee b=  a + wf->L ;
  MpIeee abs_omega=  fabs (wf->omega) ;
  ...
  do
    {
      ...
      if (extall)
      ...
      else
        {
          ...
          MpIeee width=  workspace->blist[i] - workspace->alist[i];
          
          if (0.25 * fabs(width) * abs_omega > MpIeee( "2" ))
            continue;
  ...
  *abserr = err_ext;
...

  {
    ...
    if (ratio < MpIeee( "0.01" ) || ratio > MpIeee( "100" ) || errsum > fabs (area))
      error_type = 6;
  }
