...
int
 gsl_sf_multiply_e(const MpIeee x, const MpIeee y, gsl_sf_result * result)
{
  ...
  if(x == 0.0 || y == 0.0) {
    ...
    const MpIeee f=  1.0 - 2.0 * GSL_DBL_EPSILON;
    const MpIeee min=  GSL_MIN_DBL(fabs(x), fabs(y));
    const MpIeee max=  GSL_MAX_DBL(fabs(x), fabs(y));
...
int
 gsl_sf_multiply_err_e(const MpIeee x, const MpIeee dx,
...
MpIeee gsl_sf_multiply(const MpIeee x, const MpIeee y)
{
