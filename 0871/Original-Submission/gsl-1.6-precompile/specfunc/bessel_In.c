...
int
 gsl_sf_bessel_In_scaled_e(int  n, const MpIeee x, gsl_sf_result * result)
{
  ...
  if(n == 0) {
    ...
    MpIeee Ikp1=  rat * GSL_SQRT_DBL_MIN;
    MpIeee Ik=  GSL_SQRT_DBL_MIN;
    MpIeee Ikm1;
    int  k;
    for(k=n; k >= 1; k--) {
      Ikm1 = Ikp1 + MpIeee( "2.0" )*k/ax * Ik;
      Ikp1 = Ik;
      Ik   = Ikm1;
    }
  ...
  else if( GSL_MIN( MpIeee(0.29)/(n*n), MpIeee(0.5)/(n*n + x*x) ) < 0.5*GSL_ROOT3_DBL_EPSILON) {
...
int
 gsl_sf_bessel_In_scaled_array(const int nmin, const int nmax, const MpIeee x, MpIeee * result_array)
{
  ...
  if(nmax < nmin || nmin < 0) {
    ...
    int  j;
    for(j=0; j<=nmax-nmin; j++) result_array[j] = MpIeee( "0.0" );
    if(nmin == 0) result_array[0] = MpIeee( "1.0" );
    return GSL_SUCCESS;
  }
  else if(nmax == 0) {
    gsl_sf_result I0_scaled;
    ...
    const MpIeee ax=  fabs(x);
    const MpIeee two_over_x=  2.0/ax;
...
int
 gsl_sf_bessel_In_e(const int n_in, const MpIeee x, gsl_sf_result * result)
{
  const MpIeee ax=  fabs(x);
...
int
 gsl_sf_bessel_In_array(const int nmin, const int nmax, const MpIeee x, MpIeee * result_array)
{
  MpIeee ax=  fabs(x);

  /* CHECK_POINTER(result_array) */

  if(ax > GSL_LOG_DBL_MAX - MpIeee( "1.0" )) {
    int  j;
    for(j=0; j<=nmax-nmin; j++) result_array[j] = MpIeee( "0.0" ); /* FIXME: should be Inf */
