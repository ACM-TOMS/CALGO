 ...
 */
static int
 hyperg_2F1_series(const MpIeee a, const MpIeee b, const MpIeee c,
                  const MpIeee x, 
                  gsl_sf_result * result
                  )
{
  MpIeee sum_pos=  MpIeee( "1.0" );
  MpIeee sum_neg=  MpIeee( "0.0" );
  MpIeee del_pos=  MpIeee( "1.0" );
  MpIeee del_neg=  MpIeee( "0.0" );
  ...
  do {
    ...
    del *= (a+k)*(b+k) * x / ((c+k) * (k+MpIeee( "1.0" )));  /* Gauss series */

    if(del > MpIeee( "0.0" )) {
      del_pos  =  del;
      sum_pos +=  del;
    }
    else if(del == MpIeee( "0.0" )) {
...
static
int
 hyperg_2F1_conj_series(const MpIeee aR, const MpIeee aI, const MpIeee c,
                       MpIeee x,
                       gsl_sf_result * result)
{
  if(c == 0.0) {
    ...
    MpIeee k=  MpIeee( "0.0" );
    do {
      del *= ((aR+k)*(aR+k) + aI*aI)/((k+MpIeee( "1.0" ))*(c+k)) * x;
...
static
int
 hyperg_2F1_luke(const MpIeee a, const MpIeee b, const MpIeee c,
                const MpIeee xin, 
                gsl_sf_result * result)
{
  int  stat_iter;
  ...
  const MpIeee t0=  a*b/c;
  const MpIeee t1=  (a+1.0)*(b+1.0)/(2.0*c);
  const MpIeee t2=  (a+2.0)*(b+2.0)/(2.0*(c+1.0));
  MpIeee F=  MpIeee( "1.0" );
  MpIeee prec;
  ...
  while(1) {
    ...
    MpIeee npbm1=  n + b - MpIeee( "1" );
    MpIeee npcm1=  n + c - MpIeee( "1" );
    MpIeee npam2=  n + a - MpIeee( "2" );
    MpIeee npbm2=  n + b - MpIeee( "2" );
    MpIeee npcm2=  n + c - MpIeee( "2" );
    MpIeee tnm1=  MpIeee( "2" )*n - MpIeee( "1" );
    MpIeee tnm3=  MpIeee( "2" )*n - MpIeee( "3" );
    MpIeee tnm5=  MpIeee( "2" )*n - MpIeee( "5" );
    MpIeee n2=  n*n;
    MpIeee F1=   (MpIeee( "3.0" )*n2 + (a+b-MpIeee( "6" ))*n + MpIeee( "2" ) - a*b - MpIeee( "2" )*(a+b)) / (MpIeee( "2" )*tnm3*npcm1);
    MpIeee F2=  -(MpIeee( "3.0" )*n2 - (a+b+MpIeee( "6" ))*n + MpIeee( "2" ) - a*b)*npam1*npbm1/(MpIeee( "4" )*tnm1*tnm3*npcm2*npcm1);
    MpIeee F3=  (npam2*npam1*npbm2*npbm1*(n-a-MpIeee( "2" ))*(n-b-MpIeee( "2" ))) / (MpIeee( "8" )*tnm3*tnm3*tnm5*(n+c-MpIeee( "3" ))*npcm2*npcm1);
    MpIeee E=  -npam1*npbm1*(n-c-MpIeee( "1" )) / (MpIeee( "2" )*tnm3*npcm2*npcm1);
...
static
int
 hyperg_2F1_conj_luke(const MpIeee aR, const MpIeee aI, const MpIeee c,
                     const MpIeee xin, 
                     gsl_sf_result * result)
{
  int  stat_iter;
  ...
  const MpIeee x=  -xin;
  const MpIeee x3=  x*x*x;
  const MpIeee atimesb=  aR*aR + aI*aI;
  const MpIeee apb=  2.0*aR;
  const MpIeee t0=  atimesb/c;
  const MpIeee t1=  (atimesb +     apb + 1.0)/(2.0*c);
  const MpIeee t2=  (atimesb + 2.0*apb + 4.0)/(2.0*(c+1.0));
  MpIeee F=  MpIeee( "1.0" );
  MpIeee prec;
  ...
  while(1) {
    MpIeee nm1=  n - MpIeee( "1" );
    MpIeee nm2=  n - MpIeee( "2" );
    MpIeee npam1_npbm1=  atimesb + nm1*apb + nm1*nm1;
    MpIeee npam2_npbm2=  atimesb + nm2*apb + nm2*nm2;
    MpIeee npcm1=  nm1 + c;
    MpIeee npcm2=  nm2 + c;
    MpIeee tnm1=  MpIeee( "2" )*n - MpIeee( "1" );
    MpIeee tnm3=  MpIeee( "2" )*n - MpIeee( "3" );
    MpIeee tnm5=  MpIeee( "2" )*n - MpIeee( "5" );
    MpIeee n2=  n*n;
    MpIeee F1=   (MpIeee( "3.0" )*n2 + (apb-MpIeee( "6" ))*n + MpIeee( "2" ) - atimesb - MpIeee( "2" )*apb) / (MpIeee( "2" )*tnm3*npcm1);
    MpIeee F2=  -(MpIeee( "3.0" )*n2 - (apb+MpIeee( "6" ))*n + MpIeee( "2" ) - atimesb)*npam1_npbm1/(MpIeee( "4" )*tnm1*tnm3*npcm2*npcm1);
    MpIeee F3=  (npam2_npbm2*npam1_npbm1*(nm2*nm2 - nm2*apb + atimesb)) / (MpIeee( "8" )*tnm3*tnm3*tnm5*(n+c-MpIeee( "3" ))*npcm2*npcm1);
    MpIeee E=  -npam1_npbm1*(n-c-MpIeee( "1" )) / (MpIeee( "2" )*tnm3*npcm2*npcm1);
...
static
int
 hyperg_2F1_reflect(const MpIeee a, const MpIeee b, const MpIeee c,
                   const MpIeee x, gsl_sf_result * result)
{
  const MpIeee d=  c - a - b;
  const int intd  = floor(d+0.5);
  const int d_integer = ( fabs(d - intd) < locEPS );

  if(d_integer) {
    const MpIeee ln_omx=  log(1.0 - x);
    const MpIeee ad=  fabs(d);
    ...
    if(ad < GSL_DBL_EPSILON) {
      ...
      if(stat_ad1 == GSL_SUCCESS && stat_bd1 == GSL_SUCCESS && stat_ad == GSL_SUCCESS) {
         ...
         */
        int  i;
        MpIeee sum1=  MpIeee( "1.0" );
        MpIeee term=  MpIeee( "1.0" );
        MpIeee ln_pre1_val=  lng_ad.val + lng_c.val + d2*ln_omx - lng_ad1.val - lng_bd1.val;
        MpIeee ln_pre1_err=  lng_ad.err + lng_c.err + lng_ad1.err + lng_bd1.err + GSL_DBL_EPSILON * fabs(ln_pre1_val);
        int  stat_e;
        ...
        for(i=1; i<ad; i++) {
          int  j=  i-1;
          term *= (a + d2 + j) * (b + d2 + j) / (MpIeee( "1.0" ) + d2 + j) / i * (MpIeee( "1.0" )-x);
          sum1 += term;
        }
    ...
    if(stat_ad2 == GSL_SUCCESS && stat_bd2 == GSL_SUCCESS) {
      ...
      MpIeee psi_val=  psi_1 + psi_1pd.val - psi_apd1.val - psi_bpd1.val - ln_omx;
      MpIeee psi_err=  psi_1pd.err + psi_apd1.err + psi_bpd1.err + GSL_DBL_EPSILON*fabs(psi_val);
      MpIeee fact=  MpIeee( "1.0" );
      MpIeee sum2_val=  psi_val;
      MpIeee sum2_err=  psi_err;
      MpIeee ln_pre2_val=  lng_c.val + d1*ln_omx - lng_ad2.val - lng_bd2.val;
      MpIeee ln_pre2_err=  lng_c.err + lng_ad2.err + lng_bd2.err + GSL_DBL_EPSILON * fabs(ln_pre2_val);
      int  stat_e;
      ...
      for(j=1; j<maxiter; j++) {
        ...
        MpIeee term1=  MpIeee( "1.0" )/(MpIeee)j  + MpIeee( "1.0" )/(ad+j);
        MpIeee term2=  MpIeee( "1.0" )/(a+d1+j-MpIeee( "1.0" )) + MpIeee( "1.0" )/(b+d1+j-MpIeee( "1.0" ));
        MpIeee delta=  MpIeee( "0.0" );
        psi_val += term1 - term2;
        psi_err += GSL_DBL_EPSILON * (fabs(term1) + fabs(term2));
        fact *= (a+d1+j-MpIeee( "1.0" ))*(b+d1+j-MpIeee( "1.0" ))/((ad+j)*j) * (MpIeee( "1.0" )-x);
        delta = fact * psi_val;
        sum2_val += delta;
    ...
    sgn_2 = ( GSL_IS_ODD(intd) ? -MpIeee( "1.0" ) : MpIeee( "1.0" ) );
    ...
    MpIeee sgn1;MpIeee  sgn2;
    gsl_sf_result F1, F2;
    int  status_F1;int   status_F2;

    /* These gamma functions appear in the denominator, so we
     * catch their harmless domain errors and set the terms to zero.
     */
    gsl_sf_result ln_g1ca,  ln_g1cb,  ln_g2a,  ln_g2b;
    MpIeee sgn_g1ca;MpIeee  sgn_g1cb;MpIeee  sgn_g2a;MpIeee  sgn_g2b;
    ...
    MpIeee sgn_gc;MpIeee  sgn_gd;MpIeee  sgn_gmd;
    ...
    if(ok1 && ok2) {
      MpIeee ln_pre1_val=  ln_gc.val + ln_gd.val  - ln_g1ca.val - ln_g1cb.val;
      MpIeee ln_pre2_val=  ln_gc.val + ln_gmd.val - ln_g2a.val  - ln_g2b.val + d*log(MpIeee( "1.0" )-x);
      MpIeee ln_pre1_err=  ln_gc.err + ln_gd.err + ln_g1ca.err + ln_g1cb.err;
      MpIeee ln_pre2_err=  ln_gc.err + ln_gmd.err + ln_g2a.err  + ln_g2b.err;
...
static int  pow_omx(const MpIeee x, const MpIeee p, gsl_sf_result * result)
{
  MpIeee ln_omx;
  MpIeee ln_result;
  if(fabs(x) < GSL_ROOT5_DBL_EPSILON) {
    ln_omx = -x*(MpIeee( "1.0" ) + x*(MpIeee( "1.0" )/MpIeee( "2.0" ) + x*(MpIeee( "1.0" )/MpIeee( "3.0" ) + x/MpIeee( "4.0" ) + x*x/MpIeee( "5.0" ))));
  }
  else {
    ln_omx = log(MpIeee( "1.0" )-x);
  }
  ln_result = p * ln_omx;
...
int
 gsl_sf_hyperg_2F1_e(MpIeee a, MpIeee b, const MpIeee c,
                       const MpIeee x,
                       gsl_sf_result * result)
{
  const MpIeee d=  c - a - b;
  ...
  const MpIeee rintc=  floor(c + 0.5);
  ...
  if(c_neg_integer) {
    if(! (a_neg_integer && a > c + MpIeee( "0.1" ))) DOMAIN_ERROR(result);
    if(! (b_neg_integer && b > c + MpIeee( "0.1" ))) DOMAIN_ERROR(result);
  }
  ...
  if(a >= MpIeee( "0.0" ) && b >= MpIeee( "0.0" ) && c >=MpIeee( "0.0" ) && x >= MpIeee( "0.0" ) && x < MpIeee( "0.995" )) {
...
int
 gsl_sf_hyperg_2F1_conj_e(const MpIeee aR, const MpIeee aI, const MpIeee c,
                            const MpIeee x,
                            gsl_sf_result * result)
{
...
int
 gsl_sf_hyperg_2F1_renorm_e(const MpIeee a, const MpIeee b, const MpIeee c,
                              const MpIeee x,
                              gsl_sf_result * result
                              )
{
  ...
  if(c_neg_integer) {
    if((a_neg_integer && a > c+0.1) || (b_neg_integer && b > c+0.1)) {
      ...
      MpIeee s1;MpIeee  s2;MpIeee  s3;MpIeee  s4;MpIeee  s5;
      int  stat=  0;
      ...
      if(stat != 0) {
        ...
        MpIeee ln_pre_val=  g1.val + g2.val - g3.val - g4.val - g5.val;
        MpIeee ln_pre_err=  g1.err + g2.err + g3.err + g4.err + g5.err;
        MpIeee sg=  s1 * s2 * s3 * s4 * s5;
...
int
 gsl_sf_hyperg_2F1_conj_renorm_e(const MpIeee aR, const MpIeee aI, const MpIeee c,
                                   const MpIeee x,
                                   gsl_sf_result * result
                                   )
{
  const MpIeee rintc=  floor(c  + 0.5);
  const MpIeee rinta=  floor(aR + 0.5);
  ...
  if(c_neg_integer) {
    if(a_neg_integer && aR > c+0.1) {
      ...
      if(stat != 0) {
        ...
        MpIeee ln_pre_val=  MpIeee( "2.0" )*(g1.val - g2.val) - g3.val;
        MpIeee ln_pre_err=  MpIeee( "2.0" ) * (g1.err + g2.err) + g3.err;
        int  stat_e=  gsl_sf_exp_mult_err_e(ln_pre_val, ln_pre_err,
                                              F.val, F.err,
                                              result);
        return GSL_ERROR_SELECT_2(stat_e, stat_F);
