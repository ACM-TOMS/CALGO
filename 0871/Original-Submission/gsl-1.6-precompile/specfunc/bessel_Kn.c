...
static
int
 bessel_Kn_scaled_small_x(const int n, const MpIeee x, gsl_sf_result * result)
{
  int  k;
  MpIeee y=  MpIeee( "0.25" ) * x * x;
  MpIeee ln_x_2=  log(MpIeee( "0.5" )*x);
  MpIeee ex=  exp(x);
  gsl_sf_result ln_nm1_fact;
  MpIeee k_term;
  MpIeee term1;MpIeee  sum1;MpIeee  ln_pre1;
  MpIeee term2;MpIeee  sum2;MpIeee  pre2;

  gsl_sf_lnfact_e((unsigned int)(n-1), &ln_nm1_fact);

  ln_pre1 = -n*ln_x_2 + ln_nm1_fact.val;
  if(ln_pre1 > GSL_LOG_DBL_MAX - MpIeee( "3.0" )) GSL_ERROR ("error", GSL_EOVRFLW);
  ...
  term1 = MpIeee( "0.5" ) * exp(ln_pre1) * sum1;

  pre2 = MpIeee( "0.5" ) * exp(n*ln_x_2);
  if(pre2 > MpIeee( "0.0" )) {
    const int KMAX = 20;
    gsl_sf_result psi_n;
    gsl_sf_result npk_fact;
    MpIeee yk=  MpIeee( "1.0" );
    MpIeee k_fact=  MpIeee( "1.0" );
    MpIeee psi_kp1=  -M_EULER;
    MpIeee psi_npkp1;
    gsl_sf_psi_int_e(n, &psi_n);
    gsl_sf_fact_e((unsigned int)n, &npk_fact);
    psi_npkp1 = psi_n.val + MpIeee( "1.0" )/n;
    sum2 = (psi_kp1 + psi_npkp1 - MpIeee( "2.0" )*ln_x_2)/npk_fact.val;
    for(k=1; k<KMAX; k++) {
      psi_kp1   += MpIeee( "1.0" )/k;
      psi_npkp1 += MpIeee( "1.0" )/(n+k);
      k_fact    *= k;
      npk_fact.val *= n+k;
      yk *= y;
      k_term = yk*(psi_kp1 + psi_npkp1 - MpIeee( "2.0" )*ln_x_2)/(k_fact*npk_fact.val);
      sum2 += k_term;
    }
    term2 = ( GSL_IS_ODD(n) ? -MpIeee( "1.0" ) : MpIeee( "1.0" ) ) * pre2 * sum2;
  }
  else {
    term2 = MpIeee( "0.0" );
  }
...
int  gsl_sf_bessel_Kn_scaled_array(const int nmin, const int nmax, const MpIeee x, MpIeee * result_array)
{
  ...
  if(nmin < 0 || nmax < nmin || x <= 0.0) {
    int  j;
    for(j=0; j<=nmax-nmin; j++) result_array[j] = MpIeee( "0.0" );
    ...
    for(n=nmin+1; n<=nmax+1; n++) {
      if(Knm1 < GSL_DBL_MAX) {
         ...
         */
        int  j;
        for(j=n; j<=nmax+1; j++) result_array[j-1-nmin] = MpIeee( "0.0" );
...
int
 gsl_sf_bessel_Kn_array(const int nmin, const int nmax, const MpIeee x, MpIeee * result_array)
{
