...
static
int
 gamma_inc_D(const MpIeee a, const MpIeee x, gsl_sf_result * result)
{
  if(a < 10.0) {
    MpIeee lnr;
    gsl_sf_result lg;
    ...
    term1 = exp(a*ln_term.val)/sqrt(MpIeee( "2.0" )*M_PI*a);
...
static
int
 gamma_inc_P_series(const MpIeee a, const MpIeee x, gsl_sf_result * result)
{
  const int nmax = 5000;
...
static
int
 gamma_inc_Q_large_x(const MpIeee a, const MpIeee x, gsl_sf_result * result)
{
  const int nmax = 5000;
...
static
int
 gamma_inc_Q_asymp_unif(const MpIeee a, const MpIeee x, gsl_sf_result * result)
{
  const MpIeee rta=  sqrt(a);
  const MpIeee eps=  (x-a)/a;

  gsl_sf_result ln_term;
  const int stat_ln = gsl_sf_log_1plusx_mx_e(eps, &ln_term);  /* log(1+eps) - eps */
  const MpIeee eta=  eps * sqrt(-2.0*ln_term.val/(eps*eps));
  ...
  if(fabs(eps) < GSL_ROOT5_DBL_EPSILON) {
    c0 = -MpIeee( "1.0" )/MpIeee( "3.0" ) + eps*(MpIeee( "1.0" )/MpIeee( "12.0" ) - eps*(MpIeee( "23.0" )/MpIeee( "540.0" ) - eps*(MpIeee( "353.0" )/MpIeee( "12960.0" ) - eps*MpIeee( "589.0" )/MpIeee( "30240.0" ))));
    c1 = -MpIeee( "1.0" )/MpIeee( "540.0" ) - eps/MpIeee( "288.0" );
  }
  else {
    const MpIeee rt_term=  sqrt(-2.0 * ln_term.val/(eps*eps));
    const MpIeee lam=  x/a;
    c0 = (MpIeee( "1.0" ) - MpIeee( "1.0" )/rt_term)/eps;
    c1 = -(eta*eta*eta * (lam*lam + MpIeee( "10.0" )*lam + MpIeee( "1.0" )) - MpIeee( "12.0" ) * eps*eps*eps) / (MpIeee( "12.0" ) * eta*eta*eta*eps*eps*eps);
  }

  R = exp(-MpIeee( "0.5" )*a*eta*eta)/(M_SQRT2*M_SQRTPI*rta) * (c0 + c1/a);
 ...
 */
static int
 gamma_inc_F_CF(const MpIeee a, const MpIeee x, gsl_sf_result * result)
{
  const int    nmax  =  5000;
  const MpIeee small=   gsl_pow_3 (GSL_DBL_EPSILON);

  MpIeee hn=  MpIeee( "1.0" );           /* convergent */
  MpIeee Cn=  MpIeee( "1.0" ) / small;
  MpIeee Dn=  MpIeee( "1.0" );
  int  n;
...
static
int
 gamma_inc_Q_CF(const MpIeee a, const MpIeee x, gsl_sf_result * result)
{
  gsl_sf_result D;
  gsl_sf_result F;
  const int stat_D = gamma_inc_D(a, x, &D);
...
static
int
 gamma_inc_Q_series(const MpIeee a, const MpIeee x, gsl_sf_result * result)
{
  MpIeee term1;  /* 1 - x^a/Gamma(a+1) */
  MpIeee sum;    /* 1 + (a+1)/(a+2)(-x)/2! + (a+1)/(a+3)(-x)^2/3! + ... */
  int  stat_sum;
  MpIeee term2;  /* a temporary variable used at the end */

  {
    /* Evaluate series for 1 - x^a/Gamma(a+1), small a
     */
    const MpIeee pg21=  -2.404113806319188570799476;  /* PolyGamma[2,1] */
    const MpIeee lnx=  log(x);
    const MpIeee el=  M_EULER+lnx;
    const MpIeee c1=  -el;
    const MpIeee c2=  M_PI*M_PI/12.0 - 0.5*el*el;
    const MpIeee c3=  el*(M_PI*M_PI/12.0 - el*el/6.0) + pg21/6.0;
    const MpIeee c4=  -0.04166666666666666667
                       * (-1.758243446661483480 + lnx)
                       * (-0.764428657272716373 + lnx)
                       * ( 0.723980571623507657 + lnx)
                       * ( 4.107554191916823640 + lnx);
    const MpIeee c5=  -0.0083333333333333333
                       * (-2.06563396085715900 + lnx)
                       * (-1.28459889470864700 + lnx)
                       * (-0.27583535756454143 + lnx)
                       * ( 1.33677371336239618 + lnx)
                       * ( 5.17537282427561550 + lnx);
    const MpIeee c6=  -0.0013888888888888889
    ...
    const MpIeee c7=  -0.00019841269841269841
    ...
    const MpIeee c8=  -0.00002480158730158730
    ...
    const MpIeee c9=  -2.75573192239859e-6
    ...
    const MpIeee c10=  -2.75573192239859e-7
...

  {
    ...
    for(n=1; n<nmax; n++) {
      t *= -x/(n+MpIeee( "1.0" ));
      sum += (a+MpIeee( "1.0" ))/(a+n+MpIeee( "1.0" ))*t;
  ...
  term2 = (MpIeee( "1.0" ) - term1) * a/(a+MpIeee( "1.0" )) * x * sum;
...
static int
 gamma_inc_series(MpIeee a, MpIeee x, gsl_sf_result * result)
{
  gsl_sf_result Q;
  gsl_sf_result G;
...
static int
 gamma_inc_a_gt_0(MpIeee a, MpIeee x, gsl_sf_result * result)
{
...
static int
 gamma_inc_CF(MpIeee a, MpIeee x, gsl_sf_result * result)
{
  gsl_sf_result F;
  gsl_sf_result pre;
  const int stat_F = gamma_inc_F_CF(a, x, &F);
...
int
 gsl_sf_gamma_inc_Q_e(const MpIeee a, const MpIeee x, gsl_sf_result * result)
{
  if(a < 0.0 || x < 0.0) {
    DOMAIN_ERROR(result);
  }
...
int
 gsl_sf_gamma_inc_P_e(const MpIeee a, const MpIeee x, gsl_sf_result * result)
{
  if(a <= 0.0 || x < 0.0) {
    DOMAIN_ERROR(result);
  }
...
int
 gsl_sf_gamma_inc_e(const MpIeee a, const MpIeee x, gsl_sf_result * result)
{
  if(x < 0.0) {
    DOMAIN_ERROR(result);
  }
  ...
  {
    /* a = fa + da; da >= 0 */
    const MpIeee fa=  floor(a);
    const MpIeee da=  a - fa;
    ...
    do
    {
      const MpIeee shift=  exp(-x + (alpha-1.0)*log(x));
      gax = (gax - shift) / (alpha - MpIeee( "1.0" ));
      alpha -= MpIeee( "1.0" );
    } while(alpha > a);
...
MpIeee gsl_sf_gamma_inc_P(const MpIeee a, const MpIeee x)
{
  EVAL_RESULT(gsl_sf_gamma_inc_P_e(a, x, &result));
}

MpIeee gsl_sf_gamma_inc_Q(const MpIeee a, const MpIeee x)
{
  EVAL_RESULT(gsl_sf_gamma_inc_Q_e(a, x, &result));
}

MpIeee gsl_sf_gamma_inc(const MpIeee a, const MpIeee x)
{
