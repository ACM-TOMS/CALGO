...
gsl_eigen_symmv_alloc (const size_t n)
{
  ...
  w->gc = (MpIeee*) malloc (n * sizeof (MpIeee));
  ...
  w->gs = (MpIeee*) malloc (n * sizeof (MpIeee));
      ...
      while (b > 0)
        {
          ...
          
          {
            ...
            MpIeee * const gc=  w->gc;
            MpIeee * const gs=  w->gs;
            ...
            for (i = 0; i < n_block - 1; i++)
              {
                const MpIeee c=  gc[i];const MpIeee  s=  gs[i];
                size_t k;
                
                for (k = 0; k < N; k++)
                  {
                    MpIeee qki=  gsl_matrix_get (evec, k, a + i);
                    MpIeee qkj=  gsl_matrix_get (evec, k, a + i + 1);
