...
void
FUNCTION (gsl_vector, set_all) (TYPE (gsl_vector) * v, MpIeee x)
{
  ...
  for (i = 0; i < n; i++)
    {
      *(MpIeee*) (data + MULTIPLICITY * i * stride) = x;
    }
}
...
FUNCTION (gsl_vector, set_zero) (TYPE (gsl_vector) * v)
{
  ...
  for (i = 0; i < n; i++)
    {
      *(MpIeee*) (data + MULTIPLICITY * i * stride) = zero;
    }
}
 ...
 FUNCTION(gsl_vector, set_basis) (TYPE (gsl_vector) * v, size_t i)
{
  ...
  for (k = 0; k < n; k++)
    {
      *(MpIeee*) (data + MULTIPLICITY * k * stride) = zero;
    }

  *(MpIeee*) (data + MULTIPLICITY * i * stride) = one;
