...
static int  qags(const gsl_function * f, const MpIeee a, const MpIeee b, const MpIeee epsabs, const MpIeee epsrel, const size_t limit,
  gsl_integration_workspace * workspace, MpIeee *result, MpIeee *abserr,
...
static MpIeee i_transform(MpIeee t, void *params);
...
static MpIeee i_transform(MpIeee t, void *params)
{
  ...
  MpIeee x=  (MpIeee( "1" ) - t) / t;
  MpIeee y=  GSL_FN_EVAL (f, x) + GSL_FN_EVAL (f, -x);
  return (y / t) / t;
}
...
struct il_params { MpIeee b; gsl_function * f ; } ;

static MpIeee il_transform(MpIeee t, void *params);
...
static MpIeee il_transform(MpIeee t, void *params)
{
  ...
  MpIeee b=  p->b;
  gsl_function * f = p->f;
  MpIeee x=  b - (MpIeee( "1" ) - t) / t;
  MpIeee y=  GSL_FN_EVAL (f, x);
  return (y / t) / t;
}
...
struct iu_params { MpIeee a; gsl_function * f ; } ;

static MpIeee iu_transform(MpIeee t, void *params);
      ...
      gsl_integration_rule * q)
{
  ...
  MpIeee result0;MpIeee  abserr0;MpIeee  resabs0;MpIeee  resasc0;
  MpIeee tolerance;

  MpIeee ertest=  MpIeee( "0" );
  MpIeee error_over_large_intervals=  MpIeee( "0" );
  MpIeee reseps=  MpIeee( "0" );MpIeee  abseps=  MpIeee( "0" );MpIeee  correc=  MpIeee( "0" );
  size_t ktmin = 0;
  int  roundoff_type1=  0;int   roundoff_type2=  0;int   roundoff_type3=  0;
  int  error_type=  0;int   error_type2=  0;
  ...
  *abserr = MpIeee( "0" );
  ...
  if (abserr0 <= MpIeee( "100" ) * GSL_DBL_EPSILON * resabs0 && abserr0 > tolerance)
    {
      *result = result0;
      *abserr = abserr0;
  ...
  else if ((abserr0 <= tolerance && abserr0 != resasc0) || abserr0 == MpIeee( "0.0" ))
    {
      *result = result0;
      *abserr = abserr0;
  ...
  do
    {
      ...
      MpIeee resasc1;MpIeee  resasc2;
      MpIeee resabs1;MpIeee  resabs2;
      MpIeee last_e_i;
      ...
      if (ktmin > 5 && err_ext < MpIeee( "0.001" ) * errsum)
        {
          error_type = 5;
        }
  ...
  *abserr = err_ext;
  ...
  {
    MpIeee max_area=  GSL_MAX_DBL (fabs (res_ext), fabs (area));

    if (!positive_integrand && max_area < MpIeee( "0.01" ) * resabs0)
      goto return_error;
  }

  {
    MpIeee ratio=  res_ext / area;

    if (ratio < MpIeee( "0.01" ) || ratio > MpIeee( "100.0" ) || errsum > fabs (area))
      error_type = 6;
  }
