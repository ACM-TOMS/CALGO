...
static int
 qagp(const gsl_function *f,
      const MpIeee *pts, const size_t npts,
      ...
      gsl_integration_rule * q)
{
  ...
  MpIeee result0;MpIeee  abserr0;MpIeee  resabs0;
  MpIeee tolerance;
  ...
  *abserr = MpIeee( "0" );
  ...
  for (i = 0; i < nint; i++)
    {
      MpIeee area1;MpIeee  error1;MpIeee  resabs1;MpIeee  resasc1;
      const MpIeee a1=  pts[i];
      const MpIeee b1=  pts[i + 1];
      ...
      if (error1 == resasc1 && error1 != MpIeee( "0.0" ))
        {
          ndin[i] = 1;
        }
      else
