...
static void init_grid (gsl_monte_vegas_state * s, MpIeee xl[], MpIeee xu[],
                size_t dim);
...
static void accumulate_distribution (gsl_monte_vegas_state * s, coord bin[],
                                     MpIeee y);
static void random_point (MpIeee x[], coord bin[], MpIeee *bin_vol,
                          const coord box[], 
...
static void print_lim (gsl_monte_vegas_state * state,
                       MpIeee xl[], MpIeee xu[], unsigned long dim);
...
static void print_res (gsl_monte_vegas_state * state,
                       unsigned int  itr, MpIeee res, MpIeee err, 
                       MpIeee cum_res, MpIeee cum_err,
                       MpIeee chi_sq);
                           ...
                           MpIeee *result, MpIeee *abserr)
{
  ...
  if (state->stage <= 2)
    {
      ...
      {
        MpIeee tot_boxes=  pow ((MpIeee) boxes, (MpIeee) dim);
        state->calls_per_box = GSL_MAX ( MpIeee( MpIeee(calls) / tot_boxes), MpIeee("2")).toUnsignedInt();
        calls = MpIeee(state->calls_per_box * tot_boxes).toUnsignedInt();
      }

      /* total volume of x-space/(avg num of calls/bin) */
      state->jac = state->vol * pow ((MpIeee) bins, (MpIeee) dim) / calls;
  ...
  for (it = 0; it < state->iterations; it++)
    {
      MpIeee intgrl=  MpIeee( "0.0" );MpIeee  intgrl_sq=  MpIeee( "0.0" );
      MpIeee sig=  MpIeee( "0.0" );
      MpIeee wgt;
      size_t calls_per_box = state->calls_per_box;
      MpIeee jacbin=  state->jac;
      MpIeee *x=  state->x;
      coord *bin = state->bin;
      ...
      do
        {
          MpIeee m=  MpIeee( "0" );MpIeee  q=  MpIeee( "0" );
          MpIeee f_sq_sum=  MpIeee( "0.0" );

          for (k = 0; k < calls_per_box; k++)
            {
              MpIeee fval;MpIeee  bin_vol;
...

              {
                MpIeee d=  fval - m;
                m += d / (k + MpIeee( "1.0" ));
                q += d * d * (k / (k + MpIeee( "1.0" )));
              }

              if (state->mode != GSL_VEGAS_MODE_STRATIFIED)
                {
                  MpIeee f_sq=  fval * fval;
      ...
      sig = sig / (calls_per_box - MpIeee( "1.0" ))  ;
     ...
     if (wgt > MpIeee( "0.0" ))
       {
         ...
         cum_sig = sqrt (MpIeee( "1" ) / state->sum_wgts);
       ...
       {
         cum_int += (intgrl - cum_int) / (it + MpIeee( "1.0" ));
         cum_sig = MpIeee( "0.0" );
       }         
...
gsl_monte_vegas_alloc (size_t dim)
{
  ...
  s->delx = (MpIeee*) malloc (dim * sizeof (MpIeee));
  ...
  s->d = (MpIeee*) malloc (BINS_MAX * dim * sizeof (MpIeee));
  ...
  s->xi = (MpIeee*) malloc ((BINS_MAX + 1) * dim * sizeof (MpIeee));
  ...
  s->xin = (MpIeee*) malloc ((BINS_MAX + 1) * sizeof (MpIeee));
  ...
  s->weight = (MpIeee*) malloc (BINS_MAX * sizeof (MpIeee));
...
static void
init_grid (gsl_monte_vegas_state * s, MpIeee xl[], MpIeee xu[], size_t dim)
{
  size_t j;
...
static void
accumulate_distribution (gsl_monte_vegas_state * s, coord bin[], MpIeee y)
{
  size_t j;
  size_t dim = s->dim;
...
static void
random_point (MpIeee x[], coord bin[], MpIeee *bin_vol,
              const coord box[], const MpIeee xl[], const MpIeee xu[],
              gsl_monte_vegas_state * s, gsl_rng * r)
{
  ...
  for (j = 0; j < dim; ++j)
    {
      ...
      MpIeee z=  ((box[j] + gsl_rng_uniform_pos (r)) / boxes) * bins;

      int  k=  z.toInt();

      MpIeee y;MpIeee  bin_width;
...
resize_grid (gsl_monte_vegas_state * s, unsigned int  bins)
{
  ...
  MpIeee pts_per_bin=  (MpIeee) s->bins / (MpIeee) bins;

  for (j = 0; j < dim; j++)
    {
      MpIeee xold;
      MpIeee xnew=  MpIeee( "0" );
      MpIeee dw=  MpIeee( "0" );
      int  i=  1;
...
refine_grid (gsl_monte_vegas_state * s)
{
  ...
  for (j = 0; j < dim; j++)
    {
      MpIeee grid_tot_j;MpIeee  tot_weight;
      MpIeee * weight=  s->weight;

      MpIeee oldg=  VALUE (s, 0, j);
      MpIeee newg=  VALUE (s, 1, j);
          ...
          if (VALUE (s, i, j) > 0)
            {
              ...
              /* damped change */
              weight[i] = pow (((oldg - MpIeee( "1" )) / oldg / log (oldg)), s->alpha);
            }

          tot_weight += weight[i];

#ifdef DEBUG
          {cout<<"weight["<< i<<"] = "<<setiosflags((ios::floatfield))<< weight[i];
...
cout<<resetiosflags((ios::floatfield))<<"\n";}
      ...
      {
        MpIeee pts_per_bin=  tot_weight / bins;
