...
struct gsl_dht_struct {
  ...
  MpIeee nu;    /* Bessel function order                          */
  MpIeee xmax;  /* the upper limit to the x-sampling domain       */
  MpIeee kmax;  /* the upper limit to the k-sampling domain       */
  MpIeee *  j;     /* array of computed J_nu zeros, j_{nu,s} = j[s]  */
  MpIeee *  Jjj;   /* transform numerator, J_nu(j_i j_m / j_N)       */
  MpIeee *  J2;    /* transform denominator, J_{nu+1}^2(j_m)         */
};
...
gsl_dht * gsl_dht_new(size_t size, MpIeee nu, MpIeee xmax);
 ...
 */
int  gsl_dht_init(gsl_dht * t, MpIeee nu, MpIeee xmax);

/* The n'th computed x sample point for a given transform.
 * 0 <= n <= size-1
 */
MpIeee gsl_dht_x_sample(const gsl_dht * t, int  n);


/* The n'th computed k sample point for a given transform.
 * 0 <= n <= size-1
 */
MpIeee gsl_dht_k_sample(const gsl_dht * t, int  n);
 ...
 */
int  gsl_dht_apply(const gsl_dht * t, MpIeee * f_in, MpIeee * f_out);
