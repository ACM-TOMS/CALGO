          ...
          if (tau_i != MpIeee( "0.0" )) 
            {
              gsl_matrix_view m = gsl_matrix_submatrix (A, i + 1, i + 1, 
                                                        N - (i+1), N - (i+1));
              MpIeee ei=  gsl_vector_get(&v.vector, 0);
              ...
              /* w = x - (1/2) tau * (x' * v) * v  */
              {
                MpIeee xv;MpIeee  alpha;
                gsl_blas_ddot(&x.vector, &v.vector, &xv);
                alpha = - (tau_i / MpIeee( "2.0" )) * xv;
                          ...
                          gsl_vector * sdiag)
{
  if (A->size1 !=  A->size2)
  ...
  else
    {
      ...
      for (i = N - 2; i > 0 && i--;)
        {
          ...
          MpIeee ti=  gsl_vector_get (tau, i);
      ...
      for (i = 0; i < N - 1; i++)
        {
          MpIeee Aji=  gsl_matrix_get (A, i+1, i);
                            ...
                            gsl_vector * sdiag)
{
  if (A->size1 !=  A->size2)
  ...
  else
    {
      ...
      for (i = 0; i < N - 1; i++)
        {
          MpIeee Aij=  gsl_matrix_get (A, i+1, i);
