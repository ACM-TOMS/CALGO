...
/* assumes n >= 1 */
static int  bessel_Yn_small_x(const int n, const MpIeee x, gsl_sf_result * result)
{
  int  k;
...
int
 gsl_sf_bessel_Yn_e(int  n, const MpIeee x, gsl_sf_result * result)
{
  int  sign=  1;
...
int
 gsl_sf_bessel_Yn_array(const int nmin, const int nmax, const MpIeee x, MpIeee * result_array)
{
  ...
  if(nmin < 0 || nmax < nmin || x <= 0.0) {
    ...
    if(stat == GSL_SUCCESS) {
      ...
      for(n=nmin; n<=nmax; n++) {
        result_array[n-nmin] = MpIeee( "0.0" );
      }
    }
