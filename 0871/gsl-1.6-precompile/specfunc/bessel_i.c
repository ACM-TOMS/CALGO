...
static
int
 bessel_il_CF1(const int l, const MpIeee x, const MpIeee threshold, MpIeee * ratio)
{
  const int kmax = 2000;
  ...
  for(k=1; k<=kmax; k++) {
    MpIeee ak=  (x/(MpIeee( "2.0" )*l+MpIeee( "1.0" )+MpIeee( "2.0" )*k)) * (x/(MpIeee( "2.0" )*l+MpIeee( "3.0" )+MpIeee( "2.0" )*k));
  ...
  *ratio = x/(MpIeee( "2.0" )*l+MpIeee( "3.0" )) * sum;
...
int  gsl_sf_bessel_i0_scaled_e(const MpIeee x, gsl_sf_result * result)
{
  ...
  if(ax < MpIeee( "0.2" )) {
    const MpIeee eax=  exp(-ax);
    const MpIeee y=  ax*ax;
  ...
  else if(ax < -MpIeee( "0.5" )*GSL_LOG_DBL_EPSILON) {
...
int  gsl_sf_bessel_i2_scaled_e(const MpIeee x, gsl_sf_result * result)
{
  ...
  if(ax < MpIeee( "4.0" )*GSL_SQRT_DBL_MIN) {
    UNDERFLOW_ERROR(result);
  }
  else if(ax < MpIeee( "0.25" )) {
    const MpIeee y=  x*x;
    ...
    const MpIeee c5=  1.0/518918400.0;
    const MpIeee sum=  1.0 + y*(c1 + y*(c2 + y*(c3 + y*(c4 + y*c5))));
    const MpIeee pre=  exp(-ax) * x*x/15.0;
    result->val = pre * sum;
    result->err = 2.0 * GSL_DBL_EPSILON * fabs(result->val);
    return GSL_SUCCESS;
  }
...
int  gsl_sf_bessel_il_scaled_e(const int l, MpIeee x, gsl_sf_result * result)
{
  ...
  if(x < MpIeee( "0.0" )) {
    /* i_l(-x) = (-1)^l i_l(x) */
    sgn = ( GSL_IS_ODD(l) ? -MpIeee( "1.0" ) : MpIeee( "1.0" ) );
    x = -x;
  }
  ...
  if(l < 0) {
  ...
  else if(x*x < MpIeee( "10.0" )*(l+MpIeee( "1.5" ))/M_E) {
    gsl_sf_result b;
    int  stat=  gsl_sf_bessel_IJ_taylor_e(l+0.5, x, 1, 50, GSL_DBL_EPSILON, &b);
    MpIeee pre=  exp(-ax) * sqrt((MpIeee( "0.5" )*M_PI)/x);
    ...
    MpIeee iellp1=  rat * GSL_SQRT_DBL_MIN;
    MpIeee iell=  GSL_SQRT_DBL_MIN;
    MpIeee iellm1;
    int  ell;
    for(ell = l; ell >= 1; ell--) {
      iellm1 = iellp1 + (MpIeee( "2" )*ell + MpIeee( "1" ))/x * iell;
      iellp1 = iell;
      iell   = iellm1;
    }
  ...
  else if(GSL_MIN_DBL(MpIeee("0.29")/(l*l+1.0), MpIeee("0.5")/(l*l+1.0+x*x)) < MpIeee("0.5")*GSL_ROOT3_DBL_EPSILON) {
    ...
    MpIeee rt_term=  sqrt((MpIeee( "0.5" )*M_PI)/x);
    ...
    MpIeee iellp1=  r_iellp1.val;
    MpIeee iell=  r_iell.val;
    MpIeee iellm1=  MpIeee( "0.0" );
    int  ell;
    iellp1 *= rt_term;
    iell   *= rt_term;
