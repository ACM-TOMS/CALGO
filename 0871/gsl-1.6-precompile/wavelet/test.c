...
test_1d (size_t N, size_t stride, const gsl_wavelet_type * T, size_t member)
{
  ...
  size_t i;
  MpIeee *data=  malloc (N * stride * sizeof (MpIeee));

  for (i = 0; i < N * stride; i++)
    data[i] = MpIeee( "12345.0" ) + i;
  ...
  for (i = 0; i < N; i++)
    {
      MpIeee x1=  gsl_vector_get (v1, i);
      MpIeee x2=  gsl_vector_get (v2, i);
      ...
      for (i = 0; i < N * stride; i++)
        {
          ...
          status |= (data[i] != (MpIeee( "12345.0" ) + i));
        }
...
test_2d (size_t N, size_t tda, const gsl_wavelet_type * T, size_t member, int  type)
{
  ...
  MpIeee *data=  malloc (N * tda * sizeof (MpIeee));
  ...
  for (i = 0; i < N; i++)
    {
      for (j = 0; j < N; j++)
        {
          MpIeee x1=  gsl_matrix_get (m1, i, j);
          MpIeee x2=  gsl_matrix_get (m2, i, j );
      ...
      for (i = 0; i < N ; i++)
        {
          for (j = N; j < tda; j++)
            {
              status |= (data[i*tda+j] != (MpIeee( "12345.0" ) + (i*tda+j)));
            }
        }
