...
main (int argc, char *argv[])
{
  size_t i,j;
  size_t n = 0;
  MpIeee mu=  MpIeee( "0" );MpIeee  nu=  MpIeee( "0" );MpIeee  nu1=  MpIeee( "0" );MpIeee  nu2=  MpIeee( "0" );MpIeee  sigma=  MpIeee( "0" );MpIeee  a=  MpIeee( "0" );MpIeee  b=  MpIeee( "0" );MpIeee  c=  MpIeee( "0" );
  MpIeee zeta=  MpIeee( "0" );MpIeee  sigmax=  MpIeee( "0" );MpIeee  sigmay=  MpIeee( "0" );MpIeee  rho=  MpIeee( "0" );
  MpIeee p=  MpIeee( "0" );
  MpIeee x=  MpIeee( "0" );MpIeee  y= MpIeee( "0" );MpIeee  z= MpIeee( "0" )  ;
  unsigned int  N=  0;int   t=  0;int   n1=  0;int   n2=  0 ;
  ...
  if (argc < 4) 
    {
      {cout<<"Usage: gsl-randist seed n DIST param1 param2 ...\n";}
      exit (0);
    }
...
#define INT_ARG(x) if (argc) { x=atoi((++argv)[0]);argc--;} else {error( #x);};
  ...
  if (NAME("bernoulli"))
  ...
  else if (NAME("dir-nd"))
    {
      ...
      INT_ARG(n1) ;
      xarr = (MpIeee *)malloc(n1*sizeof(MpIeee));

      for(i = 0; i < n; i++) { 
        gsl_ran_dir_nd (r, n1, xarr) ; 
        for (j = 0; j < n1; j++) { 
          if (j) putchar(' '); 
          {cout<<""<<setiosflags((ios::floatfield))<< xarr[j];
