...
test1d (void)
{
  ...
  for (i = 0; i < N; i++)
    {
      gsl_histogram_accumulate (h, (MpIeee) i, (MpIeee) i);
    };
...

  {
    ...
    for (i = 0; i < N; i++)
      {
        gsl_histogram_increment (h, (MpIeee) i);
  ...
  {
    MpIeee max=  gsl_histogram_max_val (h);
    gsl_test (max != MpIeee( "123456.0" ), "gsl_histogram_max_val finds maximum value");
  }

  {
    MpIeee min=  gsl_histogram_min_val (h);
    gsl_test (min != -MpIeee( "654321.0" ), "gsl_histogram_min_val finds minimum value");
  }
  ...
  {
    MpIeee sum= gsl_histogram_sum (h);
    gsl_test(sum != N*MpIeee( "27" )+((N-MpIeee( "1" ))*N)/MpIeee( "2" ), "gsl_histogram_sum sums all bin values");
  }
