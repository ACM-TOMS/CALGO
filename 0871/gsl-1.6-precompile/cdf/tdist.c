...
static MpIeee poly_eval(const MpIeee c[], unsigned int  n, MpIeee x)
{
  unsigned int  i;
  MpIeee y=  c[0] * x;
...
static MpIeee cornish_fisher(MpIeee t, MpIeee n)
{
  const MpIeee coeffs6[10] =  {
  ...
  MpIeee a=  n - MpIeee( "0.5" );
  MpIeee b=  MpIeee( "48.0" ) * a * a;

  MpIeee z2=  a * log1p (t * t / n);
  MpIeee z=  sqrt (z2);

  MpIeee p5=  z * poly_eval (coeffs6, 9, z2);
  MpIeee p4=  -z * poly_eval (coeffs5, 7, z2);
  MpIeee p3=  z * poly_eval (coeffs4, 5, z2);
  MpIeee p2=  -z * poly_eval (coeffs3, 3, z2);
  MpIeee p1=  z * (z2 + MpIeee( "3.0" ));
  MpIeee p0=  z;
...
static MpIeee normal_approx(const MpIeee x, const MpIeee nu)
{
  MpIeee y;
  MpIeee num;
  MpIeee diff;
  MpIeee q;
  int  i;
  MpIeee lg1;MpIeee  lg2;

  y = MpIeee( "1" ) / sqrt (MpIeee( "1" ) + x * x / nu);
  num = MpIeee( "1.0" );
  q = MpIeee( "0.0" );
  diff = MpIeee( "2" ) * GSL_DBL_EPSILON;
  for (i = 2; (i < MAXI) && (diff > GSL_DBL_EPSILON); i += 2)
    {
      diff = q;
      num *= y * y * (i - MpIeee( "1" )) / i;
      q += num / (nu + i);
      diff = q - diff;
    }
  q += MpIeee( "1" ) / nu;
  lg1 = gsl_sf_lngamma (nu / MpIeee( "2.0" ));
  lg2 = gsl_sf_lngamma ((nu + MpIeee( "1.0" )) / MpIeee( "2.0" ));
...
MpIeee gsl_cdf_tdist_P(const MpIeee x, const MpIeee nu)
{
  ...
  if (x2 < nu)
    {
      MpIeee u=  x2 / nu;
      MpIeee eps=  u / (MpIeee( "1" ) + u);

      if (x >= 0)
        {
          P = beta_inc_AXPY (MpIeee( "0.5" ), MpIeee( "0.5" ), MpIeee( "0.5" ), nu / MpIeee( "2.0" ), eps);
        }
      else
        {
          P = beta_inc_AXPY (-MpIeee( "0.5" ), MpIeee( "0.5" ), MpIeee( "0.5" ), nu / MpIeee( "2.0" ), eps);
    ...
    {
      MpIeee v=  nu / (x * x);
      MpIeee eps=  v / (MpIeee( "1" ) + v);

      if (x >= 0)
        {
          P = beta_inc_AXPY (-MpIeee( "0.5" ), MpIeee( "1.0" ), nu / MpIeee( "2.0" ), MpIeee( "0.5" ), eps);
        }
      else
        {
          P = beta_inc_AXPY (MpIeee( "0.5" ), MpIeee( "0.0" ), nu / MpIeee( "2.0" ), MpIeee( "0.5" ), eps);
        }
    }
  ...
  if (x2 < nu)
    {
      ...
      if (x >= 0)
        {
          Q = beta_inc_AXPY (-MpIeee( "0.5" ), MpIeee( "0.5" ), MpIeee( "0.5" ), nu / MpIeee( "2.0" ), eps);
        }
      else
        {
          Q = beta_inc_AXPY (MpIeee( "0.5" ), MpIeee( "0.5" ), MpIeee( "0.5" ), nu / MpIeee( "2.0" ), eps);
  ...
  else
    {
      ...
      if (x >= 0)
        {
          Q = beta_inc_AXPY (MpIeee( "0.5" ), MpIeee( "0.0" ), nu / MpIeee( "2.0" ), MpIeee( "0.5" ), eps);
        }
      else
        {
          Q = beta_inc_AXPY (-MpIeee( "0.5" ), MpIeee( "1.0" ), nu / MpIeee( "2.0" ), MpIeee( "0.5" ), eps);
        }
    }
