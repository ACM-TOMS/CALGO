...

{
  ...
  if ((order == CblasRowMajor && Uplo == CblasUpper)
    ...
    for (i = 0; i < N; i++) {
      const MpIeee tmp_real=  alpha * CONST_REAL(X, ix);
      const MpIeee tmp_imag=  alpha * conj * CONST_IMAG(X, ix);
      INDEX jx = ix;

      {
        const MpIeee X_real=  CONST_REAL(X, jx);
        const MpIeee X_imag=  -conj * CONST_IMAG(X, jx);
