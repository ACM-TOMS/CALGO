      SUBROUTINE DGHUYR( COMPQ1, COMPQ2, N, A, LDA, B, LDB, Q1, LDQ1,
...
C     DGHUET and DGHUEX are used to triangularize the subpencil
C     aAA1 - bBB1.
C
...
C     CONTRIBUTOR
C
C     M. Voigt, Technische Universitaet Chemnitz, Dec. 2008.
C     V. Sima, Research Institute for Informatics, Bucharest, Dec. 2009.
C
C     REVISIONS
C
C     V. Sima, Aug. 2009, Feb. 2010, July 2010, Sep.-Nov. 2010,
C     Jan. 2011, Mar. 2011, July 2011, June 2014, July 2014.
...
C
         ...
         DO 5 J = 1, I
            BW( I ) = .TRUE.
    5    CONTINUE
         CALL DGGES( 'V', 'V', 'S', SB02OW, I, A, LDA, B, LDB,
     $               IDUM( 1 ), DWORK, DWORK, DWORK, Q1, I, Q2, I,
     $               DWORK, -1, BW, INFO )
         CALL DGGES( 'V', 'V', 'N', SB02OW, I, A, LDA, B, LDB,
     $               IDUM( 1 ), DWORK, DWORK, DWORK, Q1, I, Q2,
     $               I, DWORK( 2 ), -1, BW, INFO )
         CALL DGGEV( 'N', 'N', 2, A, LDA, B, LDB, DWORK, DWORK, DWORK,
     $               DUM, 1, DUM, 1, DWORK( 3 ), -1, INFO )
         DWORK( 4 ) = 4*I + 16
C
         OPTWRK = MAX( 64 + MAX( 12 + INT( DWORK( 1 ) ), 4*M + 8, 4*N,
     $                           24 + INT( DWORK( 2 ) ),
     $                            6 + INT( DWORK( 3 ) ),
...
C
     ...
     $      CALL DCOPY( M-1, B( M+2, 1 ), LDB+1, DWORK( IB21+1 ), MP1 )
C
         CALL MB03KD( 'I', IDUM, 'N', K, M, KSCHUR, IWORK,
     $                 IWORK( K+1 ), IWORK( 2*K+1 ), BWORK, DWORK,
     $                 IWORK, IWORK( 3*K+1 ), DWORK( IV1 ), IWORK,
     $                 IWORK( 3*K+1 ), M1, HUND2, IWORK( 4*K+1 ),
...
C
         ...
         I2LORI = I2UPRI + DIM1
C
C        Generate input matrices for DGHUET, built of submatrices of A
C        and B.
C        Workspace:   need    32.
C
         IF( DIM1.EQ.1 ) THEN
...
C        Workspace:   need   64 + max( 63, 4*M + 8, 4*N ).
C
         IWRK  = 4*SDIM*SDIM + 1
         ITMP  = IWRK  + M*DIM1
         ITMP2 = ITMP  + M*DIM1
         ITMP3 = ITMP2 + DIM1*DIM1
         CALL DGHUET( SDIM, ULP, DWORK( IAUPLE ), SDIM, DWORK( IBUPLE ),
...
C
            ...
            I2LORI = I2UPRI + DIM1
C
C           Generate input matrices for DGHUEX built of submatrices of A
C           and B.
C           Workspace:   need   32.
C
...
C
            IWRK = 4*SDIM*SDIM + 1
            ITMP = IWRK + 2*N
            CALL DGHUEX( 'Lower', DIM1, DIM2, ULP, DWORK( IBUPLE ),
