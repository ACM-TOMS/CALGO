      SUBROUTINE DGHUET( N, PREC, A, LDA, B, LDB, Q1, LDQ1, Q2, LDQ2,
     $                   DWORK, LDWORK, INFO )
C
C     PURPOSE
C
...
C     REVISIONS
C
C     V. Sima, Aug. 2009., Oct. 2009, Nov. 2009, Oct. 2010, Nov. 2010,
C     June 2014.
...
C
      IF( N.EQ.4 ) THEN
         CALL DGGES( 'V', 'V', 'S', SB02OW, N, B, LDB, A, LDA, IDUM,
     $               DWORK, DWORK( N+1 ), DWORK( 2*N+1 ), Q2, LDQ2, Q1,
     $               LDQ1, DWORK( 3*N+1 ), LDWORK-3*N, BWORK, INFO )
         IF( INFO.NE.0 ) THEN
