...
*
      DOUBLE PRECISION    dabs,        datan,       dcos,        deps
      DOUBLE PRECISION    derbit,      dfloat,      dgamma,      dlog
      DOUBLE PRECISION    dpsi,        dsin,        dtan,        dnan
...
*
      ...
      DOUBLE PRECISION    deltax(MAXPTS),           dsda
      DOUBLE PRECISION    deltaw(MAXPTS),           exact,       onepa
      DOUBLE PRECISION    oneps2,      relavg,      relerr,      relmax
      DOUBLE PRECISION    relmin,      relulp,      result,      s
      DOUBLE PRECISION    sigarg,      sigma,       sigmax,      sigmin
      DOUBLE PRECISION    theta,       ulp,         v(2,MAXPTS)
      DOUBLE PRECISION    w(MAXPTS),   x(MAXPTS),   xarg
...
*
      ...
      s = dgamma(onepa) * dsin(onepa * theta) / oneps2**(HALF * onepa)
      IF (sigma .LT. ZERO) s = -s
      dsda = s*(dpsi(onepa) + theta/dtan(onepa * theta) -
     X    HALF*dlog(oneps2))
      exact = dsda
