...
*
      ...
      EXTERNAL            qpsi,        qvsum
*
      REAL*16             qabs,        qatan,       qcos,        qeps
      REAL*16             qerbit,      qgamma,      qlog,        qpsi
      REAL*16             qsin,        qtan,        qnan,        qvsum
*
*     Statement functions
*
      REAL*16             f,           fprime
...
*     Local variables
*
      REAL*16             alfmax,      alfmin,      alpha
      REAL*16             deltax(MAXPTS),           dsda
      REAL*16             deltaw(MAXPTS),           exact,       onepa
      REAL*16             oneps2,      relavg,      relerr,      relmax
      REAL*16             relmin,      relulp,      result,      s
      REAL*16             sigarg,      sigma,       sigmax,      sigmin
      REAL*16             theta,       ulp,         v(2,MAXPTS)
...
*
      DO 200 i = 1, nquad
          v(1,i) = deltaw(i) * f(x(i),sigma)
          v(2,i) = deltax(i) * fprime(x(i),sigma)
  200 CONTINUE
      neval = 2*nquad
...
10000 FORMAT (/,2X, 1X, 'nquad', 3X, 2X, 'sigma', 3X, 2X, 'alpha', 3X,
     X    12X, 'Quadrature Result', 15X,
     X    13X, 'Exact Integral', 14X,
     X    1X, 'Rel. Error', 1X, 'RelE (ULPs)', 2X, 'Err (bits)')
20000 FORMAT (2X, I6, 1X, F9.4, 1X, F9.4, 1P, 1X, E43.34, 1X, E43.34,
     X    1X, E10.2, 2X, 0P, 1X, F9.2, 1X, F9.2)
30000 FORMAT (2X, I6, 1X, F9.4, 1X, F9.4, 1P, 1X, E43.34, 1X, E43.34,
     X    1X, E10.2, 2X, E10.2, 0P, 1X, F9.2)
40000 FORMAT (/, 2X, 'Done')
50000 FORMAT (I1, 1X, 1X, 1P, 1X, E44.35, 1X, E44.35, 1X, I10, 1X,
     X    E43.34, 1X, E9.2, 1X, E9.2)
