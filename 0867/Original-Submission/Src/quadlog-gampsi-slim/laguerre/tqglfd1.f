...
*
      ...
      REAL*16             exact,       fn,          relavg,      relerr
      REAL*16             relmax,      relmin,      relulp,      result
      REAL*16             ulp,         v(2,MAXPTS), w(MAXPTS)
      REAL*16             x(MAXPTS),   xarg,        zinit(1)
*
      INTEGER             i,           ierr,        narg,        neval
      INTEGER             nqmax,       nqmin,       nquad,       nruns
      INTEGER             p,           pmax
*
      fprime(xarg,narg) = qfloat(narg) * xarg**(narg-1)
...
*
      WRITE (stdout, 10000)
      DO 200 i = 1, nquad
          WRITE (stdout, 20000) i, x(i), w(i), deltaw(i), deltax(i)
  200 CONTINUE
*
...
*         f(x) = x**p
*
*     Instead of invoking qgamma() and qpsi() each time, the following
...
*
          DO 300 i = 1, nquad
              v(1,i) = deltaw(i) * f(x(i), p)
              v(2,i) =  deltax(i) * fprime(x(i), p)
  300     CONTINUE
          neval = 2*nquad
...
*
      ...
      CLOSE (UNIT=stddat)
*
10000 FORMAT (/, 2X, 3X, 'i', 16X, 'x(i)', 20X, 16X, 'w(i)', 23X,
     X    17X, 'deltaw(i)', 21X, 14X, 'deltax(i)')
20000 FORMAT (2X, I4, 1X, F40.34, 1P, 1X, E43.34, 1X, E43.34, 1X,
     X    E43.34)
30000 FORMAT (/, 2X, 4X, 'p', 15X, 'Quadrature Result', 3X,
     X    24X, 'Exact Integral', 16X, 'Rel. Error', 1X, 'RelE (ULPs)',
     X    2X, 'Err (bits)')
40000 FORMAT (2X, I5, 1P, 1X, E43.34, 1X, E43.34, 1X, E10.2, 2X, 0P, 1X,
     X    F9.2, 1X, F9.2)
45000 FORMAT (2X, I5, 1P, 1X, E43.34, 1X, E43.34, 1X, E10.2, 2X, 1X,
     X    E9.2, 0P, 1X, F9.2)
50000 FORMAT (/, 2X, 'alpha = ', F20.16, '  nquad = ', I6)
60000 FORMAT (/, 2X, 'Done')
70000 FORMAT (I1, 1X, I3, 1X, 1P, E43.34, 1X, I10, 1X, E43.34, 1X, E9.2,
     X    1X, E9.2)
