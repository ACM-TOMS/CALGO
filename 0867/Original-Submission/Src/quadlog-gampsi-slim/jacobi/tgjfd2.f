...
*
*     This is the driver program for subroutine gjqfd() which returns
...
*
      ...
      DOUBLE PRECISION    dgamma,      dnan,        dvsum
*
*     Statement functions
*
      DOUBLE PRECISION    f
...
*     Local variables
*
      DOUBLE PRECISION    alpha,       beta,        deltax(MAXPTS)
      DOUBLE PRECISION    deltaw(MAXPTS),           exact,       relavg
      DOUBLE PRECISION    relerr,      relmax,      relmin,      relulp
      DOUBLE PRECISION    result,      ulp,         v(MAXPTS)
...
*
  ...
  300     CONTINUE
          ...
          exact = TWO**(alpha + beta + dfloat(p) + 1) *
     X        dgamma(alpha + 1) * dgamma(beta + dfloat(p) + 1) /
     X        dgamma(alpha + beta + p + 2)
