      SUBROUTINE  qgjqfd(x, w, deltaw, deltax, alpha, beta, nquad, ierr)
************************************************************************
...
*     External functions
*
      EXTERNAL            qgamma,      qpsi
*
      REAL*16             qext,        qgamma,      qpsi,        qsqrt
*
*     Parameter variables
*
      REAL*16             HALF
...
*
      REAL*16             an,          ap,          bn,          bp
      REAL*16             dd(MAXPTS),  e(MAXPTS),   ee(MAXPTS),  fab
      REAL*16             ff,          fj,          fl
      REAL*16             g(MAXPTS),   h(MAXPTS),   p0,          p1
      REAL*16             p2,          ph0,         ph1,         ph2
      REAL*16             phz,         s,           sab,         sum
      REAL*16             sum1,        sum2,        u0,          u0s
      REAL*16             y(MAXPTS)
*
...
*
      ...
      bp = ONE + beta
      sab = ap + bp
      u0s = qgamma(sab)/(qgamma(ap)*qgamma(bp)*TWO**(ap + beta))
      u0 = qsqrt(u0s)
      fab = alpha**2 - beta**2
      phz = HALF*u0*(qpsi(ap + bp) - qpsi(bp) - QLGTWO)
      DO 100 i = 1, nquad
          fl = qext(i)
          an = fl + alpha
          bn = fl + beta
          ...
          END IF
          e(i + 1) = TWO*qsqrt(fl*an*bn*ff/(an + bn + ONE))/(an + bn)
...
*
      DO 400 i = 1, nquad
          ...
          DO 300 j = 2, nquad
              ...
              sum = sum + p1**2
              sum1 = sum1 + p1*ph1
              sum2 = sum2 + ((g(j) - x(i))*qext(j - 1)*p1 + h(j)*p0)*
     X            p1
