...
program example2d_rrsqrtenkf
  ...
  real( kind = pre ) , allocatable , dimension( : , : ) :: sqrtcovauxrrsqrtenkf
  ...
  if ( rank == 0_pin ) then
     ...
     print*,'unitassimrrsqrtenkf = ' , unitassimrrsqrtenkf   
     print*,'unitobser           = ' , unitobser   
     print*,'filetruth           = ' , trim( filetruth )   
     print*,'filemodel           = ' , trim( filemodel )  
     print*,'fileassimrrsqrtenkf = ' , trim( fileassimrrsqrtenkf )  
  ...
  call mpi_bcast( unitassimrrsqrtenkf , 1_pin , mpi_integer , 0_pin , mpi_comm_world , ierror )
  ...
  call mpi_bcast( fileassimrrsqrtenkf , 100_pin , mpi_character , 0_pin , mpi_comm_world , ierror )
  !*
  !* solution parameters
  if ( rank == 0_pin ) then
  ...
  if ( pre == low ) then
     ...
     call mpi_bcast( sqrtcov , dimspacestate * modesanalysis , mpi_real , 0_pin , mpi_comm_world , ierror )
  else if ( pre == high ) then
     call mpi_bcast( state , dimspacestate , mpi_double_precision , 0_pin , mpi_comm_world , ierror )
     call mpi_bcast( sqrtcov , dimspacestate * modesanalysis , mpi_double_precision , 0_pin , mpi_comm_world , ierror )
  end if
  !*
  ...
  !*
  !* assimilation
  assimrrsqrtenkf( 1_pin , 1_pin : nx , 1_pin : ny ) = staterrsqrtenkf( 1_pin : nx , 1_pin : ny )
  !*
  !* writing to file
  if ( rank == 0_pin ) then
     do i = 1_pin , nx
        do j = 1_pin , ny
           ...
           write( unitassimrrsqrtenkf , '( f15.8 , f15.8 , f15.8 , f15.8 )' ) t( 1_pin ) , x( i ) , y( j ) , assimrrsqrtenkf( 1_p&
                &in , i , j )
        end do
     end do
  end if
  ...
  do l = 1_pin , nt - 1_pin
     ...
     if ( l == 1_pin ) then
        first = .true.
        call rrsqrtenkf_predictor( l , staterrsqrtenkf , sqrtcovrrsqrtenkf , samplesrrsqrtenkf , sqrtcovauxrrsqrtenkf )
     else
        call rrsqrtenkf_predictor( l , staterrsqrtenkf , sqrtcovrrsqrtenkf , samplesrrsqrtenkf , sqrtcovauxrrsqrtenkf )
     end if
     !*
     ...
     if ( ifobs ) then
        ...
        !*
        call rrsqrtenkf_corrector( l , staterrsqrtenkf , sqrtcovrrsqrtenkf , samplesrrsqrtenkf , sqrtcovauxrrsqrtenkf )
        !*
        deallocate( obsvalue )
        deallocate( sigmaobs )
        !*
     ...
     !*
     assimrrsqrtenkf( l + 1_pin , 1_pin : nx , 1_pin : ny ) = staterrsqrtenkf
     !*
     !* writing to file
     if ( rank == 0_pin ) then
        do i = 1_pin , nx
           do j = 1_pin , ny
              ...
              write( unitmodel , '( f15.8 , f15.8 , f15.8 , f15.8 )' ) t( l + 1_pin ) , x( i ) , y( j ) , modl( l + 1_pin , i , j&
                   & ) 
              write( unitassimrrsqrtenkf , '( f15.8 , f15.8 , f15.8 , f15.8 )' ) t( l + 1_pin ) , x( i ) , y( j ) , assimrrsqrten&
                   &kf( l + 1_pin , i , j ) 
           end do
        end do
     end if
  ...
  deallocate( sqrtcovauxrrsqrtenkf )
  deallocate( truth )
  deallocate( modl )
  deallocate( obser )
  ...
  if ( rank == 0_pin ) then
     ...
     close( unitmodel )
     close( unitobser )
     close( unitassimrrsqrtenkf )
  end if
  !*
  !* end parallelization
  call parallel_finalize()
  !*
end program example2d_rrsqrtenkf
