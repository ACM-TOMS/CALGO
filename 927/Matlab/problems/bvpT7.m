function [probs,funs,bcfuns,dfuns,dbcfuns,esolus,setoutputs,settolerancess]=bvpT7(ExtraArgs)
...
if nargin==1 && ~isempty(ExtraArgs)
    ...
    function F = fun(X,Z,ExtraArgs)
        ...
        pix = pi*X;
        F(1,:) = Z(2,:);
        F(2,:) = (-X.*Z(2,:)+Z(1,:)-(1.0e0+lambda*pi*pi).*cos(pix)...
            -pix.*sin(pix))./lambda;
    end
    ...
    function Exact = esolu(X,ExtraArgs)
        ...
        Cc = exp(-Aa);
        Dd = exp(-Bb);
        Exact(1,:) = cos(pi*X)+X+(X.*erf(X./sqrt(2.0e0*lambda))...
            +sqrt(2.0e0*lambda/pi).*Cc)./...
            (erf(1.0e0./sqrt(2.0e0*lambda))+sqrt(2.0e0*lambda/pi).*Dd);
        Exact(2,:) = erf((2^(1/2).*X)/(2*lambda^(1/2)))/(erf(2^(1/2)/(2*lambda^(1/2))) + (2^(1/2)*lambda^(1/2))/(pi^(1/2)*exp(1/(2*lambda)))) - pi*sin(pi*X) + 1;
    end
end