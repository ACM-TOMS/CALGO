function [probs,funs,bcfuns,dfuns,dbcfuns,esolus,setoutputs,settolerancess]=bvpT1(ExtraArgs)
...
if nargin==1 && ~isempty(ExtraArgs)
    ...
    function Exact = esolu(X,ExtraArgs)
        ...
        Exact(1,:) = (exp(-X./sqrt(lambda))-exp(-(2.e0-X)./sqrt(lambda)))./(1.e0-exp(-2.e0./sqrt(lambda)));
        Exact(2,:) = (1./(lambda^(1/2)*exp(X./lambda^(1/2))) + exp((X - 2)./lambda^(1/2))./lambda^(1/2))/(1/exp(2/lambda^(1/2)) - 1);
    end
end