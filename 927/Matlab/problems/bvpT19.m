function [probs,funs,bcfuns,dfuns,dbcfuns,esolus,setoutputs,settolerancess]=bvpT19(ExtraArgs)
...
if nargin==1 && ~isempty(ExtraArgs)
    ...
    function F = fun(X,Z,ExtraArgs)
        ...
        pix  = pi*X;
        F(1,:) = Z(2,:);
        F(2,:) = ((pi/2.e0)*sin(pix/2.e0).*exp(2.e0*Z(1,:)) ...
            -exp(Z(1,:)).*Z(2,:))./lambda;
    end
    ...
    function Df = dfun(X,Z,ExtraArgs)
        ...
        Df(1,2,:) = 1.0e0;
        Df(2,1,:) = (pi*sin(pi*X/2.e0).*exp(2.e0*Z(1,1,:))-exp(Z(1,1,:)).*Z(2,1,:))/lambda;
        Df(2,2,:) = -exp(Z(1,1,:))/lambda;        
    end
