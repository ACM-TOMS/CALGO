function [probs,funs,bcfuns,dfuns,dbcfuns,esolus,setoutputs,settolerancess]=bvpT3(ExtraArgs)
...
if nargin==1 && ~isempty(ExtraArgs)
    ...
    function F = fun(X,Z,ExtraArgs)
        ...
        F=zeros(size(Z));
        F(1,:) = Z(2,:);
        F(2,:) = (-(2.0e0+cos(pi*X)).*Z(2,:)+Z(1,:)-(1.0e0+lambda*pi*pi).*cos(pi*X) ...
            -(2.0e0+cos(pi*X))*pi.*sin(pi*X))./lambda;
    end
