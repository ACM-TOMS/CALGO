function [probs,funs,bcfuns,dfuns,dbcfuns,esolus,setoutputs,settolerancess]=bvpT17(ExtraArgs)
...
if nargin==1 && ~isempty(ExtraArgs)
    ...
    function F = fun(X,Z,ExtraArgs)
        ...
        F=zeros(size(Z));
        F(1,:) = Z(2,:);
        F(2,:) = -3.0*lambda*Z(1,:)./(lambda+X.*X).^2;
    end
    ...
    function  bc = bcfun(ya,yb,ExtraArgs)
        ...
        C0 = [1,0;0,0];
        C1 = [0,0;1,0];
        Eta = [-0.1/sqrt(lambda+0.01);0.1/sqrt(lambda+0.01) ];
        bc = C0*ya + C1*yb  - Eta;
    end
    ...
    function Df = dfun(X,Z,ExtraArgs)
        ...
        Df(1,2,:) = 1.0e0;
        Df(2,1,:) = -3.0e0*lambda/(lambda+X.*X).^2;
    end
    ...
    function Exact = esolu(X,ExtraArgs)
        ...
        Exact(1,:) = X./sqrt(lambda+X.*X);
        Exact(2,:) = 1./(X.^2 + lambda).^(1/2) - X.^2./(X.^2 + lambda).^(3/2);
    end
end