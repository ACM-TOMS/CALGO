...
void head_dimbydimTrackCodim(int minPacketSize, int maxPacketSize, trackingStats *trackCount, int codim_index, int pathMod, tracker_config_t *T, codim_t *CD, FILE *OUT, FILE *RAWOUT, FILE *FAIL, int my_id, int headnode, int num_processes);

void head_dimbydimSortCodim(int minPacketSize, int maxPacketSize, trackingStats *trackCount, int codim_index, int pathMod, tracker_config_t *T, codim_t *CD, FILE *OUT, FILE *RAWOUT, FILE *FAIL, double final_tol, int my_id, int headnode, int num_processes);

void worker_dimbydimTrackCodim(trackingStats *trackCount, int codim_index, tracker_config_t *T, codim_t *CD, FILE *OUT, FILE *RAWOUT, FILE *MIDOUT, FILE *FAIL, int my_id, int headnode, int num_processes);

int dimbydim_create_send_packet_track(int startNum, int size, FILE *START, endgame_data_t *sendPts, int *pathNum, int MPType, int pathMod, void const *ED_d, void const *ED_mp, char *jobName, int totalPaths, int sendProc);
int dimbydim_recv_store_packet_track(endgame_data_t **recvPts, int *numRecvPts, trackingStats *trackCount, tracker_config_t *T, FILE *OUT, FILE *RAWOUT, FILE *FAIL, FILE *OTHER, FILE *OTHER2, int *rV, void const *ED_d, void const *ED_mp, int (*change_prec)(void const *, int));
int dimbydim_recv_track_send_packet(int headnode, endgame_data_t **startPts, int *numStartPts, endgame_data_t **endPts, int *numEndPts, FILE *OUT, FILE *MIDOUT, tracker_config_t *T, void const *ED_d, void const *ED_mp, int (*eval_d)(point_d, point_d, vec_d, mat_d, mat_d, point_d, comp_d, void const *), int (*eval_mp)(point_mp, point_mp, vec_mp, mat_mp, mat_mp, point_mp, comp_mp, void const *), int (*change_prec)(void const *, int), int (*useSharpener)(int, int, void const *, void const *));

void worker_dimbydimSortCodim(trackingStats *trackCount, int codim_index, tracker_config_t *T, codim_t *CD, FILE *OUT, FILE *RAWOUT, FILE *FAIL, int my_id, int headnode, int num_processes);

int dimbydim_create_send_packet_sort(int startNum, int size, FILE *START, endgame_data_t *sendPts, int *pathNum, int MPType, int pathMod, void const *ED_d, void const *ED_mp, char *jobName, int totalPaths, int sendProc);
int dimbydim_recv_store_packet_sort(endgame_data_t **recvPts, int *numRecvPts, trackingStats *trackCount, tracker_config_t *T, FILE *OUT, FILE *RAWOUT, FILE *FAIL, FILE *OTHER, FILE *OTHER2, int *rV, void const *ED_d, void const *ED_mp, int (*change_prec)(void const *, int));
int dimbydim_recv_sort_send_packet(int headnode, endgame_data_t **startPts, int *numStartPts, endgame_data_t **endPts, int *numEndPts, FILE *OUT, FILE *MIDOUT, tracker_config_t *T, void const *ED_d, void const *ED_mp, int (*eval_d)(point_d, point_d, vec_d, mat_d, mat_d, point_d, comp_d, void const *), int (*eval_mp)(point_mp, point_mp, vec_mp, mat_mp, mat_mp, point_mp, comp_mp, void const *), int (*change_prec)(void const *, int), int (*useSharpener)(int, int, void const *, void const *));
...
* ARGUMENTS:                                                    *
* RETURN VALUES:                                                *
* NOTES: does dim-by-dim tracking - 'headnode'                  *
\***************************************************************/
{
  ...
  bcast_codim_t(CD, T->MPType, my_id, headnode);
  ...
  { // send the codimension data to the workers
    bcast_codimData_t(&CD->codim[codim_index], CD->curr_precision, T->MPType, my_id, headnode);
  }

  // loop over the codimensions to the find the witness supersets
  for (codim_index = 0; codim_index < num_codim; codim_index++)
  { // track the paths for this codimension
    codim = CD->codim[codim_index].codim;
    num_paths = CD->codim[codim_index].num_paths;

    head_dimbydimTrackCodim(minPacketSize, maxPacketSize, trackCount, codim_index, pathMod, T, CD, OUT, RAWOUT, FAIL, my_id, headnode, num_processes);
    ...
    if (CD->codim[codim_index].useIntrinsicSlice)
      num_crossings = parallel_midpoint_checking(midFile, str, 1, num_paths, codim, midpoint_tol, my_id, headnode, num_processes);
    else
    ...
    head_dimbydimSortCodim(minPacketSize, maxPacketSize, trackCount, codim_index, pathMod, T, CD, OUT, RAWOUT, FAIL, T->final_tol_times_mult, my_id, headnode, num_processes);
...
int setup_dimbydim_startPoint(endgame_data_t *startPt, int path_num, int MPType, void const *CD_d, void const *CD_mp, int setupSort)
/***************************************************************\
* USAGE:                                                        *
* ARGUMENTS:                                                    *
* RETURN VALUES:                                                *
...
\***************************************************************/
{
  ...
  { // setup startPt.PD_mp
    startPt->prec = CD->curr_precision;
...
\***************************************************************/
{
  ...
  create_send_packet = &dimbydim_create_send_packet_track;
  recv_store_packet = &dimbydim_recv_store_packet_track;

  // setup the current codimension
  CD->curr_codim_index = codim_index;

  // display messages
  printf("\nFinding a witness superset for codimension %d (%d of %d): %d path%s to track.\n", CD->codim[codim_index].codim, codim_index, CD->num_codim, CD->codim[codim_index].num_paths, CD->codim[codim_index].num_paths == 1 ? "" : "s");
  fprintf(OUT, "\n*****************************************************\n");
...
int store_dimbydim_sortPoint(endgame_data_t *endPt, trackingStats *trackCount, tracker_config_t *T, FILE *OUT, FILE *RAWOUT, FILE *FAIL, void const *CD_d, void const *CD_mp)
/***************************************************************\
* USAGE:                                                        *
* ARGUMENTS:                                                    *
* RETURN VALUES:                                                *
...
\***************************************************************/
{
  ...
  // setup the evaluators
  change_prec = &change_dimbydim_prec;
  create_send_packet = &dimbydim_create_send_packet_sort;
  recv_store_packet = &dimbydim_recv_store_packet_sort;
  ...
  dimbydim_classifyCodim(CD, codim_index, final_tol, T->MPType);
...
* NOTES: does dim-by-dim tracking - 'worker' process            *
\***************************************************************/
{
  ...
  CD.codim = (codimData_t *)bmalloc(num_codim * sizeof(codimData_t));
  for (codim_index = 0; codim_index < num_codim; codim_index++)
  ...
  { // track this codimension
    worker_dimbydimTrackCodim(&trackCount, codim_index, &T, &CD, OUT, RAWOUT, MIDOUT, FAIL, my_id, headnode, num_processes);
    ...
    // sort the endpoints for this codimension
    worker_dimbydimSortCodim(&trackCount, codim_index, &T, &CD, OUT, RAWOUT, FAIL, my_id, headnode, num_processes);
  }
...
\***************************************************************/
{
  ...
  change_prec = &change_dimbydim_prec;
  recv_track_send_packet = &dimbydim_recv_track_send_packet;
...
\***************************************************************/
{
  ...
  ptr_to_eval_mp = &standard_dimbydim_eval_mp;
  change_prec = &change_dimbydim_prec;
  recv_track_send_packet = &dimbydim_recv_sort_send_packet;
  useSharpener = &worker_useSharpener;
...
\***************************************************************/ 
{
  ...
  { // print the path number if needed
    ...
    setup_dimbydim_startPoint(&sendPts[i], pathNum[startNum + i], MPType, CD, CD, 0);
  }
...
\***************************************************************/
{
  ...
  { // print the path number
    ...
    { // store the endpoint using _d
      store_dimbydim_endPoint(&(*recvPts)[i], corank[i], sm[i], lg[i], trackCount, T, OUT, RAWOUT, FAIL, CD, CD);
    }
    else if (T->MPType == 1)
...
\***************************************************************/
{
  ...
  double *sm = NULL, *lg = NULL;
  int (*find_dehom)(point_d, point_mp, int *, point_d, point_mp, int, void const *, void const *) = dimbydim_dehom;
...
\***************************************************************/
{
  ...
  { // print the path number if needed
    ...
    setup_dimbydim_startPoint(&sendPts[i], pathNum[startNum + i], MPType, CD, CD, 1);
  }
...
\***************************************************************/
{
  ...
  // store the data
  for (i = 0; i < *numRecvPts; i++)
    store_dimbydim_sortPoint(&(*recvPts)[i], trackCount, T, OUT, RAWOUT, FAIL, ED_d, ED_mp);
...
\***************************************************************/
{
  ...
  { // loop over each point
    ...
    { // initialize rankDef & endPts
      ...
      { // classify
        dimbydimSortEndpoint(&rankDef, &finite, &soln, (*endPts)[i].condition_number, CD, CD->curr_codim_index, (*endPts)[i].pathNum, T, OUT, &(*endPts)[i].PD_d, &(*endPts)[i].PD_mp, (*endPts)[i].prec, (*endPts)[i].last_approx_d, (*endPts)[i].last_approx_mp, (*endPts)[i].last_approx_prec, change_dimbydim_prec);
        ...
        { // determine which category it belongs
          ...
          { // classify as non-solution
            (*endPts)[i].retVal = retVal_Bertini_Junk;
          }
          else if (rankDef == 0)
          { // classify as non-singular solution
            (*endPts)[i].retVal = NON_SINGULAR;
          }
          else
          { // classify as singular solution
            (*endPts)[i].retVal = SINGULAR;
          }
        }
      }
      else
