C    THESE ARE SUBROUTINES FOR CALCULATING THE REAL FAST FOURIER
C    FORWARD TRANSFORMATION.  THESE ROUTINE ARE TAKEN FROM THE
C    PACK FFTPACK WRITTEN BY PAUL SWARZTRAUBER OF THE NATIONAL
C    CENTER FOR ATMOSPHERIC RESEARCH.

      SUBROUTINE DRFFTI (N,WSAVE,IFAC)
      DOUBLE PRECISION  WSAVE(*)
      INTEGER  IFAC(*), N
      IF (N .EQ. 1) RETURN
      CALL DRFFTI1 (N,WSAVE(N+1),IFAC)
      RETURN
      END
      ...
      INTEGER K1, I1, II
      DOUBLE PRECISION WA(*)      
      INTEGER IFAC(*),N, NTRYH(4)
C     LOCAL VARIABLES
      DOUBLE PRECISION ARG,ARGH,ARGLD,FI,PI
      INTEGER I,IB,IDO,IP,IPM,IS,J,L1,L2,LD,NF,NFM1,NL,NQ,NR,NTRY
      INTRINSIC ATAN, COS
...
C     DATA 'PI'
  ...
  110 CONTINUE
      ...
      SUBROUTINE DRFFTF (N,R,WSAVE,IFAC)
      DOUBLE PRECISION   R(*),WSAVE(*)
      INTEGER IFAC(*),N
      IF (N .EQ. 1) RETURN
      CALL DRFFTF1 (N,R,WSAVE,WSAVE(N+1),IFAC)
      RETURN
      END
      ...
      INTEGER IFAC(*),N,K1
C     LOCAL VARIABLES
      INTEGER I,IX2,IX3,IX4,IDO,IDL1,IP,IW,KH,L1,L2,NA,NF
      NF = IFAC(2)
      NA = 1
      L2 = N
      IW = N
...
C     LOCAL VARIABLES
      DOUBLE PRECISION CR2,DR2,DI2,DR3,DI3,TAUR,TAUI,TI2,TI3,TR2,TR3
      INTEGER IDP2,IC,K,I, CI2
      INTRINSIC SQRT 
C     DATA 'TAUR, TAUI'
      TAUR = -.5D0
      TAUI = SQRT(3.D0)/2.D0
...
C     LOCAL VARIABLES
      DOUBLE PRECISION HSQT2,CI2,CI3,CI4,CR2,CR3,CR4,TI1,TI2,TI3,TI4,
     +                 TR1,TR2,TR3,TR4
      INTEGER IC,IDP2,K,I
      INTRINSIC SQRT 
...
C     LOCAL VARIABLES 
     ...
     +  DI3,DI4,DI5,DR2,DR3,DR4,DR5,PI,TI11,TI12,TI2,TI3, 
     +  TI4, TI5, TR11,TR12,TR2,TR3,TR4,TR5
      INTEGER IC,IDP2,K,I

      
C
C     DATA FOR 'PI, TR11, TI11, TR12, TI12'
      PI = 4.D0*ATAN(1.0 D0)
      TR11 = COS( 2.D0*PI/5.D0)
      TI11 = COS(PI/10.D0)
      TR12 = -COS(PI/5.D0)
      TI12 = COS(3.D0*PI/10.D0)
C
      DO 101 K=1,L1
...
C     LOCAL VARIABLES
     ...
     +                 DS2,DSP,PI
      INTEGER I,IDIJ,IDP2,IK,IPPH,IPP2,IS,J,J2,JC,LC,NBD,K,
     +        L, IC
C     DATA 'PI'
      PI=4.0D0*ATAN(1.0D0) 
      ARG = 2.0D0*PI/FLOAT(IP)
      DCP = COS(ARG)
      DSP = SIN(ARG)
      IPPH = (IP+1)/2
      IPP2 = IP+2
