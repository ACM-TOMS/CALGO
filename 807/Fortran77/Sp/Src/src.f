...
* **********************************************************************
*
        SUBROUTINE SSYRDB( UPLO, JOB, N, B, A, LDA, DRPTOL,
     >                     U, LDU, NB, TAU,
     >                     WORK, LWORK, INFO )
*
* ----------------------------------------------------------------------
...
*   triangle explicitly stored, ssyrdb reduces A to a symmetric banded
...
*           b = 0     , if n <= 1.
*
*   a       (in/out) real array, dimension ( lda, n )
...
*
*   u       (in/out) real array, dimension ( ldu, n )
...
*
*   tau     (out) real array, dimension ( n )
...
*     workspace in the routines sgeqrl, sgewyg, sgewy, and ssywy.
*
* Routines called:
*
        LOGICAL               LSAME
...
*
        EXTERNAL              SGEQRL, SGEWY, SGEWYG, SSYWY
*
*   sgeqrl  QR or QL factorization (SBR)
*   sgewy   apply WY transform from one side (SBR)
*   sgewyg  generate WY factors (SBRtoolbox)
*   ssywy   apply two-sided WY transform to a symmetric matrix (SBR)
*
        INTRINSIC             MAX, MIN
*
* ----------------------------------------------------------------------
...
*
        IF ( NB .GT. 0 ) THEN
          NB0 = MIN( NB, B, NB0 )
        ELSE
          NB0 = MIN( NBDFLT( 'SYRDB', 'SinglePrec', 'BlockingFactor',
     >                       N, N-1, B, NEEDU ),
     >               B, NB0 )
        ENDIF
*
...
*
            ...
            IWORK = IY + NB1 * LEN
*
            CALL SGEQRL( 'L', LEN, NB1, A( 1, J ), LDA, DRPTOL,
     >                   TAU( J ), WORK( IWORK ) )
            CALL SGEWYG( 'Upper', LEN, NB1, A( 1, J ), LDA, TAU( J ),
     >                   WORK( IW ), LEN, WORK( IY ), LEN, NB2,
     >                   WORK( IWORK ) )
            CALL SGEWY( 'Left', LEN, B - NB1, NB2, A( 1, LEN+1 ), LDA,
     >                  WORK( IW ), LEN, WORK( IY ), LEN,
     >                  WORK( IWORK ) )
            CALL SSYWY( 'Upper', LEN, NB2, A( 1, 1 ), LDA,
     >                  WORK( IW ), LEN, WORK( IY ), LEN,
     >                  WORK( IWORK ) )
*
            IF ( NEEDU ) THEN
              CALL SGEWY( 'Right', N, LEN, NB2, U( 1, 1 ), LDU,
     >                    WORK( IW ), LEN, WORK( IY ), LEN,
     >                    WORK( IWORK ) )
            ENDIF
*
...
*
            ...
            IWORK = IY + NB1 * LEN
*
            CALL SGEQRL( 'R', LEN, NB1, A( J+B, J ), LDA, DRPTOL,
     >                   TAU( J ), WORK( IWORK ) )
            CALL SGEWYG( 'Lower', LEN, NB1, A( J+B, J ), LDA, TAU( J ),
     >                   WORK( IW ), LEN, WORK( IY ), LEN, NB2,
     >                   WORK( IWORK ) )
            CALL SGEWY( 'Left', LEN, B - NB1, NB2, A( J+B, J+NB1 ), LDA,
     >                  WORK( IW ), LEN, WORK( IY ), LEN,
     >                  WORK( IWORK ) )
            CALL SSYWY( 'Lower', LEN, NB2, A( J+B, J+B ), LDA,
     >                  WORK( IW ), LEN, WORK( IY ), LEN,
     >                  WORK( IWORK ) )
*
            IF ( NEEDU ) THEN
              CALL SGEWY( 'Right', N, LEN, NB2, U( 1, J+B ), LDU,
     >                    WORK( IW ), LEN, WORK( IY ), LEN,
     >                    WORK( IWORK ) )
            ENDIF
     >
...
* **********************************************************************
*
        SUBROUTINE SSYGTR( UPLO, N, B, A, LDA, TAU, WORK, LWORK, INFO )
*
* ----------------------------------------------------------------------
*
* Description:
*
*   ssygtr generates a real orthogonal matrix U of size n, defined as
...
*           Householder transformations, as returned from ssyrdb.
*           On exit, a is overwritten with the orthogonal matrix U.
*
*   lda     (in) integer
*           The leading dimension of the array a.
...
*   slaset  initialize matrix (LAPACK)
*   sorgql  build orthogonal matrix after QL factorization (LAPACK)
...
*
        IF ( B1 .GE. N ) THEN
          CALL SLASET( 'All', N, N, ZERO, ONE, A, LDA )
          GOTO 999
        ENDIF
*
* ......................................................................
...
*
          DO 100 J = B+2, N
            CALL SCOPY( J-B1, A( 1, J ), 1, A( 1, J-B ), 1 )
  100     CONTINUE
*
*            --- set the last b columns and rows to identity ---
*
          CALL SLASET( 'All', B, N-B, ZERO, ZERO, A( N-B+1, 1 ), LDA )
          CALL SLASET( 'All', N-B, B, ZERO, ZERO, A( 1, N-B+1 ), LDA )
          CALL SLASET( 'All', B, B, ZERO, ONE, A( N-B+1, N-B+1 ), LDA )
*
*            --- generate the leading ( n-b )-by-( n-b )
*                orthogonal matrix                       ---
*
          CALL SORGQL( N-B, N-B, N-B, A, LDA, TAU( B+1 ),
     >                 WORK, LWORK, INFO )
*
        ELSE
*
...
*
          DO 200 J = N-1, B1, -1
            CALL SCOPY( N-J, A( J+1, J-B ), 1, A( J+1, J ), 1 )
  200     CONTINUE
*
*            --- set the first b columns and rows to identity ---
*
          CALL SLASET( 'All', N, B, ZERO, ONE, A, LDA )
          CALL SLASET( 'All', B, N-B, ZERO, ZERO, A( 1, B1 ), LDA )
*
*            --- generate the trailing ( n-b )-by-( n-b )
*                orthogonal matrix                        ---
*
          CALL SORGQR( N-B, N-B, N-B, A( B1, B1 ), LDA, TAU,
     >                 WORK, LWORK, INFO )
*
        ENDIF
*
...
* **********************************************************************
*
        SUBROUTINE SSBRDB( JOB, N, B1, B2, A, LDA, DRPTOL,
     >                     U, LDU, NB,
     >                     WORK, LWORK, INFO )
*
* ----------------------------------------------------------------------
...
*   diagonals, ssbrdb reduces A to a narrower band with b2 sub(super)
...
*           in the routines sqrupd, ssywy, and sgewy
*
* The workspace is used as follows:
*
...
*     sqrupd (which needs ( n+1 )*nb0 elements if U is required and
*     ( ldh+1 )*nb0 elements otherwise), ssywy (( b1+1 )*nb0 elements),
*     and sgewy (b1*nb0 elements).
*
* Routines called:
*
        LOGICAL               LSAME
...
*
        EXTERNAL              SGEWY, SQRUPD, SSYWY
*
*   sgewy   apply WY block transform (SBR)
*   sqrupd  QR decomposition of a block of A, update of U, and premulti-
*           plication of another block of A (SBR)
*   ssywy   apply two-sided block transform to a symmetric matrix (SBR)
*
        INTRINSIC             MAX, MIN
*
* ----------------------------------------------------------------------
...
*                columns of the band                           ---
*
          CALL SQRUPD( MIN( LDH, N-K-B2+1 ), NB0, DELTAB,
...
*
              CALL SSYWY( 'Lower', MIN( LDH, N-L+1 ), NB1,
     >                    A( 1, L ), LDA - 1,
...
*
              IF ( N .GE. ( L+LDH ) ) THEN
                CALL SGEWY( 'Right', MIN( B1, N-L-LDH+1 ),
...
*
              CALL SQRUPD( MIN( LDH, N-L-B1+1 ), NB0, DELTAB,
...
* **********************************************************************
*
        SUBROUTINE SSBRDT( JOB, N, B, A, LDA, DRPTOL,
     >                     D, E,
     >                     U, LDU, NB,
     >                     WORK, LWORK, INFO )
*
...
*   ssbrdt reduces A to tridiagonal form using a sequence of orthogonal
...
*           space by the routines slarfx, ssyrf, and sgewy.
...
*     the routines ssyrf (b elements), slarfx (b elements), and sgewyr
*     (n * nb0 elements), and
...
*   nbdflt  determine default blocking factor
*
        EXTERNAL              SCOPY, SGBWYG, SGEWY, SLARFX, SLBRFG,
     >                        SSYRF
*
*   scopy   vector copy (BLAS)
*   sgbwyg  generate WY block reflector (SBR)
*   sgewy   apply WY block reflector (SBR)
*   slbrfg  determine Householder vector (SBR)
*   ssyrf   apply two-sided transform to a symmetric matrix (SBR)
*   slarfx  apply Householder transform (LAPACK)
*
...
*
          IF ( NB .GT. 0 ) THEN
            NB0 = MIN( NB, NB0, N-2 )
          ELSE
            NB0 = MIN( NBDFLT( 'SBRDT', 'SinglePrec', 'BlockingFactor',
     >                         N, B, 1, .TRUE. ),
     >                 NB0 )
          ENDIF
*
...
*
          HEIGHT = MIN( B, N-K )
          CALL SLBRFG( HEIGHT, A( 2, K ), A( 3, K ), 1, TAU, DRPTOL )
*
...
*
          ...
          ENDIF
          WORK( V ) = ONE
          CALL SCOPY( HEIGHT-1, A( 3, K ), 1, WORK( V+1 ), 1 )
          DO 110 I = 3, HEIGHT+1
            A( I, K ) = ZERO
  110     CONTINUE
*
...
*
            CALL SSYRF( 'Lower', HEIGHT, WORK( V ), 1, TAU,
...
*
            IF ( ONFLY ) THEN
              CALL SLARFX( 'Right', N, HEIGHT, WORK( V ), TAU,
...
*
              CALL SLARFX( 'Right', HEIGHT, WIDTH, WORK( V ), TAU,
...
*
            CALL SLBRFG( HEIGHT, A( B+1, J ), A( B+2, J ), 1,
     >                   TAU, DRPTOL )
*
            IF ( BLOCK ) THEN
            ...
            ENDIF
            WORK( V ) = ONE
            CALL SCOPY( HEIGHT-1, A( B+2, J ), 1, WORK( V+1 ), 1 )
...
*                    (from the left)                              ---
*
              CALL SLARFX( 'Left', HEIGHT, WIDTH-1, WORK( V ), TAU,
...
*
                V = IFIRST - K0 - 1 + IV
                CALL SCOPY( NB1, WORK( V ), N-K0+1, WORK( ITAU ), 1 )
                CALL SGBWYG( LDWY, NB1, B-1,
     >                       WORK( V ), N-K0, WORK( ITAU ),
     >                       WORK( IW ), LDWY, WORK( IY ), LDWY, NB2 )
*
*                  --- update U by postmultiplying ---
*
                CALL SGEWY( 'Right', N, LDWY, NB2,
     >                      U( 1, IFIRST ), LDU,
     >                      WORK( IW ), LDWY, WORK( IY ), LDWY,
     >                      WORK( IWORK ) )
*
...
* **********************************************************************
*
        SUBROUTINE SSY2BC( UPLO, N, B, AFULL, LDFULL, ABAND, LDBAND,
     >                     INFO )
*
* ----------------------------------------------------------------------
*
* Description:
*
*   ssy2bc copies a symmetric banded A matrix from (upper or lower)
...
*
        ...
        REAL                  AFULL( LDFULL, * ), ABAND( LDBAND, * )
*
*   uplo    (in) character*1
...
*           b = 0     , if n = 0.
*
*   afull   (in) real array, dimension ( ldfull, n )
...
*           ldfull >= max( n, 1 ).
*
*   aband   (out) real array, dimension ( ldband, n )
...
* **********************************************************************
*
        SUBROUTINE SSY2BI( UPLO, N, B, A, LDFULL, LDBAND, INFO )
*
* ----------------------------------------------------------------------
*
* Description:
*
*   ssy2bi copies a symmetric banded A matrix from (upper or lower)
...
*
        EXTERNAL              SSB2BI
*
*   ssb2bi  in-place repacking from banded to banded storage (SBR)
*
        INTRINSIC             MAX, MIN
*
* ----------------------------------------------------------------------
...
*
          IF ( LDBAND .GT. B1 ) THEN
            CALL SSB2BI( 'Lower', N, B, A, B1, LDBAND, INFO )
          ENDIF
*
        ELSE
*
...
* **********************************************************************
*
        SUBROUTINE SSB2BC( UPLO, N, B, ASRC, LDSRC, ADST, LDDST, INFO )
*
* ----------------------------------------------------------------------
*
* Description:
*
*   ssb2bc copies a symmetric banded A matrix from upper or lower
...
*           b = 0     , if n = 0.
*
*   asrc    (in) real array, dimension ( ldsrc, n )
...
*           ldsrc >= b + 1.
*
*   adst    (out) real array, dimension ( lddst, n )
...
* **********************************************************************
*
        SUBROUTINE SSB2BI( UPLO, N, B, A, LDSRC, LDDST, INFO )
*
* ----------------------------------------------------------------------
*
* Description:
*
*   ssb2bi copies a symmetric banded A matrix from upper or lower banded
...
* **********************************************************************
*
        SUBROUTINE SSBRDD( JOBU, UPLO, N, B1, B2, A, LDA, DRPTOL, D, E,
     >                     U, LDU, NSTEPS, B, NB,
...
*   ssbrdd reduces A to a symmetric banded matrix B with b2 sub(or
...
*
        EXTERNAL              SSB2BI, SSBRDB, SSBRDT, SSBTRD
*
*   ssb2bi  in-place repacking of banded matrices (SBR)
*   ssbrdb  one-step reduction banded -> banded (SBR)
*   ssbrdt  one-step reduction banded -> tridiagonal (SBR)
*   ssbtrd  one-step reduction banded -> tridiagonal (LAPACK)
*
        INTRINSIC             MAX, MIN
*
* ----------------------------------------------------------------------
...
*
            BNEXT = NBDFLT( 'SBRDD', 'SinglePrec', 'Bandwidth',
     >                      N, BCURR, B2, NEEDU )
*
...
*
            NX = NBDFLT( 'SBRDD', 'SinglePrec', 'CrossoverPoint',
     >                   N, BCURR, B2, NEEDU )
            IF ( BCURR .LT. NX ) THEN
*
...
*
              ...
              IF ( LDCURR .NE. LDNEXT ) THEN
                CALL SSB2BI( ULCURR, N, BCURR, A, LDCURR, LDNEXT, INFO )
                LDCURR = LDNEXT
                ULCURR = 'Lower'
              ENDIF
*
              IF ( LWORK .LT. N ) THEN
                INFO = -17
              ELSE
                CALL SSBTRD( JOBU, ULCURR, N, BCURR, A, LDCURR, D, E,
     >                       U, LDU, WORK, INFO )
              ENDIF
            ELSE
*
...
*
     ...
     >             LSAME( ULCURR, 'Upper' ) ) THEN
              ...
              ENDIF
*
              CALL SSBRDT( JOBU, N, BCURR, A, LDCURR, DRPTOL, D, E,
...
*
            IF ( NBCURR .EQ. 0 ) THEN
              NX = NBDFLT( 'SBRDB', 'SinglePrec', 'BlockingFactor',
     >                     N, BCURR, BNEXT, NEEDU )
            ELSE
              NX = NBCURR
            ENDIF
     ...
     >           LSAME( ULCURR, 'Upper' ) ) THEN
            ...
            ENDIF
*
            CALL SSBRDB( JOBU, N, BCURR, BNEXT, A, LDCURR, DRPTOL,
     >                   U, LDU, NBCURR, WORK, LWORK, INFO )
...
*
     ...
     >  THEN
          CALL SSB2BI( ULCURR, N, BCURR, A, LDCURR, LDA, INFO )
        ENDIF
*
        IF ( INFO .GE. 0 )     INFO = 1
*
...
* **********************************************************************
*
        SUBROUTINE SSYRDD( JOBU, UPLO, N, B2, A, LDA, DRPTOL,
...
*   banded form.
*   Given a symmetric n-by-n matrix A, ssyrdd reduces A to a symmetric
...
*
        EXTERNAL              SORGTR, SSBRDD, SSY2BC, SSYGTR, SSYRDB,
     >                        SSYTRD
*
*   sorgtr  accumulation of the transformations (LAPACK)
*   ssbrdd  bandwidth reduction for banded matrices (SBR)
*   ssy2bc  repacking from full to banded storage (SBR)
*   ssygtr  accumulation of the transformations (SBR)
*   ssyrdb  reduction full -> banded (SBR)
*   ssytrd  one-step reduction full -> tridiagonal (LAPACK)
*
        INTRINSIC             MAX
*
* ----------------------------------------------------------------------
...
*            --- automatic bandwidth selection ---
*
          BNEXT = NBDFLT( 'SYRDD', 'SinglePrec', 'Bandwidth',
     >                    N, N - 1, B2, NEEDU )
*
*              --- if the proposed intermediate bandwidth is out
*                  of range then do a one-step reduction         ---
...
*
          CALL SSYTRD( UPLO, N, A, LDA, D, E, WORK( 1 ),
     >                 WORK( N ), LWORK-N+1, INFO )
          IF ( NEEDU ) THEN
            CALL SORGTR( UPLO, N, A, LDA, WORK( 1 ),
     >                   WORK( N ), LWORK-N+1, INFO )
          ENDIF
        ELSE
*
*            --- reduction to bandwidth > 1 width ssyrdb ---
*
          CALL SSYRDB( UPLO, 'NoU', N, BNEXT, A, LDA, DRPTOL,
     >                 WORK( 1 ), N, NBCURR, WORK( 1 ),
     >                 WORK( N+1 ), LWORK-N, INFO )
*
*            --- repack the reduced matrix to banded storage ---
*
          CALL SSY2BC( UPLO, N, BNEXT, A, LDA, ABAND, LDBAND, INFO )
*
*            --- accumulation of the transformations, if required ---
*
          IF ( NEEDU ) THEN
            CALL SSYGTR( UPLO, N, BNEXT, A, LDA, WORK( 1 ),
     >                   WORK( N+1 ), LWORK-N, INFO )
          ENDIF
*
...
*
          IF ( ( BNEXT .GT. B2 ) .AND. ( INFO .GE. 0 ) ) THEN
            IF ( NSTEPS .EQ. 0 ) THEN
              CALL SSBRDD( JOBU, 'Lower', N, BNEXT, B2, ABAND, LDBAND,
...
* **********************************************************************
*
        SUBROUTINE SLBRFG( N, ALPHA, X, INCX, TAU, DRPTOL )
*
* ----------------------------------------------------------------------
*
* Description:
*
*   slbrfg generates an elementary reflector H of order n, such that
*
*                                    T
*      H * ( alpha ) = ( beta ) ,   H  * H = I ,
*          (   x   )   (   0  )
...
*   Otherwise  1 <= tau <= 2.
*
*   This routine is a modified version of slbrfg by Bischof/Sun which
*   in turn is a modified version of the LAPACK routine slarfg.
*
* Author: Bruno Lang
*         Aachen University of Technology
*         na.blang@na-net.ornl.gov
...
*
*   x       (in/out) real array, dimension ( 1+(n-2)*incx )
...
*
        ...
        EXTERNAL              SLAMCH, SLAPY2, SNRM2, SSCAL
*
*   slamch        determine machine parameters (LAPACK)
*   slapy2        stable 2-norm of 2-vector (LAPACK)
*   snrm2         2-norm of a vector (BLAS)
*   sscal         vector scaling (BLAS)
*
        INTRINSIC             SIGN, ABS
*
* ----------------------------------------------------------------------
...
* **********************************************************************
*
        SUBROUTINE SSYRF( UPLO, N, V, INCV, TAU,
     >                    A, LDA, WORK )
*
* ----------------------------------------------------------------------
*
...
*           n >= 0.
*
*  v        (in) real array, dimension ( 1+(n-1)*incv )
...
*
        REAL                  SDOT
        EXTERNAL              SAXPY, SDOT, SSYMV, SSYR2
*
*   saxpy   add multiple of a vector to another vector (BLAS)
*   sdot    dot product of two vectors (BLAS)
*   ssymv   symmetric matrix-vector product (BLAS)
*   ssyr2   symmetric rank-2 update (BLAS)
*
* ----------------------------------------------------------------------
*
...
*              --- work( 1:n ) := x = tau * A * v ---
*
          CALL SSYMV( UPLO, N, TAU, A, LDA, V, INCV, ZERO, WORK, 1 )
*
*                                                        T
*              --- work( 1:n ) := w = x - 1/2 * tau * ( x  * v ) * v ---
*
          ALPHA = -0.5E0 * TAU * SDOT( N, WORK, 1, V, INCV )
          CALL SAXPY( N, ALPHA, V, INCV, WORK, 1 )
*
*                                T        T
*              --- A := A - v * w  - w * v  ---
*
          CALL SSYR2( UPLO, N, -ONE, V, INCV, WORK, 1, A, LDA )
*
        ENDIF
*
        RETURN
...
* **********************************************************************
*
        SUBROUTINE SGBQR2( M, N, D, A, LDA, TAU, DRPTOL, WORK )
*
* ----------------------------------------------------------------------
*
* Description:
*
*   Given an m-by-n matrix A with d subdiagonals, sgeqr2 computes
...
*           lda >= min( m, 1 ).
*
*   tau     (out) real array, dimension ( min( m-1, n ) )
...
* Routines called:
*
        EXTERNAL              SLARFX, SLBRFG
*
*   slarfx  apply Householder transformation (LAPACK)
*   slbrfg  generate Householder vector (SBR)
*
        INTRINSIC             MIN
*
* ----------------------------------------------------------------------
...
*                A( j+1:j+d, j )                               ---
*
          CALL SLBRFG( LEN, A( J, J ), A( J+1, J ), 1,
     >                 TAU( J ), DRPTOL )
*
*            --- apply the transform ---
*
...
*
            AJJ = A( J, J )
            A( J, J ) = ONE
            CALL SLARFX( 'Left', LEN, N-J, A( J, J ), TAU( J ),
     >                  A( J, J+1 ), LDA, WORK )
            A( J, J ) = AJJ
*
          ENDIF
...
* **********************************************************************
*
        SUBROUTINE SGEQRL( DECOMP, M, N, A, LDA, DRPTOL,
     >                     TAU, WORK )
*
* ----------------------------------------------------------------------
*
...
*
              I = M - N + J
              CALL SLBRFG( I, A( I, J ), A( 1, J ), 1, TAU( J ),
     >                     DRPTOL )
*
...
*
              TMP = A( I, J )
              A( I, J ) = ONE
              CALL SLARFX( 'Left', I, J-1, A( 1, J ), TAU( J ),
     >                     A, LDA, WORK )
              A( I, J ) = TMP
*
  100       CONTINUE
...
*
              TMP = A( J, J )
              A( J, J ) = ONE
              CALL SLARFX( 'Left', M-J+1, N-J, A( J, J ), TAU( J ),
     >                     A( J, J+1 ), LDA, WORK )
              A( J, J ) = TMP
*
  200       CONTINUE
...
* **********************************************************************
*
        SUBROUTINE SGBWYG( M, N, D, A, LDA, TAU,
     >                     W, LDW, Y, LDY, K )
*
* ----------------------------------------------------------------------
*
* Description:
*
*   Given a set of Householder vectors as returned by sgbqr2, this
...
*           ldw >= max( m, 1 ).
*
*   y       (out) real array, dimension ( ldy, n )
...
* Routines called:
*
        EXTERNAL              SGEMV, SSWAP
*
*   sgemv   matrix-vector product (BLAS)
*   sswap   swap two vectors (BLAS)
*
        INTRINSIC             MIN
*
* ----------------------------------------------------------------------
...
*
  ...
  110       CONTINUE
            Y( J, K ) = ONE
            CALL SSWAP( LEN, A( J+1, J ), 1, Y( J+1, K ), 1 )
*
*                --- W( :, k ) = - tau( j ) * Y( :, k ) ---
*
            DO 120 I = 1, J-1
...
*                         = ( I + W( j-1 ) * Y( k-1 )  ) * W( :, k ) ---
*
            CALL SGEMV( 'Transpose', LEN+J, K-1,
     >                  ONE, Y, LDY, W( 1, K ), 1, ZERO, TAU, 1 )
            CALL SGEMV( 'NoTranspose', LEN+J, K-1,
...
* **********************************************************************
*
        SUBROUTINE SGEWYG( UPLO, M, N, A, LDA, TAU,
     >                     W, LDW, Y, LDY, K,
     >                     WORK )
*
* ----------------------------------------------------------------------
...
* Description:
*
*   Given a set of Householder vectors as returned by sgeqrl, this
*   routine returns W and Y such that
*
*                       T
*      Q  =  I  +  W * Y
...
*           0 <= n <= m.
*
*   a       (in) real array, dimension ( lda, n )
...
*
  ...
  110           CONTINUE
                Y( J, K ) = ONE
                CALL SCOPY( M-J, A( J+1, J ), 1, Y( J+1, K ), 1 )
*
*                  --- W( :, k ) := - tau( j ) * vj ---
*
                DO 120 I = 1, J-1
...
*
                IF ( K .GT. 1 ) THEN
                  CALL SGEMV( 'Transpose', M, K-1,
     >                        ONE, Y, LDY, W( 1, K ), 1, ZERO, WORK, 1 )
                  CALL SGEMV( 'NoTranspose', M, K-1,
...
*
  ...
  310           CONTINUE
                Y( M-N+J, K ) = ONE
                CALL SCOPY( M-N+J-1, A( 1, J ), 1, Y( 1, K ), 1 )
*
*                    --- W( :, k ) := - tau( j ) * vj ---
*
                DO 320 I = M-N+J+1, M
...
* **********************************************************************
*
        SUBROUTINE SGEWY( SIDE, M, N, K, A, LDA,
     >                    W, LDW, Y, LDY,
     >                    WORK )
*
* ----------------------------------------------------------------------
...
*           lda >= max( m, 1 ).
*
*   w       (in) real array, dimension ( ldw, k )
*           The matrix W.
*
*   ldw     (in) integer
*           The leading dimension of the array w.
...
*
*   y       (in) real array, dimension ( ldy, k )
*           The matrix Y.
*
*   ldy     (in) integer
*           The leading dimension of the array y.
...
*                --- work = W  * A ---
*
            CALL SGEMM( 'Transpose', 'NoTranspose', K, N, M,
     >                  ONE, W, LDW, A, LDA, ZERO, WORK, K )
*
*                --- A = Y * work + A ---
*
            CALL SGEMM( 'NoTranspose', 'NoTranspose', M, N, K,
...
*                --- work = A * W ---
*
            CALL SGEMM( 'NoTranspose', 'NoTranspose', M, K, N,
     >                  ONE, A, LDA, W, LDW, ZERO, WORK, M )
*
*                                    T
*                --- A = A + work * Y  ---
*
            CALL SGEMM( 'NoTranspose', 'Transpose', M, N, K,
...
* **********************************************************************
*
        SUBROUTINE SQRUPD( M, N, B, A, LDA, DRPTOL,
...
* Description:
*
*   sqrupd forms the QR factorization
*
*      A  =  Q * R
*
...
*           m1 >= 0.
*
*   a1      (in/out) real array, dimension ( lda1, m )
...
*           0 <= n2 <= lda.
*
*   a2      (in/out) real array, dimension ( lda, n2 )
...
*           0 <= nb <= min( m-1, n ).
*
*   y       (out) real array, dimension ( ldwy, n )           T
*           The m-by-nb matrix Y in the representation Q = I + W * Y .
*
*   w       (out) real array, dimension ( ldwy, n )           T
...
*           for the routines sgbqr2 (n elements) and sgewy
*           ( max( m1, m ) * nb elements).
*
* Routines called:
*
        EXTERNAL              SGBQR2, SGBWYG, SGEWY
*
*   sgbqr2  QR factorization of a banded matrix (SBR)
*   sgbwyg  generate WY factors (SBR)
*   sgewy   apply WY transform (SBR)
*
* ----------------------------------------------------------------------
*
...
*            --- factorize A = Q * R ---
*
        CALL SGBQR2( M, N, B, A, LDA, WORK( ITAU ), DRPTOL,
     >               WORK( IWORK ) )
*
*            --- build W and Y factors ---
*
        LDWY = M
        CALL SGBWYG( M, N, B, A, LDA, WORK( ITAU ),
     >               W, LDWY, Y, LDWY, NB )
*
        IF ( NB .GT. 0 ) THEN
*
...
*
          IF ( NEEDA1 ) THEN
            CALL SGEWY( 'Right', M1, M, NB, A1, LDA1,
...
*
          IF ( N2 .GT. 0 ) THEN
            CALL SGEWY( 'Left', M, N2, NB, A2, LDA,
     >                  W, LDWY, Y, LDWY, WORK( IWORK ) )
          ENDIF
        ENDIF
*
...
* **********************************************************************
*
        SUBROUTINE SSYWY( UPLO, N, K, A, LDA, W, LDW, Y, LDY, WORK )
*
* ----------------------------------------------------------------------
*
* Description:
...
*           ldy >= max( n, 1 ).
*
*   work    (workspace) real array, dimension ( (n+k)*k )
*
* ----------------------------------------------------------------------
*
* Local variables:
...
* Routines called:
*
        EXTERNAL              SGEMM, SSYMM, SSYR2K
*
*   sgemm   matrix-matrix product (BLAS)
*   ssymm   symmetric matrix-matrix product (BLAS)
*   ssyr2k  symmetric rank-2 update (BLAS)
*
* ----------------------------------------------------------------------
*
...
*              --- work( 1 .. n*k ) := X1 = A * W ---
*
          CALL SSYMM( 'Left', UPLO, N, K,
     >                ONE, A, LDA, W, LDW, ZERO, WORK, N )
*
*                                                           T
*              --- work( n*k+1 .. n*k+k*k ) := X2 = 1/2 * X1  * W ---
*
          CALL SGEMM( 'Transpose', 'NoTranspose', K, K, N,
     >                0.5E0, WORK, N, W, LDW, ZERO, WORK( N*K+1 ), K )
*
*              --- work( 1 .. n*k ) := X3 = X1 + Y * X2 ---
*
          CALL SGEMM( 'NoTranspose', 'NoTranspose', N, K, K,
     >                ONE, Y, LDY, WORK( N*K+1 ), K, ONE, WORK, N )
*
*                                 T        T
*              --- A := A + X3 * Y + Y * X3  ---
*
          CALL SSYR2K( UPLO, 'NoTranspose', N, K,
