function [objPoly,ineqPolySys,lbd,ubd] ... 
    = randomwithEQ(nDim,minCliqueSize,maxCliqueSize,maxDegree,randSeed);

% randomwithEQ(nDim,minCliqueSize,maxCliqueSize,maxDegree,randSeed);
% 
% A randomly generated optimization problems with nonlinear 
% objective function and linear equations.
%
% <Input> 
...
%
% Remark:
% In this version, we can not control the number of linear equations. This
% number is the same as the number of cliques 
%
% [objPoly,ineqPolySys,low,up]=
% unconProb(nDim,minCliqueSize,maxCliqueSize,maxDegree,randSeed);
%
%
...
% 
% low: lower bound. Here, all lower bound is -1.0e+10
% up: upper bound. Here, all upper bound is -1.0e+10
%
...
%%
%% Set 'ineqPolySys' as linear equations
%%
ineqPolySys = [];
A = 2*rand(noOfCliques,nDim)-1;
for i=1:size(cliqueSet,1)
    subspaceIdxSet = find(cliqueSet(i,:));
    otherIdxSet = setdiff(1:nDim,subspaceIdxSet);
    A(i,otherIdxSet) = 0;
end
x = 1.0e-1* (2*rand(nDim,1)-1);
b = A*x;
...
for i=1:noOfCliques
   ...
   subspaceIdxSet = find(cliqueSet(i,:));
   ineqPolySys{i}.coef = [A(i,subspaceIdxSet),-b(i)]';
   ineqPolySys{i}.supports = sparse(1+length(subspaceIdxSet),nDim);
   for j=1:length(subspaceIdxSet)
       ineqPolySys{i}.supports(j,subspaceIdxSet(j)) = 1;
   end
   ineqPolySys{i}.noTerms = length(subspaceIdxSet)+1;
end
...
% $Header: /home/waki9/CVS_DB/SparsePOPdev/example/POPformat/randomwithEQ.m,v 1.1.1.1 2007/01/11 11:31:50 waki9 Exp $
