...
C     MODIFIED TO DEFAULT TO SMALL SUBSET OF TEST CASES
C
C     .. Parameters ..
      INTEGER bigar
      PARAMETER (bigar=100)
...
C     ..
C     .. Local Scalars ..
      DOUBLE PRECISION a,b,d2ldw2,dfd,dfn,dldw,dum,l,tau,w,worig
      INTEGER i,ia,ib,inun,iw,nab,nw,outun,stdout
      LOGICAL qab,qfile,qscrn,qw
C     ..
C     .. Local Arrays ..
      DOUBLE PRECISION aarray(bigar),barray(bigar),warray(bigar)
...
C     .. External Functions ..
      ...
      INTEGER obfugt
      LOGICAL qyngt
      EXTERNAL rlgt,obfugt,qyngt
C     ..
C     .. External Subroutines ..
      EXTERNAL gtstio,lldrlf,mvlogf,pause
C     ..
...
C     .. Data statements ..
     ...
     +     'log(F(exp(W)|DFN,DFD))    ','log(1 - F(exp(W)|DFN,DFD))'/
      DATA (warray(i),i=1,15)/-1000.0D0,-500.0D0,-100.0D0,-50.0D0,
     +     -10.0D0,-5.0D0,-1.0D0,0.0D0,1.0D0,5.0D0,10.0D0,50.0D0,
     +     100.0D0,500.0D0,1000.0D0/
      DATA (aarray(i),i=1,4)/-3.0D0,1.0D0,5.0D0,10.0D0/
C     ..
...
C
   20 qab = (qyngt('('' Do you want to specify A and B? (y/n)'')'))
C
C     Ask for A and B
C
      IF (qab) THEN
...
C
      IF (qyngt('('' Do you want to continue? (y/n)'')')) GO TO 20
C
