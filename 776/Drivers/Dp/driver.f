...
*  In addition, see the comments of DCHECK for the limit of the test 
*  problem sizes for the testing subroutine DLAQR3. 
*
*  ---------------- 
*
...
*     .. Local Scalars ..
      ...
      DOUBLE PRECISION   QNORM, TOL
*     ..
*     .. Local Arrays ..
      INTEGER            ITRSD( MAXSPC ), IWORK( LWORK ), ISEED( 4 ), 
     $                   NMAX( MAXSPC ) 
      DOUBLE PRECISION   AQ( LDA, MAXSPC ), DUMMY( 1 ),
...
*
      IF( LSAME( PTYPE, 'RWK' ) ) THEN
         INFO = 1 
         ISTART = -1
         CALL DSRRIT( N, NV, M, MAXIT, ISTART, Q, LDQ, AQ, LDA, T, LDT,
...
* 
         ...
         DO 30 J = 1, M
            DO 20 I = 1, N 
               Q( I,J ) = SIGN( ONE, DLARAN( ISEED ) - HALF )  
   20       CONTINUE
   30    CONTINUE
*
         CALL DSRRIT( N, NV, M, MAXIT, ISTART, Q, LDQ, AQ, LDA, T, LDT,
...
*
 ...
 1113    FORMAT( ' DORTH subroutine fails, INFO = ', I2 )
      ELSE IF( INFO.EQ.2 )THEN
         WRITE( NOUT, FMT = 1114 )INFO 
 1114    FORMAT( ' DSRRSP subroutine fails, INFO = ', I2 )
      ELSE IF( INFO.EQ.3 )THEN
         WRITE( NOUT, FMT = 1115 )INFO 
 1115    FORMAT( ' Reduced matrix T is singular, INFO = ', I2 )
      ELSE IF( INFO.EQ.4 )THEN
         WRITE( NOUT, FMT = 1116 )INFO 
 1116    FORMAT( ' DSRRIT fails to converge after MAXIT, INF0 = ', I2) 
         WRITE( NOUT, FMT = 1118 )
...
*
         CALL DGEMM( 'No transpose', 'No transpose', N, NV, NV, -ONE, 
     $               Q, LDQ, T, LDT, ONE, AQ, LDA )
*
         DO 70 I = 1, NV
            WORK( I ) = DLANGE( 'MAX', N, 1, AQ( 1, I ), 1, DUMMY )
   70    CONTINUE
*
         WRITE( NOUT, FMT = 9990 )
         WRITE( NOUT, FMT = 9989 )( WORK( I ), I = 1, NV )
*
         CALL DLASET( 'Full', NV, NV, ZERO, ONE, WORK, NV )
         CALL DGEMM( 'Transpose', 'No transpose', NV, NV, N, -ONE, Q, 
     $               LDQ, Q, LDQ, ONE, WORK, NV )
         QNORM = DLANGE( 'Max', NV, NV, WORK, NV, DUMMY )
*
...
*
 ...
 9998 FORMAT( / ' ..... END OF DSRRIT TEST .....' )
 ...
 9991 FORMAT( ' WR = ', 1P,D20.12, ' WI = ', 1P,D20.12 )
 9990 FORMAT( ' Final residual norms ',
     $      'of (A*Q - Q*T) of converged RR pairs = ' )
 9989 FORMAT( 1P,6D10.3 )
 9985 FORMAT( 6I10 )
 9988 FORMAT( ' norm(Q*Q - I)   = ', 1P,D20.12 )
*
*     Test subroutine DLAQR3
*
   90 CONTINUE 
*
      READ( NIN, FMT = * )( NMAX( I ), I = 1, K ) 
*
      CALL DCHECK( K, NMAX, INFO )
      IF( INFO .EQ. 0 )THEN
         WRITE( NOUT, FMT = 9987 ) 
 9987    FORMAT( ' Passed all DLAQR3 tests ' ) 
      ELSE 
         WRITE( NOUT, FMT = 9986 )INFO
 9986    FORMAT( ' Test routine DCHECK INFO = ', I5 ) 
      END IF
*
      GO TO 10 
*
...
****** begin of dcheck.f ********************************************
*
      SUBROUTINE DCHECK( MAX, NMAX, INFO ) 
*
*     .. Scalar Arguments .. 
      INTEGER         MAX, INFO 
*
...
*  =======
*
*  DCHECK tests the subroutine DLAQR3. 
*  On normal return, INFO is to be zero, otherwise, if 
*
*   INFO = 1, DLAQR3 has illegal parameter in the calling sequence
...
*
      ...
      PARAMETER         ( ZERO = 0.0D+0, TOL = 2.0D+01 )       
*
*     .. Local Scalars ..
...
*     .. Local Arrays ..
      INTEGER           ISEED( 4 )
      DOUBLE PRECISION  A( LDA,LDA ), B( LDB,LDB ), U( LDU,LDU ),
     $                  H( LDH, LDH ), WR( LDA ), WI(LDA), 
     $                  WORK( LWORK ), RESULT( 2 )
*
*     .. External functions .. 
      DOUBLE PRECISION  DLAPY2, DLARAN
      EXTERNAL          DLAPY2, DLARAN
*
*     .. External Subroutines ..
      EXTERNAL          DLACPY, DGEHRD, DORGHR, DLAQR3, DGET21
*
      INFO = 0  
      ISEED( 1 ) = 8321
      ISEED( 2 ) = 9735
...
*
         CALL DLACPY( 'FULL', N, N, A, LDA, B, LDB )
*
*        Hessenberg reduction
*
         CALL DGEHRD( N, 1, N, A, LDA, WR, WORK, LWORK, IERR )
         DO 50 J = 1, N - 1
            DO 40 I = J + 2, N
               U( I, J ) = A( I, J )
               A( I, J ) = ZERO
   40       CONTINUE
   50    CONTINUE
         CALL DORGHR( N, 1, N, U, LDU, WR, WORK, LWORK, IERR )
*
         CALL DLACPY( 'FULL', N, N, A, LDA, H, LDH )
*
*        Compute the Schur decomposition of upper Hessenberg matrix
*
         CALL DLAQR3( .TRUE., .TRUE., N, 1, N, A, LDA, WR, WI, 1, N, 
     $                U, LDU, WORK, IERR )
         IF( IERR .NE. 0 )THEN
             INFO =  1
             RETURN
         END IF 
*
         CALL DGET21( N, B, LDB, A, LDA, U, LDU, WORK, RESULT )
...
*
         CALL DLAQR3( .FALSE., .FALSE., N, 1, N, H, LDH, WR, WI, 
     $                1, N, U, LDU, WORK, IERR )
*
*        Check the ordering
*
...
********* begin of dget21.f ***************************************** 
*
      SUBROUTINE DGET21( N, A, LDA, B, LDB, U, LDU, WORK, RESULT )
*     ..
*     .. Scalar Arguments ..
      INTEGER            LDA, LDB, LDU, N
*     ..
*     .. Array Arguments ..
      DOUBLE PRECISION   A( LDA, * ), B( LDB, * ), RESULT( * ),
     $                   U( LDU, * ), WORK( * )
*
*  Purpose
*  =======
...
*           Not modified.
*
*  U      - DOUBLE PRECISION array of dimension ( LDU, N ).
...
*           Not modified.
*
*  WORK   - DOUBLE PRECISION array of dimension ( 2*N**2 )
*           Workspace.
*           Modified.
*
*  RESULT - DOUBLE PRECISION array of dimension ( 2 )
...
*
     ...
     $           WORK( N**2+1 ), N )
*
      CALL DGEMM( 'N', 'C', N, N, N, -ONE, WORK( N**2+1 ), N, U, LDU,
     $           ONE, WORK, N )
*
...
*
*  AQ   (output) DOUBLE PRECISION array, dimension (LDQ, M )
...
*     .. Parameters ..
      ...
      PARAMETER          ( P1 = 1.0D+0, P2 = 1.0D+0, P3 = 1.0D+0 )
*     ..
*     .. Local Scalars ..
      INTEGER            I, II, K, NS
      DOUBLE PRECISION   BETA, GAMMA, HSTEP, SIGMA
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          DBLE, SQRT
*     ..
...
*     .. Parameters ..
      ...
      PARAMETER          ( GAMMA = 1.0D-2 )
      INTEGER            LDU
      PARAMETER          ( LDU = 1001 )
*     ..
*     .. Local Scalars ..
      INTEGER            I, INFO, J, K
      DOUBLE PRECISION   DD, DELTA, HSTEP, HSTEP2
*     ..
*     .. Local Arrays ..
      DOUBLE PRECISION   A( 2, LDU ), U( LDU, 1 )
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          DBLE
*     ..
*     .. External Subroutines ..
      EXTERNAL           DPBTF2, DPBTRS
*     ..
*     .. Executable Statements ..
*
      HSTEP = ONE / DBLE( N )
...
*
      CALL DPBTF2( 'Upper', N-1, 1, A, 2, INFO )
*
      DO 30 J = 1, N - 2
         U( J, 1 ) = ZERO
   30 CONTINUE
      U( N-1, 1 ) = ONE
*
      CALL DPBTRS( 'Upper', N-1, 1, 1, A, 2, U, 301, INFO )
*
...
*
   ...
   40    CONTINUE
         AQ( N, K ) = ZERO
*
         CALL DPBTRS( 'Upper', N-1, 1, 1, A, 2, AQ( 1, K ), LDA, INFO )
*
...
*     .. Parameters ..
      ...
      DOUBLE PRECISION   ALPHA, BETA, DELT1, DELT2, LL
      PARAMETER          ( ALPHA = 2.0D+0, BETA = 5.45D+0, 
     $                     DELT1 = 8.0D-3, DELT2 = 4.0D-3,
     $                     LL = 0.51302D+0 )
*
*     .. Local Variables .. 
      INTEGER            I, K, NHALF
      DOUBLE PRECISION   HSTEP, TAU1, TAU2, DIAG1, DIAG2, ALPHA2 
*
*     set up middle parameters
*
      NHALF = N / 2 
      HSTEP = ONE / ( DBLE( NHALF ) + 1 ) 
