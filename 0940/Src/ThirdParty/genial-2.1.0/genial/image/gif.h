...
//Summary: GIF format
class GIFFile : public ImageFile
{
  public:
    GIFFile(const string &s) : ImageFile(s) {}   
};
...
template<class G> void operator>>(const GIFFile &file, Matrix<G> &X)
{
  ...
  if(Y.GetHeight()*Y.GetWidth()==0) throw error("Error: Load GIF");
...
template<class G> void operator<<(const GIFFile &file, const Matrix<G> &X)
{
  ...
  {
    typename MatrixRow<image_type>::self Xi=row(X,i);
    for(int_type j=0,jmax;j<jmax;j++)
    {
      RGBA c(Xi[j]);
  ...
  check_gdi(Y.Save(wstring(file.name().begin(),file.name().end()).c_str(),&Clsid),"Error: Save GIF");
}
