...
from scripts.python.utils.system_config import CPU_FPS, plot_output_path
from scripts.python.utils.utils import modes_string, modes_title_string, MttkrpMethod

logger = logging.getLogger('CALS MTTKRP Implementation Analyzer')


def mttkrp_performance_analysis(backend, modes, threads, methods):
    ...
    for midx, mode in enumerate(modes):
        for m in methods:
            dic = read_bench(backend, threads, modes, m)
            df = dic['data']
            dm = df.loc[df['MODE'] == midx]
            if m == MttkrpMethod.AUTO:
                flops = dm['FLOPS']
            if m == MttkrpMethod.CTF or m == MttkrpMethod.PLANC:
                y = flops / dm['TIME'] / mfps
            else:
                y = dm['FLOPS'] / dm['TIME'] / mfps
            label, color = get_label_and_color(dic['mttkrp_method'])
    ...
    fig.suptitle('Comparison (BLAS: ' + backend
                 + ', Threads: ' + str(threads)
                 + ', Modes: ' + modes_title_string(modes) + ')')

    plt.tight_layout()
    fig.savefig(plot_output_path
                + 'MTTKRP_Comp_' + backend
...
def get_label_and_color(method):
    label, color = '', ''
    if method == MttkrpMethod.AUTO:
        label = "CALS MTTKRP"
        color = 'r'
    elif method == MttkrpMethod.CTF:
        label = "CTF"
        color = 'g'
    elif method == MttkrpMethod.PLANC:
        label = "PLANC"
        color = 'b'
    else:
...
def plot_best_mttkrp(backend, modes, threads, best, ax=None, c=None, f_ylabel=True, f_xlabel=True, f_legend=True):
    mfps = CPU_FPS[str(threads)]
...
def main():
    backend = 'MKL'
    methods = [MttkrpMethod.AUTO, MttkrpMethod.CTF, MttkrpMethod.PLANC]
    for modes in [(200, 200, 200)]:
        for threads in [1, 24]:
            mttkrp_performance_analysis(backend, modes, threads, methods)
