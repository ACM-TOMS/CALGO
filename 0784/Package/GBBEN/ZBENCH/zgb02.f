      SUBROUTINE ZGB02 ( SIDE, UPLO, M, N, ALPHA, A, LDA, B, LDB,
     $                   BETA, C, LDC )
*     .. Scalar Arguments ..
...
*  =======
*
*  ZGB02 (ZSYMM)  performs one of the matrix-matrix operations
*
*     C := alpha*A*B + beta*C,
*
*  or
...
*     .. Parameters ..
     ...
     $                   ONE = ( 1.0D+0, 0.0D+0 ) )
*     .. User specified parameters for ZGB02 ..
      INTEGER            RCB, CB
...
*
      IF( ALPHA.EQ.ZERO )THEN
         CALL ZGEMM ( 'N', 'N', M, N, 0, ZERO, A, MAX( LDA, LDB ),
...
*              length of a vector referenced by ZCOPY is CB.
*
               DO 30, JJ = II, II+ISEC-1, CB
                  JSEC = MIN( CB, II+ISEC-JJ )
                  DO 20, J = JJ+1, II+ISEC-1
                     CALL ZCOPY ( MIN( JSEC, J-JJ ), A( JJ, J ), 1,
...
*
               IF( II.GT.1 )THEN
                  CALL ZGEMM ( 'T', 'N', ISEC, N, II-1, ALPHA,
...
*
               DO 70, JX = II+ISEC-1, II, -CB
                  ...
                  DO 60, J = II, JJ+JSEC-2
                     CALL ZCOPY ( MIN( JSEC, JJ+JSEC-1-J ),
     $                                        A( MAX( JJ, J+1 ), J ), 1,
...
*
               IF( II+ISEC.LE.M )THEN
                  CALL ZGEMM ( 'T', 'N', ISEC, N, M-II-ISEC+1, ALPHA,
...
*
               DO 110, II = JJ, JJ+JSEC-1, CB
                  ...
                  DO 100, I = II+1, JJ+JSEC-1
                     CALL ZCOPY ( MIN( ISEC, I-II ), A( II, I ), 1,
...
*
     ...
     $                                            ONE, C( 1, JJ ), LDC )
               ...
               IF( JJ+JSEC.LE.N )THEN
                  CALL ZGEMM ( 'N', 'T', M, JSEC, N-JJ-JSEC+1, ALPHA,
...
*
               DO 150, IX = JJ+JSEC-1, JJ, -CB
                  ...
                  DO 140, I = JJ, II+ISEC-2
                     CALL ZCOPY ( MIN( ISEC, II+ISEC-1-I ),
     $                                        A( MAX( II, I+1 ), I ), 1,
...
*
     ...
     $                                       LDA, ONE, C( 1, JJ ), LDC )
               END IF
               IF( JJ.GT.1 )THEN
                  CALL ZGEMM ( 'N', 'T', M, JSEC, JJ-1, ALPHA,
