...
*  system.
*
*  The function SGB91 returns .TRUE. if the leading dimension LD is
*  critical and .FALSE. if it is not critical. In this implementation
*  SGB91 is designed to detect critical leading dimensions in an
...
*  It may be rewarding to rewrite SGB91 for a machine with a different
*  cache policy.
*
...
*
*
*  User specified parameters for SGB91
*  ================================
*
...
*           considered critical.
*
*  SP     - Number of bytes in a single-precision word.
*
*
...
*     .. Intrinsic Functions ..
      INTRINSIC          MIN, MOD
*     .. User specified parameters for SGB91 ..
      INTEGER            LOLIM, LNSZ, NPRT, PRTSZ, SP
      PARAMETER        ( LNSZ = 64, NPRT = 128, PRTSZ = 3,
     $                   LOLIM = 64, SP = 8 )
*     .. Parameters ..
      INTEGER            ONEWAY, MXDIF
      PARAMETER        ( ONEWAY = ( LNSZ*NPRT )/SP,
     $                   MXDIF = LNSZ/( SP*PRTSZ ) )
*     ..
...
*
      ...
      ELSE
         ...
         SGB91 = MIN( UPDIF, LD-UPDIF ).LE.MXDIF
      END IF
*
      RETURN
*
