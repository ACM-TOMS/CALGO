...
*     .. Parameters ..
     ...
     $                   MXDIM = 36, MXLDA = 24, MXRUNS = 20 )
      COMPLEX            C11, ALPHA, BETA
      PARAMETER        ( C11 = ( 1.0E+0, 1.0E+0 ),
     $                   ALPHA = ( 0.9E+0, 0.05E+0 ),
     $                   BETA = ( 1.1E+0, 0.03E+0 ) )
*     .. Local Scalars ..
...
*     .. Data statements ..
     ...
     $                   NAME/ 'CSYMM ', 'CHEMM ', 'CSYRK ', 'CHERK ',
     $                   'CSYR2K', 'CHER2K', 'CTRMM ', 'CTRSM '/,
     $                   SUB/ .FALSE. /
      DATA  BLANK/'                                                  '/,
     $      LBL  /'                                                  '/
*     ..
...
*
  ...
  190    IF( GETWRD( INLN( JE+1:LLN ), LLN-JE, KB, KE ) )THEN
         ...
         END IF
         NLDA = I
      ELSE IF( INLN( IB:IE ).EQ.'CSYMM'.OR.INLN( IB:IE ).EQ.'CHEMM'.OR.
     $          INLN( IB:IE ).EQ.'CSYRK'.OR.INLN( IB:IE ).EQ.'CHERK'.OR.
     $        INLN( IB:IE ).EQ.'CSYR2K'.OR.INLN( IB:IE ).EQ.'CHER2K'.OR.
     $        INLN( IB:IE ).EQ.'CTRMM'.OR.INLN( IB:IE ).EQ.'CTRSM' )THEN
*
...
*
  210 CONTINUE
     ...
     $      'TRANS, DIAG, DIM1, DIM2, LDA are missing, or none of the'
         WRITE( NERR, FMT = * )
     $      'routines CSYMM, CHEMM, CSYRK, CHERK, CSYR2K, CHER2K,'
         WRITE( NERR, FMT = * )
     $      'CTRMM, and CTRSM are marked to be timed', '.'
         WRITE( NERR, FMT = * )
         STOP
      END IF
...
*
      DO 230, J = 1, NMAX
         DO 220, I = 1, NMAX
            A( I, J ) = C11 + CMPLX( 0.08E+0*REAL( I+( J-1 )*NMAX )/
     $              REAL( NMAX*NMAX+1 ), 0.06E+0*REAL( I+( J-1 )*NMAX )/
     $                                             REAL( NMAX*NMAX+1 ) )
  220    CONTINUE
  230 CONTINUE
      DO 250, J = 1, NMAX
         DO 240, I = 1, NMAX
            B( I, J ) = C11 + CMPLX( 0.04E+0*REAL( I+( J-1 )*NMAX )/
     $              REAL( NMAX*NMAX+1 ), 0.02E+0*REAL( I+( J-1 )*NMAX )/
     $                                             REAL( NMAX*NMAX+1 ) )
  240    CONTINUE
  250 CONTINUE
*
...
*
*        Time the internal GEMM-Based Level 3 BLAS routines (CGB02,
*        CGB03, CGB04, CGB05, CGB06, CGB07, CGB08, and CGB09).
*
         CALL CGBT01( 'G', TABSUB, SIDE, NSIDE, UPLO, NUPLO, TRNS,
...
*
         CALL CGBT01( 'U', TABSUB, SIDE, NSIDE, UPLO, NUPLO, TRNS,
     $                 NTRNS, DIAG, NDIAG, DIM1, DIM2, NDIM1, LDA, NLDA,
     $                      ALPHA, BETA, A, B, C, LD, NMAX, NERR, MXSUB,
...
*
*        Time CGEMM using user specified parameters.
*
         CALL CGBT02( TABSUB, SIDE, NSIDE, NUPLO, TRNS, NTRNS, NDIAG,
...
*
         CALL CGBTP1( TABSUB, SIDE, NSIDE, UPLO, NUPLO, TRNS, NTRNS,
...
*
         CALL CGBTP2( TAB, TABSUB, SIDE, NSIDE, UPLO, NUPLO, TRNS,
     $                 NTRNS, DIAG, NDIAG, DIM1, DIM2, NDIM1, LDA, NLDA,
