      SUBROUTINE SGB09( SIDE, UPLO, TRANSA, DIAG, M, N, ALPHA, A, LDA,
     $                   C, LDC )
*     .. Scalar Arguments ..
...
*  =======
*
*  SGB09 (STRSM) solves one of the matrix equations
*
...
*     .. External Functions ..
      LOGICAL            LSAME, SGB90, SGB91
      EXTERNAL           LSAME, SGB90, SGB91
*     .. External Subroutines ..
      EXTERNAL           XERBLA
      EXTERNAL           SGEMM, SGEMV, STRSV, SCOPY
*     .. Parameters ..
      REAL               ZERO, ONE
      INTEGER            SIP91, SIP92, SIP93
      PARAMETER        ( ZERO = 0.0E+0, ONE = 1.0E+0,
     $                   SIP91 = 91, SIP92 = 92, SIP93 = 93 )
*     .. User specified parameters for SGB09 ..
      INTEGER            RB, CB, RCB
...
*
               SMALLN = .NOT.SGB90( SIP91, M, N )
               IF( SMALLN )THEN
                  TINYN = .NOT.SGB90( SIP92, M, N )
...
*                    of A.
*
                     CALL SGEMM ( 'T', 'N', ISEC, N, II-1, -ONE,
...
*                    involving a rectangular block of A.
*
                     CALL SGEMM ( 'N', 'N', ISEC, N, II-1, -ONE,
...
*
                        DO 390, I = II+ISEC-1, II, -1
                           ...
                           END IF
                           CALL SGEMV ( 'N', JSEC, TSEC, GAMMA,
     $                        T1( 1, I-II+1+TIJ ), RB, A( I+TIJ, I ), 1,
     $                                       DELTA, T1( 1, I-II+1 ), 1 )
  390                   CONTINUE
*
*                       C := T1', the transpose of T1 is copied back
...
*                    involving a rectangular block of A.
*
                     CALL SGEMM ( 'N', 'N', M, JSEC, JJ-1, -ONE,
...
*
                     DO 430, I = 1, M
                        CALL STRSV ( 'U', 'T', DIAG, JSEC, A( JJ, JJ ),
     $                                            LDA, C( I, JJ ), LDC )
  430                CONTINUE
  440             CONTINUE
               ELSE
...
*
                     DO 500, I = 1, M
                        CALL STRSV ( 'U', 'N', DIAG, JSEC, A( JJ, JJ ),
     $                                            LDA, C( I, JJ ), LDC )
  500                CONTINUE
  510             CONTINUE
               ELSE
...
*
                     DO 580, I = 1, M
                        CALL STRSV ( 'L', 'T', DIAG, JSEC, A( JJ, JJ ),
     $                                            LDA, C( I, JJ ), LDC )
  580                CONTINUE
  590             CONTINUE
               ELSE
...
*                    of A.
*
                     CALL SGEMM ( 'N', 'T', M, JSEC, JJ-1, -ONE,
...
*
                     DO 650, I = 1, M
                        CALL STRSV ( 'L', 'N', DIAG, JSEC, A( JJ, JJ ),
     $                                            LDA, C( I, JJ ), LDC )
  650                CONTINUE
  660             CONTINUE
               ELSE
