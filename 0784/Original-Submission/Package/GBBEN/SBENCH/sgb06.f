      SUBROUTINE SGB06( UPLO, TRANS, N, K, ALPHA, A, LDA, B, LDB,
     $                   BETA, C, LDC )
*     .. Scalar Arguments ..
...
*  =======
*
*  SGB06 (SSYR2K) performs one of the symmetric rank 2k operations
*
...
*     .. External Subroutines ..
      EXTERNAL           XERBLA
      EXTERNAL           SGEMM, SAXPY, SSCAL
*     .. Parameters ..
      REAL               ONE, ZERO
      PARAMETER        ( ONE = 1.0E+0, ZERO = 0.0E+0 )
*     .. User specified parameters for SGB06 ..
      INTEGER            RCB, CB
...
*
               CALL SGEMM ( 'N', 'T', ISEC, ISEC, K, ALPHA, A( II, 1 ),
...
*
               DO 40, I = II, II+ISEC-1
                  CALL SAXPY ( I-II+1, ONE, T1( 1, I-II+1 ), 1,
     $                                                   C( II, I ), 1 )
   40          CONTINUE
*
...
*              maximum length of a vector referenced by SAXPY is CB.
*
               DO 60, JJ = II, II+ISEC-1, CB
                  JSEC = MIN( CB, II+ISEC-JJ )
                  DO 50, I = JJ, II+ISEC-1
                     CALL SAXPY ( MIN( JSEC, I-JJ+1 ), ONE,
...
*              blocks of A and B. T1 is square.
*
               CALL SGEMM ( 'T', 'N', ISEC, ISEC, K, ALPHA, A( 1, II ),
...
*
               DO 140, I = II, II+ISEC-1
                  CALL SAXPY ( II+ISEC-I, ONE, T1( I-II+1, I-II+1 ), 1,
     $                                                    C( I, I ), 1 )
  140          CONTINUE
*
...
*
               DO 160, JX = II+ISEC-1, II, -CB
                  ...
                  DO 150, I = II, JJ+JSEC-1
                     CALL SAXPY ( MIN( JSEC, JJ+JSEC-I ), ONE,
...
*
               IF( II+ISEC.LE.N )THEN
                  CALL SGEMM ( 'N', 'T', N-II-ISEC+1, ISEC, K, ALPHA,
...
*
               IF( II+ISEC.LE.N )THEN
                  CALL SGEMM ( 'T', 'N', N-II-ISEC+1, ISEC, K, ALPHA,
