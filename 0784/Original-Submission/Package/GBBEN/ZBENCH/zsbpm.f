      PROGRAM ZSBPM
*
*  ZSBPM re-writes GEMM-Based Level 3 BLAS source files replacing lines
...
*  statements in the input file, and then run ZSBPM to distribute these
...
*  'dgpm.in'). Read the file INSTALL for further instructions.
*
*
*  -- Written in May-1994.
...
*
*     .. Local Scalars ..
      INTEGER            I, IB, IE, JB, JE, KB, KE, LB, LE, NAM, NXTLN
      LOGICAL            PMEOF
*     .. External Functions ..
...
*     .. Parameters ..
      ...
      INTEGER            NLNS, LLN
      PARAMETER        ( NLNS = 14, LLN = 256 )
      CHARACTER          TMPNAM*( LLN )
      PARAMETER        ( TMPNAM = 'tmpgb.tmp' )
*     .. Local Arrays ..
      CHARACTER          PMLN*( LLN ), GBLN*( LLN ), GBNAM*( LLN ),
     $                   STRS( NLNS, 2 )*( LLN ), BNAM( NLNS )*( LLN )
      CHARACTER          PMLNA( LLN ), GBLNA( LLN ), GBNAMA( LLN ),
     $                   STRSA( LLN, NLNS, 2 ), BNAMA( LLN, NLNS )
      EQUIVALENCE      ( PMLN, PMLNA ), ( GBLN, GBLNA ),
     $                 ( GBNAM, GBNAMA ), ( STRS, STRSA ),
     $                 ( BNAM, BNAMA )
*     .. Data statements ..
      DATA               BNAM/
     $'zgb02.f'   ,'zgb03.f'   ,'zgb04.f'   ,'zgb05.f'   ,
     $'zgb06.f'   ,'zgb07.f'   ,'zgb08.f'   ,'zgb09.f'   ,
     $'zgb90.f'   ,'          ','          ','          ',
     $'zgb91.f'   ,'          '/
      DATA               STRS/
     $'zsymm.f'   ,'zhemm.f'   ,'zsyrk.f'   ,'zherk.f'   ,
     $'zsyr2k.f'  ,'zher2k.f'  ,'ztrmm.f'   ,'ztrsm.f'   ,
     $'zbigp.f'   ,'          ','          ','          ',
     $'zcld.f'    ,'          ',
     $'PARAMETER ( RCB = $$ , CB = $$ )',
     $'PARAMETER ( RCB = $$ , CB = $$ )',
...
*
   10 READ( NPM, FMT = 9000, END = 110 ) GBNAM
      IF( .NOT.GETWRD( GBNAMA, LLN, IB, IE ).OR.
     $                                     ( GBNAM( 1:1 ).EQ.'*' ) )THEN
         GO TO 10
      END IF
*
   ...
   20 NAM = -1
      ...
      DO 30, I = 1, NLNS
         IF( GBNAM( IB:IE ).EQ.STRS( I, 1 ) )THEN
            NAM = I
            IF( .NOT.GETWRD( BNAMA( 1, NAM ), LLN, LB, LE ) )THEN
               WRITE( NERR, FMT = * )
     $            'Benchmark routine name corresponding to ',
     $            GBNAM( IB:IE ), ' is missing in ZSBPM.'
               STOP
            END IF
         END IF
   30 CONTINUE
...
*
   40 READ( NPM, FMT = 9000, END = 110 ) PMLN
      IF( .NOT.GETWRD( PMLNA, LLN, JB, JE ).OR.
     $                                     ( PMLN( 1:1 ).EQ.'*' ) )THEN
         GO TO 40
      END IF
*
...
*
      NXTLN = NAM
      IF( LNCMP( PMLNA, LLN, STRSA( 1, NXTLN, 2 ), LLN ) )THEN
         OPEN( NGB, FILE = BNAM( NAM )( LB:LE ), STATUS = 'OLD' )
         OPEN( NTMP, FILE = TMPNAM, STATUS = 'NEW' )
   50    READ( NGB, FMT = 9000, END = 80 ) GBLN
         IF( LNCMP( GBLNA, LLN, STRSA( 1, NXTLN, 2 ), LLN ) )THEN
            WRITE( NTMP, FMT = 9010 ) PMLN( 1:EOLN( PMLNA, LLN ) )
   60       READ( NPM, FMT = 9000, END = 70 ) PMLN
            IF( .NOT.GETWRD( PMLNA, LLN, JB, JE ).OR.
     $                                      ( PMLN( 1:1 ).EQ.'*' ) )THEN
               GO TO 60
            END IF
            IF( .NOT.GETWRD( STRSA( 1, NXTLN+1, 1 ), LLN, KB, KE ).AND.
     $              ( LNCMP( PMLNA, LLN, STRSA( 1, NXTLN+1, 2 ), LLN ) )
     $                                                             )THEN
               NXTLN = NXTLN + 1
            END IF
         ELSE
            WRITE( NTMP, FMT = 9010 ) GBLN( 1:EOLN( GBLNA, LLN ) )
         END IF
         GO TO 50
   70    PMEOF = .TRUE.
         GO TO 50
...
*
      ...
      OPEN( NGB, FILE = BNAM( NAM )( LB:LE ), STATUS = 'NEW' )
   90 READ( NTMP, FMT = 9000, END = 100 ) GBLN
      WRITE( NGB, FMT = 9010 ) GBLN( 1:EOLN( GBLNA, LLN ) )
      GO TO 90
  100 CONTINUE
