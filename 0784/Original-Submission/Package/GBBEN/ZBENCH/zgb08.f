      SUBROUTINE ZGB08 ( SIDE, UPLO, TRANSA, DIAG, M, N, ALPHA, A, LDA,
     $                   C, LDC )
*     .. Scalar Arguments ..
...
*  =======
*
*  ZGB08 (ZTRMM) performs one of the matrix-matrix operations
*
...
*     .. External Subroutines ..
      EXTERNAL           XERBLA
      EXTERNAL           ZGEMM, ZGEMV, ZTRMV, ZCOPY
*     .. Parameters ..
      COMPLEX*16         ZERO, ONE
      INTEGER            ZIP81, ZIP82, ZIP83
      PARAMETER        ( ZERO = ( 0.0D+0, 0.0D+0 ),
     $                   ONE = ( 1.0D+0, 0.0D+0 ),
     $                   ZIP81 = 81, ZIP82 = 82, ZIP83 = 83 )
*     .. User specified parameters for ZGB08 ..
      INTEGER            RB, CB, RCB
...
*
               SMALLN = .NOT.ZGB90( ZIP81, M, N )
               IF( SMALLN )THEN
                  TINYN = .NOT.ZGB90( ZIP82, M, N )
                  DO 40, II = 1, M, RCB
                     ISEC = MIN( RCB, M-II+1 )
*
...
*
                     IF( ALPHA.NE.ONE )
     $                  CALL ZGEMM ( 'N', 'N', ISEC, N, 0, ZERO, A, LDA,
...
*
                     IF( ALPHA.NE.ONE )
     $                  CALL ZGEMM ( TRANSA, 'N', ISEC, N, 0, ZERO, A,
...
*
                     IF( II.GT.1 )THEN
                        CALL ZGEMM ( TRANSA, 'N', ISEC, N, II-1, ALPHA,
...
*
                     IF( II.GT.1 )THEN
                        CALL ZGEMM ( 'N', 'N', ISEC, N, II-1, ALPHA,
...
*
                     IF( ALPHA.NE.ONE )
     $                  CALL ZGEMM ( 'N', 'N', M, JSEC, 0, ZERO, C, LDC,
...
*
                     IF( JJ.GT.1 )THEN
                        CALL ZGEMM ( 'N', 'N', M, JSEC, JJ-1, ALPHA,
...
*
                     IF( ALPHA.NE.ONE )
     $                  CALL ZGEMM ( 'N', TRANSA, M, JSEC, 0, ZERO, C,
...
*
                     IF( JJ.GT.1 )THEN
                        CALL ZGEMM ( 'N', TRANSA, M, JSEC, JJ-1, ALPHA,
