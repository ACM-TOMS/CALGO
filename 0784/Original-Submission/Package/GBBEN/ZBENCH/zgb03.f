      SUBROUTINE ZGB03 ( SIDE, UPLO, M, N, ALPHA, A, LDA, B, LDB,
     $                   BETA, C, LDC )
*     .. Scalar Arguments ..
...
*  =======
*
*  ZGB03 (ZHEMM)  performs one of the matrix-matrix operations
*
*     C := alpha*A*B + beta*C,
*
*  or
...
*     .. Intrinsic Functions ..
      INTRINSIC          MAX, MIN, DBLE, DCMPLX, DCONJG
*     .. External Functions ..
      LOGICAL            LSAME
      EXTERNAL           LSAME
*     .. External Subroutines ..
      EXTERNAL           XERBLA
      EXTERNAL           ZGEMM, ZCOPY
*     .. Parameters ..
     ...
     $                   ZZERO = ( 0.0D+0, 0.0D+0 ),
     $                   ZONE = ( 1.0D+0, 0.0D+0 ) )
*     .. User specified parameters for ZGB03 ..
      INTEGER            RCB, CB
...
*
     ...
     $    ( ( ALPHA.EQ.ZZERO ).AND.( BETA.EQ.ZONE ) ) )
     $   RETURN
*
*     And when alpha.eq.zero.
*
      IF( ALPHA.EQ.ZZERO )THEN
         CALL ZGEMM ( 'N', 'N', M, N, 0, ZZERO, A, MAX( LDA, LDB ),
...
*
               DO 10, I = II, II+ISEC-1
                  CALL ZCOPY ( I-II+1, A( II, I ), 1, T1( 1, I-II+1 ),
     $                                                               1 )
   10          CONTINUE
*
...
*
               DO 20, I = 1, ISEC
                  T1( I, I ) = DCMPLX( DBLE( T1( I, I ) ), ZERO )
   20          CONTINUE
*
...
*
               CALL ZGEMM ( 'N', 'N', ISEC, N, ISEC, ALPHA, T1( 1, 1 ),
...
*
               IF( II.GT.1 )THEN
                  CALL ZGEMM ( 'C', 'N', ISEC, N, II-1, ALPHA,
     $                                  A( 1, II ), LDA, B( 1, 1 ), LDB,
     $                                           ZONE, C( II, 1 ), LDC )
               END IF
               IF( II+ISEC.LE.M )THEN
                  CALL ZGEMM ( 'N', 'N', ISEC, N, M-II-ISEC+1, ALPHA,
...
*
               IF( II+ISEC.LE.M )THEN
                  CALL ZGEMM ( 'C', 'N', ISEC, N, M-II-ISEC+1, ALPHA,
...
*
               DO 130, J = JJ, JJ+JSEC-1
                  CALL ZCOPY ( J-JJ+1, A( JJ, J ), 1, T1( 1, J-JJ+1 ),
     $                                                               1 )
  130          CONTINUE
*
*              Set the imaginary part of diagonal elements of T1
...
*
               DO 140, J = 1, JSEC
                  T1( J, J ) = DCMPLX( DBLE( T1( J, J ) ), ZERO )
  140          CONTINUE
*
*              T1 :=  conjg( A' ), the conjugated transpose of a
*              strictly upper triangular diagonal block of A is copied
...
*              corresponds to a full diagonal block of the matrix A.
*
               CALL ZGEMM ( 'N', 'N', M, JSEC, JSEC, ALPHA, B( 1, JJ ),
...
*
     ...
     $                                           ZONE, C( 1, JJ ), LDC )
               ...
               IF( JJ+JSEC.LE.N )THEN
                  CALL ZGEMM ( 'N', 'C', M, JSEC, N-JJ-JSEC+1, ALPHA,
...
*              corresponds to a full diagonal block of the matrix A.
*
               CALL ZGEMM ( 'N', 'N', M, JSEC, JSEC, ALPHA,
...
*
               IF( JJ+JSEC.LE.N )THEN
                  CALL ZGEMM ( 'N', 'N', M, JSEC, N-JJ-JSEC+1, ALPHA,
     $                           B( 1, JJ+JSEC ), LDB, A( JJ+JSEC, JJ ),
     $                                      LDA, ZONE, C( 1, JJ ), LDC )
               END IF
               IF( JJ.GT.1 )THEN
                  CALL ZGEMM ( 'N', 'C', M, JSEC, JJ-1, ALPHA,
