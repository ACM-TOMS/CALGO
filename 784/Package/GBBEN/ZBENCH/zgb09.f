      SUBROUTINE ZGB09 ( SIDE, UPLO, TRANSA, DIAG, M, N, ALPHA, A, LDA,
     $                   C, LDC )
*     .. Scalar Arguments ..
...
*  =======
*
*  ZGB09 (ZTRSM) solves one of the matrix equations
*
...
*     .. Local Scalars ..
     ...
     $                   CLDC, SMALLN, TINYN, TINYM
      INTEGER            I, II, IX, ISEC, J, JJ, TIJ, JX, JSEC, TSEC
      COMPLEX*16         GAMMA, DELTA
*     .. Intrinsic Functions ..
      INTRINSIC          MAX, MIN, MOD, DCONJG
*     .. External Functions ..
      LOGICAL            LSAME, ZGB90, ZGB91
      EXTERNAL           LSAME, ZGB90, ZGB91
*     .. External Subroutines ..
      EXTERNAL           XERBLA
      EXTERNAL           ZGEMM, ZGEMV, ZTRSV, ZCOPY
*     .. Parameters ..
      COMPLEX*16         ZERO, ONE
      INTEGER            ZIP91, ZIP92, ZIP93
      PARAMETER        ( ZERO = ( 0.0D+0, 0.0D+0 ),
     $                   ONE = ( 1.0D+0, 0.0D+0 ),
     $                   ZIP91 = 91, ZIP92 = 92, ZIP93 = 93 )
*     .. User specified parameters for ZGB09 ..
      INTEGER            RB, CB, RCB
      PARAMETER        ( RCB = 7, RB = 5, CB = 3 )
      COMPLEX*16         T1( RB, CB ), T2( CB, CB ), T3( RCB, RCB )
*     ..
...
*
      IF( ALPHA.EQ.ZERO )THEN
         CALL ZGEMM ( 'N', 'N', M, N, 0, ZERO, C, MAX( LDA, LDC ), C,
...
*
               SMALLN = .NOT.ZGB90( ZIP91, M, N )
               IF( SMALLN )THEN
                  TINYN = .NOT.ZGB90( ZIP92, M, N )
...
*
                     ...
                     ELSE
                        ...
                        GAMMA = ZERO
                     END IF
                     CALL ZGEMM ( 'N', 'N', ISEC, N, M-II-ISEC+1,
     $                                 GAMMA, A( II, II+ISEC-TIJ ), LDA,
     $                                         C( II+ISEC-TIJ, 1 ), LDC,
     $                                          ALPHA, C( II, 1 ), LDC )
                     IF( TINYN )THEN
*
...
*
                        DO 20, I = II+OFFD, II+ISEC-1
                           CALL ZCOPY ( I-II+1-OFFD, A( II, I ), 1,
     $                                              T3( 1, I-II+1 ), 1 )
   20                   CONTINUE
*
...
*
                        IF( CLDC )THEN
                           DO 60, J = JJ, JJ+JSEC-1
                              CALL ZCOPY ( ISEC, C( II, J ), 1,
     $                                             T1( J-JJ+1, 1 ), RB )
   60                      CONTINUE
                        ELSE
                           DO 70, I = II, II+ISEC-1
                              CALL ZCOPY ( JSEC, C( I, JJ ), LDC,
     $                                              T1( 1, I-II+1 ), 1 )
   70                      CONTINUE
                        END IF
*
...
*                       to compensate for a deficiency in ZGEMV that
...
*
                        DO 80, I = II+ISEC-1, II, -1
                           ...
                           IF( TSEC.EQ.0 )THEN
                              ...
                              TSEC = 1
                           END IF
                           CALL ZGEMV ( 'N', JSEC, TSEC, GAMMA,
     $                                          T1( 1, I-II+1+TIJ ), RB,
     $                                      T2( I-II+1+TIJ, I-II+1 ), 1,
     $                                       DELTA, T1( 1, I-II+1 ), 1 )
   80                   CONTINUE
*
...
*
                        DO 90, J = JJ, JJ+JSEC-1
                           CALL ZCOPY ( ISEC, T1( J-JJ+1, 1 ), RB,
     $                                                   C( II, J ), 1 )
   90                   CONTINUE
  100                CONTINUE
  110             CONTINUE
...
*                    of A.
*
                     CALL ZGEMM ( TRANSA, 'N', ISEC, N, II-1, -ONE,
...
*
                     IF( NOCONJ )THEN
                        DO 160, J = II+OFFD, II+ISEC-1
                           CALL ZCOPY ( J-II+1-OFFD, A( II, J ), 1,
     $                                              T2( 1, J-II+1 ), 1 )
  160                   CONTINUE
                     ELSE
                        DO 180, J = II+OFFD, II+ISEC-1
                           DO 170, I = II, J-OFFD
                              T2( I-II+1, J-II+1 ) = DCONJG( A( I, J ) )
  170                      CONTINUE
  180                   CONTINUE
                     END IF
                     DO 230, JJ = 1, N, RB
...
*                    involving a rectangular block of A.
*
                     CALL ZGEMM ( 'N', 'N', ISEC, N, II-1, -ONE,
...
*
                        DO 260, I = II, II+ISEC-1-OFFD
                           CALL ZCOPY ( II+ISEC-I-OFFD, A( I+OFFD, I ),
...
*
                     ...
                     ELSE
                        ...
                        GAMMA = ZERO
                     END IF
                     CALL ZGEMM ( TRANSA, 'N', ISEC, N, M-II-ISEC+1,
     $                                 GAMMA, A( II+ISEC-TIJ, II ), LDA,
     $                                         C( II+ISEC-TIJ, 1 ), LDC,
     $                                          ALPHA, C( II, 1 ), LDC )
                     IF( TINYN )THEN
*
...
*
                     IF( NOCONJ )THEN
                        DO 400, J = II, II+ISEC-1-OFFD
                           CALL ZCOPY ( II+ISEC-J-OFFD, A( J+OFFD, J ),
...
*                    involving a rectangular block of A.
*
                     CALL ZGEMM ( 'N', 'N', M, JSEC, JJ-1, -ONE,
...
*
                     ...
                     ELSE
                        ...
                        GAMMA = ZERO
                     END IF
                     CALL ZGEMM ( 'N', TRANSA, M, JSEC, N-JJ-JSEC+1,
     $                                  GAMMA, C( 1, JJ+JSEC-TIJ ), LDC,
     $                                        A( JJ, JJ+JSEC-TIJ ), LDA,
     $                                          ALPHA, C( 1, JJ ), LDC )
*
...
*
                     DO 560, J = JJ+OFFD, JJ+JSEC-1
                        CALL ZCOPY ( J-JJ+1-OFFD, A( JJ, J ), 1,
     $                                            T3( J-JJ+1, 1 ), RCB )
  560                CONTINUE
*
...
*
                     ...
                     ELSE
                        ...
                        GAMMA = ZERO
                     END IF
                     CALL ZGEMM ( 'N', 'N', M, JSEC, N-JJ-JSEC+1,
     $                                  GAMMA, C( 1, JJ+JSEC-TIJ ), LDC,
     $                                        A( JJ+JSEC-TIJ, JJ ), LDA,
     $                                          ALPHA, C( 1, JJ ), LDC )
*
*                    Solve X*A = C, triangular system solve involving
...
*
                        DO 700, J = JJ+JSEC-1, JJ, -1
                           ...
                           END IF
                           CALL ZGEMV ( 'N', ISEC, TSEC, GAMMA,
     $                        T1( 1, J-JJ+1+TIJ ), RB, A( J+TIJ, J ), 1,
     $                                       DELTA, T1( 1, J-JJ+1 ), 1 )
  700                   CONTINUE
*
*                       C := T1, T1 is copied back to C.
...
*                    of A.
*
                     CALL ZGEMM ( 'N', TRANSA, M, JSEC, JJ-1, -ONE,
...
*
                     DO 740, J = JJ, JJ+JSEC-1-OFFD
                        CALL ZCOPY ( JJ+JSEC-J-OFFD, A( J+OFFD, J ),
