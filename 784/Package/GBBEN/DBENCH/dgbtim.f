...
*     .. Parameters ..
     ...
     $                   MXLDA = 24, MXRUNS = 20 )
      ...
      PARAMETER        ( ONE = 1.0D+0, ALPHA = 0.9D+0, BETA = 1.1D+0 )
*     .. Local Scalars ..
...
*     .. External Subroutines ..
      EXTERNAL           DGBT01, DGBT02, DGBTP1, DGBTP2
*     .. Local Arrays ..
     ...
     $                   MXDIM, MXLDA )
      COMMON           / DBKCMN / A, B, C, USRES, GBRES, MMRES
...
*
  ...
  180    IF( GETWRD( INLNA( JE+1 ), LLN-JE-1, KB, KE ) )THEN
         ...
         END IF
         NLDA = I
      ELSE IF( INLN( IB:IE ).EQ.'DSYMM'.OR.INLN( IB:IE ).EQ.'DSYRK'.OR.
     $         INLN( IB:IE ).EQ.'DSYR2K'.OR.INLN( IB:IE ).EQ.'DTRMM'.OR.
     $                                    INLN( IB:IE ).EQ.'DTRSM' )THEN
*
...
*
  200 CONTINUE
     ...
     $      'TRANS, DIAG, DIM1, DIM2, LDA are missing, or none of the'
         WRITE( NERR, FMT = * )
     $      'routines DSYMM, DSYRK, DSYR2K, DTRMM, and DTRSM are marked'
         WRITE( NERR, FMT = * )
     $      'to be timed', '.'
         WRITE( NERR, FMT = * )
         STOP
...
*
      DO 220, J = 1, NMAX
         DO 210, I = 1, NMAX
            A( I, J ) = ONE + 0.08D+0*DBLE( I+( J-1 )*NMAX )/
     $                                               DBLE( NMAX*NMAX+1 )
  210    CONTINUE
  220 CONTINUE
      DO 240, J = 1, NMAX
         DO 230, I = 1, NMAX
            B( I, J ) = ONE + 0.04D+0*DBLE( I+( J-1 )*NMAX )/
     $                                               DBLE( NMAX*NMAX+1 )
  230    CONTINUE
  240 CONTINUE
*
...
*
*        Time the internal GEMM-Based Level 3 BLAS routines (DGB02,
*        DGB04, DGB08, DGB08, and DGB09).
*
         CALL DGBT01( 'G', TABSUB, SIDE, NSIDE, UPLO, NUPLO, TRNS,
...
*
         CALL DGBT01( 'U', TABSUB, SIDE, NSIDE, UPLO, NUPLO, TRNS,
     $                 NTRNS, DIAG, NDIAG, DIM1, DIM2, NDIM1, LDA, NLDA,
     $                      ALPHA, BETA, A, B, C, LD, NMAX, NERR, MXSUB,
...
*
*        Time DGEMM using user specified parameters.
*
         CALL DGBT02( TABSUB, SIDE, NSIDE, NUPLO, TRNS, NTRNS, NDIAG,
...
*
         CALL DGBTP1( TABSUB, SIDE, NSIDE, UPLO, NUPLO, TRNS, NTRNS,
...
*
         CALL DGBTP2( TAB, TABSUB, SIDE, NSIDE, UPLO, NUPLO, TRNS,
     $                 NTRNS, DIAG, NDIAG, DIM1, DIM2, NDIM1, LDA, NLDA,
