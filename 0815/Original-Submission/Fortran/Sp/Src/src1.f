      subroutine gfas( adjv,   alpha,  bcset,  bestz,  caset1, caset2,
...
c     ------------------------------------------------------------------
c
c     gfas: A package of FORTRAN subroutines for finding feedback arc
...
c          ne0    - number of arcs in the graph (original)
c          nv0    - number of vertices in the graph (original)
c          seed   - seed for pseudo random number generator 
c          seedd  - seed for pseudo random number generator
c
c     ------------------------------------------------------------------
      integer          errcnd, fin0,   fout0,  nrcl,   nv0,    ne0,
     +                 seed
...
c           vmap1   - node 1 of arc in the feedback arc version
c           vmap2   - node 2 of arc in the feedback arc version
c           vout    - vertex in out-adjacency list (copy)
c           vout0   - vertex in out-adjacency list (original)
c           vtx1    - node 1 of arc in the feedback vertex version
c           vtx2    - node 2 of arc in the feedback vertex version 
c
c     ------------------------------------------------------------------
     ...
     +           vout(max2e),      vout0(max2e),    vmap1(maxe),
     +           vmap2(maxe),      vtx1(maxe),      vtx2(maxe)

c     ------------------------------------------------------------------
c
c     Local scalars:
c            
c           nee     - array dimension = ne*ne
c           n2e     - array dimension = 2*ne
c           nvv     - array dimension = nv*nv
c
c     ------------------------------------------------------------------
      integer    nee,    n2e,    nvv
...
c     ------------------------------------------------------------------
      call chkfas( errcnd, look4,  maxe,   maxitr, maxv,   ne,     nv,
     +             prttyp, seed,   seedd,  vtx1,   vtx2 )
...
c     ------------------------------------------------------------------
c     Translate feedback arc set problem into feedback vertex set 
c     problem.
c     ------------------------------------------------------------------
      call fa2fv( adjv,   first,  ind,    last,   ne,     ne0,    
     +            next,   nee,    n2e,    nv,     nvv,    nv0,
     +            vmap1,  vmap2,  vtx1,   vtx2 )
      n2e=2*ne

c     ------------------------------------------------------------------
c     Solve FVS problem.
c     ------------------------------------------------------------------
      call gfvsa( alpha,  bcset,  bestz,  cset,   cset0,  din,    din0,
     +            dout,   dout0,  errcnd, fin0,   fout0,  gitrb,  lin,
     +            lin0,   look4,  lout,   lout0,  maxitr, ne,     nee,
     +            n2e,    nin,    nin0,   nout,   nout0,  nrcl,   nv,
     +            pin,    pin0,   pout,   pout0,  prttyp, out,    rcl,
     +            seed,   seedb,  vertex, vin,    vin0,   vout,   vout0,
     +            vtx1,   vtx2 )

c     ------------------------------------------------------------------
c     Translate feedback vertex set solution into feedback arc set 
c     solution.
c     ------------------------------------------------------------------
      call fv2fa( caset1, caset2, bcset,  ne,     bestz,  vmap1,  
     +            vmap2 )
...
c     ------------------------------------------------------------------
      ...
      subroutine chkfas( errcnd, look4,  maxe,   maxitr, maxv,   ne,
     +                   nv,     prttyp, seed,   seedd,  vtx1,   vtx2 )
c     ------------------------------------------------------------------
c
c     chkfas: Checks values of parameters and problem dimension for
c             validity.  Feedback arc set case.
c
c     ------------------------------------------------------------------
c
c     Passed input scalars:
...
c     ------------------------------------------------------------------
      if (look4.lt.0 .or. look4.gt.ne) then
           errcnd=4
           return
      endif
...
c     ------------------------------------------------------------------
     ...
     +                 n2e,    nin,    nin0,   nout,   nout0,  nv,
     +                 pin,    pin0,   pout,   pout0,  vertex, vin,
     +                 vin0,   vout,   vout0 )
c     ------------------------------------------------------------------
c
...
c     ------------------------------------------------------------------
      ...
      subroutine fa2fv( adjv,   first,  ind,    last,   ne,     ne0,
     +                  next,   nee,    n2e,    nv,     nvv,    nv0,
     +                  vmap1,  vmap2,  vtx1,   vtx2 )
c     ------------------------------------------------------------------
c
c     fa2fv: Transforms FAS problem into FVS problem.
c
c     ------------------------------------------------------------------
c
c     Passed input scalars:
c
c          ne     - number of arcs in graph (copy)
c          n2e    - array dimension = 2*ne
c          nv     - number of vertices in graph (copy)
c          nvv    - array dimension = nv*nv
c
c     ------------------------------------------------------------------
      integer    ne,     n2e,    nv,     nvv, nee

c     ------------------------------------------------------------------
c
c     Passed working scalars:
c
c          nadjv  - number of adjacent vertices 
c          ne0    - number of arcs in graph (original)
c          nv0    - number of vertices in graph (original)
c
c     ------------------------------------------------------------------
      integer    nadjv,  ne0,    nv0
...
c
c     ------------------------------------------------------------------ 
      integer    adjv(n2e),      first(nv),      ind(nvv),
     +           last(n2e),      next(n2e),      vmap1(nee),
     +           vmap2(nee),     vtx1(nee),      vtx2(nee)
...
c          i     - do loop index
c          j     - do loop index
c          ptr   - pointer to vertex
c          tind  - index of vertex in the translated problem (copy)
c          tmpa  - vertex
c          v1    - vertex
c          v2    - vertex
c          
c     ------------------------------------------------------------------
      integer    a,      i,      j,      ptr,    tind,   tmpa,
     +           v1,     v2

c     ------------------------------------------------------------------
c     Initialize pointers to list of adjacent vertices and indices.
c     ------------------------------------------------------------------
      do 10 i=1,nv
           first(i)=0
10    continue
...
c     ------------------------------------------------------------------
c     Initialize counter of adjacent vertices.
c     ------------------------------------------------------------------
      nadjv=0
...
c          -------------------------------------------------------------
           ...
           v2=vtx2(i)
c          -------------------------------------------------------------
c          Transform the edge = (v1,v2) into the vertex (v1,v2) having 
c          index i.
c          -------------------------------------------------------------
           ind((v1-1)*nv+v2)=i
...
c          -------------------------------------------------------------
c          Check if v1 has no outgoing arcs.
c          ------------------------------------------------------------- 
           if (first(v1).eq.0) then
c               --------------------------------------------------------
c               Add v2 to v1's out-adjacency list as the first element.
c               --------------------------------------------------------
                first(v1)=nadjv
                adjv(nadjv)=v2
           else
c               --------------------------------------------------------
c               Add v2 to v1's out-adjacency list as the last element.
c               --------------------------------------------------------
                adjv(nadjv)=v2
                next(last(v1))=nadjv
           endif
...
c     ------------------------------------------------------------------
c     Build the 'directed line-graph' of the original graph.
c     ------------------------------------------------------------------
      ne0=0
      do 60 i=1,ne
c          -------------------------------------------------------------
c          Store temporarily the tail and the head of arc i in v1 
c          and v2, respectively.
c          -------------------------------------------------------------
           v1=vtx1(i)
           v2=vtx2(i)

c          -------------------------------------------------------------
c          Store temporarily the first outgoing arc from v2 in ptr and
c          the index of the vertex (v1,v2) in tind.
c          -------------------------------------------------------------
           ptr=first(v2)
           tind=ind((v1-1)*nv+v2)

c          -------------------------------------------------------------
c          Check if v2 has no outgoing arcs.
c          -------------------------------------------------------------
40         if (ptr.eq.0) goto 50
c               --------------------------------------------------------
c               Let a be the head of the arc ptr.
c               --------------------------------------------------------
                a=adjv(ptr)

c               --------------------------------------------------------
c               Increase the number of arcs in the translated problem.
c               --------------------------------------------------------
                ne0=ne0+1

c               --------------------------------------------------------
c               Update the indices of the tail and the head of the 
c               arc (ne0) in the translated problem.  
c               --------------------------------------------------------
                vmap1(ne0)=tind
                vmap2(ne0)=ind((v2-1)*nv+a)

c               --------------------------------------------------------
c               Let ptr be the next outgoing arc from v2.
c               --------------------------------------------------------
                ptr=next(ptr)
                goto 40
50         continue     
...
c     ------------------------------------------------------------------
c     Update the number of vertices and arcs in the translated problem.
c     ------------------------------------------------------------------
      nv0=nv
      nv=ne
      ne=ne0
...
c     ------------------------------------------------------------------
c     Copy the array vmap* in vtx* and vtx* in vmap*
c     ------------------------------------------------------------------
      if (ne.lt.ne0) then
           do 70 i=1,ne
                tmpa=vtx1(i)
...
c     ------------------------------------------------------------------
      ...
      subroutine fv2fa( caset1, caset2, cset,   ne,     ncset,  vmap1,
     +                  vmap2 )
c     ------------------------------------------------------------------
c
c     fv2fa: Transforms FVS solution of FAS problem into FAS solution.
c
c     ------------------------------------------------------------------
c
c     Passed input scalars:
...
c     Passed input arrays:
c
c          cset   - cutset found for the translated problem 
c          vmap1  - node 1 of arc in the translated problem
c          vmap2  - node 2 of arc in the translated problem
c
c     ------------------------------------------------------------------
      integer    cset(ne),       vmap1(ne),      vmap2(ne)

c     ------------------------------------------------------------------
c
c     Passed output arrays:
c
c          caset1 - node 1 of arc in cutset found for the 
c                   original problem 
c          caset2 - node 2 of arc in cutset found for the 
c                   original problem
c
c     ------------------------------------------------------------------
      integer    caset1(ne),     caset2(ne)
...
c     ------------------------------------------------------------------
c     Map back to original graph.
c     ------------------------------------------------------------------
      do 10 i=1,ncset
           caset1(i)=vmap1(cset(i))
           caset2(i)=vmap2(cset(i))
10    continue
      return

c     ------------------------------------------------------------------
c     End of subroutine fv2fa.
c     ------------------------------------------------------------------
      end
      ...
      subroutine gfvsa( alpha,  bcset,  bestz,  cset,   cset0,  din,
     +                  din0,   dout,   dout0,  errcnd, fin0,   fout0,
     +                  gitrb,  lin,    lin0,   look4,  lout,   lout0,
     +                  maxitr, ne,     nee,    n2e,    nin,    nin0,
     +                  nout,   nout0,  nrcl,   nv,     pin,    pin0,
     +                  pout,   pout0,  prttyp, out,    rcl,    seed,
     +                  seedb,  vertex, vin,    vin0,   vout,   vout0,
     +                  vtx1,   vtx2 )
c     ------------------------------------------------------------------
c
c     gfvsa: A GRASP for finding approximate solutions of the
c            Feedback Vertex Set Problem (for translated Feedback
c            Arc Set Problems)
c
c     ------------------------------------------------------------------
c
...
c     ------------------------------------------------------------------
      real       alpha
      integer    look4,  maxitr, ne,     nee,    n2e,    nv,
     +           out,    prttyp
...
c          pout0  - pointer to start of out-adjacency list (original)
c          rcl    - Restricted Candidate List
c          verte  - existence of vertex in the reduced graph
c          vin    - vertex in in-adjacency list (copy)
c          vin0   - vertex in in-adjacency list (original)
c          vout   - vertex in out-adjacency list (copy)
c          vout0  - vertex in out-adjacency list (original)
...
c     ------------------------------------------------------------------
      integer    bcset(nv),      cset(nv),       cset0(nv),
     +           din(nv),        din0(nv),       dout(nv),
     +           dout0(nv),      lin(nee),       lin0(nee),
     +           lout(nee),      lout0(nee),     nin(nee),
     +           nin0(nee),      nout(nee),      nout0(nee),
     +           pin(nv),        pin0(nv),       pout(nv),
     +           pout0(nv),      rcl(nv),        vertex(nv),
     +           vin(nee),       vin0(nee),      vout(nee),
     +           vout0(nee),     vtx1(nee),      vtx2(nee)
...
c     ------------------------------------------------------------------
      call mkds( din0,   dout0,  fin0,   fout0,  lin0,   lout0,  nee,
...
c               --------------------------------------------------------
c               The cardinality of the found cutset is still less 
...
c     ------------------------------------------------------------------
c     Execution termionated successfully.
c     ------------------------------------------------------------------
      errcnd=0
      return

c     ------------------------------------------------------------------
c     End of subroutine gfvsa.
c     ------------------------------------------------------------------
      end
...
c               ------------------------------------------------------
     ...
     +                     lout,   lout0,  n2e,    nin,    nin0,
     +                     nout,   nout0,  nv,     pin,    pin0,
     +                     pout,   pout0,  vertex, vin,    vin0,
     +                     vout,   vout0 )
...
c
c     ------------------------------------------------------------------
      integer    din0(nv),       dout0(nv),      lin0(nee),
     +           lout0(nee),     nin0(nee),      nout0(nee),
     +           pin0(nv),       pout0(nv),      vertex(nv),
     +           vin0(nee),      vout0(nee)
c     ------------------------------------------------------------------
c
...
c     ------------------------------------------------------------------
      ...
      subroutine outarc( caset1, caset2, gitrb,  maxe,   ncset,  out,
     +                   seedb )
c     ------------------------------------------------------------------
c
c     outarc: Prints out best feedback arc set found.
c
c     ------------------------------------------------------------------
c
c     Passed input scalars:
...
c
c     ------------------------------------------------------------------
      integer    caset1(maxe),   caset2(maxe)
...
10    format(//,' GRASP solution---------------------------------',//)
      write(out,20) ncset
20    format('     size of feedback arc cutset: ',i14,//)
      write(out,30) gitrb
...
c     ------------------------------------------------------------------
      write(out,50)
50    format(' smallest arc cutset: ',/)
      do 70 i=1,ncset
           write(out,60) caset1(i),caset2(i)
60         format(' arc: ',2i5)
70    continue
      write(out,80)
80    format(//,' -----------------------------------------------',//)
