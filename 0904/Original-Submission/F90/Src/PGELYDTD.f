...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      SUBROUTINE PGELYDTD( JOB, SYMM, OP, ASCHUR, N, A, IA, JA, DESCA, 
     $                     C, IC, JC, DESCC, NB2, DWORK, LDWORK, IWORK, 
...
C  Purpose
C  =======
C  The subroutine solves the general Discrete Time Lyapunov Equation 
C
C     op(sub(A)) * X * op(sub(A)^T) - X  = C,
C
...
C            When solving the triangular problem in PTRLYDTD it is possible
...
C  method for solving the general discrete-time Lyapunov equation (see
...
C  Keywords
C  ========
C  QR-algorithm, real Schur form, discrete-time Lyapunov equation, 
...
C     
     ...
     $                   PDIDMAT, PDHESS, PTRLYDTD, DLATCPY, PDCOPY
C     .. 
...
C
         IF( SOLVE ) THEN
            CALL PTRLYDTD( CSYMM, OP, N, A, IA, JA, DESCA, C, IC, JC, 
...
C     
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PGELYDTD', -INFO )
         RETURN
      END IF
C
...
C
      ...
      T1 = MPI_WTIME() - T1
      IF( .NOT. SOLVE ) RETURN
c      CALL PDLAPRNT( N, N, A, 1, 1, DESCA, 0, 0, 'T', 6, DWORK(SWORK) )
c      CALL PDLAPRNT( N, N, DWORK(Z_A), 1, 1, DESCZ_A, 0, 0, 'Q', 6, 
c     $     DWORK(SWORK) )     
C
...
C     
 ...
 15         CONTINUE
      ...
      END IF
      T2 = MPI_WTIME() - T2
c      CALL PDLAPRNT( N, N, C, 1, 1, DESCC, 0, 0, 'CC', 6, DWORK(SWORK) )
C     
C     Solve the reduced triangular problem
C
      T3 = MPI_WTIME()
      CALL PTRLYDTD( CSYMM, OP, N, A, IA, JA, DESCA, C, IC, JC, DESCC, 
...
C
      ...
      END IF
      T3 = MPI_WTIME() - T3
c      CALL PDLAPRNT( N, N, C, 1, 1, DESCC, 0, 0, 'XX', 6, DWORK(SWORK) )
C     
