...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      SUBROUTINE PGLYCTCON( OP, N, A, IA, JA, DESCA, E, IE, JE, DESCE, 
...
C  Lyapunov Equation 
C
C     op(A) * X * op(E^T) + op(E) * X * op(A^T) = C, 
C
...
C                      LOCc(N+IROFFB), 
C            where BUFF is the workspace needed by PTRGLYCTD (i.e., the 
C            same as for PGEGLYCTD with ASCHUR = 'S'). 
C            
...
C            corresponds to the number of call to PDLACON and PGEGLYCTD.
C
...
C  Keywords
C  ========
C  Z-GLYCT, 1-norm based lower bound, generalized discrete-time Lyapunov 
...
C     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, PDLACON, PGEGLYCTD, PDMATADD
C     ..
...
C
      IF( INFO.LT.0 ) THEN
         CALL PXERBLA( ICTXT, 'PGLYCTCON', -INFO )
         RETURN
      END IF
C
