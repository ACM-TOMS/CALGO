...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      SUBROUTINE PTRGLYDTD( CSYMM, OP, N, A, IA, JA, DESCA, E, IE, JE, 
     $                      DESCE, C, IC, JC, DESCC, NB2, DWORK, LDWORK, 
...
C  =======
C
C  PTRGLYDTD solves the (quasi-)triangular generalized discrete-time 
C  Lyapunov Equation (GLYDT)
C
C     op(sub(A)) * X * op(sub(A)^T) - op(sub(E)) * X * op(sub(E)^T) = C,
C
...
C
C  This routine should *not* be called directly, but through PGEGLYDTD.
C
C  C  Notes
C  =====
C
...
C            When solving the triangular problem in PTRGLYDTD it is possible
...
C  solving the triangular generalized discrete-time Lyapunov equation.
...
C     .. Local Scalars ..
     ...
     $         IWRKND, EXAINF, IPW, GI, GJ, NBCAE, POS, JEND, EYE, EXD,
     ...
     $         INDXE, INDXU, PHASES, PHASE, MNPDIM,KKK, XIJT, THREADS,
...
C     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, CHK1MAT, SG03AX,
...
C     
      ...
      END IF
C     
C     Check if the call to PTRGLYDTD was a workspace query, if so
...
C     
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PTRGLYDTD', -INFO )
         RETURN
      END IF
C     
...
C     
                  ...
                  SCSRC = CCSRC
C     
C     Solve sub-system (i,j) - GLYDT for i=j and C=C^T, and GSYL else. 
C     
                  IF (SNODE) THEN
C     
...
#ifdef USE_OMP
                           IF( RSIDE ) THEN
                              CALL RECGSYL_P( THREADS, 3, SCALOC, 
     $                             ROWS, COLS, 
     $                             DWORK(AEII), NB+1, DWORK(AEJJ), 
     $                             NB+1, DWORK(AEII+(NB+1)*ROWS), 
     $                             NB+1, DWORK(AEJJ+(NB+1)*COLS), 
     $                             NB+1, DWORK(CIJ), NB+1, LINFO,
     $                             MACHINE, DWORK(IPW), LDWORK-IPW+1 )
                           ELSEIF( .NOT. RSIDE ) THEN
                              CALL RECGSYL_P( THREADS, 5, SCALOC, 
     $                             ROWS, COLS, 
     $                             DWORK(AEII), NB+1, DWORK(AEJJ), 
     $                             NB+1, DWORK(AEII+(NB+1)*ROWS), 
     $                             NB+1, DWORK(AEJJ+(NB+1)*COLS), 
...
#else
                           IF( RSIDE ) THEN
                              CALL RECGSYL( 3, SCALOC, ROWS, COLS, 
     $                             DWORK(AEII), NB+1, DWORK(AEJJ), 
     $                             NB+1, DWORK(AEII+(NB+1)*ROWS), 
     $                             NB+1, DWORK(AEJJ+(NB+1)*COLS), 
     $                             NB+1, DWORK(CIJ), NB+1, LINFO,
     $                             MACHINE, DWORK(IPW), LDWORK-IPW+1 )
                           ELSEIF( .NOT. RSIDE ) THEN
                              CALL RECGSYL( 5, SCALOC, ROWS, COLS, 
     $                             DWORK(AEII), NB+1, DWORK(AEJJ), 
     $                             NB+1, DWORK(AEII+(NB+1)*ROWS), 
     $                             NB+1, DWORK(AEJJ+(NB+1)*COLS), 
     $                             NB+1, DWORK(CIJ), NB+1, LINFO,
...
#else
                           IF( RSIDE ) THEN
                              CALL RECGSYL( 3, SCALOC, ROWS, ROWS, 
     $                             DWORK(AEII), NB+1, DWORK(AEII), 
     $                             NB+1, DWORK(AEII+(NB+1)*ROWS), 
     $                             NB+1, DWORK(AEII+(NB+1)*ROWS), 
     $                             NB+1, DWORK(CIJ), NB+1, LINFO,
     $                             MACHINE, DWORK(IPW), LDWORK-IPW+1 )
                           ELSEIF( .NOT. RSIDE ) THEN
                              CALL RECGSYL( 5, SCALOC, ROWS, ROWS, 
     $                             DWORK(AEII), NB+1, DWORK(AEII), 
     $                             NB+1, DWORK(AEII+(NB+1)*ROWS), 
     $                             NB+1, DWORK(AEII+(NB+1)*ROWS), 
     $                             NB+1, DWORK(CIJ), NB+1, LINFO,
...
#ifdef USE_OMP
                        IF( RSIDE ) THEN 
                           CALL RECGLYDT_P( THREADS, 0, SCALOC, 
     $                          ROWS, 
     $                          DWORK(AEII), 
     $                          NB+1, DWORK(AEII+ROWS*(NB+1)), NB+1, 
     $                          DWORK(CIJ), NB+1, LINFO, MACHINE, 
     $                          DWORK(IPW), LDWORK-IPW+1 )
                        ELSE
                           CALL RECGLYDT_P( THREADS, 1, SCALOC, 
     $                          ROWS, 
     $                          DWORK(AEII), 
     $                          NB+1, DWORK(AEII+ROWS*(NB+1)), NB+1, 
     $                          DWORK(CIJ), NB+1, LINFO, MACHINE, 
...
#else
                        IF( RSIDE ) THEN 
                           CALL RECGLYDT( 0, SCALOC, ROWS, 
     $                          DWORK(AEII), 
     $                          NB+1, DWORK(AEII+ROWS*(NB+1)), NB+1, 
     $                          DWORK(CIJ), NB+1, LINFO, MACHINE, 
     $                          DWORK(IPW), LDWORK-IPW+1 )
                        ELSE
                           CALL RECGLYDT( 1, SCALOC, ROWS, 
     $                          DWORK(AEII), 
     $                          NB+1, DWORK(AEII+ROWS*(NB+1)), NB+1, 
     $                          DWORK(CIJ), NB+1, LINFO, MACHINE, 
     $                          DWORK(IPW), LDWORK-IPW+1 )
