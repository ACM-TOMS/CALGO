...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      SUBROUTINE PSYDTCON( TRANSA, TRANSB, ISGN, COMM, M, N, A, IA, 
     $                     JA, DESCA, B, IB, JB, DESCB, MB2, DWORK, 
...
C  M*N x M*N matrix Z^(-1) corresponding to the triangular SYDT Equation 
C
C     op(A) * X * op(B) +/- X = C, 
C
...
C        then Z * x = c corresponds to A * X * B +/- X = C     (1)
C     
C     If TRANSA = 'N' and TRANSB = 'T', 
C        then Z * x = c corresponds to A * X * B^T +/- X = C   (2)
C
C     If TRANSA = 'T' and TRANSB = 'N', 
C        then Z * x = c corresponds to A^T * X * B +/- X = C   (3)
C
C     If TRANSA = 'T' and TRANSB = 'T', 
C        then Z * x = c corresponds to A^T * X * B^T +/- X = C (4) 
C  
...
C                      LOCc(N+IROFFB), 
C            where BUFF is the workspace needed by PTRSYDTD (i.e., the 
C            same as for PGESYDTD with ASCHUR = BSCHUR = 'S').
C            
...
C            LIWORK >= MAX( 2*BUFF, DBA + DBB ) + LOCr(M*N+IROFFA), 
...
C            corresponds to the number of call to PDLACON and PGESYDTD.
C
C  Error indicator
C
...
C  Keywords
C  ========
C  Z-SYDT, 1-norm based lower bound, discrete-time Sylvester equation, 
...
C     .. Local Scalars ..
      ...
      CHARACTER*1        TRANSA1, TRANSA2, TRANSB1, TRANSB2
...
C     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, PDLACON, PGESYDTD, CHK1MAT
C     ..
...
C 
         IF( SHIFT ) THEN
            IF( M.EQ.N ) MWORK = MAX( AROWS*ACOLS,BROWS*BCOLS )
            IF( M.GT.N .AND. NPCOL.EQ.1 ) MWORK = AROWS*ACOLS
            IF( M.GT.N .AND. NPCOL.GT.1 ) MWORK = 
     $           MAX( AROWS*ACOLS, AROWS*BCOLS )
            IF( M.LE.N .AND. NPROW.EQ.1 ) MWORK = BROWS*BCOLS
            IF( M.LE.N .AND. NPROW.GT.1 ) MWORK = 
     $           MAX(BROWS*BCOLS , AROWS*BCOLS )
            IF( TRANA .AND. .NOT. TRANB ) 
     $           MWORK = MAX( AROWS*ACOLS, MWORK )
            IF( TRANB .AND. .NOT. TRANA )
     $           MWORK = MAX( BROWS*BCOLS, MWORK )
         ELSE
            MWORK = 0
         END IF
         BUFF = MAX( 1, EXMEMA + EXMEMB + EXMEMC ) + 2*(MB+1)*(NB+1) + 
     $          (MB+1)**2 + (NB+1)**2 + MAX( MB, NB ) + SIZE_C + MWORK
         WRK = BUFF + 2*CONWRK + SIZE_C
         IWRK = MAX( 2*BUFF, DBA + DBB ) + CONWRK
         IF( LQUERY ) THEN
            DWORK(1) = WRK
            IWORK(1) = IWRK
            INFO = 0
...
C
      IF( INFO.LT.0 ) THEN
         CALL PXERBLA( ICTXT, 'PSYDTCON', -INFO )
         RETURN
      END IF
C
...
C
      ...
      IPW  = IPX + CONWRK
C
C     Divide the integer workspace between PDLACON and PGESYDTD
C
      IPIW1 = 1
      IPIW2 = IPIW1 + CONWRK
C
...
C
         ...
         T = MPI_WTIME()
         IF( KASE.EQ.1 ) THEN
            CALL PGESYDTD( 'Solve', 'Schur', 'Schur', TRANSA1, TRANSB1, 
     $                     ISGN, COMM, M, N, A, IA, JA, DESCA, B, IB, 
     $                     JB, DESCB, DWORK(IPC), IC, JC, DESCC, MB2,
     $                     DWORK(IPW), LDWORK-IPW+1, IWORK(IPIW2), 
     $                     LIWORK-IPIW2+1, NOEXSY, SCALE, INFO )
         ELSEIF( KASE.EQ.2 ) THEN
            CALL PGESYDTD( 'Solve', 'Schur', 'Schur', TRANSA2, TRANSB2, 
     $                     ISGN, COMM, M, N, A, IA, JA, DESCA, B, IB, 
     $                     JB, DESCB, DWORK(IPC), IC, JC, DESCC, MB2,
     $                     DWORK(IPW), LDWORK-IPW+1, IWORK(IPIW2), 
     $                     LIWORK-IPIW2+1, NOEXSY, SCALE, INFO )
