...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC   
      SUBROUTINE PLYDTCON( OP, N, A, IA, JA, DESCA, NB2, DWORK, LDWORK, 
...
C     .. Scalar Arguments ..
      CHARACTER*1          OP
      INTEGER              N, IA, JA, LDWORK, LIWORK, NOITER, INFO,
     $                     NB2
...
C     If OP = 'N', then Z * x = c corresponds to A * X * A^T - X = C (1)
C     If OP = 'T', then Z * x = c corresponds to A^T * X * A - X = C (2)
C
C  Notes
C  =====
C
...
C                      LOCc(N+IROFFB), 
C            where BUFF is the workspace needed by PTRLYDTD (i.e., the 
C            same as for PGELYDTD with ASCHUR = 'S'). 
C            
...
C            corresponds to the number of call to PDLACON and PGELYDTD.
C
...
C  Keywords
C  ========
C  Z-LYDT, 1-norm based lower bound, continous-time Lyapunov equation, 
...
C     .. Local Scalars ..
     ...
     $                   XSIZE, ROUNDS, XPCOL, XCOLS, GKASE, GKASE_RST,
     $                   NPCOL0
...
C     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, PDLACON, PGELYDTD, PDMATADD
C     ..
...
C 
         BUFF = MAX( 1, EXMEMA + EXMEMC ) + 4*(NB+1)**2 + NB + SIZE_C
...
C
      IF( INFO.LT.0 ) THEN
         CALL PXERBLA( ICTXT, 'PLYDTCON', -INFO )
         RETURN
      END IF
C
...
C
         ...
         T = MPI_WTIME()
         IF( KASE.EQ.1 ) THEN
            CALL PGELYDTD( 'Solve', 'Nonsymmetric', LOP1, 'Schur', N, 
     $                     A, IA, JA, DESCA, DWORK(IPC), IC, JC, DESCC, 
     $                     NB2, DWORK(IPW), LLDWORK, IWORK, LIWORK, 
     $                     NOEXSY, SCALE, INFO )
         ELSEIF( KASE.EQ.2 ) THEN
            CALL PGELYDTD( 'Solve', 'Nonsymmetric', LOP2, 'Schur', N, 
     $                     A, IA, JA, DESCA, DWORK(IPC), IC, JC, DESCC, 
     $                     NB2, DWORK(IPW), LLDWORK, IWORK, LIWORK, 
     $                     NOEXSY, SCALE, INFO )
         END IF
