...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      SUBROUTINE PGLYDTCON( OP, N, A, IA, JA, DESCA, E, IE, JE, DESCE, 
     $                      NB2, DWORK, LDWORK, IWORK, LIWORK, EST, 
     $                      NOITER, INFO )
C 
...
C     .. Scalar Arguments ..
      CHARACTER*1          OP
      INTEGER              N, IA, JA, IE, JE, LDWORK, LIWORK, NOITER, 
     $                     INFO, NB2
      DOUBLE PRECISION     EST
C     ..
C     .. Array Arguments ..
      INTEGER              DESCA( * ), DESCE( * ), IWORK( * )
      DOUBLE PRECISION     A( * ), E( * ), DWORK( * )
C 
C  Purpose
C  =======
C  The subroutine gives a 1-norm based bound of the spectral norm of the
C  N*N x N*N matrix Z^(-1) corresponding to the triangular generalized 
C  Lyapunov Equation 
C
C     op(A) * X * op(A^T) - op(E) * X * op(E^T) = C, 
C
C  where A, E, C and X are NxN matrices.    
C  The matrix pair (A,E) have to be in generalized Schur form.
C
C  Notes
C  =====
C
...
C            for updates of the matrix C in PTRGLYDTD (see the references 
C            for details).
...
C                      LOCc(N+IROFFB), 
C            where BUFF is the workspace needed by PTRGLYDTD (i.e., the 
C            same as for PGEGLYDTD with ASCHUR = 'S'). 
C            
...
C            corresponds to the number of call to PDLACON and PGEGLYDTD.
C
...
C  Keywords
C  ========
C  Z-GLYDT, 1-norm based lower bound, generalized discrete-time Lyapunov 
C  equation, sep^-1, conditionnumber
C
C  =====================================================================
C
C     .. Parameters ..
...
C     .. Local Scalars ..
     ...
     $                   CCOLS, IPW, IPV, IPC, SND, IPX, XCSRC, DBAE,
     $                   LLDWORK, LLDV, NN, NPROCS, IPX2, IPIW1, IPIW2,
     $                   IROFFAE, LIA, LJA, AERSRC, AECSRC, AECOLS,
     $                   CROWS, AEROWS, IC, JC, VECDIM, NB, BUFF, 
     $                   DIMVEC, EXMEMAE, EXMEMC, CONWRK, EAST, WEST, 
     $                   INDXX, XSIZE, ROUNDS, XPCOL, XCOLS, GKASE,
     $                   GKASE_RST, NPCOL0
...
C     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, PDLACON, PGEGLYDTD, PDMATADD
C     ..
...
C     
      ...
      END IF
      IF( INFO.EQ.0 ) THEN
         CALL CHK1MAT( N, 2, N, 2, IE, JE, DESCE, 10, INFO )
      END IF
C     
...
C     
      ...
      END IF
      IF( INFO.EQ.0 ) THEN
         IF( DESCE( MB_ ).NE.DESCE( NB_ ) ) INFO = -(100*10 + MB_)
      END IF
      IF( INFO.EQ.0 ) THEN
         IF( DESCE( MB_ ).NE.DESCA( NB_ ) ) INFO = -(100*10 + MB_)
      END IF
C
...
C
C     Compute IROFFAE, etc.
C
         IROFFAE = MOD( IA - 1, DESCA(MB_) )
         CALL INFOG2L( IA, JA, DESCA, NPROW, NPCOL, MYROW, MYCOL,
     $                 LIA, LJA, AERSRC, AERSRC )
         AEROWS = NUMROC( N+IROFFAE, DESCA( MB_ ), MYROW, AERSRC, NPROW)
         AECOLS = NUMROC( N+IROFFAE, DESCA( NB_ ), MYCOL, AERSRC, NPCOL)
         CROWS = AEROWS
         CCOLS = AECOLS
C
...
C
         CALL DESCINIT( DESCC, N+IROFFAE, N+IROFFAE, DESCA(MB_), 
     $                  DESCA(NB_), AERSRC, AERSRC, ICTXT,  
     $                  MAX( 1, CROWS ), INFO )
         IC = 1 + IROFFAE
         JC = 1 + IROFFAE
         VECDIM = (N+IROFFAE)**2
...
C
      IF( INFO.LT.0 ) THEN
         CALL PXERBLA( ICTXT, 'PGLYDTCON', -INFO )
         RETURN
      END IF
C
...
C
     ...
     $                    EAST )
            ...
            XCOLS = NUMROC( N + IROFFAE, NB, XPCOL, AECSRC, NPCOL )
            XSIZE = CROWS * XCOLS
...
C
         ...
         T = MPI_WTIME()
         IF( KASE.EQ.1 ) THEN
            CALL PGEGLYDTD( 'Solve', 'Nonsymmetric', LOP1, 'Schur', N, 
     $                      A, IA, JA, DESCA, E, IE, JE, DESCE, 
     $                      DWORK(IPC), IC, JC, DESCC, NB2, DWORK(IPW), 
     $                      LLDWORK, IWORK, LIWORK, NOEXSY, SCALE, 
     $                      INFO )
         ELSEIF( KASE.EQ.2 ) THEN
            CALL PGEGLYDTD( 'Solve', 'Nonsymmetric', LOP2, 'Schur', N, 
     $                      A, IA, JA, DESCA, E, IE, JE, DESCE, 
     $                      DWORK(IPC), IC, JC, DESCC, NB2, DWORK(IPW), 
     $                      LLDWORK, IWORK, LIWORK, NOEXSY, SCALE, 
     $                      INFO )
