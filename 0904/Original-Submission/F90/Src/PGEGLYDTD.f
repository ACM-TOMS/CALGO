...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      SUBROUTINE PGEGLYDTD( JOB, SYMM, OP, AESCHR, N, A, IA, JA, DESCA, 
...
C  Purpose
C  =======
C  The subroutine solves the general generalized discrete-time
...
C  method for solving the general generalized discrete-time Lyapunov 
...
C  Limitations
C  ===========
C  In contrary to SLICOTs SG03AX this routine do not scale against 
...
C  choice is controlled by logical parameter GLYDT_PDORGQR, see below.
C
C  Keywords
C  ========
C  QZ-algorithm, generalized Schur form, generalized discrete-time 
...
C     ..Parameters ..
     ...
     $                     WRKSPQ = -1, MAXBULG = 1, 
     $                     GLYDT_PDORGQR = .FALSE., ZERO = 0.0D0, 
...
C     
     ...
     $                   PDGEMM, PDLACPY, PDHEXTR, PTRGLYDTD, DESCINIT,
...
C     
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PGEGLYDTD', -INFO )
         RETURN
      END IF
C
...
C     GLYDT_PDORGQR. See also "Known issues" above.
C
      IF( GLYDT_PDORGQR ) THEN
         IF( .NOT. SCHRAE .AND. SOLVE ) THEN
...
C
      T3 = MPI_WTIME()
      CALL PTRGLYDTD( CSYMM, OP, N, A, IA, JA, DESCA, E, IE, JE, DESCE, 
