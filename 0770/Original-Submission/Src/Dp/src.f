        SUBROUTINE DBVSIS (X,Y,NP,N,K,OPT,D0,DNP,D20,D2NP,CONSTR,EPS,
     *                     BETA,BETAI,RHO,RHOI,KMAX,MAXSTP,XTAB,NTAB,
     *                     SBOPT,Y0OPT,Y1OPT,Y2OPT,ERRC,ERRE,D,D2,
     *                     DIAGN,Y0TAB,Y1TAB,Y2TAB,WORK,NWORK)

C  DBVSIS is merely a support routine which calls DBVSSC and DBVSSE
...
C  The use of DBVSIS is not recommended when more evaluations of the
C  same spline are required; in this case it is better to separately
C  call DBVSSC and then DBVSSE repeatedly.
C  For an explanation of input and output parameters, the user is
C  referred to the comments of DBVSSC and DBVSSE.
     ...
     *          Y0OPT,Y1OPT,Y2OPT,DIAGN(0:NP-1),ERRC,ERRE,NWORK
        ...
        DOUBLE PRECISION X(0:NP),Y(0:NP),D0,DNP,D20,D2NP,XTAB(0:NTAB),
...
C  ---------------------------------------------------------------------
        ...
        SUBROUTINE DBVSSC (X,Y,NP,N,K,OPT,D0,DNP,D20,D2NP,CONSTR,EPS,
     *                     BETA,BETAI,RHO,RHOI,KMAX,MAXSTP,ERRC,D,D2,
     *                     DIAGN,WORK,NWORK)

C  -------------------------------------------------
C            Lines 49-549 are comment lines.
C            The actual code begins at line 555.
C  -------------------------------------------------

C  ABSTRACT:
C
C  DBVSSC is designed to compute the coefficients (first and, if
...
C  DBVSSC furnishes the input parameters for DBVSSE, which, in turn,
...
C  CALLING SEQUENCE:
C
C       CALL DBVSSC (X,Y,NP,N,K,OPT,D0,DNP,D20,D2NP,CONSTR,EPS,BETA,
...
C                            subroutine DTDC using subroutine DAL2DP 
...
C                            in input by the user using DAL2DP; note
...
C            DBVSSC resets it to 0.0001 which has turned out to be a
...
C            below. If kmax.LE.0, DBVSSC resets it to 10 .
...
C            If maxstp.LE.0, DBVSSC resets it to 10 .
...
C            their shape (see subroutine DMSK1 for details). So, on out-
...
C            On output, D is computed by the routine DAL2 if  p=1  and
C            is computed by the routine DAL2DP if  p=2  or  p=3 .
C
C
C  OUTPUT PARAMETERS
C
...
C            the main subroutines  DBVSSC and DBVSSE .
...
C  the comments in subroutine DCSDRC.
C
...
C              computed ?
C
C  Let DPRJ1: RxR -> R and DPRJ2: RxR -> R be, respectively, the 
C  projection maps from uv-plane onto the u-axis and v-axis and let us 
C  denote with  B(i) := DPRJ1(D(i)) :
C
C      ALGORITHM A1[B0].
C        1. Set A(0):=B(0); J:=np.
C        2. For i=1,2,...,np
C           2.1. Set A(i):= DPRJ2( D(i-1).INT.{ A(i-1) x B(i) } ) .
...
C           2.1. Choose d(i).IN.DPRJ1( D(i).INT.{ A(i) x { d(i+1) }}).
C        3. Stop.
C
C  For more theoretical details about A1 and A2 see \1\ , and for
C  practical details see subprograms DALG1, DAL2, DAL2DP. In the latter
...
C  subroutine DSTINF) which contains all the data needed for the
C  construction of the operators DPRJ0, DPRJ1 and DPRJ2.
C
...
C  interval  (x(i),x(i+1)) , i=0,1,...,np , stored in the array INFO.
     ...
     *          DIAGN(0:NP-1),NWORK,I1,I2,I3,I4,I5,I6,I7,I8,I9,I10
        ...
        DOUBLE PRECISION X(0:NP),Y(0:NP),D0,DNP,D20,D2NP,EPS,BETA,BETAI,
...
C  DBVSSC is essentially an interfacing routine which relieves the
C  user of a longer calling sequence. The structure of the method can 
C  be seen in DBVC and in the subroutines called.

        CALL DBVC (X,Y,NP,N,K,OPT,D0,DNP,D20,D2NP,CONSTR,WORK(I1),
...
C  ---------------------------------------------------------------------
        ...
        SUBROUTINE DBVSSE (X,Y,NP,N,K,XTAB,NTAB,SBOPT,Y0OPT,Y1OPT,Y2OPT,
     *                     ERRC,D,D2,Y0TAB,Y1TAB,Y2TAB,ERRE,WORK,NWORK)

C  -------------------------------------------------
C            Lines 621-754 are comment lines.
C            The actual code begins at line 760.
C  -------------------------------------------------

C  ABSTRACT:
C
C  DBVSSE is designed to evaluate the interpolating, shape-preserving
C  spline computed in subroutine DBVSSC.
C
C
C  REMARK:
C
...
C  be previously computed in subroutine DBVSSC. Then, given a set of
C  tabulation points  xtab(i) , i=0,1,...,ntab , DBVSSE computes the
...
C  CALLING SEQUENCE:
C
C        CALL DBVSSE (X,Y,NP,N,K,XTAB,NTAB,SBOPT,Y0OPT,Y1OPT,Y2OPT,
...
C            x(i).LT.x(i+1), i=0,1,...,np , checked in DBVSSC.
C  Y       : floating array, of bounds 0:NP, containing the data
C            ordinates  y(i), i=0,1,...,np.
C  NP      : integer variable, defining the number of interpolation
C            points. Restriction: np.GE.2 , checked in DBVSSC.
C  N       : integer variable, containing the degree of s.
C            Restriction: n.GE.3 , checked in DBVSSC
C  K       : integer variable, containing the class of continuity of s.
...
C            execution of subroutine DBVSSC.
C  D       : floating array, of bounds 0:NP, containing the first
...
C            the status of the code. DBVSSE has only two levels of error
C            (see DBVSSC for comparison): success and severe error,
...
C                     error, which means that the output of DBVSSC is 
C                     meaningless. Check the input parameters of DBVSSC.
...
C  -------------------------------------------------
     ...
     *          I1,I2,I3,I4,I5,I6,I7,I8,I9,I10
        ...
        DOUBLE PRECISION X(0:NP),Y(0:NP),XTAB(0:NTAB),Y0TAB(0:NTAB),
...
C  DBVSSE is essentially an interfacing routine which relieves the
C  user of a longer calling sequence. The structure of the method can
C  be seen in DBVE and in the subroutines called.

        CALL DBVE (X,Y,NP,N,K,XTAB,NTAB,SBOPT,Y0OPT,Y1OPT,Y2OPT,
...
C  DALG1 implements the algorithm A1[B(0)] described in subr. DBVSSC.
C
...
C      More precisely, A(i) = [a2(1,i),a2(2,i)] .
        ...
        DOUBLE PRECISION A1(1:2,0:NP),INFO(1:COMM+PART*NP+NP+1),EPS,
     *                   A2(1:2,0:NP)

        SAVE FL0
        DOUBLE PRECISION FL0
        DATA FL0/0.0D00/
...
C  Step 2.1.
                ...
                CALL DPRJ1 (A2(1,I-1),A2(2,I-1),A1(1,I),A1(2,I),I,INFO,
...
C  DALG1D computes the sequence of sets A(i), i=0,1,...,np, implementing
...
C  input parameter INFO is described in DSTINF; the input parameters A1
C  and A2 are described in subprogram DALG1.
C
C  Item of possible interest is:
C
C  ERRC1  : Integer parameter, containing a control variable which is
C           then used in subr. DFPSVF
...
C           errc1 = 1 - A(i) is empty for some i.
        ...
        DOUBLE PRECISION DSTAR,A1(1:2,0:NP),INFO(1:COMM+PART*NP+NP+1),
     *                   EPS,A2(1:2,0:NP)
...
C  ---------------------------------------------------------------------
        ...
        SUBROUTINE DALG3 (INFO,NP,COMM,PART,OPT,D0,DNP,EPS,KMAX,MAXSTP,
     *                    BETA,BETAI,A1,A2,D,ERRC,DIAGN)

C  DALG3 computes a sequence of slopes ( d(0), d(1), ..., d(np) ) which
...
C  The input parameter INFO is described in subprogram DSTINF.
     ...
     *          I,K,P,Q
        ...
        DOUBLE PRECISION INFO(1:COMM+PART*NP+NP+1),D0,DNP,EPS,BETA,
...
C  Start step 1: store the sets  B(i), i=0,1,...,np , into the array A1.
        ...
        DO 10 I=1,NP
            CALL DPRJ0 (I,INFO,COMM,PART,NP,A1(1,I-1),A1(2,I-1))
10      CONTINUE
        A1(1,NP)=INFO(4)
        A1(2,NP)=INFO(5)
...
C  Start step 2. Call DALG1 to compute the array A2 containing the
C  sets A(i) , i=0,1,...,np.
...
C  Compute  betai(phi(A(0).INT.beta(A(0))) .
            ...
            CALL DINTRS (MIN(BETA(A2(1,0)),BETA(A2(2,0))),
...
30          CONTINUE
            IF (DTST(A1(1,0),A1(2,0),A2(1,0),A2(2,0),EPS).OR.
     *                                             K.EQ.KMAX)  GO TO 50
...
C  is looked for.
            ...
            CALL DFPSVF (A1,A2,NP,INFO,COMM,PART,EPS,MAXSTP,
     *                   BETA,ERRC,DSTAR)
...
C  DAL2 computes a sequence of slopes (d(0),d(1),...,d(np)) implementing
...
C  described in DBVSSC; the input parameter INFO is described in DSTINF.
C
C  Item of possible interest is:
C
...
C         is the feasible interval for d(i) .
        ...
        DOUBLE PRECISION A2(1:2,0:NP),INFO(1:COMM+PART*NP+NP+1),
     *                   D(0:NP),P1,P2

        SAVE FL1D2
        DOUBLE PRECISION FL1D2
        DATA FL1D2/0.5D00/

        D(NP)=(A2(1,NP)+A2(2,NP))*FL1D2

        DO 10 I=NP,1,-1
            CALL DPRJ2(A2(1,I-1),A2(2,I-1),D(I),D(I),I,INFO,
     *                       COMM,PART,NP,P1,P2)
            D(I-1)=(P1+P2)*FL1D2
10      CONTINUE
...
C  DAL2DP links algorithm A2 and a dynamic programming scheme
...
C  is the sequence of optimal values, DAL2DP use the following dynamic
...
C  in subprogram DSTINF and the input parameter A2 is described in DAL2.
...
C  the admissible domain.
        ...
        DOUBLE PRECISION A2(1:2,0:NP),INFO(1:COMM+PART*NP+NP+1),D(0:NP),
     ...
     *                   P1,P2,D0,D1,DSL
        ...
        DO 5 J=0,NSUBD
            PART0(J)=A2(1,0)+J*H0
5       CONTINUE
        PART0(NSUBD+1)=DSL(A2(1,0),A2(2,0),INFO(IND))
        D(0)=DSL(A2(1,0),A2(2,0),INFO(IND))
        DO 10 J=0,NSUBD+1
...
10      CONTINUE
        ...
        DO 40 I=1,NP
            ...
            DO 15 J=0,NSUBD
                PART1(J)=A2(1,I)+J*H1
15          CONTINUE
            PART1(NSUBD+1)=DSL(A2(1,I),A2(2,I),INFO(IND+I))
            PSI1MN=FLE30
            DO 20 J=0,NSUBD+1
                D1=PART1(J)
                CALL DPRJ2 (A2(1,I-1),A2(2,I-1),D1,D1,I,INFO,
     *                      COMM,PART,NP,P1,P2)
                D0=DSL(P1,P2,D(I-1))
                IF (H0.GT.FL0) THEN
                    JD0=DMNIND(D0,PART0)
...
C  ---------------------------------------------------------------------
        ...
        DOUBLE PRECISION FUNCTION DBL(X,N,L,X0,XN,TB,FLAG,LAUX0)

C  DBL computes the value assumed by the n-degree Bernstein polynomial
...
C          In the case  flag=0  DBL  assumes to perform the first
C          evaluation of the polynomial, and computes the values
C          tb(i)*l(i) , i=0,1,...,n . In the case  flag=1  DBL
...
C          intermediate results.
        ...
        DOUBLE PRECISION X,L(0:N),X0,XN,TB(0:N),LAUX0(0:N),XNMX,XMX0,
     *                   AUX,FL1
...
C  ---------------------------------------------------------------------
        ...
        DOUBLE PRECISION FUNCTION DBL1(X,N,L,X0,XN,TB,FLAG,LAUX1)

C  DBL1 computes the value assumed by the first derivative of an
...
C          terms used for computing the Bernstein polynomial.
C  FLAG  : integer variable, containing a control parameter.
C          In the case  flag=0  DBL1  assumes to perform the first
C          evaluation of the polynomial, and computes the values
C          tb(i)*(l(i+1)-l(i)) , i=0,1,...,n-1 . In the case  flag=1
C          DBL1 assumes to perform subsequent evaluations, and uses 
...
C          intermediate results.
        ...
        DOUBLE PRECISION X,L(0:N),X0,XN,TB(0:N-1),LAUX1(0:N-1),XNMX,
     *                   XMX0,AUX,FL1
...
C  ---------------------------------------------------------------------
        ...
        DOUBLE PRECISION FUNCTION DBL2(X,N,L,X0,XN,TB,FLAG,LAUX2)

C  DBL2 computes the value assumed by the second derivative of an
C  n-degree Bernstein polynomial of a function  l  in the interval
C  (x0,xn)  at the point  x .
C  The evaluation is made using a Horner scheme, and the instructions
C  which do not depend upon  x  are executed under the control of
...
C          terms used for computing the Bernstein polynomial.
C  FLAG  : integer variable, containing a control parameter.
C          In the case  flag=0  DBL2  assumes to perform the first
C          evaluation of the polynomial, and computes the values
C          tb(i)*(l(i+2)-2*l(i+1)+l(i)) , i=0,1,...,n-2 .
C          In the case  flag=1  DBL2 assumes to perform subsequent 
...
C          intermediate results.
        ...
        DOUBLE PRECISION X,L(0:N),X0,XN,TB(0:N-2),LAUX2(0:N-2),XNMX,
     *                   XMX0,AUX,FL1
...
10          CONTINUE
        ...
        DO 20 I=N-3,0,-1
            AUX=XNMX*AUX
            DBL2=LAUX2(I)*AUX+XMX0*DBL2
20      CONTINUE

        DBL2=N*(N-1)*DBL2/(XN-X0)**N
...
C  ---------------------------------------------------------------------
        ...
        SUBROUTINE DBNTAB (X,Y,NP,XTAB,NTAB,Y0OPT,Y1OPT,Y2OPT,N,K,D,D2,
     *                     TB,L,LAUX0,LAUX1,LAUX2,Y0TAB,Y1TAB,Y2TAB)

C  DBNTAB evaluates the spline and/or its first derivative and/or its
...
C  see subroutines DTRMB, DLSPIS.


        INTEGER NP,NTAB,Y0OPT,Y1OPT,Y2OPT,N,K,IND,J

        DOUBLE PRECISION X(0:NP),Y(0:NP),XTAB(0:NTAB),D(0:NP),D2(0:NP),
     *                   TB(1:N*(N+1)/2+N),L(0:N),LAUX0(0:N),LAUX1(0:N),
     *                   LAUX2(0:N),Y0TAB(0:NTAB),Y1TAB(0:NTAB),
     *                   Y2TAB(0:NTAB),DBL,DBL1,DBL2

        DO 40 J=0,NTAB

C  Call subprogram  DBSEAR  to compute the index  ind  such that
C       x(ind).LE.xtab(j).LT.x(ind+1) .

            CALL DBSEAR(X,NP,XTAB(J),IND)

C  Call subprogram  DLSPIS  to compute the linear shape-preserving
C  interpolating spline  l  at
C      x(ind)+p*(x(ind+1)-x(ind))/n , p=0,1,...,n .

            CALL DLSPIS(X,Y,D,D2,NP,N,K,IND,L)

            IF(Y0OPT.EQ.1) THEN

C  Evaluate the spline at  xtab(j) .

                Y0TAB(J)=DBL(XTAB(J),N,L,X(IND),X(IND+1),
     *                       TB(N*(N+1)/2),0,LAUX0)
            END IF
...
C  Evaluate the first derivative of the spline at  xtab(j) .
                ...
                Y1TAB(J)=DBL1(XTAB(J),N,L,X(IND),X(IND+1),
     *                        TB((N-1)*N/2),0,LAUX1)
            END IF
...
C  Evaluate the second derivative of the spline at  xtab(j) .
                ...
                Y2TAB(J)=DBL2(XTAB(J),N,L,X(IND),X(IND+1),
...
C  point  xtab , DBSEAR finds the index  ind  such that
C
C              x(ind) .LE. xtab .LT. x(ind+1)
C
C  using a standard binary search. DBSEAR  sets  ind=0  or  ind=np-1
...
C  ---------------------------------------------------------------------
        ...
        SUBROUTINE DBVC (X,Y,NP,N,K,OPT,D0,DNP,D20,D2NP,CONSTR,INFO,
     *                   COMM,PART,EPS,KMAX,MAXSTP,A1,A2,DAUX2,
     *                   DAUX3,BETA,BETAI,RHO,RHOI,D,D2,ERRC,DIAGN)

C  DBVC checks input parameters and computes the required spline.
C
...
C  The other parameters are described in the called subprograms.
     ...
     *          ERRC,DIAGN(0:NP-1),P,Q,R,I
        ...
        DOUBLE PRECISION X(0:NP),Y(0:NP),D0,DNP,D20,D2NP,
...
C  Call subprogram DSTINF to set the information array INFO.
...
30      CONTINUE
        ...
        CALL DSTINF (OPT,D0,DNP,CONSTR,N,K,X,Y,D,NP,COMM,PART,EPS,
     *                     BETA,BETAI,DAUX2,DAUX3,INFO,ERRC,DIAGN)

C  Call subprogram DALG3 to compute the array D containing the first
C  derivative at initial points.

        CALL DALG3 (INFO,NP,COMM,PART,OPT,D0,DNP,EPS,KMAX,MAXSTP,
     *              BETA,BETAI,A1,A2,D,ERRC,DIAGN)
...
C  possible, to boundary conditions.
            ...
            CALL DSCDRC (N,X,Y,D,OPT,NP,EPS,D20,D2NP,
...
C  ---------------------------------------------------------------------
        ...
        SUBROUTINE DBVE (X,Y,NP,N,K,XTAB,NTAB,SBOPT,Y0OPT,Y1OPT,
     *                   Y2OPT,D,D2,ERRC,TB,L,LAUX0,LAUX1,LAUX2,
     *                   Y0TAB,Y1TAB,Y2TAB,ERRE)

C  DBVE checks input parameters and evaluates the required spline.
C
...
C  Call subprogram DTRMB to compute the binomial terms needed
...
C  sbopt=1:  sequential search is required.
            ...
            CALL DSQTAB (X,Y,NP,XTAB,NTAB,Y0OPT,Y1OPT,Y2OPT,N,K,D,D2,
     *                   TB,L,LAUX0,LAUX1,LAUX2,Y0TAB,Y1TAB,Y2TAB)
        ELSE

C  sbopt=2: binary search is required.

            CALL DBNTAB (X,Y,NP,XTAB,NTAB,Y0OPT,Y1OPT,Y2OPT,N,K,D,D2,
     *                   TB,L,LAUX0,LAUX1,LAUX2,Y0TAB,Y1TAB,Y2TAB)
        END IF
...
C  ---------------------------------------------------------------------
        ...
        SUBROUTINE DFPSVF (A1,A2,NP,INFO,COMM,PART,EPS,MAXSTP,
     *                     BETA,ERRC,DSTAR)

C  DFPSVF finds, if possible, dstar.IN.[a1(1,0),a1(2,0)] such that
...
C  A2 are described in  DALG1.
        ...
        DOUBLE PRECISION A1(1:2,0:NP),A2(1:2,0:NP),
...
C  direction with respect to the middle of the interval.
        ...
        DO 20 STEP=1,MAXSTP
            ...
            DO 10 I=1,SUBSTP,2
                DSTAR=MID-I*H
                CALL DALG1D (DSTAR,A1,NP,INFO,COMM,PART,EPS,
     *                       A2,ERRC1)
                IF (ERRC1.EQ.0.AND.(A2(1,NP)-EPS.LE.BETA(DSTAR).AND.
     *              BETA(DSTAR).LE.A2(2,NP)+EPS))               RETURN
                DSTAR=MID+I*H
...
C  DINTRS computes the intersection of the two intervals  [a,b]
C  and [c,d]. [p1,p2] is the result. The output  p1.GT.p2 means that
C  the intervals are disjoint. DINTRS assumes  a.LE.b  and  c.LE.d .


        DOUBLE PRECISION A,B,C,D,P1,P2
...
C  DLSPIS   evaluates the control points of the Bernstein-Bezier net
C  l:=l(x) of the interpolating spline  s:=s(x) , s.IN.BS(n;k) in the
C  interval  [x(ind),x(ind+1)] . For a description of the function  l
C  see the comments in subroutines  DBVSSC and DSCDRC. Here we only
...
C          of the control points.
        ...
        DOUBLE PRECISION X(0:NP),Y(0:NP),D(0:NP),D2(0:NP),L(0:N),H,
     *                   ALPHA,Q1,Q2,FL1,FL2,FL4

        SAVE FL1,FL2,FL4
        DATA FL1,FL2,FL4/1.0D00,2.0D00,4.0D00/
...
C  ---------------------------------------------------------------------
        ...
        DOUBLE PRECISION FUNCTION DMDIAN(A,B,C)
...
C  the other two.
        ...
        DMDIAN=MIN(MAX(A,B),MAX(B,C),MAX(C,A))
...
C  DMNIND finds the index of the component of the array PART closest
C  to d .
        ...
        DOUBLE PRECISION D,PART(0:NSUBD+1),AUX(0:NSUBD+1),MINDIS

        SAVE FL30
        DOUBLE PRECISION FL30
        DATA FL30/1.0D30/
...
C  ---------------------------------------------------------------------
        ...
        DOUBLE PRECISION FUNCTION DMNMOD(A,B)

C  Given two real numbers a and b, DMNMOD returns the number between
C  a and b which is closest to zero.


        DOUBLE PRECISION A,B,FL1,FL2

        SAVE FL1,FL2
        DATA FL1,FL2/1.0D00,2.0D00/

        DMNMOD=((SIGN(FL1,A)+SIGN(FL1,B))/FL2)*MIN(ABS(A),ABS(B))
...
C  DMSK1 compares the constraints required in input by the user and
C  stored in the array CONSTR with the shape of the data, stored by
C  DSTINF in the array INFO. If the required and real shapes do not
C  agree, DMSK1 resets both INFO and CONSTR with the 'intersection'
...
C  in DALG3 will be based on these new values for selecting the domains
...
C  routine DSTINF; CONSTR, COMM, PART, NP are explaained in subroutine
C  DBVSSC.
        ...
        DOUBLE PRECISION INFO(1:COMM+PART*NP+NP+1)
...
C  ---------------------------------------------------------------------
        ...
        SUBROUTINE DMSK2(INFO,COMM,PART,IND1,NP,D0,DNP,EPS,
     *                   ERRC,DIAGN)
...
C  DSTINF and DBVSSC respectively.


        INTEGER COMM,PART,IND1,NP,ERRC,DIAGN(0:NP-1)

        DOUBLE PRECISION INFO(1:COMM+PART*NP+NP+1),D0,DNP,EPS,P1,P2,A,B

        CALL DPRJ0 (1,INFO,COMM,PART,NP,P1,P2)
        ...
        CALL DPRJ0(NP,INFO,COMM,PART,NP,A,B)
        CALL DPRJ1(A,B,INFO(4),INFO(5),NP,INFO,COMM,PART,NP,P1,P2)
...
C  Given the integer i , DPRJ0 computes the set B(i) performing the
...
C  parameter INFO is described in subroutine DSTINF.
C
C  OUTPUT PARAMETERS:
C
...
C        resulting interval.
        ...
        DOUBLE PRECISION INFO(1:COMM+PART*NP+NP+1),P1,P2,N,K,DEL
...
C        resulting interval.
        ...
        DOUBLE PRECISION A,B,C,D,INFO(1:COMM+PART*NP+NP+1),P1,P2,N,K,
     *                   DEL,F1,F2,F3,X
...
C  Given the set s=[a,b]x[c,d] and the integer i, DPRJ2 performs the
C  intersection of S with the domain D(i) and the projection of the
...
C  ---------------------------------------------------------------------
        ...
        SUBROUTINE DSCDRC (N,X,Y,D,OPT,NP,EPS,D20,D2NP,RHO,RHOI,
     *                     A1,A2,H,D2,ERRC)

C  DSCDRC computes the sequence  d2(i) , i=0,1,...,np , of second
...
C  by subroutine DALG3. Let A, B and C be the second, third and 
...
C  input ones D2,ERRC are documented in subroutine DBVSSC.
        ...
        DOUBLE PRECISION X(0:NP),Y(0:NP),D20,D2NP,EPS,D(0:NP),D2(0:NP),
     *                   H(0:NP-1),A1(1:2,0:NP),A2(1:2,0:NP),A,B,C,DD,E,
     *                   F,G,HH,ALPHA,GAMMA,DIFF2,P1,P2,Q1,Q2,FL0,FL1,
     *                   FL2,FL4,RHO,RHOI,DSL

        SAVE FL0,FL1,FL2,FL4
        DATA FL0,FL1,FL2,FL4/0.0D00,1.0D00,2.0D00,4.0D00/
...
C  derivatives.
        ...
        DO 30 I=1,NP-1
            ...
            CALL DINTRS (A1(1,I),A1(2,I),A2(1,I),A2(2,I),P1,P2)
...
C  satisfied by admissible derivatives.
            ...
            CALL DINTRS(A1(1,NP),A1(2,NP),Q1,Q2,P1,P2)
...
C  admissible and the boundary interval.
                ...
                D2(NP)=DSL(A1(1,NP),A1(2,NP),(P1+P2)/FL2)
                D2(0)=DSL(A1(1,0),A1(2,0),RHOI(D2(NP)))
...
C  values. Otherwise take simply the boundary values.
     ...
     *           D2NP.LT.A1(1,NP)-EPS .OR. D2NP.GT.A1(2,NP)+EPS ) THEN
            ...
            D2(0)=DSL(A1(1,0),A1(2,0),D20)
            D2(NP)=DSL(A1(1,NP),A1(2,NP),D2NP)
...
C  ---------------------------------------------------------------------
        ...
        DOUBLE PRECISION FUNCTION DSL(A,B,C)

C  Given the interval [a,b] and the number c, dsl is c if c belongs
...
C  ---------------------------------------------------------------------
        ...
        SUBROUTINE DSQTAB (X,Y,NP,XTAB,NTAB,Y0OPT,Y1OPT,Y2OPT,N,K,D,D2,
     *                     TB,L,LAUX0,LAUX1,LAUX2,Y0TAB,Y1TAB,Y2TAB)

C  DSQTAB evaluates the spline and/or its first derivative and/or its
C  second derivative at the points  xtab(j) , j=0,1,...,ntab  using
...
C  ---------------------------------------------------------------------
        ...
        SUBROUTINE DSTINF (OPT,D0,DNP,CONSTR,N,K,X,Y,D,NP,COMM,PART,EPS,
     *                     BETA,BETAI,DAUX2,DAUX3,INFO,ERRC,DIAGN)

C  DSTINF computes the information needed in the other parts of the
...
C          interpolant (see subr. DTDC described later), if p=4 it 
...
C          of INFO is not referenced.
     ...
     *          I,R,Q,P,IND1,IND2,IND3
        ...
        DOUBLE PRECISION D0,DNP,X(0:NP),Y(0:NP),D(0:NP),EPS,BETA,BETAI,
     *                   DAUX2(1:NP-1),DAUX3(0:NP-1),
     *                   INFO(1:COMM+PART*NP+NP+1),D2IM1,D2I,IAUX0,
     *                   IAUXNP

        SAVE FL0,FLEM4,FL2
        DOUBLE PRECISION FL0,FLEM4,FL2
        DATA FL0,FLEM4,FL2/0.0D00,1.0D-04,2.0D00/
...
C  those given by the array CONSTR
        ...
        IF (R.EQ.4) THEN
            CALL DMSK1(INFO,CONSTR,COMM,PART,IND1,NP)
        END IF
...
C  is compared with the boundary conditions
        ...
        IF (Q.EQ.3) THEN
            CALL DMSK2(INFO,COMM,PART,IND1,NP,D0,DNP,EPS,
     *                       ERRC,DIAGN)
        END IF
...
C  subr. DTDC, is stored in the fourth part of INFO.

        ELSE IF (P.EQ.3) THEN
            CALL DTDC(NP,X,COMM,PART,INFO,DAUX2,DAUX3)
...
C  Given the initial points ( x(i), y(i) ) , i=0,1,...,np , DTDC
...
C  parameter INFO is described in subr. DSTINF .
C
C  The computed values are stored in the last part of INFO.
        ...
        DOUBLE PRECISION X(0:NP),INFO(1:COMM+PART*NP+NP+1),DD2(1:NP-1),
     *                   DD3(0:NP-1),Q1,Q2,TIT,F,P1,P2,TI,TMAX,TMIN,SI,
     *                   Q12,Q32,E1,E2,E3,D1,D2,DMNMOD,DMDIAN

        SAVE FL0,FL2,FL3
        DOUBLE PRECISION FL0,FL2,FL3
        DATA FL0,FL2,FL3/0.0D00,2.0D00,3.0D00/
...
C  ds(i) : initialization
             ...
             E1=DMNMOD(DD3(I-2),DD3(I-1))
             E2=DMNMOD(DD3(I-1),DD3(I))
             E3=DMNMOD(DD3(I),DD3(I+1))
             Q1=INFO(IND+I-1)+(X(I)-X(I-1))*DMNMOD(DD2(I-1)+E1*
     *          (X(I)-X(I-2)),DD2(I)+E2*(X(I)-X(I+1)))
             Q2=INFO(IND+I)-(X(I+1)-X(I))*DMNMOD(DD2(I)+E2*
...
C  ds(1): initialization
        ...
        E1=DMNMOD(DD3(0),DD3(1))
        E2=DMNMOD(DD3(1),DD3(2))
        Q1=DMDIAN(INFO(IND),Q12,Q32)
        Q2=INFO(IND+1)-(X(2)-X(1))*DMNMOD(DD2(1)+E1*(X(1)-X(0)),
     *                                    DD2(2)+E2*(X(1)-X(3)))
        F=(Q1+Q2)/FL2
...
C  refinement
        ...
        TIT=DMNMOD(Q1,Q2)
        D2=DMNMOD(DD2(1),DD2(2))
        P1=INFO(IND)+DD2(1)*(X(1)-X(0))
        P2=INFO(IND+1)+D2*(X(1)-X(2))
        TI=DMNMOD(P1,P2)
        SI=DMNMOD(INFO(IND),INFO(IND+1))
        TMIN=MIN(FL0,FL3*SI,FL3*TI/FL2,TIT)
        TMAX=MAX(FL0,FL3*SI,FL3*TI/FL2,TIT)
        INFO(IND1+1)=F+DMNMOD(TMIN-F,TMAX-F)

C  ds(np-1): initialization

        E1=DMNMOD(DD3(NP-3),DD3(NP-2))
        E2=DMNMOD(DD3(NP-2),DD3(NP-1))
        Q1=INFO(IND+NP-2)+(X(NP-1)-X(NP-2))*DMNMOD(DD2(NP-2)+
...
C  ds(np):
     ...
     *     DD3(NP-2)*(X(NP)-X(NP-2))*(X(NP)-X(NP-1))
        INFO(IND1+NP)=DMDIAN(INFO(IND+NP-1),Q1,Q2)
...
C  DTRMB    computes the binomial terms
...
C          elements   TB(i*(i+1)/2),...,TB((i*(i+1)/2)+i) .
        ...
        DOUBLE PRECISION TB(1:N*(N+1)/2+N),FL1
...
C  DTST checks if two intervals [a,b] and [c,d] differ less than eps.
C  DTST assumes  a.LE.b  and  c.LE.d .


        DOUBLE PRECISION A,B,C,D,EPS

        DTST=ABS(A-C).LE.EPS.AND.ABS(B-D).LE.EPS
