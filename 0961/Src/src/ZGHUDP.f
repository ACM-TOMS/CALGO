      SUBROUTINE ZGHUDP( COMPQ, ORTH, N, A, LDA, DE, LDDE, B, LDB, FG,
...
C             = 1: QZ iteration failed in the routine DGHUSP (QZ
...
C     DGHUSP is applied to compute the structured Schur form, i.e., the
C     factorizations
C
C     ~                     (  S11  S12  )
...
C
C     Second, the routine ZGHUXP is applied, to compute a unitary matrix
C     ~
C     Q, such that
C
C                        ~    ~
...
C     Systems, Magdeburg, Dec. 2011.
C
C     REVISIONS
C
C     V. Sima, Jul. 2013, June 2014.
...
C     .. Parameters ..
      ...
      INTEGER            DKIND
      PARAMETER          ( DKIND = KIND( ONE ) )
      COMPLEX(DKIND) ::  CZERO, CONE, CIMAG
...
C     .. Array Arguments ..
      ...
      COMPLEX(DKIND) ::  A( LDA, * ), B( LDB, * ), DE( LDDE, * ),
     $                   FG( LDFG, * ), Q( LDQ, * ), ZWORK( * )
C
C     .. Local Scalars ..
     ...
     $                   ITAU, IW, IW1, IWRK, J, J1, J2, JM1, JP2, M,
     $                   MINDB, MINDW, MINZW, N2, NB, NC, NN,
     $                   OPTDW, OPTZW
...
C     .. External Subroutines ..
      EXTERNAL           DCOPY, DLACPY, DSCAL, ZGHUXP, DGHUSP, XERBLA,
...
C
         IF( LQUERY ) THEN
            CALL DGHUSP( JOB, CMPQ, N2, DWORK, N, DWORK, N, DWORK, N,
...
C
C     Set the pointers for the inputs and outputs of DGHUSP.
...
C
C     STEP 1: Apply DGHUSP to transform the extended pencil to real
C             skew-Hamiltonian/skew-Hamiltonian Schur form.
C
C             Real workspace:
C                  need   4*N*N + 2*N + N*N + N, if COMPQ = 'N';
C                         8*N*N + 2*N + 3*N*N,   if COMPQ = 'C'.
C                  prefer larger.
C
      INFO = NBL
      CALL DGHUSP( JOB, CMPQ, N2, DWORK( IA ), N, DWORK( IDE ), N,
...
C
      ...
      DO 180 J = 1, N
         DO 170 I = 1, J
            A( I, J ) = CMPLX( DWORK( IW ), KIND = DKIND )
            IW = IW + 1
  170    CONTINUE
         IW = IW + N - J
  180 CONTINUE
C
      ...
      DO 200 J = 1, N
         DO 190 I = 1, J - 1
            DE( I, J ) = CMPLX( DWORK( IW ), KIND = DKIND )
            IW = IW + 1
  190    CONTINUE
         DE( J, J ) = CZERO
         IW = IW + N - J + 1
...
C
      ...
      DO 220 J = 1, N
         DO 210 I = 1, MIN( J + 1, N )
            B( I, J ) = CMPLX( DWORK( IW ), KIND = DKIND )
            IW = IW + 1
  210    CONTINUE
         IW = IW + N - J - 1
  220 CONTINUE
C
      ...
      DO 240 J = 1, N
         DO 230 I = 1, J - 1
            FG( I, J ) = CMPLX( DWORK( IW ), KIND = DKIND )
            IW = IW + 1
  230    CONTINUE
         FG( J, J ) = CZERO
         IW = IW + N - J + 1
...
C
      ...
      DO 260 J = 1, N2
         DO 250 I = 1, N2
            Q( I, J ) = CMPLX( DWORK( IW ), KIND = DKIND )
            IW = IW + 1
  250    CONTINUE
  260 CONTINUE
C
...
C
            ...
            TMP         =  A( J+1, J )
            A( J+1, J ) = CZERO
            CALL ZHGEQZ( 'S', 'I', 'I', 2, 1, 2, B( J, J ), LDB,
     $                   A( J, J ), LDA, ZWORK( IEV ), ZWORK( IEV+2 ),
     $                   ZWORK( IQ ), 2, ZWORK( IQ2 ), 2, ZWORK( IWRK ),
...
C
  ...
  300 CONTINUE
C
C     STEP 2: Apply ZGHUXP to reorder the eigenvalues with strictly
C             negative real part to the top.
C
      CMPQ = 'Update'
      INFO = NBL
      CALL ZGHUXP( CMPQ, N2, A, LDA, DE, LDDE, B, LDB, FG, LDFG, Q, LDQ,
     $             NEIG, TOL, INFO )
C
      IF( QR )
     $   NEIG = NEIG/2
