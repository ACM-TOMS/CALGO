...
C     .. External Subroutines ..
     ...
     $                   DLACPY, DORGQR, DSWAP, MA02ED, DGHFXC, DGHURV,
     $                   DGHFYR, XERBLA
C
...
C
      ...
      ELSE
         ...
         END IF
         MINDB = I*NN
         MINDW = J*NN + MAX( NN + MAX( M + 252, 432 ),
     $                            MAX( IS + 48, 171 ) )
      END IF
      LQUERY = LDWORK.EQ.-1
C
...
C
         IF( LQUERY ) THEN
            CALL DGHURV( JOB, CMQI, CMQI, CMUI, CMUI, N, DWORK, N,
...
C
            IF( LCMP ) THEN
               ...
               ELSE
                  IW    = 0
               END IF
               CALL DGHFYR( CMQI, CMPI, CMPI, N, DWORK, N, DWORK, N,
...
C
      ...
      END IF
C
C     STEP 1: Apply DGHURV to compute the generalized symplectic
C             URV decomposition.
C
C     Set the pointers for the inputs and outputs of DGHURV.
C
...
C                         w  = 3/2*N**2+MAX(N, 48)+6, if  COMPQ = 'N'
C                                                     and COMPU = 'N';
C                         w  =   3*N**2+MAX(N, 48)+6, otherwise;
C                         prefer larger.
...
C
      ...
      IWRK = IT + NN
C
      CALL DGHURV( JOB, CMQI, CMQI, CMUI, CMUI, N, Z, LDZ, DWORK( IH ),
...
C                         prefer larger.
C     Integer workspace:  need   N + 48.
C     Logical workspace:  need   M.
C
C     Save Z12, T12, and H12, since they are overwritten by DGHFYR.
C
      IW = IT + NM
      IS = IH + NM
      IF( LCMPU ) THEN
...
C
  ...
  100 CONTINUE
C
      CALL DGHFYR( CMQI, CMPI, CMPI, N, DWORK( IT ), N, Z, LDZ,
     $             DWORK( IH ), N, DWORK( IQ4 ), N, DWORK( IU3 ), N,
     $             DWORK( IQ3 ), N, IWORK( MP1 ), LIWORK-M,
...
C
  ...
  120 CONTINUE
C
C     STEP 4: Apply DGHFXC to reorder the eigenvalues with strictly
...
C     Integer workspace:  need   5*N/2 + 1.
C
      CALL MA02ED( 'Upper', N, DWORK( IT ), N )
C
      CALL DGHFXC( CMQI, CMUI, N2, Z, LDZ, DWORK( IT ), N,
     $             DWORK( IZ12 ), N, DWORK( IH ), N, DWORK( IH12 ), N,
     $             Q, LDQ, U, LDU, U( 1, NP1 ), LDU, NEIG, IWORK( MP1 ),
     $             LIWORK-M, DWORK( IWRK ), LDWORK-IWRK+1, INFO )
      IF( INFO.GT.0 )
     $   RETURN
C
      IF( QR )
...
C
               DO 170 J = MP1, M + NEIG
                  IWORK( J ) = 0
  170          CONTINUE
               CALL DGEQP3( N, NEIG, Q, LDQ, IWORK( MP1 ), DWORK,
...
C
               DO 220 J = MP1, M + NEIG
                  IWORK( J ) = 0
  220          CONTINUE
               CALL DGEQP3( N, NEIG, U, LDU, IWORK( MP1 ), DWORK,
     $                      DWORK( IWRK ), LDWORK-IWRK+1, INFO )
            END IF
            OPTDW = MAX( OPTDW, INT( DWORK( IWRK ) ) + IWRK - 1 )
C
