...
C     Oct. 2012, July 2013, June 2014, July 2014.
...
C     .. External Subroutines ..
     ...
     $                   MB01KD, MB01LD, DGHUTR, DGHUXC, DGHUYR, XERBLA
C
...
C
      IFO = 1
C
C     STEP 1: Apply DGHUTR to transform the pencil to real
C             skew-Hamiltonian/Hamiltonian Schur form.
C
C     Set the computation option and pointers for the inputs and outputs
C     of DGHUTR. If possible, array Q is used as vectorized workspace.
C
...
C
      IF( LINIQ ) THEN
         ...
         IB   = 2*IC2 + 1
         IC2  =   IC2 + 1
         CALL DGHUTR( 'T', CMPQ, CMPQ, N, A, LDA, DE, LDDE, B, LDB, FG,
     ...
     $                DWORK( IWRK ), LDWORK-IWRK+1, INFO )
      ELSE
         ...
         IC2  = IB  + MM
         IWRK = IC2 + MM
         CALL DGHUTR( 'E', CMPQ, CMPQ, N, A, LDA, DE, LDDE, B, LDB, FG,
...
C
C     STEP 4: Apply DGHUXC to reorder the eigenvalues with strictly
...
C
      IWRK = IH12 + NN
C
      CALL DGHUXC( CMPQ, N2, DWORK( IS11 ), N, DWORK( IS12 ), N,
