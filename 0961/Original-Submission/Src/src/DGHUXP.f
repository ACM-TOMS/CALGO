      SUBROUTINE DGHUXP( COMPQ, N, A, LDA, D, LDD, B, LDB, F, LDF, Q,
...
C             = 1: error occured during execution of DGHUEX;
C             = 2: error occured during execution of DGHUEY.
C
C     METHOD
C
...
C     elementary subroutines DGHUEX and DGHUEY are called for the
...
C     CONTRIBUTOR
C
C     V. Sima, Research Institute for Informatics, Bucharest, Nov. 2010.
C     M. Voigt, Max Planck Institute for Dynamics of Complex Technical
C     Systems, Magdeburg, Jan. 2012.
C
C     REVISIONS
C
C     V. Sima, July 2013, June 2014, July 2014.
C     M. Voigt, July 2013.
C
C     KEYWORDS
C
...
C     .. Local Scalars ..
     ...
     $                   J, K, LDW, M, M1, MM, MP, NC, NCOL, NROWS,
     $                   OPTDW, R, SDIM, UPDS
...
C     .. External Subroutines ..
     ...
     $                   DLASET, DSCAL, MB01LD, MB01RU, MB01RX, DGHUEX,
     $                   DGHUEY, XERBLA
C
...
C
      INTEGER            BLKUP, ICOL, ICOLS, JE, JS, NB
      INTEGER,           ALLOCATABLE :: IBWORK( : )
...
C
C     Set pointers for the inputs and outputs of DGHUEX.
C
      IQ1   = 1
      IQ2   = IQ1 + 16
      IA    = IQ2 + 16
...
C              B(ib1:ib3-1,ib1:ib3-1) to DWORK as inputs for DGHUEX.
...
C
               CALL DGHUEX( 'Triangular', DIM1, DIM2, PREC, DWORK( IB ),
...
C
               ...
               IBWORK( ICOL+2 ) = IB2
               IBWORK( ICOL+3 ) = IB3
               IBWORK( ICOL+4 ) = IBWORK( ICOL ) + SDIM*SDIM
               CALL DLACPY( 'Full', SDIM, SDIM, DWORK( IQ2 ), SDIM,
     $                      DBWORK( IBWORK( ICOL ) ), SDIM )
               ICOL = ICOL + 4
C
...
C           WHILE( JE.GT.2 ) DO
   40       CONTINUE
            IF( JE.GE.MM+1 ) THEN
               ...
               JS = IBWORK( ICOL+3 )
               DO 50 J = JE, MM + 1, -1
                  DIM1 = IBWORK( ICOL+2 ) - IBWORK( ICOL+1 )
                  DIM2 = IBWORK( ICOL+3 ) - IBWORK( ICOL+2 )
                  SDIM = DIM1 + DIM2            
                  CALL DGEMM(  'Transpose', 'No Transpose', SDIM, NCOL,
     $                         SDIM, ONE, DBWORK( IBWORK( ICOL ) ),
     $                         SDIM, A( IBWORK( ICOL+1 ), JS ), LDA,
     $                         ZERO, DWORK( IWRK2 ), SDIM )
                  CALL DLACPY( 'Full', SDIM, NCOL, DWORK( IWRK2 ), SDIM,
     $                         A( IBWORK( ICOL+1 ), JS ), LDA )
                  ICOL = ICOL + 4
                  NC = NC + DIM1
                  IF( NC.LE.NB ) THEN
                     NCOL = NCOL+DIM1
                     JS = IBWORK( ICOLS+3 ) - NCOL
                     BLKUP = BLKUP+1
                  END IF
   50          CONTINUE
               JE =  JE - BLKUP
   ...
   80       CONTINUE
            IF( JE.GE.MM+1 ) THEN
               ...
               DO 90 J = JE, MM + 1, -1
                  ...
                  CALL DGEMM(  'Transpose', 'No Transpose', SDIM, NCOL,
     $                         SDIM, ONE, DBWORK( IBWORK( ICOL ) ),
     $                         SDIM, D( IBWORK( ICOL+1 ), JS ), LDD,
     $                         ZERO, DWORK( IWRK2 ), SDIM )
                  CALL DLACPY( 'Full', SDIM, NCOL, DWORK( IWRK2 ), SDIM,
     $                         D( IBWORK( ICOL+1 ), JS ), LDD )
                  ICOL = ICOL + 4
                  NC = NC + DIM1
                  IF( NC.LE.NB ) THEN
                     NCOL = NCOL+DIM1
  ...
  120       CONTINUE
            IF( JE.GE.MM+1 ) THEN
               ...
               DO 130 J = JE, MM + 1, -1
                  ...
                  CALL DGEMM(  'Transpose', 'No Transpose', SDIM, NCOL,
     $                         SDIM, ONE, DBWORK( IBWORK( ICOL ) ),
     $                         SDIM, B( IBWORK( ICOL+1 ), JS ), LDB,
     $                         ZERO, DWORK( IWRK2 ), SDIM )
                  CALL DLACPY( 'Full', SDIM, NCOL, DWORK( IWRK2 ), SDIM,
     $                         B( IBWORK( ICOL+1 ), JS ), LDB )
                  ICOL = ICOL + 4
                  NC = NC + DIM1
                  IF( NC.LE.NB ) THEN
                     NCOL = NCOL+DIM1
  ...
  160       CONTINUE
            IF( JE.GE.MM+1 ) THEN
               ...
               DO 170 J = JE, MM + 1, -1
                  ...
                  CALL DGEMM(  'Transpose', 'No Transpose', SDIM, NCOL,
     $                         SDIM, ONE, DBWORK( IBWORK( ICOL ) ),
     $                         SDIM, F( IBWORK( ICOL+1 ), JS ), LDF,
     $                         ZERO, DWORK( IWRK2 ), SDIM )
                  CALL DLACPY( 'Full', SDIM, NCOL, DWORK( IWRK2 ), SDIM,
     $                         F( IBWORK( ICOL+1 ), JS ), LDF )
                  ICOL = ICOL + 4
                  NC = NC + DIM1
                  IF( NC.LE.NB ) THEN
                     NCOL = NCOL+DIM1
...
C
C     Set pointers for the inputs and outputs of DGHUEY.
C
      IQUPLE = 1
      IAUPLE = IQUPLE + 16
      IBUPLE = IAUPLE +  8
...
C
         ...
         IQLORI = IQUPRI + DIM1
C
C        Generate input matrices for DGHUEY built of submatrices of A,
C        D, B, and F.
C
         IF( DIM1.EQ.2 ) THEN
...
C
         CALL DGHUEY( SDIM, DWORK( IAUPLE ), DIM1, DWORK( IBUPLE ),
  ...
  310    CONTINUE
         IF( JE.GE.MM ) THEN
            ...
            DO 320 J = JE, MM, -1
               ...
               SDIM = DIM1 + DIM2            
               CALL DGEMM(  'Transpose', 'No Transpose', SDIM, NCOL,
     $                      SDIM, ONE, DBWORK( IBWORK( ICOL ) ), SDIM,
     $                      A( IBWORK( ICOL+1 ), JS ), LDA, ZERO,
     $                      DWORK( IWRK2 ), SDIM )
               CALL DLACPY( 'Full', SDIM, NCOL, DWORK( IWRK2 ), SDIM,
     $                      A( IBWORK( ICOL+1 ), JS ), LDA )
               ICOL = ICOL + 4
