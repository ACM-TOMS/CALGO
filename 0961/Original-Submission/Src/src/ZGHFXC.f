...
C     elementary subroutines ZGHFEX and ZGHFEY are called for the
...
C     REVISIONS
C
C     V. Sima, Dec. 2010, Jan. 2011, June 2014.
...
C
C     .. Array Arguments ..
      COMPLEX(DKIND) ::  A( LDA, * ), B( LDB, * ), C( LDC, * ),
     $                   D( LDD, * ), F( LDF, * ), Q( LDQ, * ),
     $                   U1( LDU1, * ), U2( LDU2, * )
C
C     .. Local Scalars ..
      ...
      COMPLEX(DKIND) ::  CJF, SI1, SI2, SI3, TMP
C
C     .. Local Arrays ..
      DOUBLE PRECISION   DUM( 1 )
      COMPLEX(DKIND) ::  HLP( 2, 2 )
C
...
C     .. External Subroutines ..
      EXTERNAL           ZGHFEX, ZGHFEY, XERBLA, ZLASET, ZROT
C
C     .. Intrinsic Functions ..
      INTRINSIC          CONJG, DBLE, MAX, MIN, MOD
C
...
C
               CALL ZGHFEX( HLP, 2, A( J, J ), LDA, B( J, J ),
...
C        Perform eigenvalue exchange.
C
         CALL ZGHFEY( A( M, M ), D( M, M ), C( M, M ), B( M, M ),
...
C
            ...
            HLP( 2, 2 ) = CONJG( C( J+1, J+1 ) )
C
            CALL ZGHFEX( HLP, 2, A( J, J ), LDA, B( J, J ), LDB, CO1,
     $                   SI1, CO2, SI2, CO3, SI3 )
C
C           Update A, C, and D.
C
