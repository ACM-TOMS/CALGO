...
C     .. External Subroutines ..
      EXTERNAL           DCOPY, DLACPY, DSCAL, ZGHUXC, DGHUST, XERBLA,
...
C
         IF( LQUERY ) THEN
            CALL DGHUST( JOB, CMPQ, N2, DWORK, N, DWORK, N, DWORK, N,
...
C
C     Set the pointers for the inputs and outputs of DGHUST.
...
C
C     STEP 1: Apply DGHUST to transform the extended pencil to real
...
C                  prefer larger.
C
      CALL DGHUST( JOB, CMPQ, N2, DWORK( IA ), N, DWORK( IDE ), N,
...
C
  ...
  300 CONTINUE
C
C     STEP 2: Apply ZGHUXC to reorder the eigenvalues with strictly
C             negative real part to the top.
C
      CMPQ = 'Update'
      CALL ZGHUXC( CMPQ, N2, A, LDA, DE, LDDE, B, LDB, FG, LDFG, Q, LDQ,
     $             NEIG, TOL, INFO )
C
      IF( QR )
     $   NEIG = NEIG/2
