function [probs,funs,bcfuns,dfuns,dbcfuns,esolus,setoutputs,settolerancess]=bvpT9(ExtraArgs)
...
if nargin==1 && ~isempty(ExtraArgs)
    ...
    function F = fun(X,Z,ExtraArgs)
        ...
        F=zeros(size(Z));
        F(1,:) = Z(2,:);
        F(2,:) = -(4.0e0*X.*Z(2,:)+2.0e0*Z(1,:))./(lambda+X.*X);
    end
    ...
    function Df = dfun(X,Z,ExtraArgs)
        ...
        Df(1,2,:) = 1.0e0;
        Df(2,1,:) = -2.0e0/(lambda+X.*X);
        Df(2,2,:) = -4.0e0*X./(lambda+X.*X);
    end
    ...
    function Exact = esolu(X,ExtraArgs)
        ...
        Exact(1,:) = 1.0e0./(X.*X+lambda);
        Exact(2,:) = -(2.*X)./(X.^2 + lambda).^2;
    end
end