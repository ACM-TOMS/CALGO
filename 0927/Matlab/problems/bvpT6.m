function [probs,funs,bcfuns,dfuns,dbcfuns,esolus,setoutputs,settolerancess]=bvpT6(ExtraArgs)
...
if nargin==1 && ~isempty(ExtraArgs)
    ...
    function F = fun(X,Z,ExtraArgs)
        ...
        pix = pi*X;
        F(1,:) = Z(2,:);
        F(2,:) = (-X.*Z(2,:)-lambda*pi*pi.*cos(pix)-pi*X.*sin(pix))./lambda;
    end
    ...
    function Exact = esolu(X,ExtraArgs)
        ...
        pix = pi*X;
        Sqep = sqrt(2.e0*lambda);
        Exact(1,:) = cos(pix)+erf(X./Sqep)./erf(1.e0./Sqep);
        Exact(2,:) = 2^(1/2)./(pi^(1/2)*lambda^(1/2)*exp(X.^2./(2*lambda))*erf(2^(1/2)./(2*lambda^(1/2)))) - pi*sin(pi*X);
    end
end