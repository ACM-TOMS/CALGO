function [probs,funs,bcfuns,dfuns,dbcfuns,esolus,setoutputs,settolerancess]=bvpT16(ExtraArgs)
...
if nargin==1 && ~isempty(ExtraArgs)
    ...
    function Exact = esolu(X,ExtraArgs)
        ...
        Exact(1,:) = sin(pi*X./(2.0e0*sqrt(lambda)));
        Exact(2,:) = (pi*cos((pi*X)/(2*lambda^(1/2))))/(2*lambda^(1/2));
    end
end