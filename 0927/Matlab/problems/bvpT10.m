function [probs,funs,bcfuns,dfuns,dbcfuns,esolus,setoutputs,settolerancess]=bvpT10(ExtraArgs)
...
if nargin==1 && ~isempty(ExtraArgs)
    ...
    function Exact = esolu(X,ExtraArgs)
        ...
        Exact(1,:) = 1.0e0+erf(X./sqrt(2.0e0*lambda))./erf(1.0e0./sqrt(2.0e0*lambda));
        Exact(2,:) = 2^(1/2)./(pi^(1/2)*lambda^(1/2)*exp(X.^2./(2*lambda))*erf(2^(1/2)/(2*lambda^(1/2))));
    end
end