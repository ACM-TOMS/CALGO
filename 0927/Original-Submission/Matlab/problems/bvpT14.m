function [probs,funs,bcfuns,dfuns,dbcfuns,esolus,setoutputs,settolerancess]=bvpT14(ExtraArgs)
...
if nargin==1 && ~isempty(ExtraArgs)
    ...
    function F = fun(X,Z,ExtraArgs)
        ...
        pix = pi*X;
        F(1,:) = Z(2,:);
        F(2,:) = (Z(1,:)-lambda*pi*pi*cos(pix)-cos(pix))./lambda;
    end
    ...
    function Exact = esolu(X,ExtraArgs)
        ...
        Exact(1,:) = cos(pi*X)+exp(-(1.0e0+X)./sqrt(lambda))+...
            exp(-(1.0e0-X)./sqrt(lambda));
        Exact(2,:) = exp((X - 1)./lambda^(1/2))./lambda^(1/2) - pi*sin(pi*X) - 1./(lambda^(1/2)*exp((X + 1)./lambda^(1/2)));
    end
end