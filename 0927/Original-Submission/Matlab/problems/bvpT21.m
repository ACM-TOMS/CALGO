function [probs,funs,bcfuns,dfuns,dbcfuns,esolus,setoutputs,settolerancess]=bvpT21(ExtraArgs)
...
if nargin==1 && ~isempty(ExtraArgs)
    ...
    function F = fun(X,Z,ExtraArgs)
        ...
        F=zeros(size(Z));
        F(1,:) = Z(2,:);
        F(2,:) = (Z(1,:).*(1.e0+Z(1,:))-exp((-2.e0.*X)/lambda))./(lambda*lambda);
    end
    ...
    function Df = dfun(X,Z,ExtraArgs)
        ...
        Df(1,2,:) = 1.0e0;
        Df(2,1,:) = (1.e0+2.e0*Z(1,1,:))/(lambda*lambda);
    end
    ...
    function Exact = esolu(X,ExtraArgs)
        ...
        Exact(1,:) = exp(-X./(lambda));
        Exact(2,:) = -exp(-X./(lambda))./((lambda));
    end
end