function [probs,funs,bcfuns,dfuns,dbcfuns,esolus,setoutputs,settolerancess]=bvpT24(ExtraArgs)
...
if nargin==1 && ~isempty(ExtraArgs)
    ...
    function F = fun(X,Z,ExtraArgs)
        ...
        Ga = 1.4e0;
        F(1,:) = Z(2,:);
        F(2,:) = (((1.e0+Ga)/2.e0-lambda*Apx).*Z(1,:).*Z(2,:)-Z(2,:)./Z(1,:)-...
            (Apx./Ax).*(1.e0-(Ga-1.e0)*Z(1,:).^2/2.e0))./(lambda*Ax.*Z(1,:));
    end
    ...
    function Df = dfun(X,Z,ExtraArgs)
        ...
        Ga = 1.4e0;
        Df(1,2,:) = 1.0e0;
        Df(2,1,:) = ((2.e0*Z(2,1,:))./Z(1,1,:).^3+Apx./(Ax.*Z(1,1,:).^2)+...
            (Apx.*(Ga-1.e0))./(2.e0*Ax))./(lambda*Ax);
        Df(2,2,:) = (((1.e0+Ga)/2.e0-lambda*Apx).*Z(1,1,:)-1.e0./Z(1,1,:))./(lambda*Ax.*Z(1,1,:));
    end
