function [y,iflag,problem,rhs,rnsq,fty]=fixjac_a(problem,iorder,ltol,tol,t,y,lambda,fty,defcor,defnew,rhs,J)
%
...
%
    ...
    if rnsq>huge || ( iorder==8 && rnsq>xlarge )
        if problem.debug, fprintf('Large residual, rnsq = %g\n',rnsq); end
        iflag=-2;
        return
    end
    rhs=rhstri;
    ...
    while true
        ...
        if problem.debug, fprintf('iter %d rnsq %g\n',iter,rnsq); end
        if rnsq<=epsmch
            iflag=0;
            return;
        end
        ...
        rnold=rnsq;
        [rhstri,rnsq,fty,problem]=rhscal_a(problem,h,t,ytrial,lambda,defcor);
        ...
        if iter>=lmtfrz || rnsq>rnold/rngrow
            ...
            if problem.debug, fprintf('failure of fixed jacobian, iflag = %d\n',iflag); end
            return
        end
        ...
        if errok
            if problem.debug, fprintf('fixed jacobian convergence, iter: %d, rnsq: %g\n',iter,rnsq); end
            iflag=0;
            return;
        end
    end
