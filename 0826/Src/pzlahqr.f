...
*           ZLAHQR     -> Serial QR used to determine shifts and eigenvalues
...
*           ZLAMSH     -> Sends multiple shifts through a small submatrix to
*                         see how the consecutive subdiagonals change (if
*                         PZLACONSB indicates we can start a run in the middle)
*           PZLAWIL    -> Given the shift, get the transformation
*           PZLACP3    -> Parallel array to local replicated array copy & back.
*           ZLAREF     -> Row/column reflector applier.  Core routine here.
...
*     .. Local Scalars ..
      LOGICAL            SKIP
      INTEGER            CONTXT, DOWN, HBL, I, I1, I2, IAFIRST,
     $                   IBULGE, ICBUF, ICOL, ICOL1, ICOL2, IDIA,
     $                   IERR, II, IRBUF, IROW, IROW1, IROW2, ISPEC,
     $                   ISTART, ISTARTCOL, ISTARTROW, ISTOP, ISUB,
     $                   ISUP, ITERMAX, ITMP1, ITMP2, ITN, ITS,
     $                   IZBUF, J, JAFIRST, JBLK, JJ, K, KI, L, LCMRC,
     $                   LDA, LDZ, LEFT, LIHIH, LIHIZ, LILOH, LILOZ,
     $                   LOCALI1, LOCALI2, LOCALK, LOCALM, M, MODKM1,
     $                   MYCOL, MYROW, NBULGE, NH, NODE, NPCOL,
     $                   NPROW, NQ, NR, NUM, NZ, RIGHT, ROTN, UP, 
     $                   VECSIDX
      DOUBLE PRECISION   CS, OVFL, S, SMLNUM, ULP, UNFL
      COMPLEX*16         CDUM, H00, H10, H11, H12, H21, H22, H33, H44,
     $                   H43H34, SN, SUM, T1, T1COPY, T2, T3, V1SAVE, 
     $                   V2, V2SAVE, V3, V3SAVE
*     ..
*     .. Local Arrays ..
...
*     .. External Subroutines ..
     ...
     $                   PZLACP3, PZLASMSUB, PZLAWIL, ZCOPY, ZGEBR2D,
     $                   ZGEBS2D, ZGERV2D, ZGESD2D, ZGSUM2D, ZLAHQR,
     $                   ZLAMSH, ZLAREF, ZLARFG
*     ..
*     .. Intrinsic Functions ..
*
...
*
   ...
   20       CONTINUE
         ELSE
            CALL ZLAHQR( .FALSE., .FALSE., 2*JBLK, 1, 2*JBLK, S1, 2*
     $                  IBLK, WORK( IRBUF+1 ), 1, 2*JBLK, Z, LDZ, IERR )
*
...
*
         CALL INFOG1L( I1, HBL, NPROW, MYROW, IAFIRST, LOCALI1, 
     $                 ICOL1 )
...
*              communication can be broken into several parts for efficient
*              parallelism:
...
*
   ...
   50                CONTINUE
     ...
     $                                       DCONJG( V2 )
                        SMALLA( J, 4, KI ) = SMALLA( J, 4, KI ) - SUM*
     $                                       DCONJG( V3 )
   60                CONTINUE
                  END IF
               END IF
...
*
   ...
   70                CONTINUE
     ...
     $                        T3*SMALLA( J, 5, KI )
                        ...
                        SMALLA( J, 4, KI ) = SMALLA( J, 4, KI ) - SUM*
     $                                       DCONJG( V2 )
                        SMALLA( J, 5, KI ) = SMALLA( J, 5, KI ) - SUM*
     $                                       DCONJG( V3 )
   80                CONTINUE
                  END IF
               END IF
...
*
     ...
     $                          LDA + IROW1, LDA
                           SUM = DCONJG( T1 )*A( J ) + DCONJG( T2 )*
     $                           A( J+1 ) + DCONJG( T3 )*A( J+2 )
                           A( J ) = A( J ) - SUM
                           A( J+1 ) = A( J+1 ) - SUM*V2
                           A( J+2 ) = A( J+2 ) - SUM*V3
   90                   CONTINUE
...
*
     ...
     $                                      WORK( IRBUF+1 ), 2, UP,
     $                                      MYCOL )
                              DO 290 J = ICOL1, ICOL2
                                 SUM = DCONJG( T1 )*WORK( IRBUF+2*( 
     $                                 J-ICOL1 )+1 ) + DCONJG( T2 )*
...
*
     ...
     $            GO TO 380
               IF( ( MYROW.NE.ICURROW( KI ) ) .AND.
     $             ( DOWN.NE.ICURROW( KI ) ) )GO TO 380
               ISTART = MAX( K1( KI ), M )
               ISTOP = MIN( K2( KI ), I-1 )
               IF( ( ISTART.EQ.ISTOP ) .OR.
     $             ( MOD( ISTART-1, HBL ).GE.HBL-2 ) .OR.
     $             ( ICURROW( KI ).NE.MYROW ) ) THEN
                  DO 370 K = ISTART, ISTOP
                     V2 = WORK( VECSIDX+( K-1 )*3+1 )
                     V3 = WORK( VECSIDX+( K-1 )*3+2 )
                     T1 = WORK( VECSIDX+( K-1 )*3+3 )
                     NR = MIN( 3, I-K+1 )
...
*
     ...
     $                   ( MOD( K-1, HBL ).LT.HBL-2 ) ) THEN
                     ...
                     CALL INFOG1L( I1, HBL, NPROW, MYROW, IAFIRST, 
     $                             IROW1, IROW2 )
                     IROW2 = NUMROC( ITMP1, HBL, MYROW, IAFIRST, 
     $                               NPROW )
                     IF( ( MOD( K-1, HBL ).EQ.HBL-2 ) .AND.
     $                   ( NPCOL.GT.1 ) ) THEN
...
*
     ...
     $                                      IROW2-IROW1+1, MYROW, LEFT )
                              ...
                              DO 430 J = IROW1, IROW2
                                 SUM = T1*WORK( II+J ) + T2*WORK( JJ+J )
     $                                 + T3*Z( ICOL1+J )
                                 WORK( II+J ) = WORK( II+J ) - SUM
                                 WORK( JJ+J ) = WORK( JJ+J ) - SUM*
     $                                                     DCONJG( V2 )
                                 Z( ICOL1+J ) = Z( ICOL1+J ) - SUM*
     $                                                     DCONJG( V3 )
  430                         CONTINUE
                              IF( ISTART.EQ.ISTOP ) THEN
                                 CALL ZGESD2D( CONTXT, IROW2-IROW1+1, 2,
     ...
     $                                         LEFT )
                              ...
                              DO 440 J = IROW1, IROW2
                                 SUM = T1*WORK( II+J ) + T2*Z( J+ICOL1 )
     $                                 + T3*Z( J+ICOL1+LDZ )
                                 WORK( II+J ) = WORK( II+J ) - SUM
...
*
  ...
  560                      CONTINUE
     ...
     $                                      WORK( IRBUF+1 ), 1, UP,
     $                                      MYCOL )
                              DO 570 J = LILOH, LIHIH
                                 SUM = DCONJG( T1 )*WORK( 
     $                                 IRBUF+J-LILOH+1 ) + DCONJG( T2 )
     $                                 *A( ( J-1 )*LDA+ITMP1 )
...
*
         ...
         IF( JBLK.LE.2*IBLK ) THEN
            ...
            CALL ZLAHQR( .FALSE., .FALSE., JBLK, 1, JBLK, S1, 2*IBLK,
