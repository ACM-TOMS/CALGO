...
*  ===================
*
*  TOTMEM   INTEGER, default = 4000000
...
*  MEM      COMPLEX*16 array, dimension ( TOTMEM / DBLESZ )
*
...
*     .. Parameters ..
     ...
     $                     RSRC_ = 7, CSRC_ = 8, LLD_ = 9 )
      ...
      PARAMETER          ( DBLESZ = 16, TOTMEM = 220000000,
...
*     .. Local Scalars ..
     ...
     $                   IPOSTPAD, IPREPAD, IPW, IPWR, IPZ, IPVL, J, K,
     $                   KFAIL, KPASS, KSKIP, KTESTS, LDA, LDZ, LWORK,
     $                   M,MYCOL, MYROW, N, NB, NGRIDS, NMAT, NNB, NOUT,
...
*     .. Local Arrays ..
     ...
     $                   PVAL( NTESTS ), QVAL( NTESTS )
      DOUBLE PRECISION   CTIME( 2 ), WTIME( 2 ), RWORK( 5000 ),
     $                   RESULT( 2 )
...
*     .. External Subroutines ..
     ...
     $                   PZGET22, PZEVCINFO, SLBOOT, SLCOMBINE,
     $                   SLTIMER
*     ..
*     .. External Functions ..
      INTEGER            ILCM, NUMROC
...
*
     ...
     $                       MYCOL, IROW, ICOL, II, JJ )
                  IF( MYROW.EQ.II .AND. MYCOL.EQ.JJ ) THEN
                     MEM( IPWR-1+JJJ ) = MEM( IPA-1+(ICOL-1)*LDA+IROW )
                  ELSE 
                     MEM( IPWR-1+JJJ ) = ZERO
                  ENDIF
               END DO
               CALL ZGSUM2D( CONTXT, 'All', ' ', N, 1, MEM( IPWR ), 1, 
     $                      -1, -1 )
*
               CALL SLBOOT( )
