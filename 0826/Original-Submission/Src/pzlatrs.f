      SUBROUTINE PZLATRS( UPLO, TRANS, DIAG, NORMIN, N, A, IA, JA, 
...
*  =======
*
*  PZLATRS solves one of the triangular systems
*
...
*     .. Local Scalars ..
      ...
      INTEGER            CONTXT, CSRC, I, ICOL, ICOLX, IMAX, IROW, IROWX, 
     $                   ITMP1, ITMP1X, ITMP2, ITMP2X, J, JFIRST, JINC, 
     $                   JLAST, LDA, LDX, MB, MYROW, MYCOL, NB, NPCOL,
     $                   NPROW, RSRC
...
*     .. External Functions ..
      LOGICAL            LSAME
      INTEGER            IDAMAX
      DOUBLE PRECISION   PDLAMCH, PDZASUM
      COMPLEX*16         ZLADIV
      EXTERNAL           IDAMAX, LSAME, PDLAMCH, PDZASUM, ZLADIV
*     ..
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, DSCAL, DGSUM2D, INFOG2L, 
     $                   PDLABAD, PXERBLA, PZAMAX, PZAXPY, PZDOTC,
     $                   PZDOTU, PZDSCAL, PZLASET, PZSCAL, PZTRSV,
     $                   ZGEBS2D, ZGEBR2D
*     ..
*     .. Intrinsic Functions ..
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( CONTXT, 'PZLATRS', -INFO )
         RETURN
      END IF
*
...
*
      SMLNUM = PDLAMCH( CONTXT, 'Safe minimum' )
      BIGNUM = ONE / SMLNUM
      CALL PDLABAD( CONTXT, SMLNUM, BIGNUM )
      SMLNUM = SMLNUM / PDLAMCH( CONTXT, 'Precision' )
      BIGNUM = ONE / SMLNUM
      SCALE = ONE
...
*
            CNORM( 1 ) = ZERO
            DO 10 J = 2, N
               CNORM( J ) = PDZASUM( J-1, A, IA, JA+J-1, DESCA, 1 )
   10       CONTINUE
         ELSE
*
...
*
            DO 20 J = 1, N - 1
               CNORM( J ) = PDZASUM( N-J, A, IA+J, JA+J-1, DESCA, 1 )
   20       CONTINUE
            CNORM( N ) = ZERO
         END IF
         CALL DGSUM2D( CONTXT, 'Row', ' ', N, 1, CNORM, 1, -1, -1 )
      END IF
*
...
C              TJJS = A( J, J )
     ...
     $                      MYROW, MYCOL, IROW, ICOL, ITMP1, ITMP2 )
               IF( ( MYROW.EQ.ITMP1 ) .AND. ( MYCOL.EQ.ITMP2 ) ) THEN
                  ...
                  CALL ZGEBS2D( CONTXT, 'All', ' ', 1, 1, TJJS, 1 )
               ELSE
                  CALL ZGEBR2D( CONTXT, 'All', ' ', 1, 1, TJJS, 1, 
     $                          ITMP1, ITMP2 )
               END IF
               TJJ = CABS1( TJJS )
*
...
*
         CALL PZTRSV( UPLO, TRANS, DIAG, N, A, IA, JA, DESCA, X, IX, 
     $                JX, DESCX, 1 )
      ELSE
*
...
C              XJ = CABS1( X( J ) )
     ...
     $                      MYCOL, IROWX, ICOLX, ITMP1X, ITMP2X )
               IF(( MYROW.EQ.ITMP1X ) .AND. ( MYCOL.EQ.ITMP2X )) THEN
                  XJTMP = X( IROWX )
                  CALL ZGEBS2D( CONTXT, 'All', ' ', 1, 1, XJTMP, 1 )
               ELSE
                  CALL ZGEBR2D( CONTXT, 'All', ' ', 1, 1, XJTMP, 1, 
     $                          ITMP1X, ITMP2X )
               END IF
               XJ = CABS1( XJTMP )
               IF( NOUNIT ) THEN
...
*
                  CALL PZLASET( ' ', N, 1, CZERO, CZERO, X, IX, JX, 
     $                          DESCX )
                  IF(( MYROW.EQ.ITMP1X ) .AND. ( MYCOL.EQ.ITMP2X )) THEN
                     X( IROWX ) = CONE
                  ENDIF
...
*                 Scale x by 1/2.
*
                  CALL PZDSCAL( N, HALF, X, IX, JX, DESCX, 1 )
                  XJTMP = XJTMP*HALF
                  SCALE = SCALE*HALF
               END IF
*
...
*
     ...
     $                            X, IX+J, JX, DESCX, 1 )
                  ...
                  IF( MYCOL.EQ.ITMP2X ) THEN
                     CALL ZGEBS2D( CONTXT, 'Row', ' ', 1, 1, CSUMJ, 1 )
                  ELSE
                     CALL ZGEBR2D( CONTXT, 'Row', ' ', 1, 1, CSUMJ, 1, 
     $                             MYROW, ITMP2X )
                  END IF
               ELSE
*
*                 Otherwise, scale column of A by USCAL before dot product.
*                 Below is not the best way to do it.
*
                  IF( UPPER ) THEN
C                    DO 130 I = 1, J - 1
C                       CSUMJ = CSUMJ + ( A( I, J )*USCAL )*X( I )
C 130                CONTINUE
                     ZDUM = DCONJG( USCAL )
...
C                 XJ = CABS1( X( J ) )
                  XJTMP = XJTMP - CSUMJ
                  XJ = CABS1( XJTMP )
C                  IF( ( MYROW.EQ.ITMP1X ) .AND. ( MYCOL.EQ.ITMP2X ) )
C     $               X( IROWX ) = XJTMP
                  IF( NOUNIT ) THEN
C                    TJJS = A( J, J )*TSCAL
                     CALL INFOG2L( IA+J-1, JA+J-1, DESCA, NPROW, NPCOL, 
...
*
                  REC = REC*HALF
                  IF( NOUNIT ) THEN
C                    TJJS = DCONJG( A( J, J ) )*TSCAL
                     CALL INFOG2L( IA+J-1, JA+J-1, DESCA, NPROW, NPCOL, 
     $                         MYROW, MYCOL, IROW, ICOL, ITMP1, ITMP2 )
                     IF( ( MYROW.EQ.ITMP1 ) .AND. ( MYCOL.EQ.ITMP2 ) ) 
     $                  THEN
                        TJJS = DCONJG( A( (ICOL-1)*LDA+IROW ) )*TSCAL
                        CALL ZGEBS2D( CONTXT, 'All', ' ', 1, 1, TJJS,1)
                     ELSE
                        CALL ZGEBR2D( CONTXT, 'All', ' ', 1, 1, TJJS,1, 
     $                                ITMP1, ITMP2 )
...
*
                  IF( UPPER ) THEN
C                    DO 180 I = 1, J - 1
C                       CSUMJ = CSUMJ + ( DCONJG( A( I, J ) )*USCAL )*
C    $                          X( I )
C 180                CONTINUE
                     ZDUM = DCONJG( USCAL )
                     CALL PZSCAL( J-1, ZDUM, A, IA, JA+J-1, DESCA, 1 )
