...
*          column.
*
*  WORK    (local workspace) COMPLEX*16 array, dimension ( 2*DESCT(LLD_) )
*          Additional workspace may be required if PZLATRS is updated
*          to use WORK.
*
...
*  been implemented in PZLATRS which is called by this routine to solve
*  the triangular systems.  PZLATRS just calls PZTRSV.
*
...
*     .. Local Scalars ..
      ...
      INTEGER            CONTXT, CSRC, I, II, IS, J, K, KI, LDT, LDW, 
     $                   MB, NB, NPCOL, NPROW, RSRC, LDVL, LDVR
      DOUBLE PRECISION   OVFL, REMAXD, SCALE, SMIN, SMLNUM, ULP, UNFL
      COMPLEX*16         CDUM, REMAXC, SHIFT, TEMP
*     ..
*     .. Local Arrays ..
      INTEGER            DESCW( DLEN_ )
*     ..
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, IGAMN2D, INFOG2L, DGSUM2D,
     $                   PDLABAD, PDZASUM, PXERBLA, PZAMAX, PZCOPY, 
     $                   PZDSCAL, PZGEMV, PZLASET, PZLATRS, ZGSUM2D
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          ABS, DBLE, DCMPLX, DCONJG, DIMAG, MAX, MIN, MOD
*     ..
*     .. Statement Functions ..
      DOUBLE PRECISION   CABS1, pzlantr
*     ..
...
*
      DO 20 I = 1, N
         CALL INFOG2L( I, I, DESCT, NPROW, NPCOL, MYROW, MYCOL,
...
*     but not used.  For use in PZLATRS.
*
      RWORK( 1 ) = ZERO
      DO 30 J = 2, N
...
*     over the process rows?
      CALL DGSUM2D( CONTXT, 'Row', ' ', N, 1, RWORK, 1, -1, -1 )
*
      IF( RIGHTV ) THEN
*
...
*
            CALL INFOG2L( 1, 1, DESCW, NPROW, NPCOL, MYROW, MYCOL,
     $                    IROW, ICOL, ITMP1, ITMP2 )
            IF( ( MYROW.EQ.ITMP1 ) .AND. ( MYCOL.EQ.ITMP2 ) ) THEN
               WORK( 1 ) = CONE
            END IF
...
*
            IF( KI.GT.1 ) THEN
               CALL PZCOPY( KI-1, T, 1, KI, DESCT, 1, WORK, 1, 
     $                      1, DESCW, 1 )
            ENDIF
            DO 40 K = 1, KI-1
...
*
            IF( KI.GT.1 ) THEN
               CALL PZLATRS( 'Upper', 'No transpose', 'Non-unit', 'Y',
...
*
            IF( .NOT.OVER ) THEN
               CALL PZCOPY( KI, WORK, 1, 1, DESCW, 1, VR, 1, IS, 
     $                      DESCVR, 1 )
*
...
*
               CALL PZLASET( ' ', N-KI, 1, CZERO, CZERO, VR, KI+1, 
     $                       IS, DESCVR )
            ELSE
               IF( KI.GT.1 )
...
*
     ...
     $                    IROW, ICOL, ITMP1, ITMP2 )
            ...
            CALL ZGSUM2D( CONTXT, 'ALL', ' ', 1, 1, SHIFT, 1, -1, -1 )
*
            CALL INFOG2L( N, 1, DESCW, NPROW, NPCOL, MYROW, MYCOL,
     $                    IROW, ICOL, ITMP1, ITMP2 )
            IF( ( MYROW.EQ.ITMP1 ) .AND. ( MYCOL.EQ.ITMP2 ) ) THEN
               WORK( IROW ) = CONE
            END IF
...
*
            IF( KI.LT.N ) THEN
               CALL PZCOPY( N-KI, T, KI, KI+1, DESCT, N, WORK, KI+1, 
     $                      1, DESCW, 1 )
            ENDIF
            DO 90 K = KI + 1, N
               CALL INFOG2L( K, 1, DESCW, NPROW, NPCOL, MYROW, MYCOL,
...
*
            IF( KI.LT.N ) THEN
               CALL PZLATRS( 'Upper', 'Conjugate transpose', 'Nonunit',
     $                     'Y', N-KI, T, KI+1, KI+1, DESCT, WORK, KI+1, 
     $                      1, DESCW, SCALE, RWORK, INFO )
               CALL INFOG2L( KI, 1, DESCW, NPROW, NPCOL, MYROW, MYCOL,
     $                       IROW, ICOL, ITMP1, ITMP2 )
               IF( MYROW.EQ.ITMP1 .AND. MYCOL.EQ.ITMP2 ) THEN
                  WORK( IROW ) = DCMPLX( SCALE )
...
*
            IF( .NOT.OVER ) THEN
               CALL PZCOPY( N-KI+1, WORK, KI, 1, DESCW, 1, VL, KI, 
     $                      IS, DESCVL, 1 )
*
...
*
               CALL PZLASET( ' ', KI-1, 1, CZERO, CZERO, VL, 1, 
     $                       IS, DESCVL )
            ELSE
               IF( KI.LT.N )
     $            CALL PZGEMV( 'N', N, N-KI, CONE, VL, 1, KI+1, 
     $                        DESCVL, WORK, KI+1, 1, DESCW, 1, 
     $                        DCMPLX( SCALE ), VL, 1, KI, DESCVL, 1 )
*
