				  ...
				  int                           halton_index   )
{
  ...
  std::set<NOMAD::direction_type>::const_iterator end = dir_types.end() ;
  for ( std::set<NOMAD::direction_type>::const_iterator it = dir_types.begin() ;
	it != end ; ++it ) {

    if ( *it == NOMAD::UNDEFINED_DIRECTION || *it == NOMAD::NO_DIRECTION )
      continue;
