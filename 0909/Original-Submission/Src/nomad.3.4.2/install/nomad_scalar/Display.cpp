...
/*---------------------------------------------------------*/
/*      destructor (close all remaining opened blocks)     */
/*---------------------------------------------------------*/
NOMAD::Display::~Display ( void )
{
  while ( !_indent_str.empty() ) {
    this->close_block();
    (*this) << std::endl;
  }
}
...
std::ostream & NOMAD::operator << ( std::ostream & out , NOMAD::stop_type st )
{
  ...
  case NOMAD::MAX_ITER_REACHED:
    ...
    break;
  case NOMAD::MIN_EPS_REACHED:
    out << "min precision (eps) reached";
    break;
  ...
  case NOMAD::X0_FAIL:
    ...
    break;
  case NOMAD::TRIAL_PT_CONSTRUCTION:
  ...
  case NOMAD::MAX_CACHE_MEMORY_REACHED:
    ...
    break;
  case NOMAD::CTRL_C:
    out << "terminated by pressing ctrl-c";
    break;
