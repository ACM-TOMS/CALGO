...
namespace NOMAD {
  ...
  /**
     ...
     \endverbatim
  */
  class Display : private NOMAD::Uncopyable {
  ...
  public:
    ...
    */
    void flags ( std::_Ios_Fmtflags f ) const { _out.flags(f); }

    /// Set the format flags (2/2).
    /**
       \param f The flags -- \b IN.
    */
    void setf ( std::_Ios_Fmtflags f ) const { _out.setf(f); }

    /// Unset the format flags.
    /**
       \param f The flags -- \b IN.
    */
    void unsetf ( std::_Ios_Fmtflags f ) const { _out.unsetf(f); }

    /// Get the current format flags.
    /**
       \return The format flags.
    */
    std::_Ios_Fmtflags flags ( void ) const { return _out.flags(); }
