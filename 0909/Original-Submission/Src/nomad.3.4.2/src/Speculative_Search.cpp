					 ...
					 const NOMAD::Eval_Point *& new_infeas_inc   )
{
  ...
  for ( int i = 0 ; i < 2 ; ++i ) {
	...
	factor *= NOMAD::Mesh::get_mesh_update_basis();
      ...
      NOMAD::Direction new_dir ( n , 0.0 , x[i]->get_direction()->get_type() );
      new_dir.Point::operator = ( factor * *x[i]->get_direction() );
