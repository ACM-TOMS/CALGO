...
NOMAD::stop_type NOMAD::Mads::run ( void )
{
  ...
  try {
    ...
    // check if there is an extended poll for categorical variables,
    // and disable extended poll if no categorical variables:
    if ( _p.get_signature()->has_categorical() ) {
      if ( _p.get_extended_poll_enabled() && !_extended_poll )
	throw NOMAD::Exception ( "Mads.cpp" , __LINE__ ,
	      "categorical variables: no extended poll has been defined" );
    }
			 ...
			 bool                     & forbid_poll_size_stop   )
{
  ...
  while ( true ) {
	...
	throw NOMAD::Exception ( "Mads.cpp" , __LINE__ ,
      ...
      if ( !stop && dirs.empty() ) {
	if ( display_degree == NOMAD::FULL_DISPLAY )
	  out << "Mads::poll(): could not get directions: stop" << std::endl;
	stop        = true;
	stop_reason = NOMAD::TRIAL_PT_CONSTRUCTION;
	return;
      }
	...
	// add the new point to the ordered list of poll trial points:
  ...
  _stats.add_poll_pts ( static_cast<int>(_ev_control.get_eval_lop().size()) );
