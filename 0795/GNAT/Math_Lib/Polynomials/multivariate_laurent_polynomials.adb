...
package body Multivariate_Laurent_Polynomials is
...
-- CONSTRUCTORS :
  ...
  --   and n = Number_of_Unknowns(p) >= 1.
    ...
    begin
      nt.cf := t.cf;
      nt.dg := new Integer_Vectors.Vector(t.dg'first..t.dg'last-1);
  ...
  begin
    if (p = Null_Poly) or else (n = 0)
     then return null;
          ...
          return Create(p,n,Maximal_Degree(p,ind));
    end if;
  end Create;
  ...
  function Create ( p : Poly ) return Eval_Coeff_Poly is
    ...
    begin
      cnt := cnt + 1;
      lt.cf := Convert(cnt);
      lt.dg := new Integer_Vectors.Vector'(t.dg.all);
      Plus_Term(lp,lt);
      cont := true;
    end Label_Term;
  ...
  begin
    Clear(t2);
    Integer_Vectors.Copy(Link_to_Vector(t1.dg),Link_to_Vector(t2.dg));
    copy(t1.cf,t2.cf);
  end Copy;
...
-- SELECTORS :
  ...
  begin
    res := new Integer_Vectors.Vector'(1..n => -MAX_INT);
    Degree_Terms(p);
    return res;
  end Maximal_Degrees;
...
-- ARITHMETICAL OPERATIONS :
  ...
  begin
    Integer_Vectors.Copy(Link_to_Vector(t.dg),Link_to_Vector(temp.dg));
    temp.cf := -t.cf;
    Plus_Term(p,temp);
    Integer_Vectors.Clear(Link_to_Vector(temp.dg));
    clear(temp.cf);
  end Min_Term;
  ...
  procedure Mult_Term ( p : in out Poly; t : in Term ) is
    ...
    begin
      Integer_Vectors.Plus_Vector(Link_to_Vector(tp.dg),Link_to_Vector(t.dg));
    ...
    begin
      ...
      if t.dg(i) > 0
       then dt.cf := convert(cnt);
            dt.dg := new Integer_Vectors.Vector'(t.dg.all);
...
-- DESTRUCTORS :
  ...
  begin
    Integer_Vectors.Clear(Link_to_Vector(t.dg));
    clear(t.cf);
  end Clear;
...
end Multivariate_Laurent_Polynomials;
