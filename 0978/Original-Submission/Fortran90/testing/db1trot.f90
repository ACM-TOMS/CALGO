subroutine DB1TROT( nn, nvals, nix, ixvals, lenx, x, y, xs, ys, &
...
!  =======
!
!  DB1TROT tests the Level 1 BLAS subroutine DROT to apply
!  a plane rotation to a pair of vectors x and y.
!
!  Arguments
!  =========
!
...
!  .. Local Scalars ..
   ...
   real(wp) :: c, cs, rogue1, rogue2, s, ss, tmax, trat, umax, urat, &
      xexp, xmax, xmex, xnrm, xtmp, yexp, ymax, ymex, ynrm, ytmp
!  ..
!  .. Local Arrays ..
...
!
         do ir = 1, nr
           ...
           ss = svals(ir)
!
!          Call DROT to apply the rotation described by c and s
!
           n = ns
           x(1:lenx) = xs(1:lenx)
           incx = incxs
...
!
           ...
           call DARGCKX( nerrs, subnam, 'X', lenx, iwork1, x, xs, nout )
           call IARGCKS( nerrs, subnam, 'INCX', incx, incxs, nout )
           call DARGCKX( nerrs, subnam, 'Y', lenx, iwork2, y, ys, nout )
...
!
           ...
           do i = 1, ns
             ...
             xnrm = abs( cs*xexp ) + abs( ss*yexp )
             ytmp = -ss*xexp + cs*yexp
             ynrm = abs( ss*xexp ) + abs( cs*yexp )
             xexp = xtmp
             yexp = ytmp
!
