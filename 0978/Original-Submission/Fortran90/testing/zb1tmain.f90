...
10 continue
   ...
   if( lsamen( 4, path, 'amax' ) ) then
      call ZB1TAMAX( nn, nvals, nix, ixvals, lenx, x1, x2, &
                     work, nout )
   else if( lsamen( 4, path, 'asum' ) ) then
      call ZB1TASUM( nn, nvals, nix, ixvals, lenx, x1, y1, x2, &
                     work, thresh, nout )
   else if( lsamen( 4, path, 'axpy' ) ) then
      call ZB1TAXPY( nn, nvals, nix, ixvals, ncal, calvals, lenx, &
                     x1, y1, x2, y2, work, work2, iwork, thresh, nout )
   else if( lsamen( 4, path, 'copy' ) ) then
      call ZB1TCOPY( nn, nvals, nix, ixvals, lenx, x1, y1, x2, y2, &
                     work, iwork, thresh, nout )
   else if( lsamen( 4, path, 'dot ' ) ) then
      call ZB1TDOTC( nn, nvals, nix, ixvals, lenx, x1, y1, x2, y2, &
                     work, work2, thresh, nout )
      call ZB1TDOTU( nn, nvals, nix, ixvals, lenx, x1, y1, x2, y2, &
                     work, work2, thresh, nout )
   else if( lsamen( 4, path, 'nrm2' ) ) then
      call ZB1TNRM2( nn, nvals, nix, ixvals, lenx, x1, x2, y1, &
                     work, thresh, nout )
   else if( lsamen( 4, path, 'rot ' ) ) then
      call ZB1TROT( nn, nvals, nix, ixvals, lenx, x1, y1, x2, y2, &
                    work, work2, iwork, iwork2, thresh, nout )
      call ZB1TDROT( nn, nvals, nix, ixvals, lenx, x1, y1, x2, y2, &
                     work, work2, iwork, iwork2, thresh, nout )
   else if( lsamen( 4, path, 'rotg' ) ) then
      call ZB1TRTG( 'ZROTG ', thresh, nout )
      call ZB1TRTG( 'ZLARTG', thresh, nout )
   else if( lsamen( 4, path, 'scal' ) ) then
      call ZB1TSCAL( nn, nvals, nix, ixvals, ncal, calvals, lenx, &
                     x1, x2, work, iwork, thresh, nout )
      call ZB1TDSCAL( nn, nvals, nix, ixvals, nal, alvals, lenx, &
                      x1, x2, work, iwork, thresh, nout )
   else if( lsamen( 4, path, 'ssq ' ) ) then
      call ZB1TSSQ( nn, nvals, nix, ixvals, lenx, x1, y1, x2, &
                    work, thresh, nout )
   else if( lsamen( 4, path, 'swap' ) ) then
      call ZB1TSWAP( nn, nvals, nix, ixvals, lenx, x1, y1, x2, y2, &
