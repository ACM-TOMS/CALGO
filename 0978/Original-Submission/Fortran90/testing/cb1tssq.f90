subroutine CB1TSSQ( nn, nvals, nix, ixvals, lenx, x, xs, z, &
      work, thresh, nout )
   use LA_CONSTANTS32, only: wp, zero, ulp, half, one, two, &
...
!  =======
!
!  CB1TSSQ tests the sum of squares routine CLASSQ.  Tests are done
...
!
!            Call CLASSQ to compute the sum of squares
!
             scl = s
             sumsq = ssq
             n = ns
...
!
             if( ns > 0 .and. incxs == 0 ) then
               y1 = sqrt(abs(v0))
               y2 = sqrt( real(ns,wp) )*SLAPY2( real(x(1)), aimag(x(1)) )
               ymin = min( y1, y2 )
               ymax = max( y1, y2 )
               if( LA_ISNAN( y1 ) .or. LA_ISNAN( y2 ) ) then
                 znrm = LA_XVALS( 3, v0 )
