subroutine ZB1TRTG( rtname, thresh, nout )
...
!  =======
!
!  ZB1TRTG tests the generation of plane rotations in LINPACK ZROTG
!  and LAPACK ZLARTG.  Tests are done with +/- combinations of the
!  following values:
!
!  0, very small, ulp, 1/3, 1, 1/ulp, very big
!
!  Arguments
!  =========
!
!  RTNAME  (input) CHARACTER*(*)
!          The subroutine to test, either ZROTG or ZLARTG.
!
!  THRESH  (input) REAL
...
!
   if( LSAMEN( 5, rtname, 'ZROTG' ) ) then
      likbla = 1
   else if( LSAMEN( 6, rtname, 'ZLARTG' ) ) then
      likbla = 0
   else
      return
   end if
...
!
         call ZRTG01( f, g, cs, sn, r, result )
!
...
99 format( /, 'Tests for ZROTG/ZLARTG:', / &
           '   1:  C*F + S*G   = R', / &
           '   2:  -conjg(S)*F + C*G  = 0', / &
           '   3:  C**2 + conjg(S)*S = 1', / &
           '   4:  C >= 0',/ &
