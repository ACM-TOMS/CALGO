subroutine ZB1TROT( nn, nvals, nix, ixvals, lenx, x, y, xs, ys, &
                    work1, work2, iwork1, iwork2, thresh, nout )
   use LA_CONSTANTS, only: wp, zero, half, one, two, ulp
!
...
!  =======
!
!  ZB1TROT tests the Level 1 BLAS subroutine ZROT to apply
!  a plane rotation to a pair of complex vectors x and y.
!
!  Arguments
!  =========
!
...
!  .. Local Scalars ..
   ...
   real(wp) :: c, cs, tmax, trat, umax, urat, xnrm, ynrm
   complex(wp) :: rogue1, rogue2, s, ss, xexp, xmax, xmex, xtmp, &
      yexp, ymax, ymex, ytmp
!  ..
!  .. Local Arrays ..
   real(wp) :: cvals(nr)
...
!
   ...
   subnam = 'ZROT  '
!
!  Test for each of the following rotations (c is always positive
!  when constructed by ZROTG, but ZROT doesn't actually check this):
!     1) c = 0, s = (1,0)
!     2) c = 0, s = (0,1)
...
!
         do ir = 1, nr
           ...
           ss = svals(ir)
!
!          Call ZROT to apply the rotation described by c and s
!
           n = ns
           x(1:lenx) = xs(1:lenx)
           incx = incxs
...
!
           ...
           call ZARGCKX( nerrs, subnam, 'X', lenx, iwork1, x, xs, nout )
           call IARGCKS( nerrs, subnam, 'INCX', incx, incxs, nout )
           call ZARGCKX( nerrs, subnam, 'Y', lenx, iwork2, y, ys, nout )
...
!
           ...
           iy = 1
           if( incys < 0 ) iy = 1 - (ns-1)*incys
           xexp = cmplx(work1(2*ns-1),work1(2*ns),wp)
           yexp = cmplx(work2(2*ns-1),work2(2*ns),wp)
           do i = 1, ns
             if( incxs /= 0 ) then
