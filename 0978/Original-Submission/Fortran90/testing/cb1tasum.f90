subroutine CB1TASUM( nn, nvals, nix, ixvals, lenx, x, xs, z, &
      work, thresh, nout )
...
!  =======
!
!  CB1TASUM tests the 1-norm routine SCASUM. Tests are done with
...
!
   do in = 1, nn
     ns = nvals(in)
     if( ns > 1 ) then
       call random_number(work(1:2*(ns-1)))
     end if
     do i = 1, 2*(ns-1)
     ...
     do iv = 1, nv
       v0 = values(iv)
       if( abs(v0) > bignum ) v0 = ( v0*half ) / real(ns+1,wp)
       z(1) = cmplx( v0, -v0, wp )
       do iw = 1, nv
         v1 = values(iw)
         if( abs(v1) > bignum ) v1 = ( v1*half ) / real(ns+1,wp)
         wnrm = zero
         do i = 1, ns-1
           z(i+1) = cmplx( v1*work(2*i-1), v1*work(2*i), wp )
           wnrm = wnrm + CABS1(z(i+1))
         end do
!
...
!
         do j = 1, nix
           xs(1:lenx) = cmplx( bignum, -bignum, wp )
           incxs = ixvals(j)
           ix = 1
...
!
!          Call SCASUM to compute the 1-norm
!
           n = ns
           x(1:lenx) = xs(1:lenx)
           incx = incxs
           snrm = SCASUM( n, x, incx )
!
