subroutine DB1TASUM( nn, nvals, nix, ixvals, lenx, x, xs, z, &
      work, thresh, nout )
   use LA_CONSTANTS, only: wp, zero, one, two, ulp, safmin, safmax, &
                             smlnum, bignum
   use LA_XISNAN
   use LA_XXVALS
!
...
!  =======
!
!  DB1TASUM tests the 1-norm routine DASUM. Tests are done with
...
!
!          Call DASUM to compute the 1-norm
!
           n = ns
           x(1:lenx) = xs(1:lenx)
           incx = incxs
