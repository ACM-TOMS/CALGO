subroutine DTOMCOPY( n, nrep, x, ldx, y, ldy, xs, ys, thresh, nout )
...
!  ..
!
!  DTOMCOPY times DCOPY from the Level 1 BLAS, a subroutine to copy
...
!
   ...
   t1 = DSECND()
   do j = 1, nrep
     call DCOPY( n, x(1,j), incx, y(1,j), incy )
   end do
   t2 = DSECND()
   tottim = t2 - t1
   write(nout,99) 'DCOPY ', ns, tottim / nrep
!
