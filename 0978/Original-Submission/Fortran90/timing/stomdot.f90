subroutine STOMDOT( n, nrep, x, ldx, y, ldy, xs, ys, thresh, nout )
...
!  ..
!
!  STOMDOT times SDOT from the Level 1 BLAS, a function to compute the
...
!
   ...
   t1 = SECOND()
   do j = 1, nrep
     sbla(j) = SDOT( n, x(1,j), incx, y(1,j), incy )
   end do
   t2 = SECOND()
   tottim = t2 - t1
   write(nout,99) 'SDOT  ', ns, tottim / nrep
!
...
!
   ...
   call SARGCKV( nerrs, subnam, 'Y', ns, y(1,nrep), ys, nout )
