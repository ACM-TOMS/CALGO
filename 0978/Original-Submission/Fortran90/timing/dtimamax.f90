subroutine DTIMAMAX( n, nrep, x, xs, rtim, nout )
...
!  ..
!
!  DTIMAMAX times IDAMAX from the Level 1 BLAS, a function to find
...
!
   ...
   t1 = DSECND()
   do j = 1, nrep
     ibla(j) = IDAMAX( n, x, incx )
   end do
   t2 = DSECND()
   tottim = t2 - t1
   rtim = tottim / nrep
!  write(nout,99) 'IDAMAX', ns, rtim
!
