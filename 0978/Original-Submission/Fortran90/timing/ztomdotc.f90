subroutine ZTOMDOTC( n, nrep, x, ldx, y, ldy, xs, ys, work, thresh, nout )
...
!  ..
!
!  ZTOMDOTC times ZDOTC from the Level 1 BLAS, a function to compute the
...
!
   ...
   t1 = DSECND()
   do j = 1, nrep
     cbla(j) = ZDOTC( n, x(1,j), incx, y(1,j), incy )
   end do
   t2 = DSECND()
   tottim = t2 - t1
   write(nout,99) 'ZDOTC ', ns, tottim / nrep
!
...
!
   ...
   call ZARGCKV( nerrs, subnam, 'Y', ns, y(1,nrep), ys, nout )
