subroutine CTOMASUM( n, nrep, x, ldx, xs, work, thresh, nout )
...
!  ..
!
!  CTOMASUM times SCASUM from the Level 1 BLAS, a function to compute
...
!  ..
!  .. External Functions ..
   real(wp) :: SCASUM, SECOND
   external :: SCASUM, SECOND
!  ..
!
   nfail = 0
   ntests = 0
...
!
   ...
   do j = 1, nrep
     n = ns
     incx = incxs
     sbla(j) = SCASUM( n, x(1,j), incx )
   end do
   t2 = SECOND()
   tottim = tottim + t2 - t1
   write(nout,99) 'SCASUM', ns, tottim / nrep
!
