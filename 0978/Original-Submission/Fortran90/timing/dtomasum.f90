subroutine DTOMASUM( n, nrep, x, ldx, xs, thresh, nout )
...
!  ..
!
!  DTOMASUM times DASUM from the Level 1 BLAS, a function to compute
...
!  ..
!  .. External Functions ..
   real(wp) :: DASUM, DSECND
   external :: DASUM, DSECND
!  ..
!
   nfail = 0
   ntests = 0
...
!
   ...
   t1 = DSECND()
   do j = 1, nrep
     sbla(j) = DASUM( n, x(1,j), incx )
   end do
   t2 = DSECND()
   tottim = t2 - t1
   write(nout,99) 'DASUM ', ns, tottim / nrep
!
