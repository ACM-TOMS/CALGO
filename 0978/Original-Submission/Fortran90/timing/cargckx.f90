subroutine CARGCKX( nerrs, subnam, varnam, n, skip, x, xs, nout )
...
!  =======
!
!  CARGCKX checks if two complex vectors are identical in the entries
!  where SKIP is 0 and reports any errors.
!
!  Arguments
!  =========
!
...
!  ..
   ...
   do i = 1, n
      if( skip(i) == 0 .and. x(i) /= xs(i) ) then
         write(nout,99) subnam, varnam, i, real(xs(i)), aimag(xs(i)), &
            real(x(i)), aimag(x(i))
         ierrs = ierrs + 1
      end if
   end do
