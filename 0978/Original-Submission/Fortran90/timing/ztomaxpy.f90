subroutine ZTOMAXPY( n, nrep, nal, alvals, x, ldx, y, ldy, xs, ys, &
                     work, thresh, nout )
...
!  ..
!
!  ZTOMAXPY times ZAXPY from the Level 1 BLAS, a subroutine to compute
...
!
     ...
     do j = 1, nrep
       ...
       incx = incxs
       incy = incys
       call ZAXPY( n, alpha, x(1,j), incx, y(1,j), incy )
     end do
     t2 = DSECND()
     tottim = tottim + t2 - t1
     write(nout,99) 'ZAXPY ', ns, alphas, tottim / nrep
!
...
!
     ...
     call ZARGCKV( nerrs, subnam, 'X', n, x(1,nrep), xs, nout )
