subroutine ZTIMASUM( n, nrep, x, xs, work, thresh, rtim, nout )
...
!  ..
!
!  ZTIMASUM times DZASUM from the Level 1 BLAS, a function to compute
...
!
   ...
   t1 = DSECND()
   do j = 1, nrep
     sbla(j) = DZASUM( n, x, incx )
   end do
   t2 = DSECND()
   tottim = t2 - t1
   rtim = tottim / nrep
!  write(nout,99) 'DZASUM ', ns, rtim
!
