subroutine ZTIMDOTU( n, nrep, x, y, xs, ys, work, thresh, rtim, nout )
...
!  ..
!
!  ZTIMDOTU times ZDOTU from the Level 1 BLAS, a function to compute the
...
!
   ...
   t1 = DSECND()
   do j = 1, nrep
     cbla(j) = ZDOTU( n, x, incx, y, incy )
   end do
   t2 = DSECND()
   tottim = t2 - t1
   rtim = tottim / nrep
!  write(nout,99) 'ZDOTU ', ns, rtim
!
