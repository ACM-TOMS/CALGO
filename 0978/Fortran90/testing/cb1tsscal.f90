subroutine CB1TSSCAL( nn, nvals, nix, ixvals, nal, alvals, lenx, x, xs, &
...
!  =======
!
!  CB1TSSCAL tests the scaling routine CSSCAL.
!
!  Arguments
!  =========
!
...
!  .. Local Scalars ..
   ...
   real(wp) :: alpha, alphas, tmax, trat, xnrm
   complex(wp) :: rogue, xexp, xmax, xmex
!  ..
...
!
       do k = 1, nal
         alphas = alvals(k)
!
!        Call CSSCAL to scale the vector x
!
         n = ns
         alpha = alphas
         x(1:lenx) = xs(1:lenx)
...
!
         ...
         if( ns > 0 .and. incxs == 0 ) then
           xexp = cmplx( (alphas**ns)*work(2*ns-1), &
                  (alphas**ns)*work(2*ns), wp )
           xnrm = abs( xexp )
           if( xnrm == zero ) then
             trat = abs( x(ix) ) / ulp
           else
           ...
           tmax = -one
           do i = 1, ns
             xexp = cmplx( alphas*work(2*i-1), alphas*work(2*i), wp )
             xnrm = abs( xexp )
             if( xnrm == zero ) then
               trat = abs( x(ix) ) / ulp
             else
