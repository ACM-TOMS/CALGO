subroutine CB1TAMAX( nn, nvals, nix, ixvals, lenx, x, xs, &
      work, nout )
...
!  =======
!
!  CB1TAMAX tests the infinity-norm routine ICAMAX. Tests are done
...
!  .. Local Scalars ..
   character*6 :: subnam
   integer :: i, ibla, iexp, imax, imax2, in, incx, incxs, iu, iv, iw, &
      ix, ixmax, j, k, n, nerrs, nfail, nmax, ns, ntests, nv, nw
   real(wp) :: pnan, rogue
   complex(wp) :: xmax
!  ..
!  .. Local Arrays ..
...
!
!              Call ICAMAX to find the maximum element
!
               n = ns
               x(1:lenx) = xs(1:lenx)
               incx = incxs
               ibla = ICAMAX( n, x, incx )
!
