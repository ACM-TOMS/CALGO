...
10 continue
   ...
   if( lsamen( 4, path, 'amax' ) ) then
      call SB1TAMAX( nn, nvals, nix, ixvals, lenx, x1, x2, &
                     work, nout )
   else if( lsamen( 4, path, 'asum' ) ) then
      call SB1TASUM( nn, nvals, nix, ixvals, lenx, x1, y1, x2, &
                     work, thresh, nout )
   else if( lsamen( 4, path, 'axpy' ) ) then
      call SB1TAXPY( nn, nvals, nix, ixvals, nal, alvals, lenx, &
                     x1, y1, x2, y2, work, work2, iwork, thresh, nout )
   else if( lsamen( 4, path, 'copy' ) ) then
      call SB1TCOPY( nn, nvals, nix, ixvals, lenx, x1, y1, x2, y2, &
                     work, iwork, thresh, nout )
   else if( lsamen( 4, path, 'dot ' ) ) then
      call SB1TDOT( nn, nvals, nix, ixvals, lenx, x1, y1, x2, y2, &
                    work, work2, thresh, nout )
   else if( lsamen( 4, path, 'nrm2' ) ) then
      call SB1TNRM2( nn, nvals, nix, ixvals, lenx, x1, x2, y1, &
                     work, thresh, nout )
   else if( lsamen( 4, path, 'rot ' ) ) then
      call SB1TROT( nn, nvals, nix, ixvals, lenx, x1, y1, x2, y2, &
                    work, work2, iwork, iwork2, thresh, nout )
   else if( lsamen( 4, path, 'rotg' ) ) then
      call SB1TRTG( 'SROTG ', thresh, nout )
      call SB1TRTG( 'SLARTG', thresh, nout )
   else if( lsamen( 4, path, 'scal' ) ) then
      call SB1TSCAL( nn, nvals, nix, ixvals, nal, alvals, lenx, &
                     x1, x2, work, iwork, thresh, nout )
   else if( lsamen( 4, path, 'ssq ' ) ) then
      call SB1TSSQ( nn, nvals, nix, ixvals, lenx, x1, y1, x2, &
                    work, thresh, nout )
   else if( lsamen( 4, path, 'swap' ) ) then
      call SB1TSWAP( nn, nvals, nix, ixvals, lenx, x1, y1, x2, y2, &
