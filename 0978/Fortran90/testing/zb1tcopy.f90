subroutine ZB1TCOPY( nn, nvals, nix, ixvals, lenx, x, y, xs, ys, &
...
!  =======
!
!  ZB1TCOPY tests the vector copy routine ZCOPY.
!
!  Arguments
!  =========
!
...
!  .. Local Scalars ..
   ...
   real(wp) :: tmax, trat, wxi, ynrm
   complex(wp) :: rogue, yexp, ymax, ymex
!  ..
!
   nfail = 0
   ntests = 0
   rogue = cmplx( 999.999_wp, -999.999_wp, wp )
   subnam = 'ZCOPY '
!
...
!
         ...
         call ZARGCKX( nerrs, subnam, 'Y', lenx, iwork, y, ys, nout )
...
!
         ...
         do i = 1, ns
           ...
           if( incys == 0 ) then
             yexp = cmplx( work(ns), -work(ns), wp )
           else
             yexp = cmplx( wxi, -wxi, wp )
           end if
           ynrm = abs( yexp )
           if( ynrm == zero ) then
             trat = abs( y(iy) ) / ulp
