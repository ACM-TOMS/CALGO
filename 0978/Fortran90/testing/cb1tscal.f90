subroutine CB1TSCAL( nn, nvals, nix, ixvals, nal, alvals, lenx, x, xs, &
      work, iwork, thresh, nout )
   use LA_CONSTANTS32, only: wp, zero, one, two, ulp, bignum
!
...
!  =======
!
!  CB1TSCAL tests the scaling routine CSCAL.
!
!  Arguments
!  =========
!
...
!
!  ALVALS  (input) COMPLEX array, dimension (NAL)
...
!  .. Local Scalars ..
   ...
   real(wp) :: tmax, trat, xnrm
   complex(wp) :: alpha, alphas, rogue, xexp, xmax, xmex
!  ..
...
!
       do k = 1, nal
         alphas = alvals(k)
!
!        Call CSCAL to scale the vector x
!
         n = ns
         alpha = alphas
         x(1:lenx) = xs(1:lenx)
...
!
         ...
         call CARGCKX( nerrs, subnam, 'X', lenx, iwork, x, xs, nout )
...
!
         ...
         if( ns > 0 .and. incxs == 0 ) then
           xexp = (alphas**ns)*cmplx( work(2*ns-1), work(2*ns), wp )
           xnrm = abs( xexp )
           if( xnrm == zero ) then
             trat = abs( x(ix) ) / ulp
           else
           ...
           tmax = -one
           do i = 1, ns
             xexp = alphas*cmplx( work(2*i-1), work(2*i), wp )
             xnrm = abs( xexp )
             if( xnrm == zero ) then
               trat = abs( x(ix) ) / ulp
             else
