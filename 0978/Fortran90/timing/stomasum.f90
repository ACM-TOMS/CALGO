subroutine STOMASUM( n, nrep, x, ldx, xs, thresh, nout )
...
!  ..
!
!  STOMASUM times SASUM from the Level 1 BLAS, a function to compute
...
!
   ...
   t1 = SECOND()
   do j = 1, nrep
     sbla(j) = SASUM( n, x(1,j), incx )
   end do
   t2 = SECOND()
   tottim = t2 - t1
   write(nout,99) 'SASUM ', ns, tottim / nrep
!
