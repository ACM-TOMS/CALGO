subroutine ZTOMCOPY( n, nrep, x, ldx, y, ldy, xs, ys, thresh, nout )
   use LA_CONSTANTS, only: wp, zero, one, ulp, czero
!
!  .. Scalar Arguments ..
...
!  ..
!
!  ZTOMCOPY times ZCOPY from the Level 1 BLAS, a subroutine to copy
...
!
   ...
   do j = 1, nrep
     ...
     incx = incxs
     incy = incys
     call ZCOPY( n, x(1,j), incx, y(1,j), incy )
   end do
   t2 = DSECND()
   tottim = tottim + t2 - t1
   write(nout,99) 'ZCOPY ', ns, tottim / nrep
!
