subroutine STOMPY2( n, nrep, x, ldx, y, ldy, z, ldz, xs, ys, &
      thresh, nout )
...
!  ..
!
!  STOMPY2 times SLAPY2 from LAPACK, a function to compute the
...
!
   ...
   do j = 1, nrep
     do k = 1, n
       z(k,j) = SLAPY2( x(k,j), y(k,j) )
     end do
   end do
   t2 = SECOND()
   tottim = t2 - t1
   write(nout,99) 'SLAPY2', ns, tottim / nrep
!
!  See what values changed inside the subroutine
!
   nerrs = 0
   call IARGCKS( nerrs, subnam, 'N', n, ns, nout )
   call SARGCKV( nerrs, subnam, 'X', n, x(1,nrep), xs, nout )
   call SARGCKV( nerrs, subnam, 'Y', n, y(1,nrep), ys, nout )
!
