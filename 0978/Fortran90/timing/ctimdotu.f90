subroutine CTIMDOTU( n, nrep, x, y, xs, ys, work, thresh, rtim, nout )
...
!  ..
!
!  CTIMDOTU times CDOTU from the Level 1 BLAS, a function to compute the
...
!
   ...
   t1 = SECOND()
   do j = 1, nrep
     cbla(j) = CDOTU( n, x, incx, y, incy )
   end do
   t2 = SECOND()
   tottim = t2 - t1
   rtim = tottim / nrep
!  write(nout,99) 'CDOTU ', ns, rtim
!
