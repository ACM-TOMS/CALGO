subroutine STIMAMAX( n, nrep, x, xs, rtim, nout )
...
!  ..
!
!  STIMAMAX times ISAMAX from the Level 1 BLAS, a function to find
...
!
   ...
   t1 = SECOND()
   do j = 1, nrep
     ibla(j) = ISAMAX( n, x, incx )
   end do
   t2 = SECOND()
   tottim = t2 - t1
   rtim = tottim / nrep
!  write(nout,99) 'ISAMAX', ns, rtim
!
