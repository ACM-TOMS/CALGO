function ratios = aditest_CPF(numeval)
% function ratios = aditest_CPF(numeval)
...
for k = 0:numeval
  ...
  if intlabFlag
    tic
    gx = gradientinit(x);
    gf = MinpackCPF_F(gx,Prob);
    time.intlab = time.intlab+toc;
  end
  
  tic
  [F,J2] = MinpackCPF_F_Jac(x,Prob);
  time.adigator = time.adigator + toc;
  %
  if madFlag
    tic
    mx = fmad(x,S);
    mf = MinpackCPF_F(mx,Prob);
    time.mad = time.mad+toc;
  end
  
  tic
  f = MinpackCPF_F(x,Prob);
  time.func = time.func+toc;
  
  tic
  [F,J] = MinpackCPF_FJ(x,Prob);
  ...
  if adimatFlag
    ...
    for i = 1:size(S,2)
      J3(:,i) = g_MinpackCPF_F(S(:,i),x,Prob);
    end
