function ratios = aditest_CTS(numeval)
% function ratios = aditest_CTS(numeval)
...
for k = 0:numeval
  ...
  if intlabFlag
    tic
    gx = gradientinit(x);
    gf = MinpackCTS_F(gx,Prob);
    time.intlab = time.intlab+toc;
  end
  
  tic
  [F,J2] = MinpackCTS_F_Jac(x,Prob);
  time.adigator = time.adigator + toc;
  %
  if madFlag
    tic
    mx = fmad(x,S);
    mf = MinpackCTS_F(mx,Prob);
  ...
  if adimatFlag
    ...
    for i = 1:size(S,2)
      J3(:,i) = g_MinpackCTS_F(S(:,i),x,Prob);
    end
