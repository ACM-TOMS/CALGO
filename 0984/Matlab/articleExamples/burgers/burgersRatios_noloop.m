function ratios = burgersRatios_noloop(nn,numeval)
% function ratios = burgersRatios_noloop(nn,numeval) 
% This file computes Burgers ODE Jacobian numerous times using ADiGator,
% ADiMat, INTLAB, and MAD, and the modified burgers ODE function (without
...
for i = 0:numeval
  ...
  tic
  f = burgersfun_noloop(1,y,N);
  ti.fun = ti.fun+toc;
  
  if intlabFlag
    tic;
    gy = gradientinit(y);
    gf = burgersfun_noloop(1,gy,N);
  ...
  ay.f = y;
  ay.dy = ones(n,1);
  f1 = burgersfun_noloop_ADiGatorJac(1,ay,N);
  ti.adigator = ti.adigator+toc;
  
  if madFlag
    tic;
    my = fmad(y,S);
    mf = burgersfun_noloop(1,my,N);
    ti.mad = ti.mad + toc;
  end
  ...
  if adimatFlag
    ...
    for j = 1:size(S,2)
      g_y = S(:,j);
      JJ(:,j) = g_burgersfun_noloop(1,g_y,y,N);
    end
