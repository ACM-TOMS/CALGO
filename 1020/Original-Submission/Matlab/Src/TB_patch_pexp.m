classdef TB_patch_pexp < TB_patch_poly
...
methods
   ...
   function P = TB_patch_pexp(p, xx, w)
      
      % Construction of a polynomial exponential TB-spline patch of even degree p
      %
      % INPUT
      %   p     : TB-spline degree
      ...
      P.w = abs(w);
      cwl = 2 * cosh(P.w * (P.U(end) - P.U(1)) / 2);
      P.ss = eye(P.n, 1);
      for q = 3:2:P.n
         i = q:-1:3;
   ...
   function M = TB_evaluation_all(P, xx, cl)
      ...
      % Evaluation of all polynomial exponential TB-splines in given points
      %
      % INPUT
      %   P     : TB-spline patch
      ...
      if P.w * (P.U(end) - P.U(1)) < tol
         ...
         if any(j)
            sswl = (sinh(P.w * (P.U(end) - P.U(1)) / 2))^2;
            cswl = 2 * cosh(P.w * (P.U(end) - P.U(1)) / 2) / sswl;
            swx1 = sinh(P.w * (xx(j) - P.U(1)) / 2);
            swx2 = sinh(P.w * (P.U(end) - xx(j)) / 2);
            xl1 = swx1.^2 / sswl;
            xl2 = swx1 .* swx2 * cswl;
            xl3 = swx2.^2 / sswl;
            M(1, j) = 1;
   ...
   function K = TB_diffend_all(P, r, el)
      ...
      % Full differentiation of all polynomial exponential TB-splines 
      ...
      if P.w * (P.U(end) - P.U(1)) < tol
         ...
         if el
            ...
            if P.p > 0 && r > 0
               cwl = 2 * cosh(P.w * (P.U(end) - P.U(1)) / 2);
               swl = 2 * sinh(P.w * (P.U(end) - P.U(1)) / 2);
               k = min(r, P.p);
               i = (1:k)';
   ...
   function M = TB_differentiation_all(P, r, xx, cl)
      ...
      % Differentiation of all polynomial exponential TB-splines in given points
      %
      % INPUT
      %   P     : TB-spline patch
