...
*        where EPS = SLAMCH('Epsilon'). This authorises SGESVDQ to
...
*        the transposed R** of the computed trinagular factor R. This involves
...
*  the Householder vectorsas stored by SGEQP3. If JOBU = 'F', these Househlder 
...
*          { MAX( M, 1 ),  if JOBU = 'A'
*  LWSVD = MAX( 5*N, 1 )
*  LWLQF = MAX( N/2, 1 ), LWSVD2 = MAX( 5*(N/2), 1 ), LWUNLQ = MAX( N, 1 ),
...
*         N + MAX( LWQP3, N/2+LWLQF, N/2+LWSVD2, N/2+LWUNLQ, LWORQ) ) if the  
*                         full SVD is requested with JOBV='A' or 'V', and 
*                         JOBR ='N'
*  = MAX( N + MAX( LWQP3, LWCON, LWSVD, LWORQ ), 
*         N + MAX( LWQP3, LWCON, N/2+LWLQF, N/2+LWSVD2, N/2+LWUNLQ, LWORQ ) )  
*                         if the full SVD is requested with JOBV='A' or 'V', and  
*                         JOBR ='N', and also a scaled condition number estimate 
*                         requested.
*  = MAX( N + MAX( LWQP3, LWSVD, LWORQ ),
*         N + MAX( LWQP3, N/2+LWQRF, N/2+LWSVD2, N/2+LWORQ2, LWORQ ) ) if the
*                         full SVD is requested with JOBV='A', and JOBR ='T'
*  = MAX( N + MAX( LWQP3, LWCON, LWSVD, LWORQ ),
...
*   SVD routine (SGESVD) to do its own preemptive scaling if potential over-
*   or underflows are detected. To avoid repeated scanning of the array A,
*   an optimal implementation would do all necessary scaling before calling
*   SGESVD and the scaling in SGESVD can be switched off.
...
*     exact zeros in SGESVD() aplied to the (possibly truncated) 
