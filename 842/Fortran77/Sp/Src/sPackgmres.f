...
*
        subroutine sgmres(n,m,b,x,H,w,r0,V,yCurrent,xCurrent,rotSin,
...
* ----------------------
       integer  irc(*)
       real x(*), H(m+1,*), w(*), r0(*), V(n,*), yCurrent(*)
...
* ---------------
       ...
       integer  iwarn, ihist
       integer  compRsd
       real    beta, bn, sA, sb, sPA, sPb, bea, be
...
*
         call saxpy(jH,ONE,r0,1,H(1,jH),1)
         call sgemv('N',n,jH,-ONE,V(1,1),n,r0,1,ONE,w,1)
         dloo = snrm2(jH,r0,1)**2
       endif
*
*         dnormw = snrm2(n,w,1)
*
...
* Apply previous Givens rotations to the new column of H
       do j=1,jH-1
         call srot (1, H(j,jH), 1, H(j+1,jH), 1, rotCos(j), rotSin(j))
       enddo
       auxHjj = H(jH,jH)
       auxHjp1j= H(jH+1,jH)
       call srotg(auxHjj, auxHjp1j, rotCos(jH),rotSin(jH))
...
* rotation to the last entry of the collumn
       call srot (1, H(jH,jH), 1, H(jH+1,jH), 1, rotCos(jH), rotSin(jH))
       H(jH+1,jH) = ZERO
*
