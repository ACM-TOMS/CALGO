...
*     .. Parameters ..
     ...
     $                   LLD_, MB_, M_, NB_, N_, RSRC_
      LOGICAL            DEBUG, CRSOVER, READEIG
      PARAMETER          ( BLOCK_CYCLIC_2D = 1, DLEN_ = 9, DTYPE_ = 1,
     $                     CTXT_ = 2, M_ = 3, N_ = 4, MB_ = 5, NB_ = 6,
     $                     RSRC_ = 7, CSRC_ = 8, LLD_ = 9,
     $                     DEBUG = .FALSE., CRSOVER = .FALSE.,
     $                     READEIG = .TRUE. )
      INTEGER            NTINY
      PARAMETER          ( NTINY = 11 )
      INTEGER            NL
...
*     .. Local Scalars ..
     ...
     $                   DUM4, COS, SIN, ELEM1, ELEM2, ELEM3, ELEM4,
     $                   SN, CS, ELEM5, TMP
*     ..
*     .. Local Arrays ..
...
*
         IF( DEBUG ) WRITE(*,*) 'Quick return: invalid arg(s)'
...
*
         IF( DEBUG ) WRITE(*,*) 'Copy eigenvalues from PDGEBAL'
         DO 10 I = 1, ILO - 1
...
*
         IF( DEBUG ) WRITE(*,*) 'Initialize Z, if requested'
         IF( INITZ )
...
*
     ...
     $        DESCH(RSRC_).NE.0 .OR. DESCH(CSRC_).NE.0 ) THEN
            IF( DEBUG ) WRITE(*,*) 'PDHSEQR -> PDLAQR0 1'
            CALL PDLAQR0( WANTT, WANTZ, N, ILO, IHI, H, DESCH, WR, WI, 
     $                    ILO, IHI, Z, DESCZ, WORK, LWORK, IWORK, 
     $                    LIWORK, INFO, TIMINGS )
            IF( DEBUG ) WRITE(*,*) 'PDHSEQR <- PDLAQR0 1'
            IF( INFO.GT.0 .AND. ( DESCH(RSRC_).NE.0 .OR. 
     $           DESCH(CSRC_).NE.0 ) ) THEN
*     
*              ==== A PDLAQR0 failure!  PDLAQR1 sometimes succeeds
*              .    when PDLAQR0 fails. ====
*
               IF( DEBUG ) WRITE(*,*) 'A PDLAQR0 failure'
               KBOT = INFO
               IF( DEBUG ) WRITE(*,*) 'PDHSEQR -> PDLAQR1 1'
...
*              .    when PDLAQR1 fails. ====
*
               IF( DEBUG ) WRITE(*,*) 'A rare PDLAQR1 failure'
               KBOT = INFO
*
               IF( N.GE.NL ) THEN
*
...
*
                  IF( DEBUG ) WRITE(*,*) 'PDHSEQR -> PDLAQR0 2'
...
*
         IF( DEBUG ) WRITE(*,*) 'Read out eigenvalues...'
*     
...
*     
         DO 30 I = ILO, IHI-1
            ...
            IF( TMP3.NE.0.0D+00 ) THEN
               ...
               CALL PDELGET( 'All', ' ', TMP4, H, I+1, I+1,
     $              DESCH )
