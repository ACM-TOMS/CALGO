      SUBROUTINE PDLAQR1( WANTT, WANTZ, N, ILO, IHI, A, DESCA, WR, WI,
...
*     .. Local Scalars ..
     ...
     $                   RIGHT, ROTN, UP, VECSIDX, TOTIT, ILOC1, JLOC1,
     $                   ARSRC1, ACSRC1, ARSRC2, ACSRC2, ARSRC3, ACSRC3,
     $                   ILOC2, JLOC2, ILOC3, JLOC3, ILOC4, JLOC4,
     $                   ARSRC4, ACSRC4, TOTNS
     ...
     $                   V3SAVE, TMP1, TMP2, TMP3, TMP4, COS, SIN,
     $                   DUM1, DUM2, DUM3, DUM4, ELEM1, ELEM2, ELEM3,
     $                   ELEM4, SN, CS, TMP, ELEM5
*     ..
*     .. Local Arrays ..
...
*     ..
*     .. External Functions ..
      INTEGER            ILCM, NUMROC, ICEIL, INDXG2P
      DOUBLE PRECISION   PDLAMCH
      EXTERNAL           ILCM, NUMROC, PDLAMCH, ICEIL, INDXG2P
*     ..
...
*
      INFO = 0
      IF( DEBUG ) WRITE(*,*) 'PDLAQR1: WANTT, WANTZ, N, ILO, IHI=',
     $     WANTT, WANTZ, N, ILO, IHI
*
...
*
     ...
     $   JJ = JJ + 1
      ...
      IF( LWORK.EQ.-1 .OR. ILWORK.EQ.-1 ) THEN
         WORK(1) = DBLE( INT( 3*N+
     $        MAX( 2*MAX( LDA, LDZ )+2*LOCALK, JJ ) ) )
         IWORK(1) = 1
         RETURN
      ELSEIF( LWORK.LT.3*N+MAX( 2*MAX( LDA, LDZ )+2*LOCALK, JJ ) ) THEN
         INFO = -15
      END IF
...
*
      ...
      IWORK(3) = TOTNS
*
*     === New part of the code for fixing some stuffs not working very
*         good in the current version of PDLAHQR ====
*
...
*
               ARSRC1 = INDXG2P( I, HBL, MYROW, DESCA(RSRC_), NPROW )
               ACSRC1 = INDXG2P( I, HBL, MYCOL, DESCA(CSRC_), NPCOL )
               ARSRC4 = INDXG2P( I+1, HBL, MYROW, DESCA(RSRC_), NPROW )
               ACSRC4 = INDXG2P( I+1, HBL, MYCOL, DESCA(CSRC_), NPCOL )
               IF( MYROW.EQ.ARSRC1 .AND. MYCOL.EQ.ACSRC1 ) THEN
                  WR( I ) = TMP1
                  WI( I ) = ZERO
               ELSE
                  WR( I ) = ZERO
                  WI( I ) = ZERO
               END IF
              IF( MYROW.EQ.ARSRC4 .AND. MYCOL.EQ.ACSRC4 ) THEN
                  WR( I+1 ) = TMP4
                  WI( I+1 ) = ZERO
               ELSE
                  WR( I+1 ) = ZERO
...
*     
 ...
 470  CONTINUE
      ...
      DO 480 K = ILO, IHI
         IF( .NOT. PAIR ) THEN
            IF( K.LT.N ) THEN
               BORDER = MOD( K, HBL ).EQ.0 .OR. ( K.NE.1 .AND. 
     $              MOD( K, HBL ).EQ.1 )
               IF( .NOT. BORDER ) THEN
                  CALL INFOG2L( K, K, DESCA, NPROW, NPCOL, MYROW, MYCOL,
     $                 ILOC1, JLOC1, ARSRC1, ACSRC1 )
                  IF( MYROW.EQ.ARSRC1 .AND. MYCOL.EQ.ACSRC1 ) THEN
                     ELEM1 = A((JLOC1-1)*LDA+ILOC1)
                     ELEM3 = A((JLOC1-1)*LDA+ILOC1+1)
                     IF( ELEM3.NE.ZERO ) THEN
                        ELEM2 = A((JLOC1)*LDA+ILOC1)
                        ELEM4 = A((JLOC1)*LDA+ILOC1+1)
     ...
     $                       CS )
                     ...
                     ELSE 
                        IF( K.GT.1 ) THEN
                           TMP = A((JLOC1-2)*LDA+ILOC1)
                           IF( TMP.NE.ZERO ) THEN
                              ELEM1 = A((JLOC1-2)*LDA+ILOC1-1)
                              ELEM2 = A((JLOC1-1)*LDA+ILOC1-1)
                              ELEM3 = A((JLOC1-2)*LDA+ILOC1)
                              ELEM4 = A((JLOC1-1)*LDA+ILOC1)
                              CALL DLANV2( ELEM1, ELEM2, ELEM3, ELEM4, 
     $                             WR( K-1 ), WI( K-1 ), WR( K ), 
     $                             WI( K ), SN, CS )
                           ELSE
            ...
            ELSE
               CALL INFOG2L( K, K, DESCA, NPROW, NPCOL, MYROW, MYCOL,
     $              ILOC1, JLOC1, ARSRC1, ACSRC1 )
               IF( MYROW.EQ.ARSRC1 .AND. MYCOL.EQ.ACSRC1 ) 
     $              WR( K ) = A((JLOC1-1)*LDA+ILOC1)
            END IF
         ELSE
            PAIR = .FALSE.
         END IF
 480  CONTINUE
      DO 490 K = ICEIL(ILO,HBL)*HBL, IHI-1, HBL
         CALL INFOG2L( K, K, DESCA, NPROW, NPCOL, MYROW, MYCOL,
     $        ILOC1, JLOC1, ARSRC1, ACSRC1 )
         CALL INFOG2L( K, K+1, DESCA, NPROW, NPCOL, MYROW, MYCOL,
     $        ILOC2, JLOC2, ARSRC2, ACSRC2 )
         CALL INFOG2L( K+1, K, DESCA, NPROW, NPCOL, MYROW, MYCOL,
     $        ILOC3, JLOC3, ARSRC3, ACSRC3 )
         CALL INFOG2L( K+1, K+1, DESCA, NPROW, NPCOL, MYROW, MYCOL,
     $        ILOC4, JLOC4, ARSRC4, ACSRC4 )
         IF( MYROW.EQ.ARSRC2 .AND. MYCOL.EQ.ACSRC2 ) THEN
            ELEM2 = A((JLOC2-1)*LDA+ILOC2)
            IF( ARSRC1.NE.ARSRC2 .OR. ACSRC1.NE.ACSRC2 ) 
     $           CALL DGESD2D( CONTXT, 1, 1, ELEM2, 1, ARSRC1, ACSRC1 ) 
         END IF
         IF( MYROW.EQ.ARSRC3 .AND. MYCOL.EQ.ACSRC3 ) THEN
            ELEM3 = A((JLOC3-1)*LDA+ILOC3)
            IF( ARSRC1.NE.ARSRC3 .OR. ACSRC1.NE.ACSRC3 ) 
     $           CALL DGESD2D( CONTXT, 1, 1, ELEM3, 1, ARSRC1, ACSRC1 )
         END IF
         IF( MYROW.EQ.ARSRC4 .AND. MYCOL.EQ.ACSRC4 ) THEN
            WORK(1) = A((JLOC4-1)*LDA+ILOC4)
            IF( K+1.LT.N ) THEN
               WORK(2) = A((JLOC4-1)*LDA+ILOC4+1)
            ELSE
               WORK(2) = ZERO
            END IF
            IF( ARSRC1.NE.ARSRC4 .OR. ACSRC1.NE.ACSRC4 ) 
     $           CALL DGESD2D( CONTXT, 2, 1, WORK, 2, ARSRC1, 
     $           ACSRC1 )
         END IF
         IF( MYROW.EQ.ARSRC1 .AND. MYCOL.EQ.ACSRC1 ) THEN
            ELEM1 = A((JLOC1-1)*LDA+ILOC1)
            IF( ARSRC1.NE.ARSRC2 .OR. ACSRC1.NE.ACSRC2 ) 
     $           CALL DGERV2D( CONTXT, 1, 1, ELEM2, 1, ARSRC2, ACSRC2 ) 
            IF( ARSRC1.NE.ARSRC3 .OR. ACSRC1.NE.ACSRC3 ) 
     $           CALL DGERV2D( CONTXT, 1, 1, ELEM3, 1, ARSRC3, ACSRC3 )
            IF( ARSRC1.NE.ARSRC4 .OR. ACSRC1.NE.ACSRC4 ) 
     $           CALL DGERV2D( CONTXT, 2, 1, WORK, 2, ARSRC4, 
     $           ACSRC4 )
            ELEM4 = WORK(1)
            ELEM5 = WORK(2) 
