...
%
% [V,D,EXITINFO] = IRBLEIGS(A,OPTIONS) 
% [V,D,EXITINFO] = IRBLEIGS(A,M,OPTIONS)
% [V,D,EXITINFO] = IRBLEIGS('Afunc',N,OPTIONS) 
% [V,D,EXITINFO] = IRBLEIGS('Afunc',N,M,OPTIONS)
%
...
%
% IV.)  [V,D,EXITINFO] = IRBLEIGS(A) or [V,D,EXITINFO] = IRBLEIGS(A,M)
%       This option returns the same as (III) plus a two dimensional array EXITINFO 
%       that reports if the algorithm converges and the number of matrix vector 
%       products. If EXITINFO(1) = 0 then this implies normal return: all eigenvalues have 
%       converged. If EXITINFO(1) = 1 then the maximum number of iterations have been 
%       reached before all desired eigenvalues have converged. EXITINFO(2) contains the 
%       number of matrix vector products used by the code. If the maximum number of 
%       iterations are reached (EXITINFO(1) = 1), then the matrices V and D contain any 
...
while (iter <= maxit)
   ...
   % deflate if searching for interior eigenvalues.
   if (nbls > 2 & ~strcmp(sigma,'IE')) 
...
% Output option IV: Output EXITINFO. 
if nargout == 3, EXITINFO(2) = mprod; varargout{3} = EXITINFO; end
