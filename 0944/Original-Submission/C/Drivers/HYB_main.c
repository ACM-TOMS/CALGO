 ...
 *                                                                        *
 *                                                                        *
 *          DRIVER MAIN FOR THE MPI/OMP-BASED HYB IMPLEMENTATION          *
 *                            OF TALBOT SUITE                             *
 *                                                                        *
 *                                                                        *
 *                                                                        *
...
*/
{
    ...
    /*     CALL THE HYB PARALLEL FUNCTION - modified Talbot's method
           (MPI coarse grain parallelism / OMP fine grain parallelism) */
    IFAIL_tot_loc = HYB_Talbot3 (LTfun,sigma0,NTval_loc,t_loc,tol,NUMft_loc,IFAIL_loc,
    ...
    /* (4) COMPUTE THE MAX OF LOCAL ELAPSED TIMES (only in Root) */
    MPI_Reduce(&TOT_TIME_loc,&TOT_TIME,1,MPI_DOUBLE,MPI_MAX,Root,MPI_COMM_WORLD);

    /* (5) COMBINATION OF LOCAL RESULTS (only in Root) */
    ...
    if (myid == Root)
    {
        ...
        printf("\n\tinput tolerance: tol = %5.2e\t\tMPI procs: %d\tOMP threads: %d\t\telapsed time: %e\n", tol,nprocs,THREADS,TOT_TIME);
        if (NTval < 50)
        {
            if (!IFAIL_tot)
            {
