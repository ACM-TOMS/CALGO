...
echo ""
echo "run_me.sh: shell script to run driver programs"
...
echo "\n-------------------------------------------------\n"
echo "1) compile and link with Makefile: make ${Tver}\n"
make ${Tver}
echo "\n-------------------------------------------------\n"
...
1)	echo "2) run the executable: ./${Tver}_talbot.exe"
	./${Tver}_talbot.exe $Tfun
	;;

#   2) OMP
2)	echo "2) run the executable: ./OMP_talbot.exe $Tfun $threads"
	./OMP_talbot.exe $Tfun $threads
	;;

#   3) MPI
#   WITHOUT -machinefile OPTION TO mpiexec
3)	echo "2) run the executable: mpiexec -np $nprocs ./MPI_talbot.exe $Tfun"
	mpiexec  -np $nprocs ./MPI_talbot.exe $Tfun

#   OR, ALTERNATIVELY, WITH -machinefile OPTION TO mpiexec
#3)	echo "2) run the executable: mpiexec -machinefile hostlist -np $nprocs ./MPI_talbot.exe $Tfun"
#	mpiexec -machinefile hostlist -np $nprocs ./MPI_talbot.exe $Tfun
	;;
