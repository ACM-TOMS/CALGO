function [f,err] = besselintr(a,nu,m,r,varargin)
%BESSELINTR Compute the integral of a product of Bessel functions
%   F = BESSELINTR(A,NU,M,R) tries to compute the integral of the
%   product
%
%       X^M * J_NU(1)(A(1)*X) * ... * J_NU(k)(A(k)*X) / (X^2 + R^2)
%
...
%   order), M is a real number and R is a real number. J_NU(i) is the
%   Bessel function of the first kind and order NU(i).
%
%   F = BESSELINTR(A,NU,M,R,RELTOL) uses a relative error tolerance of
%   RELTOL instead of the default, which is 50*eps.
%
%   F = BESSELINTR(A,NU,M,R,RELTOL,ABSTOL) also uses an absolute error
...
%
%   [F,ERR] = BESSELINTR(...) also returns an error estimate in ERR.
...
%   function of its parameters, a warning message is given.
%
...
%            Dept. of Math. & Computer Science, Univ. Antwerp, Belgium,
%            Ronald Cools
...
%
%   Software revision date: October 22, 2007
% ----------------------------------------------------------------------

% check validity of input arguments
if nargin < 4
    error('BESSELINTR requires at least four input arguments.')
elseif nargin > 6
...
elseif imag(r) ~= 0,
    error('Coefficient r must be real.');
end
...
% compute optimal breakpoint for accuracy
i = find(tol);
if length(i) == 2, i = 2; end
x = gop(a,nu,m-2,r,tol(i),i-1);

% compute infinite range approximation
nr = ceil(log(eps)/(2*log(abs(r)/x(1))));
[f1,x,param,s,sflag] = ira(a,nu,m-2,x,k,tol(i),i-1,nr,r);
if sflag, % exceptional case, display warning message
    disp(sprintf(s));
    warning('Possible discontinuous case; result and error estimate may be inaccruate');
end

% perform finite range integration
[f2,err2] = fri(a,nu,m,r,x(1),tol);
...
x(1)^(m-k/2-2*x(2)-4) / sqrt(prod(a));
err(2) = (abs(abserr1)+err2) / am^(m+1); % renormalized!

% fudge parameter for reliability
fderr = 5; err = err * fderr;

err(1) = max(eps,err(2)/abs(f)); % a relative error less than eps ...
err(2) = err(1)*abs(f); % ... does not make much sense

if (err(i) > tol(i)) & ~sflag & (nargout <= 1),
    warning('Requested precision not reached');
end

% ----------------------------------------------------------------------

function x = gop(a,nu,m,r,tol,errflag)
%GOP Get optimal breakpoint x0 and corresponding order n
...
c = 2^(k-1)*log(1/eps)*tGJ*pi/(k*N*sum(a));
ck = sqrt(c/abs(r))/2;

% compute LambertW(ck)
if ck < 2, w1 = 0; else w1 = log(ck) - log(log(ck)); end
w2 = (ck*exp(-w1)+w1^2) / (w1+1);
...
fmin = optimfun(nmin,a,nu,m,tol,errflag,k,x0);
if fmin < 0,
    n = nmin; % smallest n suffices (optimfun is decreasing with n)
else
    nmax = nmin + 10;
    fmax = optimfun(nmax,a,nu,m,tol,errflag,k,x0);
...
n = ceil(n); % n should be integer
x = [x0,n];

% ----------------------------------------------------------------------

function f = optimfun(n,a,nu,m,tol,errflag,k,x0)
...
function [f1,x,param,s,sflag] = ira(a,nu,m,x,k,tol,errflag,nr,r)
...
% if value of n obtained from gmp was too large, determine new
...
for count = 0:nr-1,
	...
	if abs(e) <= eabs*eps % special case
		...
		if ~sflag,
		    s1 = ' a(1)'; sflag = 1;
		    for js = 2:k, % string with zero combination
			if b(js-1), op = '-'; else op = '+'; end
		        s1 = strcat(s1,op,'a(',num2str(js),')');
		    end
		    s = strcat(s,'Assuming ',s1,'=0;');
		end
		Fe = Fe + F(1:fix(-n0)+1); % gather coeffs inf integrals
	    ...
	    dflag = 0;
	    Ihl = igammarange(n0,-e,x0,length(F)-1); 
	end
    ...
    if ~isempty(tmp),
	f1 = real(Fe(tmp(1)))*inf; % first non-zero coeff dominant inf
	return
    else
	f = 2*real(If);
    end
...
% IGAMMARANGE Compute range of igamma values recursively

iu = sqrt(-1); iuy = iu*y;
f = zeros(1,s+1);

i = 1;
while x < 0, % this is rather exceptional; do not use recurrence
    f(i) = igamma(-x, iuy*x0) * (iuy)^x;
    x = x + 1; i = i + 1; s = s - 1;
end

n = max(min(round(abs(y*x0)-x),s),0); % point to start recurrence from
a = exp(-iuy*x0); c = exp(-(x+n)*log(x0));

ig = igamma(-x-n, iuy*x0);
if ig == 0, % bypass underflow in igamma with asymptotic series term
    I_n = a*c / (iuy*x0 + x + n);
else
    I_n = ig * (iuy)^(x+n);
end
I = I_n;
f(i+n) = I; b = c;
for j = n+1:s, % forward recurrence
    b = b/x0;
    I = -1/(x + j) * (I*iuy - b*a);
    f(i+j) = I;
end
...
function [f,err] = fri(a,nu,m,r,x0,tol)
...
if abs(fix(p)-p) > abs(p)*50*eps, % fractional degree -> extrapolate
    xs = z(2); z = z(2:end);
    rho = 1/4; f1 = 0; i = 0;
    [f2,err] = nqf(@fun,[xs/2,xs],tol,a,nu,m,r);
    e = err;
    I1(1) = f2; xs = xs/2;
    while abs(f2-f1) > tol(1)*abs(f2) & abs(f2-f1) > tol(2),
	i = i + 1;
	[I2(1),err] = nqf(@fun,[xs*rho,xs],tol,a,nu,m,r);
	e = e + err;
	I2(1) = I1(1) + I2(1);
