...
*
*  The program writes the results to files zfig3_12.out, zfig3_14.out,
*  zfig3_16.out, and zfig3_17.out. Each of the files contains one of the
*  ratios described above for the set of matrices specified by
*  the input file.  The ratios in each file are:
*
*  zfig3_12.out:  || s - svlues ||/(||svlues||*eps*max(m,n)) where
...
*
*  zfig3_14.out:  A ratio testing that x is the row space of A.  The
*       ratio is calculated by LAPACK's xQRT14.  The comments in
*       xQRT14 describe the  ratio in more detail.
*
*  zfig3_16.out: norm(B - A*X) / ( max(m,n) * norm(A) * norm(X) * eps )
*       as calulated by LAPACK xQRT16.
*
*  zfig3_17.out:     || r' A ||/(||A||*||B||*max(m,n,nrhs)*eps) where
...
*     .. External Subroutines ..
      EXTERNAL           DAXPY, ZGELSD, ZGELSY, ZGELSZ, ZQRT16, ZQRT25
*     ..
*     .. Intrinsic Functions ..
...
*
         CALL ZQRT25( SCALE, RANK, M, N, NRHS, A0, LDA, B0, LDB, S,
...
*
   ...
   50    CONTINUE
*
         CALL ZGELSD( M, N, NRHS, A, LDA, B, LDB, SD, RCOND, CRANK,
...
*
   ...
   90    CONTINUE
*
         CALL ZQRT16( 'No transpose', M, N, NRHS, A0, LDA, X, LDB, B,
...
*
         ACC17( IC, 1 ) = ZERO
         IF( M.GT.CRANK )
     $      ACC17( IC, 1 ) = ZQRT17( 'No transpose', 1, M, N, NRHS, A0,
...
*
         ACC14( IC, 1 ) = ZERO
         IF( N.GT.CRANK )
     $      ACC14( IC, 1 ) = ZQRT14( 'No transpose', M, N, NRHS, A0,
...
*
  ...
  130    CONTINUE
*
         CALL ZGELSY( M, N, NRHS, A, LDA, B, LDB, IPVT, RCOND, CRANK,
...
*
         ACC12( IC, 2 ) = ZQRT12( CRANK, CRANK, A, LDA, S, WORK, LWORK,
     $                    RWORK )
*
*        Test:  Compute error in solution
*
...
*
         ACC17( IC, 2 ) = ZERO
         IF( M.GT.CRANK )
     $      ACC17( IC, 2 ) = ZQRT17( 'No transpose', 1, M, N, NRHS, A0,
     $                       LDA, X, LDB, B0, LDB, C, WORK, LWORK )
*
*        Test:  Check if x is in the rowspace of A
*
         ACC14( IC, 2 ) = ZERO
         IF( N.GT.CRANK )
     $      ACC14( IC, 2 ) = ZQRT14( 'No transpose', M, N, NRHS, A0,
     $                       LDA, X, LDB, WORK, LWORK )
*
*        ******* xGELSZ **********************************
*
  ...
  210    CONTINUE
*
         CALL ZGELSZ( M, N, NRHS, A, LDA, B, LDB, IPVT, RCOND, CRANK,
     $                WORK, LWORK, RWORK, INFO )
*
         SUM = 0
         DO 230 I = 1, N
...
*        Test :  Compute relative error in sing. values of T11
*
         ACC12( IC, 3 ) = ZQRT12( CRANK, CRANK, A, LDA, S, WORK, LWORK,
     $                    RWORK )
*
*        Test:  Compute error in solution
*
...
*
         ACC17( IC, 3 ) = ZERO
         IF( M.GT.CRANK )
     $      ACC17( IC, 3 ) = ZQRT17( 'No transpose', 1, M, N, NRHS, A0,
     $                       LDA, X, LDB, B0, LDB, C, WORK, LWORK )
*
*        Test:  Check if x is in the rowspace of A
*
         ACC14( IC, 3 ) = ZERO
         IF( N.GT.CRANK )
     $      ACC14( IC, 3 ) = ZQRT14( 'No transpose', M, N, NRHS, A0,
     $                       LDA, X, LDB, WORK, LWORK )
*
  260 CONTINUE
*
