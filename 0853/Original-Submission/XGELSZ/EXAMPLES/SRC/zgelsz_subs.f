      SUBROUTINE ZGELSZ( M, N, NRHS, A, LDA, B, LDB, JPVT, RCOND, RANK,
...
*  =======
*
*  ZGELSZ computes the minimum-norm solution to a complex linear least
*  squares problem:
*      minimize || A * X - B ||
...
*             LWORK >= MAX( 3*MN+N+1, 2*MN+NRHS ),
*          where MN = min( M, N ).
*          The block algorithm requires that:
*             LWORK >= MAX( 3*MN+NB*(N+1), 2*MN+NB*NRHS ),
...
*     .. External Subroutines ..
      EXTERNAL           DLABAD, XERBLA, ZCOPY, ZLAQP3, ZLASCL, ZLASET,
     $                   ZTRSM, ZTZRZF, ZUNMQR, ZUNMRZ
*     ..
*     .. External Functions ..
      INTEGER            ILAENV
...
*
      ...
      LWKOPT = MAX( 1, 3*MN+NB*( N+1 ), 2*MN+NB*NRHS )
...
*
      ...
      END IF
*
      BNRM = ZLANGE( 'M', M, NRHS, B, LDB, WORK )
      IBSCL = 0
...
*     is the effective rank of A.
*
      CALL ZLAQP3( M, N, A, LDA, JPVT, RCOND, RANK, WORK( 1 ),
     $             WORK( MN+1 ), LWORK-MN, RWORK, INFO )
      WSIZE = MN + DBLE( WORK( MN+1 ) )
*
*     complex workspace: 3*MN+NB*(N+1). real workspace 2*N.
...
*
      CALL ZUNMQR( 'Left', 'Conjugate transpose', M, NRHS, RANK, A, LDA,
...
*
      END
      SUBROUTINE ZLAQP3( M, N, A, LDA, JPVT, RCOND, RANK, TAU, WORK,
     $                   LWORK, RWORK, INFO )
*
*     This code is part of a package for solving rank deficient least
*     squares problems, written by:
...
*  =======
*
*  ZLAQP3 uses Level 3 BLAS to compute enough of a QR factorization
...
*     ..
*     .. External Subroutines ..
      EXTERNAL           XERBLA, ZGEQRF, ZLAIC1, ZLAQPP, ZLAQPQ, ZSWAP,
     $                   ZUNMQR
*     ..
*     .. External Functions ..
      INTEGER            ILAENV
...
*CC      CALL ZGEQR2( M, NA, A, LDA, TAU, WORK, INFO )
         ...
         IF( NA.LT.N ) THEN
*CC         CALL ZUNM2R( 'Left', 'Conjugate transpose', M, N-NA, NA, A,
...
*
               CALL ZLAQPP( M, N-J+1, J-1, JB, FJB, A( 1, J ), LDA,
...
*
         IF( J.LE.MN )
     $      CALL ZLAQPQ( M, N-J+1, J-1, A( 1, J ), LDA, JPVT( J ),
...
*
      END
      SUBROUTINE ZLAQPP( M, N, OFFSET, NB, KB, A, LDA, JPVT, RCOND,
...
*
*
*  ZLAQPP uses a partial QR factorization with column pivoting applied
...
*
*  XMIN    (input/output) COMPLEX*16 array, dimension
*          MIN( (OFFSET+N), M )
...
*
*  XMAX    (input/output) COMPLEX*16 array, dimension
*          MIN( (OFFSET+N), M )
*          On entry, an approximate left singular vector of R00
...
*     .. Local Scalars ..
      ...
      DOUBLE PRECISION   SMAXPR, SMINPR, TEMP, TEMP2
      COMPLEX*16         AKK, C1, C2, S1, S2
*     ..
*     .. External Subroutines ..
      EXTERNAL           ZGEMM, ZGEMV, ZLAIC1, ZLARFG, ZSWAP
*     ..
*     .. Intrinsic Functions ..
...
*              or when the RANK of A is zero
         ...
         ELSE
*
            CALL ZLAIC1( IMIN, RANK, XMIN, SMIN, A( 1, K ),
     $                   A( RANK+1, K ), SMINPR, S1, C1 )
            CALL ZLAIC1( IMAX, RANK, XMAX, SMAX, A( 1, K ),
...
*
      END
      SUBROUTINE ZLAQPQ( M, N, OFFSET, A, LDA, JPVT, RCOND, RANK, TAU,
...
*     .. Array Arguments ..
      INTEGER            JPVT( * )
      DOUBLE PRECISION   VN1( * ), VN2( * )
      COMPLEX*16         A( LDA, * ), TAU( * ), WORK( * ), XMAX( * ),
     $                   XMIN( * )
*     ..
*
*  Purpose
*  =======
*
*  ZLAQPQ uses a partial QR factorization with column pivoting applied
...
*  passed into ZLAQPQ; however estimates of largest and smallest
*  singular values and corresponding approximate singular vectors
*  of R00 are supplied to ZLAQPQ and these are used to estimate
*  the condition number of R1. On output RANK is the order of R1.
*
*  ZLAQPQ uses Level 2 BLAS.
*
*  Arguments
*  =========
*
...
*     ..
*     .. External Subroutines ..
      EXTERNAL           ZLAIC1, ZLARF, ZLARFG, ZSWAP
*     ..
*     .. Intrinsic Functions ..
...
*              the rank is zero
         ...
         ELSE
*
            CALL ZLAIC1( IMIN, RANK, XMIN, SMIN, A( 1, I ),
     $                   A( RANK+1, I ), SMINPR, S1, C1 )
            CALL ZLAIC1( IMAX, RANK, XMAX, SMAX, A( 1, I ),
