...
*
*  The program writes the results to files dfig3_12.out, dfig3_14.out,
*  dfig3_16.out, and dfig3_17.out. Each of the files contains one of the
*  ratios described above for the set of matrices specified by
*  the input file.  The ratios in each file are:
*
*  dfig3_12.out:  || s - svlues ||/(||svlues||*eps*max(m,n)) where
...
*
*  dfig3_14.out:  A ratio testing that x is the row space of A.  The
*       ratio is calculated by LAPACK's xQRT14.  The comments in
*       xQRT14 describe the  ratio in more detail.
*
*  dfig3_16.out: norm(B - A*X) / ( max(m,n) * norm(A) * norm(X) * eps )
*       as calulated by LAPACK xQRT16.
*
*  dfig3_17.out:     || r' A ||/(||A||*||B||*max(m,n,nrhs)*eps) where
...
*
      CHARACTER*80       LINE
*
      DOUBLE PRECISION   EPS, NORMA, NORMB, ONE, SUM, RCOND, ZERO
      DOUBLE PRECISION   DMAX12, DMAX16, DMAX14, DMAX17
      DOUBLE PRECISION   YMAX12, YMAX16, YMAX14, YMAX17
      DOUBLE PRECISION   ZMAX12, ZMAX16, ZMAX14, ZMAX17
      DOUBLE PRECISION   A( LDA, LDA ), A0( LDA, LDA ), B( LDB, NRHS ),
     $                   B0( LDB, NRHS ), C( LDB, NRHS ),
     $                   X( LDB, NRHS ), XY( LDB, NRHS )
      DOUBLE PRECISION   WORK( LWORK )
      DOUBLE PRECISION   S( LDA ), SD( LDA )
      DOUBLE PRECISION   ACC12( MAXIC, 3 ), ACC14( MAXIC, 3 ),
...
*     .. External Subroutines ..
      EXTERNAL           DAXPY, DGELSD, DGELSY, DGELSZ, DQRT16, DQRT25
*     ..
*     .. Intrinsic Functions ..
...
*
         CALL DQRT25( SCALE, RANK, M, N, NRHS, A0, LDA, B0, LDB, S,
...
*
   ...
   50    CONTINUE
*
         CALL DGELSD( M, N, NRHS, A, LDA, B, LDB, SD, RCOND, CRANK,
...
*
         IF( RANK.GT.0 ) THEN
            CALL DAXPY( MNMIN, -ONE, S, 1, SD, 1 )
            ACC12( IC, 1 ) = DASUM( MNMIN, SD, 1 ) /
     $                       DASUM( MNMIN, S, 1 ) /
     $                       ( EPS*DBLE( MNMIN ) )
         ELSE
            ACC12( IC, 1 ) = ZERO
...
*
   ...
   90    CONTINUE
*
         CALL DQRT16( 'No transpose', M, N, NRHS, A0, LDA, X, LDB, B,
     $                LDB, WORK, ACC16( IC, 1 ) )
*
...
*
         ACC17( IC, 1 ) = ZERO
         IF( M.GT.CRANK )
     $      ACC17( IC, 1 ) = DQRT17( 'No transpose', 1, M, N, NRHS, A0,
...
*
         ACC14( IC, 1 ) = ZERO
         IF( N.GT.CRANK )
     $      ACC14( IC, 1 ) = DQRT14( 'No transpose', M, N, NRHS, A0,
...
*
  ...
  130    CONTINUE
*
         CALL DGELSY( M, N, NRHS, A, LDA, B, LDB, IPVT, RCOND, CRANK,
     $                WORK, LWORK, INFO )
         DO 150 I = 1, N
            DO 140 J = 1, NRHS
               X( I, J ) = B( I, J )
...
*
         ACC12( IC, 2 ) = DQRT12( CRANK, CRANK, A, LDA, S, WORK, LWORK )
*
*        Test:  Compute error in solution
*
         DO 170 I = 1, M
...
*
         ACC17( IC, 2 ) = ZERO
         IF( M.GT.CRANK )
     $      ACC17( IC, 2 ) = DQRT17( 'No transpose', 1, M, N, NRHS, A0,
     $                       LDA, X, LDB, B0, LDB, C, WORK, LWORK )
*
*        Test:  Check if x is in the rowspace of A
*
         ACC14( IC, 2 ) = ZERO
         IF( N.GT.CRANK )
     $      ACC14( IC, 2 ) = DQRT14( 'No transpose', M, N, NRHS, A0,
     $                       LDA, X, LDB, WORK, LWORK )
*
*        ******* xGELSZ **********************************
*
  ...
  210    CONTINUE
*
         CALL DGELSZ( M, N, NRHS, A, LDA, B, LDB, IPVT, RCOND, CRANK,
     $                WORK, LWORK, INFO )
         SUM = 0
         DO 230 I = 1, N
            DO 220 J = 1, NRHS
...
*        Test :  Compute relative error in sing. values of T11
*
         ACC12( IC, 3 ) = DQRT12( CRANK, CRANK, A, LDA, S, WORK, LWORK )
*
*        Test:  Compute error in solution
*
         DO 250 I = 1, M
...
*
         ACC17( IC, 3 ) = ZERO
         IF( M.GT.CRANK )
     $      ACC17( IC, 3 ) = DQRT17( 'No transpose', 1, M, N, NRHS, A0,
     $                       LDA, X, LDB, B0, LDB, C, WORK, LWORK )
*
*        Test:  Check if x is in the rowspace of A
*
         ACC14( IC, 3 ) = ZERO
         IF( N.GT.CRANK )
     $      ACC14( IC, 3 ) = DQRT14( 'No transpose', M, N, NRHS, A0,
     $                       LDA, X, LDB, WORK, LWORK )
*
  260 CONTINUE
*
