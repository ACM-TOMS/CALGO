...
*     .. External Subroutines ..
     ...
     $                   DGELSD, DGELSS, DGELSX, DGELSY, DGELSZ, DGEMM,
     $                   DLACPY, DLARNV, DLASRT, DQRT13, DQRT15, DQRT16,
     $                   DSCAL, XLAENV
*     ..
*     .. Intrinsic Functions ..
...
*
                        SRNAMT = 'DGELSZ'
                        CALL DGELSZ( M, N, NRHS, A, LDA, B, LDB, IWORK,
     $                               RCOND, CRANK, WORK, LWLSY, INFO )
                        IF( INFO.NE.0 )
     $                     CALL ALAERH( PATH, 'DGELSZ', INFO, 0, ' ', M,
     $                                  N, NRHS, -1, NB, ITYPE, NFAIL,
     $                                  NERRS, NOUT )
*
...
*                                workspace: M*N + 4*MIN(M,N) + MAX(M,N)
*
                        RESULT( 11 ) = DQRT12( CRANK, CRANK, A, LDA,
     $                                 COPYS, WORK, LWORK )
*
...
*
                        RESULT( 21 ) = ZERO
                        IF( M.GT.CRANK )
     $                     RESULT( 21 ) = DQRT17( 'No transpose', 1, M,
     $                                    N, NRHS, COPYA, LDA, B, LDB,
     $                                    COPYB, LDB, C, WORK, LWORK )
*
...
*
                        RESULT( 22 ) = ZERO
                        IF( N.GT.CRANK )
     $                     RESULT( 22 ) = DQRT14( 'No transpose', M, N,
     $                                    NRHS, COPYA, LDA, B, LDB,
     $                                    WORK, LWORK )
*
*                       Print information about the tests that did not
