...
*     .. External Subroutines ..
     ...
     $                   CGELSS, CGELSX, CGELSY, CGELSZ, CGEMM, CLACPY,
     $                   CLARNV, CQRT13, CQRT15, CQRT16, CSSCAL, SAXPY,
     $                   SLASRT, XLAENV
*     ..
*     .. Intrinsic Functions ..
...
*                       Test CGELSZ
*
*                       CGELSZ:  Compute the minimum-norm solution
*                       X to min( norm( A * X - B ) )
*                       using the rank-revealing orthogonal
*                       factorization.
*
...
*
                        SRNAMT = 'CGELSZ'
                        CALL CGELSZ( M, N, NRHS, A, LDA, B, LDB, IWORK,
     $                               RCOND, CRANK, WORK, LWLSY, RWORK,
     $                               INFO )
                        IF( INFO.NE.0 )
     $                     CALL ALAERH( PATH, 'CGELSZ', INFO, 0, ' ', M,
     $                                  N, NRHS, -1, NB, ITYPE, NFAIL,
     $                                  NERRS, NOUT )
*
*                       workspace used: 2*MNMIN+NB*NB+NB*MAX(N,NRHS)
...
*                                workspace: M*N + 4*MIN(M,N) + MAX(M,N)
*
                        RESULT( 11 ) = CQRT12( CRANK, CRANK, A, LDA,
     $                                 COPYS, WORK, LWORK, RWORK )
*
...
*
                        RESULT( 21 ) = ZERO
                        IF( M.GT.CRANK )
     $                     RESULT( 21 ) = CQRT17( 'No transpose', 1, M,
     $                                    N, NRHS, COPYA, LDA, B, LDB,
     $                                    COPYB, LDB, C, WORK, LWORK )
*
*                       Test 18:  Check if x is in the rowspace of A
*
                        RESULT( 22 ) = ZERO
                        IF( N.GT.CRANK )
     $                     RESULT( 22 ) = CQRT14( 'No transpose', M, N,
     $                                    NRHS, COPYA, LDA, B, LDB,
     $                                    WORK, LWORK )
*
*                       Print information about the tests that did not
