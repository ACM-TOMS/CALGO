...
*
         ELSE IF( LSAMEN( 3, C3, 'QPF' ) ) THEN
            EMN = MIN( M, N )
*           In MULTS the first term 2*EN*EM is a correction of the
*                incorrect term 2*EN*EN in LAPACK 3.0
            MULTS = 2*EN*EM + EMN*( 3*EM+5*EN+2*EM*EN-( EMN+1 )*
     $              ( 4+EN+EM-( 2*EMN+1 ) / 3 ) )
*           In ADDS the first term EN*EM is a correction of the
*                incorrect term EN*EN in LAPACK 3.0     
            ADDS = EN*EM + EMN*( 2*EM+EN+2*EM*EN-( EMN+1 )*
     $             ( 2+EN+EM-( 2*EMN+1 ) / 3 ) )
*
