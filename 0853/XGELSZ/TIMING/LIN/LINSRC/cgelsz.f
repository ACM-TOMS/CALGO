      SUBROUTINE CGELSZ( M, N, NRHS, A, LDA, B, LDB, JPVT, RCOND, RANK,
...
*     03/05/2004
*
*     instrumented version of CGELSZ to count ops
*
*     .. Scalar Arguments ..
...
*  =======
*
*  CGELSZ computes the minimum-norm solution to a complex linear least
*  squares problem:
*      minimize || A * X - B ||
...
*     .. Local Scalars ..
     ...
     $                   NB, NB1, NB2, NB3, NB4, TRSM, TZRZF, UNMQR,
     $                   UNMRZ
      REAL               ADDS, ANRM, BIGNUM, BNRM, EM, EN, ER, MULTS,
     $                   SMLNUM, T1, T2, WSIZE
*     ..
*     .. External Subroutines ..
      EXTERNAL           CCOPY, CLAQP3, CLASCL, CLASET, CTRSM, CTZRZF,
     $                   CUNMQR, CUNMRZ, SLABAD, XERBLA
*     ..
*     .. External Functions ..
      INTEGER            ILAENV
      REAL               CLANGE, SECOND, SLAMCH, SOPBL3, SOPLA
      EXTERNAL           CLANGE, ILAENV, SECOND, SLAMCH, SOPBL3, SOPLA
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          ABS, MAX, MIN, REAL, CMPLX
*     .. Data statements ..
      DATA               GELSZ / 1 / , LAQP3 / 2 / , TRSM / 5 / ,
...
*
      OPCNT( GELSZ ) = OPCNT( GELSZ ) + REAL( 2 )
...
*
         OPCNT( GELSZ ) = OPCNT( GELSZ ) + REAL( 6*M*N )
...
*
      ...
      END IF
*
      BNRM = CLANGE( 'M', M, NRHS, B, LDB, WORK )
      IBSCL = 0
      IF( BNRM.GT.ZERO .AND. BNRM.LT.SMLNUM ) THEN
*
*        Scale matrix norm up to SMLNUM
*
         OPCNT( GELSZ ) = OPCNT( GELSZ ) + REAL( 6*M*NRHS )
...
*
      OPS = 0
      T1 = SECOND( )
      CALL CLAQP3( M, N, A, LDA, JPVT, RCOND, RANK, WORK( 1 ),
...
*
     ...
     $       ( 2+EN+EM-( 2*ER+1 ) / 3 ) )
      OPCNT( LAQP3 ) = OPCNT( LAQP3 ) + 2*ADDS + 6*MULTS
*
*     Calculate approximate flop count in condition estimation.
*
      OPCNT( GELSZ ) = OPCNT( GELSZ ) + REAL( 6*RANK*RANK-5*RANK-1 ) +
     $                 OPS
...
*
     ...
     $                 SOPLA( 'CUNMQR', M, NRHS, RANK, 0, 0 )
      T1 = SECOND( )
      CALL CUNMQR( 'Left', 'Conjugate transpose', M, NRHS, RANK, A, LDA,
...
*
      OPCNT( TRSM ) = OPCNT( TRSM ) + SOPBL3( 'CTRSM ', RANK, NRHS, 0 )
      T1 = SECOND( )
...
*
     ...
     $                INFO )
      END IF
      IF( IBSCL.EQ.1 ) THEN
         OPCNT( GELSZ ) = OPCNT( GELSZ ) + REAL( N*NRHS*6 )
