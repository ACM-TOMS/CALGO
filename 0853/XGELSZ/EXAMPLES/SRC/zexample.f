...
*     03/05/2004
*
*     Example -- a simple example that demonstrates the use of ZGELSZ
*
...
*
      ...
      INTEGER            JPVT( N )
      COMPLEX*16         A( M, N ), B( M, N ), WORK( LWORK )
...
*
      DO 20 I = 1, M
         WRITE( *, '(3(A,G8.2,A,G8.2,A))' )( '(', DBLE( A( I, J ) ),
     $      ',', DIMAG( A( I, J ) ), ')  ', J = 1, N )
   20 CONTINUE
*
      WRITE( *, * )
      WRITE( *, * )'and b is '
      DO 30 I = 1, M
         WRITE( *, '(A,G13.5,A,G13.5,A)' )'(', DBLE( 1.0*B( I, 1 ) ),
     $      ',', DIMAG( B( I, 1 ) ), ')  '
   30 CONTINUE
*
*     call zgelsz
      INFO = 0
      RCOND = 1.0D-10
      CALL ZGELSZ( M, N, NRHS, A, M, B, M, JPVT, RCOND, RANK, WORK,
     $             LWORK, RWORK, INFO )
*
*     print the results
      WRITE( *, * )
