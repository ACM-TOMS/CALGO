...
%	
%	[W,BETA] = SCFIX(TYPE,W,BETA) attempts to fix a problem in the
%	given polygon that arises from the posing of the parameter
%	problem.  SCFIX is used when a call to xxPARAM results in an
%	error and so advises.  In this case the polygon as given
%	violates some fairly arbitrary constraint.  SCFIX remedies the
%	situation by renumbering the vertices, or, if necessary, adding
%	a trivial (zero-turn) vertex.  TYPE is one of
%	{'hp','d','de','st','r'}.  If one additional input and output
%	argument is given, it represents the indices of the strip ends
%	or the rectangle corners.
%	
%       See also SCCHECK, SCADDVTX.
% 
%	Written by Toby Driscoll.  Last modified 5/31/95.
