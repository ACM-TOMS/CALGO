...
*/
    ...
    int     nudim = 4,         /* nudim >= ncomp */
            ...
            ltol[4],           /* (In par) Array of size ntol. For each i, ltol(i) gives the index of the component
                                          of the computed solution U controlled by the ith tolerance vtol(i) */
            iflbvp,            /* (Out par) Output error flag
                                          =  0  success
                                          = -1  one of the input parameters is invalid
                                          =  1  the number of mesh points needed for the next iteration would exceed nmax */
            nmsh = NXval,      /* (In/Out par) number of mesh points */
            nmax = MAXxx,      /* (Out par) Maximum number of mesh points. nmax <= MAXxx */
    ...
    /* U is an (Out par) array of pointer, where each component U[k] is a pointer to a matrix.
