 ...
 **/
{
    ...
    /*  FLAG TO ACTIVATE OR NOT THE MATLAB PARALLEL COMPUTING TOOLBOX (PCT)
        PCT = 0     false: without PCT and 'Vectorized' option to 'on'
            = 1     true:  with PCT and parfor in LT_samples.m     */
    int PCT = 0;
/*  int PCT = 1; */
    ...
    char *LTS = "LT samples by solving ODE problems by means of ode45.m";
    mexPrintf("%s", str);
        ...
        switch (jFUN)
        {
            case 1 : /* (OpenMP coarse-grained parallel modified method of Talbot Suite DE)  */
                ...
                for ( THREADS=1;  THREADS<=maxTHREADS;  THREADS++ )
                {
                    ...
                    if ( PCT )
                    {
                        prhs1[0] = mxCreateDoubleScalar((double)THREADS); /* 1st input param */
                        if ( mexCallMATLAB(  0,  plhs1,   1,  prhs1, "parpool") )
                            mexErrMsgTxt( "No parpool available.");
                        mxDestroyArray(prhs1[0]); /* deallocate memory */
                    }
