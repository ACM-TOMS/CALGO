 ...
 **/
{
    ...
    double Tmin=100.0,   Tmax=500.0,   Xmin= 10.0,   Xmax= 20.0; /* endpoints */
    ...
    char *STR = "OMP_LTsamples_ode";          /* OpenMP parallel LT samples */
            ...
            case 1 : /* (OpenMP coarse-grained parallel modified method of Talbot Suite DE)  */
                ...
                for ( THREADS=1;  THREADS<=maxTHREADS;  THREADS++ )
                {
                    ...
                    for (Nt=0; Nt<NTIMES; Nt++)
                    {
                            ...
                            QueryPerformanceCounter(&TICKS1);
                        #endif
                                IFAIL_tot = OMP_Talbot11_DE ( OMP_LTsamples_ode, sigma0,NXval,x,NTval,t,tol,NUMft,IFAIL,Nsings,SINGS,MULT,Tmin,Tmax,THREADS);
                        #ifdef __unix__
                            clock_gettime(CLOCK_REALTIME, &ts2);
                            T0 = (double)(ts2.tv_sec - ts1.tv_sec);   /* sec      */
                            T1 = (double)(ts2.tv_nsec - ts1.tv_nsec); /* nano sec */
            ...
            case 2 : /* (OpenMP fine-grained parallel modified method of Talbot Suite DE)  */
                ...
                for ( THREADS=1;  THREADS<=maxTHREADS;  THREADS++ )
                {
                    ...
                    for (Nt=0; Nt<NTIMES; Nt++)
                    {
                            ...
                            QueryPerformanceCounter(&TICKS1);
                        #endif
                                IFAIL_tot = OMP_Talbot12_DE ( OMP_LTsamples_ode, sigma0,NXval,x,NTval,t,tol,NUMft,IFAIL,Nsings,SINGS,MULT,Tmin,Tmax,THREADS);
                        #ifdef __unix__
                            clock_gettime(CLOCK_REALTIME, &ts2);
                            T0 = (double)(ts2.tv_sec - ts1.tv_sec);   /* sec      */
                            T1 = (double)(ts2.tv_nsec - ts1.tv_nsec); /* nano sec */
