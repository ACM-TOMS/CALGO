...
double complex *OMP_LTsamples_fun (unsigned int NXYval, double XY[], unsigned int NOPTS, double complex S[], double tol, int THREADS);
...
/** RUN AS:
            $ ./ex5_time.exe tol jFUN NTval NXYval
        ...
        NXYval   number of (x,y)-values (5, 20, 120)
 **/
{
    ...
    if (argc > 4)
    {
        ...
        NXYval = (unsigned int)atoi(argv[4]); /* 4th argument to main */
    }
    else
    {
            ...
            case 1 : /* (OpenMP coarse-grained parallel modified method of Talbot Suite DE)  */
                ...
                for ( THREADS=1;  THREADS<=maxTHREADS;  THREADS++ )
                {
                    ...
                    for (Nt=0; Nt<NTIMES; Nt++)
                    {
                            ...
                            QueryPerformanceCounter(&TICKS1);
                        #endif
                                IFAIL_tot = OMP_Talbot11_DE ( OMP_LTsamples_fun, sigma0,Nrows,XY,NTval,t,tol,NUMft,IFAIL,Nsings,SINGS,MULT,Tmin,Tmax,THREADS);
                        #ifdef __unix__
                            clock_gettime(CLOCK_REALTIME, &ts2);
                            T0 = (double)(ts2.tv_sec - ts1.tv_sec);   /* sec      */
                            T1 = (double)(ts2.tv_nsec - ts1.tv_nsec); /* nano sec */
            ...
            case 2 : /* (OpenMP fine-grained parallel modified method of Talbot Suite DE)  */
                ...
                for ( THREADS=1;  THREADS<=maxTHREADS;  THREADS++ )
                {
                    ...
                    for (Nt=0; Nt<NTIMES; Nt++)
                    {
                            ...
                            QueryPerformanceCounter(&TICKS1);
                        #endif
                                IFAIL_tot = OMP_Talbot12_DE ( OMP_LTsamples_fun, sigma0,Nrows,XY,NTval,t,tol,NUMft,IFAIL,Nsings,SINGS,MULT,Tmin,Tmax,THREADS);
                        #ifdef __unix__
                            clock_gettime(CLOCK_REALTIME, &ts2);
                            T0 = (double)(ts2.tv_sec - ts1.tv_sec);   /* sec      */
                            T1 = (double)(ts2.tv_nsec - ts1.tv_nsec); /* nano sec */
