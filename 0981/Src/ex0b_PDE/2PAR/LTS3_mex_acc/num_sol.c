...
void mexFunction (int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    int IFAIL_tot=0, *IFAIL; /* global and local error flags */
    ...
    int maxTHREADS;
        ...
        omp_set_num_threads(maxTHREADS);  /* set the number of openMP parallel threads */
    #else
        maxTHREADS = 1;
    #endif
    ...
    switch (jFUN)
    {
    case 1 : /* MODIFIED TALBOT'S METHOD: coarse-grained parallelism */
        thrds1 = maxTHREADS;   thrds2=1;
        mexPrintf("\nModified Talbot's method [coarse-grained parallelism: %d threads]\n", thrds1);
        IFAIL_tot = OMP_Talbot11_DE ( OMP_LTsamples_fun, sigma0,NXval,x,NTval,t,tol,NUMft,IFAIL,Nsings,SINGS,MULT,Tmin,Tmax,thrds1);
        break;

    case 2 : /* MODIFIED TALBOT'S METHOD: fine-grained parallelism */
        thrds1 = maxTHREADS;   thrds2=1;
        mexPrintf("\nModified Talbot's method [fine-grained parallelism: %d threads]\n", thrds1);
        IFAIL_tot = OMP_Talbot12_DE ( OMP_LTsamples_fun, sigma0,NXval,x,NTval,t,tol,NUMft,IFAIL,Nsings,SINGS,MULT,Tmin,Tmax,thrds1);
        break;

    case 3 : /* MODIFIED TALBOT'S METHOD: nested parallelism */
        thrds1 = 4;   thrds2 = maxTHREADS/4;
        mexPrintf("\nModified Talbot's method [nested parallelism: %d outer parallel threads, %d inner parallel threads]\n", thrds1,thrds2);
        #ifdef _OPENMP
            mexPrintf("\n\n\tCHECK: NESTED PARALLELISM is %s\n", omp_get_nested() ? "supported" : "not supported");
            if ( !omp_get_nested() )
            {   omp_set_nested(1); /* true */
                mexPrintf("\t                          Now it is %s\n", omp_get_nested() ? "supported" : "not supported");
            }
        #endif
