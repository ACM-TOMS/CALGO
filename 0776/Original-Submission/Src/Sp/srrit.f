      SUBROUTINE SRRIT( N, NV, M, MAXIT, ISTART, Q, LDQ, AQ, LDA, T,
...
*            1,   Orthonormalization subroutine ORTH fails. 
*            2,   error from subroutine SRRSTP.
*            3,   matrix T is singular, see COND.
...
*     .. Parameters ..
      ...
      REAL               STPFAC, ALPHA, BETA
      PARAMETER          ( STPFAC = 1.5, ALPHA = ONE, BETA = 1.1E+0 )
      REAL               ORTTOL
      PARAMETER          ( ORTTOL = TWO )
      REAL               GRPTOL, CNVTOL
      PARAMETER          ( GRPTOL = 1.0E-4, CNVTOL = 1.0E-3 )
*
      INTEGER            INIT, NOUT
...
*     .. Local Scalars ..
     ...
     $                   NOGRP, NV0, NXTORT, NXTSRR
      REAL               AE, AQMAX, ARSD, CTR, OAE, OARSD, OCTR, REC,
     $                   TCOND
*     ..
*     .. Local Arrays ..
      INTEGER            ISEED( 4 )
...
*     ..
*     .. External Functions ..
      REAL               COND, SLANGE, SLARAN
      EXTERNAL           COND, SLANGE, SLARAN
*     ..
*     .. External Subroutines ..
      EXTERNAL           GROUP, ORTH, RESID, SRRSTP, XERBLA
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          ABS, INT, LOG, LOG10, MAX, MIN, REAL
*     ..
...
*
 ...
 9999    FORMAT( 30X, 'IT = ', I4 )
      END IF
*
      CALL SRRSTP( N, L, M, Q, LDQ, AQ, LDA, T, LDT, WR, WI, WORK, M,
     $             WORK( M*M+1 ), IERR, ATQ )
      IF( IERR.NE.0 )THEN 
         INFO = 2
         RETURN
...
*
      CALL RESID( N, L, M, Q, LDQ, AQ, LDA, T, LDT, RSD )
*
      IF( PINFO ) THEN
         WRITE( NOUT, FMT = 9998 )( WR( I ), I = 1, M )
 9998    FORMAT( ' WR   =', 1P,6E12.3 )
         WRITE( NOUT, FMT = 9997 )( WI( I ), I = 1, M )
 9997    FORMAT( ' WI   =', 1P,6E12.3 )
         WRITE( NOUT, FMT = 9996 )( RSD( I ), I = 1, M )
 9996    FORMAT( ' RSD  =', 1P,6E12.3 )
      END IF
*
   90 CONTINUE
*
...
*
      CALL GROUP( L, M, WR, WI, RSD, NGRP, CTR, AE, ARSD, GRPTOL )
      CALL GROUP( L, M, WORK( M2+1 ), WORK( M2+M+1 ), WORK( M2+2*M+1 ),
...
*
 ...
 9994    FORMAT( ' CTR  =', 1P,E12.3, '  AE =', 1P,E12.3, 
     $'  ARSD = ', 1P,E12.3 )
      END IF
*
      IF( NGRP.NE.NOGRP )
     ...
     $   GO TO 100
*
      IF( ABS( AE-OAE ).GT.CTR*CNVTOL*REAL( ITRSD( L )-IWORK( M+L ) ) )
     $   GO TO 100
*
...
*
      NXTSRR = MIN( MAXIT, MAX( INT( STPFAC*REAL( IT ) ), INIT ) )
      IDSRR = NXTSRR - IT
      IF( NGRP.NE.NOGRP )
     $   GO TO 110
      IF( NGRP.EQ.0 )
     $   GO TO 110
      IF( ARSD.GE.OARSD )
     $   GO TO 110
      REC = ALPHA + BETA*REAL( IWORK( M+L )-ITRSD( L ) )*
...
*
  ...
  120    CONTINUE
  130 CONTINUE
      TCOND = COND( M, WORK, M, IWORK, WORK( M*M+1 ), IERR )
      IF( IERR.NE.0 )THEN 
         INFO = 3
         RETURN
      END IF
...
*
      ...
      AQMAX = SLANGE( 'M', N, M-L+1, AQ( 1, L ), LDA, DUMMY )
      REC = ONE / AQMAX
      DO 190 J = L, M
         DO 180 I = 1, N
            Q( I, J ) = AQ( I, J )*REC
...
*
******** begin of srrstp.f ********************************************        
      SUBROUTINE SRRSTP( N, L, M, Q, LDQ, AQ, LDA, T, LDT, WR, WI, U,
     $                   LDU, WORK, INFO, ATQ )
*     .. 
*     .. Scalar Arguments ..
...
*  =======
*
*  SRRSTP performs a Schur-Rayleigh-Ritz refinement on the set of
...
*
*  U       (output) REAL array, dimension(M,M)
...
*          On exit, if INFO is set to 
*            0,  normal return
*            1,  error from reduction T to Hessenberg form (SGEHRD).
*            2,  error from forming orthogonal matrix U (SORGHR).
*            3,  error from Schur decomposition routine (SLAQR3).
*
*  ATQ    External procedure name, which passes the matrix-vectors 
*         product operation.
*
...
*     ..
*     .. External Subroutines ..
      EXTERNAL           SGEHRD, SGEMM, SLAQR3, SLASET, SORGHR
*     ..
*     .. Executable Statements ..
*
      INFO = 0
...
*
      NC = M - L + 1
      CALL ATQ( N, L, M, Q, LDQ, AQ, LDA )
      CALL SGEMM( 'Transpose', 'No transpose', M, NC, N, ONE, Q, LDQ,
     $            AQ( 1, L ), LDA, ZERO, T( 1, L ), LDT )
*
*     Hessenberg reduction
*
      CALL SGEHRD( M, L, M, T, LDT, WORK, WORK( M+1 ), M, IERR )
      IF( IERR.NE.0 )THEN
         INFO = 1
         RETURN
      END IF
   ...
   10    CONTINUE
   20 CONTINUE
      CALL SORGHR( M, L, M, U, LDU, WORK, WORK( M+1 ), M, IERR )
*
*     Compute Schur decomposition
*
      CALL SLAQR3( .TRUE., .TRUE., M, L, M, T, LDT, WR, WI, 1, M, 
     $             U, LDU, WORK, IERR )
      IF( IERR.NE.0 )THEN
         INFO = 3
         RETURN
...
*
******** begin of orth.f **********************************************        
      SUBROUTINE ORTH( N, L, M, Q, LDQ, INFO )
*     .. 
*     .. Scalar Arguments ..
...
*  =======
*
*  ORTH orthonormalizes columns L through M of the array Q with respect
...
*  this is not done in MAXTRY (5) attempts the program stops. ORTH also
...
*     .. Parameters ..
      ...
      PARAMETER          ( MAXTRY = 5 ) 
      REAL               ZERO, ONE, TOL
      PARAMETER          ( ZERO = 0.0E+0, ONE = 1.0E+0, TOL = 0.5E+0 )
*     ..
*     .. Local Scalars ..
...
*
         DO 20 K = 1, JM1
            QQ = SDOT( N, Q( 1, K ), 1, Q( 1, J ), 1 )
            CALL SAXPY( N, -QQ, Q( 1, K ), 1, Q( 1, J ), 1 )
   20    CONTINUE
         GO TO 10
*
   30 CONTINUE
...
*
******** begin of resid.f *******************************************
      SUBROUTINE RESID( N, L, M, Q, LDQ, AQ, LDA, T, LDT, RSD )
*     .. 
*     .. Scalar Arguments ..
...
*          The leading dimension of Q, LDQ >= max(1,N).
*
*  AQ      (input) REAL array, dimension(LDA, M)
*          On entry, AQ contains the product A*Q.
*
*  LDA     (input) INTEGER
*          The leading dimension of AQ, LDA >= max(1,N).
*
*  T       (input) REAL array, dimension( LDT,M )
*          On entry, T contains of Schur form.
*
*  LDT     (input) INTEGER
*          The leading dimension of T, LDT >= max(1,M).
*
*  RSD     (output) REAL array, dimension(M)
...
*
******** begin of group.f **********************************************
      SUBROUTINE GROUP( L, M, WR, WI, RSD, NGRP, CTR, AE, ARSD, GRPTOL )
*     ..
*     .. Scalar Arguments ..
      INTEGER            L, M, NGRP
      REAL               AE, ARSD, CTR, GRPTOL
*     ..
*     .. Array Arguments ..
      REAL               RSD( * ), WI( * ), WR( * )
*     ..
...
*
*  WR,WI   (input) REAL array, dimension(M)
...
*          magnitude.      
*
*  RSD     (input) REAL array, dimension(M)
...
*     End of GROUP
      END
******** begin of cond.f **********************************************
      REAL             FUNCTION COND( M, H, LDH, IPVT, MULT, INFO )
*     .. 
*     .. Scalar Arguments ..
      INTEGER            INFO, LDH, M
*     ..
...
*
*  H       (input/output) REAL array, dimension( LDH,M )
...
*
*  MULT    (workspace) REAL array, dimension(2*M)
*
*  INFO    (output) INTEGER
*          On exit, if INFO is set to
*             0,  normal return.
...
*     .. Parameters ..
      REAL               ZERO, ONE, UPPER
      PARAMETER          ( ZERO = 0.0E+0, ONE = 1.0E+0, UPPER = 1.0E+8 )
*     ..
*     .. Local Scalars ..
...
*
******** begin of slaqr3.f ********************************************
      SUBROUTINE SLAQR3( WANTT, WANTZ, N, ILO, IHI, H, LDH, WR, WI,
...
*     ..
*     .. External Subroutines ..
      EXTERNAL           SCOPY, SLABAD, SLANV2, SLARFG, SROT, SLAEXC
*     ..
*     .. Intrinsic Functions ..
  ...
  250    CONTINUE
         IF( IN.EQ.IHI .OR. H( IN+1, IN ).EQ.ZERO )THEN
            IF( ABS( H( IN, IN ) ).GT.WR1 )THEN
                CALL SLAEXC( WANTZ, N, H, LDH, Z, LDZ, IC, 1, 1, WORK,
     $                       IERR )
                IF( IERR.NE.0 )THEN
                   INFO = N + I
                   RETURN
         ...
         ELSE
            ...
            IF( SLAPY2( H( IN, IN ), TEMP ).GT.WR1 )THEN
               CALL SLAEXC( WANTZ, N, H, LDH, Z, LDZ, IC, 1, 2, WORK,
     $                      IERR )
               IF( IERR.NE.0 )THEN
                   INFO = N + I
                   RETURN
...
*
  230      CONTINUE 
           IF( IN.EQ.IHI .OR. H( IN+1, IN ).EQ.ZERO ) THEN
               IF( ABS( H( IN, IN ) ).GT.WR1 )THEN 
                  CALL SLAEXC( WANTZ, N, H, LDH, Z, LDZ, IC, 1, 1, 
     $                         WORK, IERR )
                  IF( IERR.NE.0 )THEN
                     INFO = N + I
                     RETURN
            ...
            ELSE
               TEMP = SQRT(ABS(H( IN+1, IN )))*SQRT(ABS(H( IN, IN+1 )))
               IF( SLAPY2( H( IN, IN ), TEMP ).GT.WR1 )THEN
                  CALL SLAEXC( WANTZ, N, H, LDH, Z, LDZ, IC, 1, 2,  
     $                         WORK, IERR )
                  IF( IERR.NE.0 )THEN
                     INFO = N + I
                     RETURN
...
*
  260       CONTINUE 
            IF( IN.EQ.IHI .OR. H( IN+1, IN ).EQ.ZERO ) THEN
               IF( ABS( H( IN, IN ) ).GT.WR1 )THEN 
                  CALL SLAEXC( WANTZ, N, H, LDH, Z, LDZ, IC, 2, 1,  
     $                         WORK, IERR )
                  IF( IERR.NE.0 )THEN
                     INFO = N + I - 1
                     RETURN
            ...
            ELSE
               TEMP = SQRT(ABS(H( IN+1, IN )))*SQRT(ABS(H( IN, IN+1 ))) 
               IF( SLAPY2( H( IN, IN ), TEMP) .GT.WR1 )THEN
                  CALL SLAEXC( WANTZ, N, H, LDH, Z, LDZ, IC, 2, 2,  
     $                          WORK, IERR )
                  IF( IERR.NE.0 )THEN
                     INFO = N + I - 1
                     RETURN
