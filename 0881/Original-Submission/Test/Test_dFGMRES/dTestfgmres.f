...
*
      ...
      parameter (lwork = ldstrt**2 + ldstrt*(2*lda+5) + 6*lda + 1)
*
      integer i, j, n, m, m2
...
*
      ...
      write(*,*) 'in double precision real arithmetic'
...
*******************************************************
*
      call init_dfgmres(icntl,cntl)
      call init_dgmres(icntl2,cntl2)
*
*************************
...
*****************************************
*
10     call drive_dfgmres(n,n,m,lwork,work,
...
* gmres iteration 
           call dcopy(n,work(colx),1,work(irc(6)+n),1)
 20        call drive_dgmres(n,n,m2,irc(7),
...
* Perform the matrix vector product for the inner GMRES iteration
             call dgemv('N',n,n,ONE,a,lda,work(colx2),1,
     &                 ZERO,work(colz2),1)
             goto 20
...
*      work(colz) <-- work(colx) work(coly)
             call dgemv('C',n,nbscal2,ONE,work(colx2),n,
     &               work(coly2),1,ZERO,work(colz2),1)
             goto 20
           endif
           call dcopy(n,work(irc(6)),1,work(colz),1)
           goto 10
         else
* (m2.le.0)
