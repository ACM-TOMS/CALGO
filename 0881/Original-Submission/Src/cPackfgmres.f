...
C*
*
        subroutine drive_cfgmres(n,nloc,m,lwork,work,
     &                         irc,icntl,cntl,info,rinfo)
*
*  Purpose
*  =======
*    drive_cfgmres is the driver routine for solving the linear system 
...
* Authors :  Valerie Fraysse, Luc Giraud, Serge Gratton
*             Parallel Algorithms - CERFACS
* Comments: Totally rewritten from the cfgmres implementation
*
...
* setup some pointers on the workspace
       ...
       Zptr     = lwork - (m*nloc)+1
*
       call cfgmres(nloc,m,lwork,work(bptr),work(xptr),work(dotptr),
...
*
        subroutine cfgmres(n,m,lwork,b,x,dot,H,w,r0,V,Z,yCurrent,
     &                xCurrent,rotSin,rotCos,irc,icntl,cntl,info,rinfo)
*
*
*  Purpose
*  =======
*  cfgmres solves the linear system Ax = b using the
...
* ----------------------
       integer  irc(7)
       complex x(n), dot(m), H(m+1,m+1), w(n)
       complex r0(n), V(n,m+1), Z(n,m)
       complex xCurrent(n), rotSin(m), yCurrent(m)   
       complex rotCos(m)
*
* Local variables
* ---------------
...
*
        ...
        irc(7) = irc(7) - n 
*
*       Most outer loop : cfgmres iteration
*
*       REPEAT
 7      continue
*
...
*
          call cgemv('N',n,jH,ONE,Z(1,m-jH+1),n,
...
*
*
        subroutine init_cfgmres(icntl,cntl)
*
*  Purpose
*  =======
