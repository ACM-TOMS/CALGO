      SUBROUTINE SGEQPW( M, LWSIZE, NB, OFFSET, LACPTD, A, LDA,
...
*     .. Local Scalars ..
      ...
      REAL               GAMMA, AKK, TEMP, TEMP2, SMAX
*     ..
*     .. External Subroutines ..
      EXTERNAL           SNRM2, SSCAL, SSWAP, SLARFG,
     $                   SLARF, ISAMAX, SLAUC1, SLAPY2,
     $                   SLASMX
      INTEGER            ISAMAX
      REAL               SNRM2, SLAPY2, SLASMX
      LOGICAL            SLAUC1
*     ..
*     .. Intrinsic Functions ..
...
*
      DO 10 I = 1,LWSIZE
         WORK( I ) = SNRM2( M-OFFSET, A( OFFSET+1, OFFSET+I ), 1 )
...
*
     ...
     $            ISAMAX( LWSIZE-LACPTD, WORK( LACPTD+1 ), 1 )
         K = OFFSET + LACPTD + 1
*
*        Exchange current column and pivot column.
*
         IF( PVTIDX.NE.K ) THEN
            CALL SSWAP( M, A( 1, PVTIDX ), 1, A( 1, K ), 1 )
            I1 = JPVT( PVTIDX )
            JPVT( PVTIDX ) = JPVT( K )
            JPVT( K ) = I1
...
*        =====================================
*
         IF( SLAUC1( K, X, SMIN, A( 1, K ), GAMMA, SMAX*IRCOND ) )
     $        THEN
*
...
*
            IF( K.LT.M ) THEN
               CALL SLARFG( M-K+1, A( K, K ), A( K+1, K ), 1,
     $                      TAU( K ) )
            ELSE
               CALL SLARFG( 1, A( M, K), A( M, K ), 1, TAU ( K ) )
            END IF
*
...
*
            IF( LACPTD.LT.LWSIZE ) THEN
               AKK = A( K, K )
               A( K, K ) = ONE
               CALL SLARF( 'Left', M-K+1, LWSIZE-LACPTD,
