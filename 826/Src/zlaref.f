...
*
   ...
   10          CONTINUE
               DO 20 J = ITMP2-MOD(ITMP2-ITMP1+1,2)+1, ITMP2
                  SUM = DCONJG( T1 )*A( IROW1, J ) + DCONJG( T2 )*
     $                  A( IROW1+1, J ) + DCONJG( T3 )*A( IROW1+2, J )
...
*
   ...
   70          CONTINUE
     ...
     $                     T3*Z( J, ICOL1+2 )
                     ...
                     Z( J, ICOL1+4 ) = Z( J, ICOL1+4 ) - SUM*DCONJG(V33)
   80             CONTINUE
               END IF
               ICOL1 = ICOL1 + 3
   90       CONTINUE
  ...
  100          CONTINUE
     ...
     $                     T3*Z( J, ICOL1+2 )
                     Z( J, ICOL1 ) = Z( J, ICOL1 ) - SUM
                     Z( J, ICOL1+1 ) = Z( J, ICOL1+1 ) - SUM*DCONJG(V2)
                     Z( J, ICOL1+2 ) = Z( J, ICOL1+2 ) - SUM*DCONJG(V3)
  110             CONTINUE
               END IF
               ICOL1 = ICOL1 + 1
  120       CONTINUE
