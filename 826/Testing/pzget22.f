      SUBROUTINE PZGET22( TRANSA, TRANSE, TRANSW, N, A, DESCA, E, 
     $                    DESCE, W, WORK, DESCW, RWORK, RESULT )
*
*  Mark R. Fahey
*  June 23, 2000
*
...
*     .. Local Scalars ..
      ...
      INTEGER            ITRNSE, ITRNSW, J, JCOL, JROW, JVEC
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, DESCINIT, DGAMX2D, DGAMN2D,
...
*
      ...
      IF( ITRNSE.EQ.0 ) THEN
         DO 20 JVEC = 1, N
            TEMP1 = ZERO
            DO 10 J = 1, N
               CALL INFOG2L( J, JVEC, DESCE, NPROW, NPCOL, MYROW, 
     $                       MYCOL, IROW, ICOL, II, JJ )
               IF( ( MYROW.EQ.II ) .AND. ( MYCOL.EQ.JJ ) ) THEN
                  TEMP1 = MAX( TEMP1, CABS1( E( (ICOL-1)*LDE+IROW )))
               ENDIF
   10       CONTINUE
            IF( MYCOL.EQ.JJ ) THEN
     ...
     $                   -1, -1, -1 )
            ...
            ENDIF
   20    CONTINUE
         CALL DGAMX2D( CONTXT, 'Row', ' ', 1, 1, ENRMAX, 1, RA, CA,
     $                 -1, -1, -1 )
         CALL DGAMN2D( CONTXT, 'Row', ' ', 1, 1, ENRMIN, 1, RA, CA,
     $                 -1, -1, -1 )
      ELSE
         DO 50 J = 1, N
            TEMP1 = ZERO
...
*     Norm of A:
*
      ANORM = MAX( PZLANGE( NORMA, N, N, A, 1, 1, DESCA, RWORK ), 
     $             UNFL )
*
*     Norm of E:
*
      ENORM = MAX( PZLANGE( NORME, N, N, E, 1, 1, DESCE, RWORK ), 
     $             ULP )
*
*     Norm of error:
*
...
*
         IF( ITRNSE.EQ.0 ) THEN
            CALL PZAXPY( N, WTEMP, E, 1, JCOL, DESCE, 1, WORK, 1, 
     $                   JCOL, DESCW, 1 )
         ELSE IF( ITRNSE.EQ.1 ) THEN
            CALL PZAXPY( N, WTEMP, E, JCOL, 1, DESCE, N, WORK, 1, 
     $                   JCOL, DESCW, 1 )
         ELSE
            CALL PZAXPY( N, DCONJG( WTEMP ), E, JCOL, 1, DESCE, N, 
     $                   WORK, 1, JCOL, DESCW, 1 )
            DO JROW = 1, N
               CALL INFOG2L( JROW, JCOL, DESCW, NPROW, NPCOL, MYROW, 
     $                       MYCOL, IROW, ICOL, II, JJ )
               IF( ( MYROW.EQ.II ) .AND. ( MYCOL.EQ.JJ ) ) THEN
                  WORK( (JCOL-1)*LDW+JROW ) = DCONJG( WORK( (JCOL-1)
     $                *LDW+JROW ) )
               ENDIF
            END DO
         END IF
  100 CONTINUE
*
      CALL PZGEMM( TRANSA, TRANSE, N, N, N, CONE, A, 1, 1, DESCA, E, 
     $             1, 1, DESCE, -CONE, WORK, 1, 1, DESCW )
*
      ERRNRM = PZLANGE( 'One', N, N, WORK, 1, 1, DESCW, RWORK ) 
     $                 / ENORM
*
