...
! .. Use Statements ..
      ...
      USE givens_rotations, ONLY : d_rotm
! ..
! .. Parameters ..
...
! on exit.
      ...
      CALL d_rotm(d(1),d(2),x(1),x(2),k,xv,incx,yv,incy,param)
...
! scaling test d(1) = d(2) < 1/g^2
      ...
      k = 2
      d = one/d
      title = 'Scaling test d(1) = d(2) = 1/(1.01*g^2)'
      WRITE (6,'(//">>> ",a)') title
      WRITE (6,'(/" Input Data")')
      WRITE (6,'(" x-vector:", 2e16.8)') x(1:2)
      WRITE (6,'(" d-vector:", 2e16.8)') d(1:2)
      CALL d_rotm(d(1),d(2),x(1),x(2),k,xv,1,yv,1,param)
      CALL outres(param,x,d,k,xv,yv,incx,incy)
...
! d(1) = d(2) = huge/four; type = anti-diagonal
      ...
      WRITE (6,'(" x-vector:", 2e16.8)') x(1:2)
      WRITE (6,'(" d-vector:", 2e16.8)') d(1:2)
      CALL d_rotm(d(1),d(2),x(1),x(2),-1,xv,1,yv,1,param)
      CALL outres(param,x,d,k,xv,yv,incx,incy)
