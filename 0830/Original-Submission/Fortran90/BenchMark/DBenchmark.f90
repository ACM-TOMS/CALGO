...
! all methods for each run.
      WRITE (*,fmt='(/1x,A)') &
        ' Benchmark Givens Transformations '// &
	&'on 2N by N random matrices:'
...
! Print header information
      ...
      WRITE (4,fmt='(22x,''Double Precision Version''/)')
...
! Construct and apply standard transformations:
      ...
      DO k = 1, ntimes
        ...
        DO j = 1, n
          DO i = j + 1, m
            CALL drotg(b(j,j),b(j,i),c,s)
            CALL drot(n-j+1,b(j+1,j),1,b(j+1,i),1,c,s)
          END DO
        END DO
      END DO
      tsg = elaptime(old_time)
...
! Check solutions:
      ...
      DO k = 1, ntimes
        ...
        DO j = 1, n
          DO i = j + 1, m
! Construct and apply modified transformation:
            CALL drotmg(d(j),d(i),a(j,j),a(j,i),param)
            CALL drotm(n-j+1,a(j+1,j),1,a(j+1,i),1,param)
          END DO
        END DO
      END DO
      tmg = elaptime(old_time)
