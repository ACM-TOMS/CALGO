...
! Get a new row.  Update and then downdate.
      ...
      b(1:n+1,n+1) = r
      DO j = 1, n ! ADD DATA
        CALL s_rot(b(j,j),b(j,n+1),n-j+1,b(j+1,j),1,b(j+1,n+1),1,c,s)
      END DO
      b(1:n+1,n+1) = r
      DO j = 1, n ! DROP DATA
        CALL s_rot(b(j,j),b(j,n+1),-(n-j+1)-1,b(j+1,j),1,b(j+1,n+1),1,c,s)
      END DO
...
! Get a new row.  Update and then downdate.
      ...
      d(n+1) = one
      DO j = 1, n ! ADD DATA
        CALL s_rotm(d(j),d(n+1),a(j,j),a(j,n+1),n-j+1,a(j+1,j),1,a(j+1,n+1),1, &
          param)
      END DO
      a(1:n+1,n+1) = r
      d(n+1) = one
      DO j = 1, n
        d1 = -d(j) ! DROP DATA
        CALL s_rotm(d1,d(n+1),a(j,j),a(j,n+1),n-j+1,a(j+1,j),1,a(j+1,n+1),1, &
          param)
        d(j) = -d1
      END DO
