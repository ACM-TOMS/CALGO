      REAL             FUNCTION SCOSPX (X)
C>> 1996-01-29 SCOSPX WVS JPL Add better acknowledgement for origins.
C>> 1994-10-20 SCOSPX Krogh  Changes to use M77CON
C>> 1994-04-22 SCOSPX CLL Made SP and DP codes similar.
C>> 1993-05-06 SCOSPX WVS JPL Convert from NSWC to Math 77
C ----------------------------------------------------------------------
...
C
     ...
     *            A2)*T + A1)*T + 0.5E0) + 0.5E0
         ...
         T = A*A
         SCOSPX = ((((((SA6*T + SA5)*T + SA4)*T + SA3)*T + SA2)*T
     *                  + SA1)*T + SA0)*A
      endif
      GO TO 30
C
...
C
   ...
   21 continue
      if(eps .lt. cutoff) then
         T = 16.E0*A*A
         SCOSPX = (((((((((((((B13*T + B12)*T + B11)*T + B10)*T + B9)*T+
     *                B8)*T + B7)*T + B6)*T + B5)*T + B4)*T + B3)*T +
     *                B2)*T + B1)*T + 0.5E0) + 0.5E0
      else
         T = A*A
         SCOSPX = ((((((SB6*T + SB5)*T + SB4)*T + SB3)*T + SB2)*T
     *                  + SB1)*T + 0.5E0) + 0.5E0
      endif
C
C                        TERMINATION
