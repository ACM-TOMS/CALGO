function [objPoly,I01,Icomp] = QmatToSlackBinQOP(Qmat0,lambda)
%QMATTOSLACKBINQOP Add slack variables and convert it to BBCPOP
% Usage: [objPoly,I01,Icomp] = QmatToSlackBinQOP(Qmat0,lambda);
%
% Suppose that we have a binary QOP:
% min. x'Qx s.t. x\in\{0,1\}^n            (1)
%
% QMATTOSLACKBINQOP first reformulate (1) by adding slack variable u 
% min. x'Qx s.t. x+u=e, x_iu_i = 0 (for all i), x,u\in[0,1]^n
%
% and then convert it to BBCPOP by applying Lagrangian relaxation
% min. x'Qx + lambda\|x+u-e\|^2  
% s.t. x_iu_i = 0 (for all i), x,u\in[0,1]^n          (2)
%
% It is reported that a DNN relaxation of (2) is stronger than that of (1).
% See Section 6.2, 6.3 and 7.3 of
% 
...
% 
	...
	Icomp=logical([speye(nDim,nDim),speye(nDim,nDim)]);
    I01=false(1,2*nDim);
end
