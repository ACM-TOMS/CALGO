...
-- CREATORS :
  ...
  begin
   -- compute the inner normal :
    ...
    Normalize(mic.nor);
   -- orientate the inner normal :
    if mic.nor(mic.nor'last) < 0
     then Min_Vector(mic.nor);
    end if;
    ...
    if not done
     then declare
          ...
          begin
            ...
            mic.nor := new Integer_Vectors.Vector'(nor);
            mic.sub := null;
...
-- CONSTRUCTORS :
  ...
  begin
    Deep_Clear(mic2);
    if mic1.nor /= null
     then mic2.nor := new Integer_Vectors.Vector'(mic1.nor.all);
    end if;
    if mic1.pts /= null
