...
with Float_Face_Enumerators;           use Float_Face_Enumerators;

package body Float_Faces_of_Polytope is

-- AUXILIAIRIES :

  function Create_Edge ( pts : Float_Vectors_of_Vectors.Vector;
                         i,j : integer ) return Face is

  -- DESCRIPTION :
  --   Creates the edge spanned by pts(i) and pts(j).

    res : Face(0..1) := new Float_Vectors_of_Vectors.Vector(0..1);
  ...
  function Create_Face ( pts : Float_Vectors_of_Vectors.Vector;
                         f : Integer_Vectors.Vector ) return Face is

  -- DESCRIPTION :
  --   Returns vector of points pts(f(i)) that span the face.

    res : Face(f'range) := new Float_Vectors_of_Vectors.Vector(f'range);
  ...
  procedure Move_to_Front ( pts : in out Float_Vectors_of_Vectors.Vector;
                            x : in Float_Vectors.Vector ) is
    ...
    if k > n
     then return res;
       ...
       declare
         ...
         pts : Float_Vectors_of_Vectors.Vector(1..m) := Shallow_Create(p);
         res_last : Faces := res;
       begin
         if k = 1
          then
            ...
            declare
              procedure Append_Face ( fa : in Integer_Vectors.Vector;
                                      cont : out boolean ) is
...
-- SELECTORS :
  ...
  function Is_In ( f : Face; x : Float_Vectors.Vector ) return boolean is
  begin
    for i in f'range loop
      if f(i).all = x
       then return true;
...
-- DESTRUCTORS :
  ...
  begin
    if f /= null
     then Float_Vectors_of_Vectors.Clear(f.all);
    end if;
  end Shallow_Clear;
