package body Lists_of_Integer_Vectors is

-- CONSTRUCTORS :

  function Deep_Create ( v : Integer_Vectors_of_Vectors.Vector ) return List is
  ...
  function Shallow_Create ( v : Integer_Vectors_of_Vectors.Vector )
                          return List is
  ...
  function Deep_Create ( l : List ) return Integer_Vectors_of_Vectors.Vector is

    res : Integer_Vectors_of_Vectors.Vector(1..Length_Of(l));
    tmp : List := l;

  begin
    for i in res'range loop
      declare
        v : constant Integer_Vectors.Vector := Head_Of(tmp).all;
      begin
        res(i) := new vector'(v);
      end;
      tmp := Tail_Of(tmp);
...
-- SELECTORS :
  ...
  begin
    if not Sub_List(l1,l2)
     then return false;
     elsif not Sub_List(l2,l1)
	 then return false;
	 else return true;
    end if;
  end Is_Equal;
...
-- DESTRUCTORS :
  ...
  begin
    Lists_of_Link_to_Integer_Vectors.Clear
                       (Lists_of_Link_to_Integer_Vectors.List(l));
  end Shallow_Clear;
