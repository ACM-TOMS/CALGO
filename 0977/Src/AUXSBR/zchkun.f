*{SIGMA/AUXSBR/zchkun.f}
*
      SUBROUTINE ZCHKUN( JOB, M, N, V, LDV, DMIN, DMAX, OFFMAX, 
     $           X, LDX, INFO )
...
*     Purpose
*     ~~~~~~~
*     ZCHKUN checks whether V is left or right numerically unitary.
*     If JOBU = 'C' ('N'), it computes X = V^H * V (X=V * V^H) and then 
...
*......................................................................	
      INFO = 0 
      IF ( .NOT.( LSAME(JOB,'C') .OR. LSAME(JOB,'N') ) ) THEN 
          INFO = -1
      ELSE IF ( LSAME(JOB,'C') .AND. ( M.LT.N) ) THEN
          INFO = -1
      ...
      ELSE IF ( LDV.LT.M ) THEN
          INFO = -5
      ELSE IF ( LSAME(JOB,'C') .AND. (LDX.LT.N ) ) THEN
          INFO = -10
      ELSE IF ( LSAME(JOB,'N') .AND. (LDX.LT.M ) ) THEN
          INFO = -10 
      END IF
      IF( INFO.NE.0 ) THEN
         CALL XERBLA( 'ZCHKUN', -INFO )
         RETURN
      END IF	    
*......................................................................	   
...
*......................................................................
      IF ( LSAME( JOB, 'C' ) ) THEN 
*        .. test V^H * V  against the N x N identity         
         CALL ZHERK( 'U', 'C', N, M, ONE, V, LDV, ZERO, X, LDX )
         K = N 
      ELSE
*        .. test V * V^H against the M x M identity
         CALL ZHERK( 'U', 'N', M, N, ONE, V, LDV, ZERO, X, LDX )
         K = M 
      END IF
*     
      TEMP = -ONE
