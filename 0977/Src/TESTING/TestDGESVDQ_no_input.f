*{SIGMA/TESTING/TestDGESVDQ_no_input.f}
      PROGRAM D_S_GESVDQ_test_no_input
*
...
*
********************************************************************************
*  Systematic stress testing of the REAL versions of XGESVDQ.                  *
*  The procedures under testing are SGESVDQ and DGESVDQ.                       *
...
*     .. Scalar Arguments ..
     ...
     $                   NEXP, NUMRANK, iexp, N1, IWOFF, MINWRK, 
     $                   CHKMWRK, PASSED, FAILED
...
*     .. Array Arguments ..
      ...
      REAL               SQ(LDA)
      DOUBLE PRECISION   A( LDA, LDA2 ), A1(LDA,LDA2)
      DOUBLE PRECISION   VT(LDVT,LDVT), ZU1(LDU,LDU), ZVT(LDVT,LDVT) 
*
      DOUBLE PRECISION   AJ(LDA,LDA2),  UJ(LDU,LDU), VJ(LDA2,LDA2)
*     ..       
      DOUBLE PRECISION   ZA(LDA,LDA2), ZU(LDU,LDU), ZV(LDV,LDV)
      REAL               CA(LDA,LDA2), CU(LDU,LDU), CV(LDV,LDV) 
      REAL               CVT(LDVT,LDVT)
*     .. workspaces      
      DOUBLE PRECISION   ZWORK(LCWORK)
      REAL               CWORK(LCWORK)
...
*     .. new SVD subrutines under examination
      EXTERNAL         SGESVDQ, DGESVDQ
*     .. auxiliary subroutines
      EXTERNAL         DCHKOR, DCMPSV, DMGEN, XGES2D, XGED2S  
*     .. External Functions (SIGMA library) ..            
      EXTERNAL         DRESVD
      DOUBLE PRECISION DRESVD
*     
*     .. External Subroutines (BLAS, LAPACK)...................................
      EXTERNAL         DLASET, DSCAL, DLACPY, DGESVD, DGESVJ 
*     .. External Functions (BLAS, LAPACK)......................................   
      DOUBLE PRECISION DNRM2
      LOGICAL                                  LSAME      
      REAL                     SLAMCH
      DOUBLE PRECISION                 DLAMCH
      EXTERNAL         DNRM2,  SLAMCH, DLAMCH, LSAME 
*     .. External MKL
*      EXTERNAL         mkl_free_buffers
*
*..............................................................................
*     
      WRITE(*,*)' Test program for {S,D}GESVDQ - implementation test'
      WRITE(*,*)' For A (M x N), M >= N, {S,D}GESVDQ computes the SVD '
      WRITE(*,*)' A = U * S * V^H .'
      WRITE(*,*)' Data type: REAL  and DOUBLE PRECISION '
      WRITE(*,*)' Reference routine: DOUBLE PREC.Jacobi SVD, DGESVJ.'
      WRITE(*,*)
...
*      READ(*,*) ICASEPIV
      ...
      WRITE(*,*) '(2) The SVD of the transposed R**T                   '
...
*      READ(*,*) NEXP
     ...
     $               ' Keep calm and try again.'
          STOP
      END IF 
      WRITE(*,604,ADVANCE="NO")
 604  FORMAT(' Work in the minimal workspace (to check whether its',
     $       ' size is properly determined) (0=no, 1=yes) = ')
*      READ(*,*) CHKMWRK
      CHKMWRK = 0
      WRITE(*,*) CHKMWRK
      IF ( ( CHKMWRK .NE. 0 ) .AND. ( CHKMWRK .NE. 1 ) ) THEN 
          WRITE(*,*) ':( ..bad input; bailing out.', 
     $               ' Keep calm and try again.'
          STOP
      END IF
      WRITE(*,*)
      WRITE(*,*) 'The number of test matrices = ', 8*8*13*13*3
      WRITE(*,*) 'Depending on the machine, the test run takes up to'
      WRITE(*,*) '10-15 minutes. At the end, a brief report is printed'
      WRITE(*,*) 'and detailed information on all test cases is written'
      WRITE(*,*) 'files and can be visualised using the provided Matlab'
      WRITE(*,*) 'script TestSummary. See the User guide for details.'
      WRITE(*,*) 
      WRITE(*,*) 'Review the above input and hit ENTER to continue'
      READ(*,*)
*     ................................................................. 
      it = 0       
      SCOND = 1.0D0
...
*     .. for each triple (SCOND, MODE, SMODE) try few random examples     
 ...
 51   CONTINUE 
      ...
      SJEME(it,10) = iexp
      WRITE(*,*) 'Test case # ', it 
*     .. generate the matrix  as DOUBLE PRECISION     
      CALL DMGEN( M, N, A, LDA, SCOND, COND, SMODE,    
...
*    
*     .. compute the scaled condition number using DGESVD. It should
*     be close to the desired values given on input to ZMGEN     
      CALL DLACPY( 'A', M, N, A, LDA, A1,  LDA )
      DO 122 j = 1, N 
         ANORM = DNRM2( M, A1(1,j), 1 )
          IF (ANORM .NE. DZERO) CALL DSCAL(M, DONE/ANORM, A1(1,j),1)
122   CONTINUE        
      CALL DGESVD( 'N', 'N', M, N, A1, LDA, DSJ, ZU1, LDU, 
     $     VT, LDVT, ZWORK, LCWORK, INFO )   
      N1 = 0 
      DO 221 j = 1, N 
...
* 
*.......................................................................
* RUNNING THE DOUBLE PRECISION ONE SIDED JACOBI >> DGESVJ <<
* THIS IS THE REFERENCE ROUTINE 
*-----------------------------------------------------------------------     
       CALL DLACPY( 'A', M, N, A, LDA, AJ,  LDA )
       WRITE(*,*) '>> Computing the SVD (DGESVJ, reference values), ...'
*>>>>>>.. SVD routine used for reference values        
       CALL DGESVJ( 'G', JOBU, JOBV, M, N, AJ, LDA, DSJ, N, VJ,  
     $                   LDV, ZWORK, LCWORK, INFO ) 
*>>>>>>
       WRITE(*,*) '   DGESVJ ... done.'
*
       CALL DLACPY( 'A', M, N, AJ, LDA, UJ, LDU )
*
*      After the call to a reference routine, the reference values are
*      UJ  :: M x N matrix of the  left singular vectors, DOUBLE PRECISION
*      VJ  :: N x N matrix of the right singular vectors, DOUBLE PRECISION  
...
*
       IF ( RSVEC ) THEN 
           CALL DCHKOR( 'T', N, N, VJ, LDV, DMINVZ, DMAXVZ,  
...
*      
       IF ( LSVEC ) THEN 
           CALL DCHKOR( 'T', M, N, UJ, LDA, DMINUZ, DMAXUZ,  
...
*          
       ...
       IF ( LSVEC .AND. RSVEC ) THEN 
           ZGESVJ_R = DRESVD( M, N, N, A1, LDA, AJ, LDA, 'N', VJ, LDV,  
     $                DSJ, 'F', DWORK )       
...
*.......................................................................
*.......................................................................
* RUNNING THE PRECONDITIONED DGESVD >> DGESVDQ <<
* THIS IS THE ROUTINE UNDER EXAMINATION
*----------------------------------------------------------------------- 
*
       CALL DLACPY( 'A', M, N, A, LDA, ZA,  LDA )
       WRITE(*,*) '>> Computing the SVD (DGESVDQ), ...'
*>>>>>>.. SVD routine under examination (DOUBLE PRECISION)  
       IF ( CHKMWRK .EQ. 0 ) THEN
       CALL DGESVDQ( JOBA, JOBRWPVT, JOBT, JOBUQ, JOBVQ, M, N, ZA, LDA,
     $      DSQ, ZU, LDU, ZVT, LDV, NUMRANK, IWORK, 
     $      ZWORK, LCWORK, DWORK, LDWORK, INFO ) 
       ELSE
       CALL DGESVDQ( JOBA, JOBRWPVT, JOBT, JOBUQ, JOBVQ, M, N, ZA, LDA,
     $      DSQ, ZU, LDU, ZVT, LDV, NUMRANK, IWORK, 
     $      ZWORK, -1, DWORK, LDWORK, INFO ) 
       MINWRK = INT(ZWORK(2))
       IWOFF = LCWORK - MINWRK + 1 
       CALL DGESVDQ( JOBA, JOBRWPVT, JOBT, JOBUQ, JOBVQ, M, N, ZA, LDA,
     $      DSQ, ZU, LDU, ZVT, LDV, NUMRANK, IWORK, 
     $      ZWORK(IWOFF), MINWRK, DWORK, LDWORK, INFO )          
       END IF       
*>>>>>>
       DO 30 j = 1, N
         DO 40 i = 1, N 
...
*      tested  are
*      ZU  :: M x {N,M} matrix of the  left singular vectors, DOUBLE PRECISION
*      ZV  :: N x N matrix of the right singular vectors,     DOUBLE PRECISION  
...
*       
       IF ( RSVEC ) THEN 
           ...
           END IF
*          .. test orthogonality
           CALL DCHKOR( 'T', N, N1, ZV, LDV, DMINVZ, DMAXVZ,
     $          OFFMAXVZ,  ZWORK, N, INFO )
*          .. compare with reference values and compute max error*gap           
           CALL DCMPSV( 'M', N,NUMRANK, VJ,LDV, DSJ, ZV,LDV, SVECERR_VZ,
...
* 
       IF ( LSVEC ) THEN 
           ...
           END IF
*          .. test orthogonality
           CALL DCHKOR( 'T', M, N1, ZU, LDU, DMINUZ, DMAXUZ, 
     $          OFFMAXUZ,  ZWORK, N, INFO )
*          .. compare with reference values and compute max error*gap            
           CALL DCMPSV( 'M', M,NUMRANK, UJ,LDU, DSJ, ZU,LDU, SVECERR_UZ,
     $          DUMMY, IVIV, ZWORK, INFO )
           WRITE(*,*)
           WRITE(*,1464) '   -> Max_i<>j (U^* * U)_ij  = ', OFFMAXUZ
           WRITE(*,1464) '   -> Max_i || U(:,i) ||     = ', DMAXUZ
...
*          .. compute the residual in double precision           
           CALL DLACPY( 'A', M, N, A, LDA, A1, LDA ) 
           ZGESVDQ_R =  DRESVD( M,N, NUMRANK, A1,LDA, ZU,LDU, 'T', ZVT, 
     $                 LDVT, DSQ, 'F', DWORK ) 
...
*.......................................................................
*.......................................................................
* RUNNING THE PRECONDITIONED SGESVD >> SGESVDQ <<
* THIS IS THE ROUTINE UNDER EXAMINATION
*----------------------------------------------------------------------- 
*
       CALL XGED2S( M, N, A, LDA, CA, LDA )
       WRITE(*,*) '>> Computing the SVD (SGESVDQ), ...'
*>>>>>>.. SVD routine under examination (REAL)  
       IF ( CHKMWRK .EQ. 0 ) THEN 
           CALL SGESVDQ( JOBA, JOBRWPVT, JOBT, JOBUQ, JOBVQ, M, N, CA, 
     $          LDA, SQ, CU, LDU, CVT, LDV, NUMRANK, IWORK, 
     $          CWORK, LCWORK, RWORK, LDWORK, INFO )
      ELSE          
           CALL SGESVDQ( JOBA, JOBRWPVT, JOBT, JOBUQ, JOBVQ, M, N, CA, 
     $          LDA, SQ, CU, LDU, CVT, LDV, NUMRANK, IWORK, 
     $          CWORK, -1, RWORK, LDWORK, INFO )
*
           MINWRK = INT(CWORK(2))
           IWOFF = LCWORK - MINWRK + 1 
           CALL SGESVDQ( JOBA, JOBRWPVT, JOBT, JOBUQ, JOBVQ, M, N, CA, 
     $          LDA, SQ, CU, LDU, CVT, LDV, NUMRANK, IWORK, 
     $          CWORK(IWOFF), MINWRK, RWORK, LDWORK, INFO ) 
      END IF 
*>>>>>>
       DO 50 j = 1, N
         DO 60 i = 1, N 
...
*      After the call to the routine under examination, the values to be 
*      tested  are
*      CU  :: M x {N,M} matrix of the  left singular vectors, REAL
*      CV  :: N x N matrix of the right singular vectors,     REAL  
*             (possibly N x NUMRANK; always only N x NUMRANK part tested)
...
*       
       IF ( RSVEC ) THEN 
           ...
           ELSE
               N1 = N
           END IF
           CALL XGES2D( N, N1, CV, LDV, VT, LDVT )
*          .. test orthogonality
           CALL DCHKOR( 'T', N, N1, VT, LDVT, DMINV, DMAXV,
     $          OFFMAXV, ZWORK, N, INFO )
*          .. compare with reference values and compute max error*gap            
           CALL DCMPSV( 'M', N,NUMRANK, VJ,LDV, DSJ, VT,LDVT, SVECERR_V,
     $          DUMMY, IVIV, ZWORK, INFO )
           WRITE(*,*)
...
* 
       IF ( LSVEC ) THEN 
           ...
           ELSE
               N1 = NUMRANK
           END IF
           CALL XGES2D( M, N1,  CU, LDU, ZU1, LDU )
*          .. test orthogonality
           CALL DCHKOR( 'T', M, N1, ZU1, LDU, DMINU, DMAXU, 
     $          OFFMAXU, ZWORK, N, INFO )
*          .. compare with reference values and compute max error*gap            
           CALL DCMPSV( 'M', M,NUMRANK, UJ,LDU, DSJ, ZU1,LDU, SVECERR_U,
     $          DUMMY, IVIV, ZWORK, INFO )
           WRITE(*,*)
...
*           
 ...
 102     CONTINUE  
     ...
     $            ' ||A||_2 was ', SVERR1
        ...
        IF ( LSVEC .AND. RSVEC ) THEN 
*          .. compute the residual in double precision           
           CALL XGES2D( M, N, CU, LDU, ZU1, LDU )
           CALL XGES2D( N, N, CVT, LDV, VT, LDV )
           CALL DLACPY( 'A', M, N, A, LDA, A1, LDA )
           CGESVDQ_R = DRESVD( M,N, NUMRANK, A1,LDA, ZU1,LDU, 'T', VT,
     $                 LDV, DWORK, 'F', DWORK(N+1) )
...
700      CONTINUE            
         WRITE(*,*) 'Maximal SGESVDQ residual was ', CGESVDQ_R
         WRITE(*,*) 'Maximal DGESVDQ residual was ', ZGESVDQ_R
     ...
     $        (ZGESVDQ_R .LE. DLAMCH('E')*M100*SQRT(DBLE(N))) ) THEN
         ...
         WRITE(*,*) 'Maximal DGESVJ (reference routine) residual was',
     $              ZGESVJ_R
...
760   CONTINUE    
      ...
      WRITE(*,*) ' empirical value EPS x max(M,100). '
      WRITE(*,*) 'The maximal measured max|d_sigma_i|/||A||_F was ', 
     $            SVERR1, ' (SGESVDQ) and ', SVERRZ1, ' (DGESVDQ)'
...
*       
     ...
     $              ' max (|d_sigma_i|/sigma_i)/(EPS*SCOND)'
         WRITE(*,*) '(','Ideally, it is smaller than Max(M,100) = ', 
     $              M100, ';'
...
770      CONTINUE           
         WRITE(*,*) 'Maximal SGESVDQ quotient', 
     $              ' (|d_sigma_i|/sigma_i)/(EPS*SCOND) was ', SVERR
         WRITE(*,*) 'Maximal DGESVDQ quotient', 
...
800    CONTINUE           
       WRITE(*,*) 'Maximal |u_i**u_j|_i.NE.j in  SGESVDQ  was ', 
     $            OFFMAXU
       IF ( OFFMAXU .LE. SLAMCH('E')*M*5.0E0 ) THEN
          WRITE(*,*) '..............................', ' :) passed.'
          PASSED = PASSED + 1 
       ELSE
          WRITE(*,*) '!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!', ' :( FAILED.'
          FAILED = FAILED + 1
       END IF        
       WRITE(*,*) 'Maximal |u_i**u_j|_i.NE.j in DGESVDQ  was ', 
     $            OFFMAXVZ
       IF ( OFFMAXVZ .LE. DLAMCH('E')*M*5.0D0 ) THEN
          WRITE(*,*) '..............................', ' :) passed.'
          PASSED = PASSED + 1 
       ELSE
          WRITE(*,*) '!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!', ' :( FAILED.'
          FAILED = FAILED + 1 
       END IF
         WRITE(*,*) 'Maximal |u_i**u_j|_i.NE.j in DGESVJ  was ', 
     $              OFFMAXUZ
         IF ( OFFMAXUZ .LE. DLAMCH('E')*M*5.0D0 ) THEN
            WRITE(*,*) '..............................', ' :) passed.'
            PASSED = PASSED + 1 
         ELSE
            WRITE(*,*) '!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!', ' :( FAILED.'
...
880      CONTINUE      
         WRITE(*,*) 'Maximal |v_i**v_j|_i.NE.j in  SGESVDQ  was ', 
     $               OFFMAXV
         IF ( OFFMAXV .LE. SLAMCH('E')*M*5.0E0 ) THEN
            WRITE(*,*) '..............................', ' :) passed.'
            PASSED = PASSED + 1 
         ELSE
            WRITE(*,*) '!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!', ' :( FAILED.'
            FAILED = FAILED + 1 
         END IF   
         WRITE(*,*) 'Maximal |v_i**v_j|_i.NE.j in DGESVDQ  was ', 
     $               OFFMAXUZ
         IF ( OFFMAXUZ .LE. DLAMCH('E')*M*5.0D0 ) THEN
            WRITE(*,*) '..............................', ' :) passed.'
            PASSED = PASSED + 1
         ELSE
            ...
            FAILED = FAILED + 1 
         END IF   
         WRITE(*,*) 'Maximal |v_i**v_j|_i.NE.j in DGESVJ  was ', 
     $               OFFMAXVZ
         IF ( OFFMAXVZ .LE. DLAMCH('E')*M*5.0D0 ) THEN
            WRITE(*,*) '..............................', ' :) passed.'
            PASSED = PASSED + 1 
...
*
      ...
      WRITE(*,*) '1. SGESVDQ.table', 
     $           ' (all measured errros for SGESVDQ)     '
      WRITE(*,*) '2. DGESVDQ.table', 
     $           ' (all measured errros for DGESVDQ)     '
      WRITE(*,*) '3. DGESVJ.table', 
...
*        
      OPEN (33, FILE = 'SGESVDQ.table')
      DO 2000 i = 1, it
          WRITE(33,3333) (CHKP1(i,j), j=1, 14)
 2000 CONTINUE
      CLOSE( 33 )
*        
      OPEN (37, FILE = 'DGESVDQ.table')
      DO 2002 i = 1, it
         WRITE(37,5555) (ZCHKP2(i,j), j=1, 14)
 2002 CONTINUE
      CLOSE( 37 )   
*        
      OPEN (36, FILE = 'DGESVJ.table')
      DO 5000 i = 1, it
...
*        
 ...
 3945 FORMAT( 4I7, 6I5)     
*        
      END PROGRAM D_S_GESVDQ_test_no_input
