*{SIGMA/AUXSBR/dcmpsv.f}
*
*        SUBROUTINE DCMPSV( JOB, M, N, U, LDU, S, X, LDX, E, E1, 
*     $             F, ZW, INFO )
...
*       Purpose
*       ~~~~~~~
*       DCMPSV compares the refence values of the singular vectors 
...
*...............................................................................
*       [X] (input)
*       [X] s DOUBLE PRECISION ARRAY, LDX-by-N
...
*     .. Array Arguments 	  
      DOUBLE PRECISION   U(LDU,*),  X(LDX, *), ZW(*)
...
*              
           IF ( S(j) .GT. ZERO ) THEN 
*
           CALL DCOPY( M,     X(1,j), 1, ZW, 1 )
           A  = DDOT( M,      U(1,j), 1, ZW, 1 )
           CALL DAXPY( M, -A, U(1,j), 1, ZW, 1 )
           B = DNRM2( M, ZW, 1 )
           IF ( j .EQ. 1 ) THEN 
              GAP  = (S(1) - S(2)) / (S(1))
              GAP1 = S(1) - S(2)
...
*
 1      CONTINUE 
      ELSE
        CALL DCOPY( M,     X(1,1), 1, ZW, 1 )  
        A  = DDOT ( M,     U(1,1), 1, ZW, 1 )
        CALL DAXPY( M, -A, U(1,1), 1, ZW, 1 )
        E  = DNRM2( M, ZW, 1 )   
        E1 = E 
        IF ( ALL ) F(1) = E 
      END IF
