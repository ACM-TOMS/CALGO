...
*  ===========
*
*       SUBROUTINE ZGESVJX( JOBA, JOBU, JOBV, M, N, A, LDA, SVA, MV, V,
...
*  ========
*
* ZGESVJX computes the singular value decomposition (SVD) of a complex
...
*                 If INFO .GT. 0,
*                 the procedure ZGESVJX did not converge in the given number
...
*
*          If INFO .GT. 0 :
*          the procedure ZGESVJX did not converge in the given number of
*          iterations (sweeps) and SCALE*SVA(1:N) may not be accurate.
*
* MV (input)
*          MV is INTEGER
*          If JOBV .EQ. 'A', then the product of Jacobi rotations in ZGESVJX
...
*                    This is useful information in cases when GESVJ did
...
*          > 0 : ZGESVJX did not converge in the maximal allowed number 
...
*     .. Local Scalars ..
     ...
     $        ISWROT, jbc, jgl, KBL, LKAHEAD, MVL, N2,  
...
*
     ...
     $          ( APPLV .AND. ( LDV.LT.MV ) ) ) THEN
      ...
      ELSE IF( LWORK.LT.( M+N ) ) THEN
         INFO = -13
      ELSE IF( LRWORK.LT.MAX( N, 6 ) ) THEN
         INFO = -15   
      ELSE
         INFO = 0
      END IF
...
*     if ZGESVJX is used as a computational routine in the preconditioned
