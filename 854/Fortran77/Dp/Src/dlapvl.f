      SUBROUTINE DLAPVL( LHAM, N, K, NB, A, LDA, QG, LDQG, XA, LDXA,
...
C             On exit, DWORK(1)  returns the optimal value of LDWORK.
C
C     LDWORK  INTEGER
C             The length of the array DWORK.  LDWORK >= MAX(1,3*NB).
C
C     METHOD
C
...
C     NUMERICAL ASPECTS
C
C     The algorithm requires ??? floating point operations and is
...
C     Technical Report, TU Berlin, 2002.
C
C     CONTRIBUTOR
C
C     D. Kressner, Technical Univ. Berlin, Germany, Jan. 2003.
C
C     KEYWORDS
C
...
C     .. Scalar Arguments ..
      LOGICAL           LHAM
      INTEGER           K, LDA, LDQG, LDWORK, LDXA, LDXG, LDXQ, LDYA,
     $                  N, NB
C     .. Array Arguments ..
...
C     .. Local Scalars ..
      INTEGER           I, J, NB1, NB2
      DOUBLE PRECISION  ALPHA, C, S, TAUQ, TEMP, TTEMP
...
C
     ...
     $                   QG(K+I+1,I) )
            ...
            TEMP = A(K+I+1,I)
            CALL DLARTG( TEMP, ALPHA, C, S, A(K+I+1,I) )
...
C
     ...
     $                 S )
            ...
            CALL DROT( N-I-1, A(K+I+2,I+1), 1, QG(K+I+1,I+3), LDQG, C,
     $                 S )
            TEMP = A(K+I+1,I+1)
            TTEMP = QG(K+I+1,I+2)
...
C           w1 = U1'*u2
            CALL DGEMV( 'Transpose', N-I-1, I, ONE, QG(K+I+2,1), LDQG,
     $                  A(K+I+2,I), 1, ZERO, DWORK, 1 )
C           xa = xa + XA1*w1
            CALL DGEMV( 'No transpose', N-I-1, I, ONE, XA(I+2,1), LDXA,
     $                  DWORK, 1, ONE, XA(I+2,NB+I), 1 )
C           w2 = U2'*u2
            CALL DGEMV( 'Transpose', N-I-1, I-1, ONE, A(K+I+2,1), LDA,
     $                  A(K+I+2,I), 1, ZERO, DWORK(NB1), 1 )
C           xa = xa + XA2*w2
            CALL DGEMV( 'No transpose', N-I-1, I-1, ONE, XA(I+2,NB1),
     $                  LDXA, DWORK(NB1), 1, ONE, XA(I+2,NB+I), 1 )
C           temp = YA1'*u2
            CALL DGEMV( 'Transpose', N-I-1, I, ONE, YA(K+I+2,1), LDYA,
     $                  A(K+I+2,I), 1, ZERO, XA(1,NB+I), 1 )
C           xa = xa + U1*temp
            CALL DGEMV( 'No Transpose', N-I-1, I, ONE, QG(K+I+2,1),
...
C           temp = XA2'*u1
            CALL DGEMV( 'Transpose', N-I-1, I-1, ONE, XA(I+2,NB1), LDXA,
     $                  A(K+I+2,I), 1, ZERO, DWORK(NB2), 1 )
C           ya = ya + U2*temp
            CALL DGEMV( 'No transpose', N-I-1, I-1, ONE, A(K+I+2,1),
...
C           H(i+1,i+2:n) = H(i+1,i+2:n) + xa(i+2:n)';
            CALL DAXPY( N-I-1, ONE, XA(I+2,NB+I), 1, A(I+1,I+2), LDA )
C           H(i+1,i+2:n) = H(i+1,i+2:n) + YA(i+1,:) * U(i+2:n,:)'
            CALL DAXPY( N-I-1, YA(I+1,NB+I), A(K+I+2,I), 1, A(I+1,I+2),
     $                  LDA )
C
C           Annihilate updated parts in YA.
C
...
C           xq = xq + XQ1*w1
            CALL DGEMV( 'No transpose', N-I-1, I, ONE, XQ(I+2,1), LDXQ,
     $                  DWORK, 1, ONE, XQ(I+2,NB+I), 1 )
C           xq = xq + XQ2*w2
...
C           temp = XQ2'*u2
            CALL DGEMV( 'Transpose', N-I-1, I-1, ONE, XQ(I+2,NB1), LDXQ,
     $                  A(K+I+2,I), 1, ZERO, XQ(1,NB+I), 1 )
C           xq = xq + U2*temp
            CALL DGEMV( 'No Transpose', N-I-1, I-1, ONE, A(K+I+2,1),
...
C           temp = XG1'*u2
            CALL DGEMV( 'Transpose', N-I-1, I, ONE, XG(K+I+2,1), LDXQ,
     $                  A(K+I+2,I), 1, ZERO, DWORK(NB2), 1 )
C           xg = xg + U1*temp
            CALL DGEMV( 'No Transpose', N-I-1, I, ONE, QG(K+I+2,1),
...
C
            ...
            CALL DAXPY( N-I, ONE, XG(K+I+1,NB+I), 1, QG(I+1,I+2),
     $                  LDQG )
            CALL DAXPY( N-I, XG(I+1,NB+I), A(K+I+1,I), 1, QG(I+1,I+2),
     $                  LDQG )
C
C           Annihilate updated parts in XG.
C
