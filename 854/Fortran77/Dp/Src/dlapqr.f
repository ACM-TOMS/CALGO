      SUBROUTINE DLAPQR( WANTT, WANTQ, WANTZ, N, ILO, IHI, ILOQ, IHIQ,
...
C     PURPOSE
C
C     DLAPQR is an auxiliary routine called by DGEPQR to deal with
C     small subtasks of the product eigenvalue problem.
C
C     ARGUMENTS
C
C     Mode Parameters
...
C             Specifies the rows of Q and Z to which transformations
...
C             transformations accumulated by DHGPQR.
...
C             the same order as on the diagonales of the Schur forms of
C             A and B.
C
C     Workspace
C
...
C                   value;
C             > 0:  if INFO = i, then DLAPQR failed to compute the Schur
...
C     METHOD
C
C     The implemented algorithm is a double-shift version of the periodic
C     QR algorithm described in [1,3] with some minor modifications [2].
C     The eigenvalues are computed via an implicit complex single shift
C     algorithm.
C
C     REFERENCES
C
...
C     .. External Subroutines ..
      EXTERNAL           DCOPY, DLABAD, DLADFB, DLAPV2, DLARFG, DLARFX,
     $                   DLARNV, DLARTG, DROT, XERBLA
...
C
            B(KK,KK) = ZERO
            CALL DLADFB( WANTT, WANTQ, WANTZ, N, K, I, ILOQ, IHIQ, KK,
...
C
         CALL DLAPV2( A(I-1,I-1), LDA, B(I-1,I-1), LDB, ALPHAR(I-1),
