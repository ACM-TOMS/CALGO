      SUBROUTINE DTRQML( SIDE, UPLO, TRANS, M, N, ALPHA, T, LDT, A, LDA,
     $                   DWORK, LDWORK, INFO )
      IMPLICIT NONE
C
C     PURPOSE
...
C     step, the upper triangle of T is used; in the second step, the
C     contribution of the subdiagonal is added. If the workspace can
C     accomodate parts of A, a fast BLAS 3 DTRMM operation is used in
C     the first step. 
C
C     REFERENCES
C
...
C
      ...
      ELSE IF ( N.LT.0 ) THEN
         INFO = -5
      ELSE IF ( LDT.LT.1 .OR. ( LSIDE .AND. LDT.LT.M ) .OR.
     $                   ( .NOT.LSIDE .AND. LDT.LT.N ) ) THEN
         INFO = -8
...
C
   ...
   30       CONTINUE
     ...
     $               LDT, A, LDA )
C
C        Add the contribution of the subdiagonal of T.
C
         PDW = PSAV
         XDIF = 1 - XDIF
         IF( LSIDE ) THEN
...
C
         IF ( LSIDE ) THEN
            DO 80 J = 1, N
C              
C              Compute the contribution of the subdiagonal of T to
...
C
   ...
   70          CONTINUE
C              
C              Multiply the upper triangle of T by the j-th column
...
C              
   ...
   90          CONTINUE
C              
C              Multiply the i-th row of A by the upper triangle of T,
