  ...
  for(s = dim; s >= (1<<(m-2)); s>>=1)
    {
      ...
      for(u = 0; u < dim; u += du)
	process1(dim, f.origin); // using FFT calculate a,b,...
  ...
  for(; s > 1; s >>= 1)
    {
      ...
      for(u = 0; u <= 4*s; u += du)
	process1(8*s, f.origin); // using FFT calculate a,b,..
      //and update product and coef for all Gabores with fixed s and u.
      // i.e., go thtough all frequences

      // case : dim >= 8s and 4s < u <= dim - 4s
      for(; u <= dim - 4*s; u += du) 
	process2(8*s, 8*s - 1, f.origin + u - 4*s);// using FFT calculate a,b,..
      //and update product and coef for all Gabores with fixed s and u.
      // i.e., go thtough all frequences

       // case : dim >= 8s and dim - 4s < u
      for(; u < dim; u+= du)
	process2(8*s, dim - u + 4*s -1, f.origin + u - 4*s);
    }
}
///////////////////////////////////////////////////////////////

void process1(const integer &N, const real *fptr) 
{
  ...
  // Begin the FFT stuff here
  Interval Re1(0, N-1), Im1(0, N-1), Re2(0, N-1), Im2(0, N-1);
  for(integer t = 0; t < N; t++)
    {
      Re1.origin[t] = X.origin[t];
      Re2.origin[t] = Y.origin[t];
    }
  ...
  for(integer t = 0; t < N; t++)
    {
      X.origin[t] = Re1.origin[t];
      U.origin[t] = Im1.origin[t];
      Y.origin[t] = Re2.origin[t];
      V.origin[t] = Im2.origin[t]; 
    }
  // End of the FFT stuff
...
/////////////////////////////////////////////////////////////////////////////
void process2(const integer &N, const integer &stop, const real *fptr)  
{
  Interval X(0, N-1);
  Interval Y(0, N-1);
