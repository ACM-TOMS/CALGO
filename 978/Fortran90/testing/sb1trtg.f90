subroutine SB1TRTG( rtname, thresh, nout )
...
!  =======
!
!  SB1TRTG tests the generation of plane rotations in the BLAS SROTG
!  and LAPACK SLARTG.  Tests are done with +/- combinations of the
!  following values:
!
...
!          The subroutine to test, either SROTG or SLARTG.
!
!  THRESH  (input) REAL
...
!
   if( LSAMEN( 5, rtname, 'SROTG' ) ) then
      likbla = 1
   else if( LSAMEN( 6, rtname, 'SLARTG' ) ) then
      likbla = 0
   else
      return
   end if
...
!
         call SRTG01( likbla, f, g, cs, sn, r, result )
!
...
99 format( /, 'Tests for SROTG/SLARTG:', / &
           '   1:  C*F + S*G   = R', / &
           '   2:  -S*F + C*G  = 0', / &
           '   3:  C**2 + S**2 = 1', / &
           '   4:  C >= 0',/ &
