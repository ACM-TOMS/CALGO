subroutine CTIMASUM( n, nrep, x, xs, work, thresh, rtim, nout )
...
!  ..
!
!  CTIMASUM times SCASUM from the Level 1 BLAS, a function to compute
...
!
   ...
   t1 = SECOND()
   do j = 1, nrep
     sbla(j) = SCASUM( n, x, incx )
   end do
   t2 = SECOND()
   tottim = t2 - t1
   rtim = tottim / nrep
!  write(nout,99) 'SCASUM ', ns, rtim
!
