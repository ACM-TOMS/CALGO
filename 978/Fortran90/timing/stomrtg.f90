subroutine STOMRTG( n, nrep, x, ldx, y, ldy, z, ldz, xs, ys, &
                    thresh, nout )
...
!  ..
!
!  STOMRTG times SLARGV from LAPACK, a subroutine to compute a vector
...
!
       ...
       t1 = SECOND()
       do j = 1, nrep
         call SLARGV( n, x(1,j), incx, y(1,j), incy, z(1,j), incz )
       end do
       t2 = SECOND()
       tottim = t2 - t1
       write(nout,99) 'SLARGV', ns, iztype, tottim / nrep
!
...
!
       ...
       worst(1:ntests) = zero
       do i = 1, ns
         call SRTG01( likbla, sx*xs(i), sy*ys(i), z(i,j), y(i,j), x(i,j), result )
!
