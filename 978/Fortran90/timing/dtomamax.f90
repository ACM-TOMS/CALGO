subroutine DTOMAMAX( n, nrep, x, ldx, xs, nout )
...
!  ..
!
!  DTOMAMAX times IDAMAX from the Level 1 BLAS, a function to find
...
!
   ...
   t1 = DSECND()
   do j = 1, nrep
     ibla(j) = IDAMAX( n, x(1,j), incx )
   end do
   t2 = DSECND()
   tottim = t2 - t1
   write(nout,99) 'IDAMAX', ns, tottim / nrep
!
