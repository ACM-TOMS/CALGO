subroutine CTOMDOTC( n, nrep, x, ldx, y, ldy, xs, ys, work, thresh, nout )
...
!  ..
!
!  CTOMDOTC times CDOTC from the Level 1 BLAS, a function to compute the
...
!
   ...
   t1 = SECOND()
   do j = 1, nrep
     cbla(j) = CDOTC( n, x(1,j), incx, y(1,j), incy )
   end do
   t2 = SECOND()
   tottim = t2 - t1
   write(nout,99) 'CDOTC ', ns, tottim / nrep
!
