subroutine DTIMASUM( n, nrep, x, xs, thresh, rtim, nout )
...
!  ..
!
!  DTIMASUM times DASUM from the Level 1 BLAS, a function to compute
...
!
   ...
   t1 = DSECND()
   do j = 1, nrep
     sbla(j) = DASUM( n, x, incx )
   end do
   t2 = DSECND()
   tottim = t2 - t1
   rtim = tottim / nrep
!  write(nout,99) 'DASUM ', ns, rtim
!
