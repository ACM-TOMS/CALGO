subroutine STIMDOT( n, nrep, x, y, xs, ys, thresh, rtim, nout )
...
!  ..
!
!  STIMDOT times SDOT from the Level 1 BLAS, a function to compute the
...
!
   ...
   t1 = SECOND()
   do j = 1, nrep
     sbla(j) = SDOT( n, x, incx, y, incy )
   end do
   t2 = SECOND()
   tottim = t2 - t1
   rtim = tottim / nrep
!  write(nout,99) 'SDOT  ', ns, rtim
!
...
!
   ...
   call SARGCKV( nerrs, subnam, 'Y', ns, y, ys, nout )
