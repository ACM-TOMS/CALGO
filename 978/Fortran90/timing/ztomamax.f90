subroutine ZTOMAMAX( n, nrep, x, ldx, xs, work, nout )
...
!  ..
!
!  ZTOMAMAX times IZAMAX from the Level 1 BLAS, a function to find
...
!
   ...
   do j = 1, nrep
     n = ns
     incx = incxs
     ibla(j) = IZAMAX( n, x(1,j), incx )
   end do
   t2 = DSECND()
   tottim = tottim + t2 - t1
   write(nout,99) 'IZAMAX', ns, tottim / nrep
!
