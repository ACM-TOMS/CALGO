subroutine ZTOMDOTU( n, nrep, x, ldx, y, ldy, xs, ys, work, thresh, nout )
...
!  ..
!
!  ZTOMDOTU times ZDOTU from the Level 1 BLAS, a function to compute the
...
!
   ...
   t1 = DSECND()
   do j = 1, nrep
     cbla(j) = ZDOTU( n, x(1,j), incx, y(1,j), incy )
   end do
   t2 = DSECND()
   tottim = t2 - t1
   write(nout,99) 'ZDOTU ', ns, tottim / nrep
!
