subroutine ZTOMRTG( n, nrep, x, ldx, y, ldy, z, ldz, xs, ys, &
                    work, thresh, nout )
...
!  ..
!
!  ZTOMRTG times ZLARGV from LAPACK, a subroutine to compute a vector
...
!
       ...
       t1 = DSECND()
       do j = 1, nrep
         call ZLARGV( n, x(1,j), incx, y(1,j), incy, z(1,j), incz )
       end do
       t2 = DSECND()
       tottim = t2 - t1
       write(nout,99) 'ZLARGV', ns, iztype, tottim / nrep
!
...
!
       ...
       worst(1:ntests) = zero
       do i = 1, ns
         call ZRTG01( sx*xs(i), sy*ys(i), z(i,j), y(i,j), x(i,j), result )
!
