subroutine ZTIMDOTC( n, nrep, x, y, xs, ys, work, thresh, rtim, nout )
...
!  ..
!
!  ZTIMDOTC times ZDOTC from the Level 1 BLAS, a function to compute the
...
!
   ...
   t1 = DSECND()
   do j = 1, nrep
     cbla(j) = ZDOTC( n, x, incx, y, incy )
   end do
   t2 = DSECND()
   tottim = t2 - t1
   rtim = tottim / nrep
!  write(nout,99) 'ZDOTC ', ns, rtim
!
!  See what values changed inside the subroutine
!
   nerrs = 0
   call IARGCKS( nerrs, subnam, 'N', n, ns, nout )
   call ZARGCKV( nerrs, subnam, 'X', ns, x, xs, nout )
   call IARGCKS( nerrs, subnam, 'INCX', incx, incxs, nout )
   call ZARGCKV( nerrs, subnam, 'Y', ns, y, ys, nout )
