PROGRAM SAMPLEP_MAIN
! Test code that runs the VTMOP driver on one of the test problems from
! VTMOP_FUNC_MOD, parallelizing between iteration tasks and concurrent
! function evaluations.
!
! Before running, make sure to set the following environment variables:
!    OMP_NESTED=TRUE
!    OMP_NUM_THREADS=T1,T2
! where T1 and T2 are thread counts. The recommended values are
!    T1 = MAX(number of processors, P);
!    T2 = CEIL(number of processors / T1).
! 
! The outputs are printed to the file samplep_out.txt
...
! Output file.
CHARACTER(LEN=20), PARAMETER :: OUTFILE = "samplep_out.txt"
...
CALL VTMOP_SOLVE( D, P, LB, UB, OBJ_F, PARETO_X, PARETO_F, IERR,   &
                  ...
                  EVAL_SURROGATES=LSHEP_EVAL, PFLAG=3, ICHKPT=1    )
IF (IERR .GE. 10) THEN
...
ELSE IF (CHECK_MAE .AND. MAE > TOLERANCE) THEN
   ...
   WRITE(*,*) "An installation error may have occurred."
ELSE
   WRITE(*,*) "The parallel installation appears to be correct."
   WRITE(*,*) "The full output is contained in the file ", OUTFILE
END IF
RETURN
END SUBROUTINE CHECK_SOLUTION

END PROGRAM SAMPLEP_MAIN
