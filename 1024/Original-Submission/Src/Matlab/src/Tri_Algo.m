function [inorout,p_prime,alpha_coe,gap,iter_num]=Tri_Algo(input_a,epsilon,query,varargin)
...
matA=input_a;
[m,n]=size(matA);
diffmat=matA-query(:,ones(1,n));
eudis=sqrt(sum(diffmat.^2,1));
veps=max(epsilon*mean(eudis),epsilon);
p = query;
