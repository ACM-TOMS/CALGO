...
%	DEINVMAP(WP,W,BETA,Z,C,QDAT) computes the inverse of the
...
%	arguments are as in DEPARAM.  QDAT may be omitted.
%
...
%
%	DEINVMAP(WP,W,BETA,Z,C,QDAT,Z0) has two interpretations.  If the ODE
...
%	length(WP).
%
%       DEINVMAP(WP,W,BETA,Z,C,QDAT,Z0,OPTIONS) uses a vector of parameters
%       that control the algorithm.  See SCIMAPOPT.
%
%	See also SCIMAPOPT, DEPARAM, DEMAP.
%
...
if ode
  if isempty(z0)
    ...
    [z0,w0] = scimapz0('de',wp,w,beta,z,c,qdat);
  else
  ...
  z0 = [real(z0);imag(z0)];
  [t,y] = ode45('deimapf1',0,1,z0,odetol);
  [m,leny] = size(y);
...
if newton
  ...
  while ~all(done) & k < maxiter
    ...
    m = length(F);
    dF = c*(zn(~done).').^(-2).*...
      exp(sum(beta(:,ones(m,1)) .* log(1-(zn(~done,ones(n,1)).')./z(:,ones(m,1)))));
    zn(~done) = zn(~done) + F(:)./dF(:);
    done(~done) = (abs(F)< tol);
    k = k+1;
  end
  if any(abs(F)> tol)
    disp('Warning in deinvmap: Solution may be inaccurate')
