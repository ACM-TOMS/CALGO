function [xf,termcode,path] = fsolve(fvec,x0,details,fparam,jac,scale)
...
if details(1)
  disp('ITN    F-COUNT       NORM(F)')
end
  
while (termcode == 0)
   if details(1)
      disp(sprintf('%3d %8d %15g',itncount,nofun,norm(FVc)))
   end
   itncount = itncount + 1;
...
if (details(1) == 2)
   ...
   disp(sprintf('Final norm(F(x)): %.6g',norm(FVc)))
