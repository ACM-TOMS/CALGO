...
for k=1:length(time_steps)
  ...
  % kappa
  kappa_mre_fs(k) = mean(mre(1));
  % theta
  theta_mre_fs(k) = mean(mre(2));
  % sigma
  sigma_mre_fs(k) = mean(mre(3));
  % rho
  rho_mre_fs(k) = mean(mre(4));
  ...
  % kappa
  kappa_mre_cgmm(k) = mean(mre(1));
  % theta
  theta_mre_cgmm(k) = mean(mre(2));
  % sigma
  sigma_mre_cgmm(k) = mean(mre(3));
  % rho
  rho_mre_cgmm(k) = mean(mre(4));
end
...
title('PCSV partial CMM mean relative estimation errors');
...
print(cgmm_config.plots.mre.all('pcsv_partial'), cgmm_config.plots.device)
...
print(cgmm_config.plots.mre.individual('pcsv_partial'), cgmm_config.plots.device)
