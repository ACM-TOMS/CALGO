...
C
    1 WRITE (*,200)
  200 FORMAT (//5X,'Specify the name of a file containing ',
     .        'an input data set'/
     .        5X,'(at most 60 characters):'/)
      READ (*,205,ERR=1) FNAME
  205 FORMAT (A60)
      OPEN (LIN,FILE=FNAME,STATUS='OLD',ERR=1)
C
...
C
    5 WRITE (*,210)
  210 FORMAT (//5X,'Create file dnsplin1.ps0 containing a ',
     .        'plot '/
     .        5X,'of the initial estimate? (y/n)'/)
      READ (*,215,ERR=5) CHR
  215 FORMAT (A1)
      IF (CHR .NE. 'Y'  .AND.  CHR .NE. 'y'  .AND.
     .    CHR .NE. 'N'  .AND.  CHR .NE. 'n') GO TO 5
...
C
   13 WRITE (*,220)
  220 FORMAT (//5X,'Create file dnsplin1.ps2 containing a ',
     .        'curvature plot? (y/n)'/)
      READ (*,215,ERR=13) CHR
      IF (CHR .NE. 'Y'  .AND.  CHR .NE. 'y'  .AND.
     .    CHR .NE. 'N'  .AND.  CHR .NE. 'n') GO TO 13
      IF (CHR .EQ. 'Y'  .OR.  CHR .EQ. 'y') THEN
