...
long  std_kepler(realNUM t, realNUM v[], realNUM p[], int ORDER, realNUM cvfd[][ORDER+1])
{
...
long  std_kepler_columns()
{
	 return 5;
}

long  std_kepler_pos_der(char *der)
{
...
long  std_kepler_variable_column(int v, char *der)
{
	 return position_variable(v, std_kepler_pos_der, der);
}

long  std_kepler_function_column(int f, char *der)
{
	 return position_function(f, std_kepler_pos_der, der);
}
