!*********************************************************************************************************************************
!*
!* PROGRAM: EXAMPLE2D_RRSQRTKF
!*
!* PURPOSE: test the Reduced Rank Square Root Kalman Filter
!*
!* DEPENDENCIES:
!*                - PRECISION
!*                - TOOLS
...
program example2d_rrsqrtkf
  ...
  real( kind = pre ) , allocatable , dimension( : , : ) :: sqrtcovauxrrsqrtkf
  ...
  if ( rank == 0_pin ) then
     ...
     print*,'unitassimrrsqrtkf = ' , unitassimrrsqrtkf   
     print*,'unitobser         = ' , unitobser   
     print*,'filetruth         = ' , trim( filetruth )   
     print*,'filemodel         = ' , trim( filemodel )  
     print*,'fileassimrrsqrtkf = ' , trim( fileassimrrsqrtkf )  
  ...
  call mpi_bcast( unitassimrrsqrtkf , 1_pin , mpi_integer , 0_pin , mpi_comm_world , ierror )
  ...
  call mpi_bcast( fileassimrrsqrtkf , 100_pin , mpi_character , 0_pin , mpi_comm_world , ierror )
  !*
  !* solution parameters
  if ( rank == 0_pin ) then
  ...
  !*
  !* assimilation
  assimrrsqrtkf( 1_pin , 1_pin : nx , 1_pin : ny ) = staterrsqrtkf( 1_pin : nx , 1_pin : ny )
  !*
  !* writing to file
  if ( rank == 0_pin ) then
     do i = 1_pin , nx
        do j = 1_pin , ny
           ...
           write( unitassimrrsqrtkf , '( f15.8 , f15.8 , f15.8 , f15.8 )' ) t( 1_pin ) , x( i ) , y( j ) , assimrrsqrtkf( 1_pin ,&
                & i , j )
        end do
     end do
  end if
  ...
  do l = 1_pin , nt - 1_pin
     ...
     call rrsqrtkf_predictor( l , staterrsqrtkf , sqrtcovrrsqrtkf , sqrtcovauxrrsqrtkf )
     !*
     ...
     if ( ifobs ) then
        ...
        !*
        call rrsqrtkf_corrector( l , staterrsqrtkf , sqrtcovrrsqrtkf , sqrtcovauxrrsqrtkf )
        !*
        deallocate( obsvalue )
        deallocate( sigmaobs )
        !*
     ...
     !*
     assimrrsqrtkf( l + 1_pin , 1_pin : nx , 1_pin : ny ) = staterrsqrtkf
     !*
     !* writing to file
     if ( rank == 0_pin ) then
        do i = 1_pin , nx
           do j = 1_pin , ny
              ...
              write( unitmodel , '( f15.8 , f15.8 , f15.8 , f15.8 )' ) t( l + 1_pin ) , x( i ) , y( j ) , modl( l + 1_pin , i , j&
                   & ) 
              write( unitassimrrsqrtkf , '( f15.8 , f15.8 , f15.8 , f15.8 )' ) t( l + 1_pin ) , x( i ) , y( j ) , assimrrsqrtkf( &
                   &l + 1_pin , i , j ) 
           end do
        end do
     end if
  ...
  if ( rank == 0_pin ) then
     ...
     close( unitmodel )
     close( unitobser )
     close( unitassimrrsqrtkf )
  end if
  !*
  !* end parallelization
  call parallel_finalize()
  !*
end program example2d_rrsqrtkf
