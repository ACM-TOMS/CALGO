...

{
  ...
  if ((order == CblasRowMajor && Uplo == CblasUpper)
    ...
    for (i = 0; i < N; i++) {
      ...
      MpIeee Aii_real=  CONST_REAL(Ap, TPUP(N, i, i));
      /* Aii_imag is zero */
      REAL(Y, iy) += temp1_real * Aii_real;
      IMAG(Y, iy) += temp1_imag * Aii_real;
      for (j = j_min; j < j_max; j++) {
        MpIeee Aij_real=  CONST_REAL(Ap, TPUP(N, i, j));
        MpIeee Aij_imag=  conj * CONST_IMAG(Ap, TPUP(N, i, j));
    ...
    for (i = 0; i < N; i++) {
      ...
      INDEX jx = OFFSET(N, incX) + j_min * incX;
      INDEX jy = OFFSET(N, incY) + j_min * incY;
      MpIeee Aii_real=  CONST_REAL(Ap, TPLO(N, i, i));
      /* Aii_imag is zero */
      REAL(Y, iy) += temp1_real * Aii_real;
      IMAG(Y, iy) += temp1_imag * Aii_real;
      for (j = j_min; j < j_max; j++) {
        MpIeee Aij_real=  CONST_REAL(Ap, TPLO(N, i, j));
        MpIeee Aij_imag=  conj * CONST_IMAG(Ap, TPLO(N, i, j));
        REAL(Y, jy) += temp1_real * Aij_real - temp1_imag * (-Aij_imag);
        IMAG(Y, jy) += temp1_real * (-Aij_imag) + temp1_imag * Aij_real;
        x_real = CONST_REAL(X, jx);
        x_imag = CONST_IMAG(X, jx);
