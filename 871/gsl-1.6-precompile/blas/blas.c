...
int
 gsl_blas_sdsdot(MpIeee alpha, const gsl_vector_float * X,
                 const gsl_vector_float * Y, MpIeee *result)
{
  if (X->size == Y->size)
    {
      *result =
...
int
 gsl_blas_ddot(const gsl_vector * X, const gsl_vector * Y, MpIeee *result)
{
  if (X->size == Y->size)
    {
      *result =
...
MpIeee gsl_blas_snrm2(const gsl_vector_float * X)
{
  return cblas_snrm2 (INT (X->size), X->data, INT (X->stride));
}

MpIeee gsl_blas_dnrm2(const gsl_vector * X)
{
  return cblas_dnrm2 (INT (X->size), X->data, INT (X->stride));
}
/*
MpIeee gsl_blas_scnrm2(const gsl_vector_complex_float * X)
{
  return cblas_scnrm2 (INT (X->size), X->data, INT (X->stride));
}

MpIeee gsl_blas_dznrm2(const gsl_vector_complex * X)
{
...
MpIeee gsl_blas_sasum(const gsl_vector_float * X)
{
  return cblas_sasum (INT (X->size), X->data, INT (X->stride));
}

MpIeee gsl_blas_dasum(const gsl_vector * X)
{
  return cblas_dasum (INT (X->size), X->data, INT (X->stride));
}
/*
MpIeee gsl_blas_scasum(const gsl_vector_complex_float * X)
{
  return cblas_scasum (INT (X->size), X->data, INT (X->stride));
}

MpIeee gsl_blas_dzasum(const gsl_vector_complex * X)
{
...
int
 gsl_blas_saxpy(MpIeee alpha, const gsl_vector_float * X, gsl_vector_float * Y)
{
  if (X->size == Y->size)
    {
...
int
 gsl_blas_daxpy(MpIeee alpha, const gsl_vector * X, gsl_vector * Y)
{
  if (X->size == Y->size)
    {
...
int
 gsl_blas_srotg(MpIeee a[], MpIeee b[], MpIeee c[], MpIeee s[])
{
  cblas_srotg (a, b, c, s);
  return GSL_SUCCESS;
}
...
int
 gsl_blas_drotg(MpIeee a[], MpIeee b[], MpIeee c[], MpIeee s[])
{
  cblas_drotg (a, b, c, s);
  return GSL_SUCCESS;
}
...
int
 gsl_blas_srot(gsl_vector_float * X, gsl_vector_float * Y, MpIeee c, MpIeee s)
{
  if (X->size == Y->size)
    {
...
int
 gsl_blas_drot(gsl_vector * X, gsl_vector * Y, const MpIeee c, const MpIeee s)
{
  if (X->size == Y->size)
    {
...
int
 gsl_blas_srotmg(MpIeee d1[], MpIeee d2[], MpIeee b1[], MpIeee b2, MpIeee P[])
{
...
int
 gsl_blas_drotmg(MpIeee d1[], MpIeee d2[], MpIeee b1[], MpIeee b2, MpIeee P[])
{
...
int
 gsl_blas_srotm(gsl_vector_float * X, gsl_vector_float * Y, const MpIeee P[])
{
  if (X->size == Y->size)
    {
...
int
 gsl_blas_drotm(gsl_vector * X, gsl_vector * Y, const MpIeee P[])
{
  if (X->size != Y->size)
    {
...
void
gsl_blas_sscal (MpIeee alpha, gsl_vector_float * X)
{
  cblas_sscal (INT (X->size), alpha, X->data, INT (X->stride));
}

void
gsl_blas_dscal (MpIeee alpha, gsl_vector * X)
{
...
void
gsl_blas_csscal (MpIeee alpha, gsl_vector_complex_float * X)
{
  cblas_csscal (INT (X->size), alpha, X->data, INT (X->stride));
}

void
gsl_blas_zdscal (MpIeee alpha, gsl_vector_complex * X)
{
...
int
 gsl_blas_sgemv(CBLAS_TRANSPOSE_t TransA, MpIeee alpha,
                const gsl_matrix_float * A, const gsl_vector_float * X,
                MpIeee beta, gsl_vector_float * Y)
{
...
int
 gsl_blas_dgemv(CBLAS_TRANSPOSE_t TransA, MpIeee alpha, const gsl_matrix * A,
                const gsl_vector * X, MpIeee beta, gsl_vector * Y)
{
  const size_t M = A->size1;
  const size_t N = A->size2;
...
int
 gsl_blas_ssymv(CBLAS_UPLO_t Uplo, MpIeee alpha, const gsl_matrix_float * A,
                const gsl_vector_float * X, MpIeee beta, gsl_vector_float * Y)
{
  const size_t M = A->size1;
  const size_t N = A->size2;
...
int
 gsl_blas_dsymv(CBLAS_UPLO_t Uplo, MpIeee alpha, const gsl_matrix * A,
                const gsl_vector * X, MpIeee beta, gsl_vector * Y)
{
  const size_t M = A->size1;
  const size_t N = A->size2;
...
int
 gsl_blas_sger(MpIeee alpha, const gsl_vector_float * X,
...
int
 gsl_blas_dger(MpIeee alpha, const gsl_vector * X, const gsl_vector * Y,
               gsl_matrix * A)
{
  const size_t M = A->size1;
  const size_t N = A->size2;
...
/*
int
 gsl_blas_cher(CBLAS_UPLO_t Uplo, MpIeee alpha,
               const gsl_vector_complex_float * X,
               gsl_matrix_complex_float * A)
{
  const size_t M = A->size1;
...
int
 gsl_blas_zher(CBLAS_UPLO_t Uplo, MpIeee alpha, const gsl_vector_complex * X,
...
int
 gsl_blas_ssyr(CBLAS_UPLO_t Uplo, MpIeee alpha, const gsl_vector_float * X,
               gsl_matrix_float * A)
{
  const size_t M = A->size1;
  const size_t N = A->size2;
...
int
 gsl_blas_dsyr(CBLAS_UPLO_t Uplo, MpIeee alpha, const gsl_vector * X,
               gsl_matrix * A)
{
  const size_t M = A->size1;
  const size_t N = A->size2;
...
int
 gsl_blas_ssyr2(CBLAS_UPLO_t Uplo, MpIeee alpha, const gsl_vector_float * X,
                const gsl_vector_float * Y, gsl_matrix_float * A)
{
  const size_t M = A->size1;
  const size_t N = A->size2;
...
int
 gsl_blas_dsyr2(CBLAS_UPLO_t Uplo, MpIeee alpha, const gsl_vector * X,
...
int
 gsl_blas_sgemm(CBLAS_TRANSPOSE_t TransA, CBLAS_TRANSPOSE_t TransB,
                MpIeee alpha, const gsl_matrix_float * A,
                const gsl_matrix_float * B, MpIeee beta, gsl_matrix_float * C)
{
...
int
 gsl_blas_dgemm(CBLAS_TRANSPOSE_t TransA, CBLAS_TRANSPOSE_t TransB,
                MpIeee alpha, const gsl_matrix * A, const gsl_matrix * B,
                MpIeee beta, gsl_matrix * C)
{
  const size_t M = C->size1;
  const size_t N = C->size2;
  const size_t MA = (TransA == CblasNoTrans) ? A->size1 : A->size2;
...
int
 gsl_blas_ssymm(CBLAS_SIDE_t Side, CBLAS_UPLO_t Uplo, MpIeee alpha,
                const gsl_matrix_float * A, const gsl_matrix_float * B,
                MpIeee beta, gsl_matrix_float * C)
{
  const size_t M = C->size1;
  const size_t N = C->size2;
...
int
 gsl_blas_dsymm(CBLAS_SIDE_t Side, CBLAS_UPLO_t Uplo, MpIeee alpha,
                const gsl_matrix * A, const gsl_matrix * B, MpIeee beta,
                gsl_matrix * C)
{
  const size_t M = C->size1;
  const size_t N = C->size2;
...
int
 gsl_blas_ssyrk(CBLAS_UPLO_t Uplo, CBLAS_TRANSPOSE_t Trans, MpIeee alpha,
                const gsl_matrix_float * A, MpIeee beta, gsl_matrix_float * C)
{
  const size_t M = C->size1;
  const size_t N = C->size2;
...
int
 gsl_blas_dsyrk(CBLAS_UPLO_t Uplo, CBLAS_TRANSPOSE_t Trans, MpIeee alpha,
                const gsl_matrix * A, MpIeee beta, gsl_matrix * C)
{
  const size_t M = C->size1;
  const size_t N = C->size2;
  const size_t K = (Trans == CblasNoTrans) ? A->size2 : A->size1;
...
/*
int
 gsl_blas_cherk(CBLAS_UPLO_t Uplo, CBLAS_TRANSPOSE_t Trans, MpIeee alpha,
                const gsl_matrix_complex_float * A, MpIeee beta,
...
int
 gsl_blas_zherk(CBLAS_UPLO_t Uplo, CBLAS_TRANSPOSE_t Trans, MpIeee alpha,
                const gsl_matrix_complex * A, MpIeee beta,
                gsl_matrix_complex * C)
{
  const size_t M = C->size1;
  const size_t N = C->size2;
...
int
 gsl_blas_ssyr2k(CBLAS_UPLO_t Uplo, CBLAS_TRANSPOSE_t Trans, MpIeee alpha,
                 const gsl_matrix_float * A, const gsl_matrix_float * B,
                 MpIeee beta, gsl_matrix_float * C)
{
  const size_t M = C->size1;
...
int
 gsl_blas_dsyr2k(CBLAS_UPLO_t Uplo, CBLAS_TRANSPOSE_t Trans, MpIeee alpha,
                 const gsl_matrix * A, const gsl_matrix * B, MpIeee beta,
                 gsl_matrix * C)
{
  const size_t M = C->size1;
...
int
 gsl_blas_cher2k(CBLAS_UPLO_t Uplo, CBLAS_TRANSPOSE_t Trans,
                 const gsl_complex_float alpha,
                 const gsl_matrix_complex_float * A,
                 const gsl_matrix_complex_float * B, MpIeee beta,
                 gsl_matrix_complex_float * C)
{
  const size_t M = C->size1;
  const size_t N = C->size2;
...
int
 gsl_blas_zher2k(CBLAS_UPLO_t Uplo, CBLAS_TRANSPOSE_t Trans,
                 const gsl_complex alpha, const gsl_matrix_complex * A,
                 const gsl_matrix_complex * B, MpIeee beta,
                 gsl_matrix_complex * C)
{
  const size_t M = C->size1;
  const size_t N = C->size2;
...
int
 gsl_blas_strmm(CBLAS_SIDE_t Side, CBLAS_UPLO_t Uplo,
                CBLAS_TRANSPOSE_t TransA, CBLAS_DIAG_t Diag, MpIeee alpha,
