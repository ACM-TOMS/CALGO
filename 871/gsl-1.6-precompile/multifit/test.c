...
void
test_lmder (gsl_multifit_function_fdf * f, MpIeee x0[], 
            MpIeee * X, MpIeee F[], MpIeee * cov);

void
test_fdf (const char * name, gsl_multifit_function_fdf * f, 
          MpIeee x0[], MpIeee x[], MpIeee sumsq,
          MpIeee sigma[]);
...
void
test_fdf (const char * name, gsl_multifit_function_fdf * f, 
          MpIeee x0[], MpIeee x_final[], 
          MpIeee f_sumsq, MpIeee sigma[])
{
  const gsl_multifit_fdfsolver_type *T;
  gsl_multifit_fdfsolver *s;
...
#ifdef DEBUG
       {cout<<"iter = "<< 
         iter<<"  status = "<< status<<"  |f| = "<<setiosflags((ios::scientific & ios::floatfield))<<setprecision(18)<< gsl_blas_dnrm2 (s->f);
cout.precision(6);cout.fill(' ');cout.width(0);cout.setf(ios::dec,ios::basefield);
cout<<resetiosflags((ios::scientific & ios::floatfield))<<" x = \n";}
  ...
  
  {
    ...
    {
      MpIeee s2=  pow(gsl_blas_dnrm2 (s->f), MpIeee( "2.0" ));

      gsl_test_rel (s2, f_sumsq, 1e-5, "%s, lmsder, |f|^2", name);

      for (i = 0; i < p; i++) 
        {
          MpIeee ei=  sqrt(s2/(n-p))*sqrt(gsl_matrix_get(covar,i,i));
