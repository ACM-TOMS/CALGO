...
//  gsl_root_fsolver * s = (gsl_root_fsolver *) malloc (sizeof (gsl_root_fsolver));
  gsl_root_fsolver* s = new gsl_root_fsolver();
...
int
 gsl_root_fsolver_set(gsl_root_fsolver * s, gsl_function * f, MpIeee x_lower, MpIeee x_upper)
{
  cout << "in fsolver_set"<<endl;
  if (x_lower > x_upper)
    {
  ...
  cout << "params:"<<endl;
  cout << "solver="<<s<<endl;
  cout << "funcpointer f="<<f<<endl;
  cout << "x_lower="<<x_lower<<endl;
  cout << "x_upper="<<x_upper<<endl;
  cout << "root=" << flush << MpIeee("0.5") * (x_lower + x_upper) << endl;

  s->function = f;
  //cout << "function assigned" << endl;
  s->root = MpIeee("0.5") * (x_lower + x_upper);  /* initial estimate */
  //cout << "root assigned"<<endl;
  s->x_lower = x_lower;
  //cout << "lower assigned"<<endl;
  s->x_upper = x_upper;
  //cout << "upper assigned"<<endl;
  
  cout << "HIER MOETE WE ZIJN root=" << flush<< s->root << endl;
