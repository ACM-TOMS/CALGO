...
int
 rhs_linear(MpIeee t, const MpIeee y[], MpIeee f[], void *params)
{
  f[0] = MpIeee( "2.0" );
...
int
 jac_linear(MpIeee t, const MpIeee y[], MpIeee *dfdy, MpIeee dfdt[],
            void *params)
{
  dfdy[0] = MpIeee( "0.0" );
  dfdt[0] = MpIeee( "0.0" );
...
int
 rhs_exp(MpIeee t, const MpIeee y[], MpIeee f[], void *params)
{
  f[0] = y[0];
...
int
 jac_exp(MpIeee t, const MpIeee y[], MpIeee *dfdy, MpIeee dfdt[],
         void *params)
{
  dfdy[0] = y[0];
  dfdt[0] = MpIeee( "0.0" );
...
int
 rhs_sin(MpIeee t, const MpIeee y[], MpIeee f[], void *params)
{
  f[0] = -y[1];
  f[1] = y[0];
...
int
 jac_sin(MpIeee t, const MpIeee y[], MpIeee *dfdy, MpIeee dfdt[],
         void *params)
{
  dfdy[0] = MpIeee( "0.0" );
  dfdy[1] = -MpIeee( "1.0" );
...
int
 rhs_xsin(MpIeee t, const MpIeee y[], MpIeee f[], void *params)
{
  static int  n=  0;
...
int
 jac_xsin(MpIeee t, const MpIeee y[], MpIeee *dfdy, MpIeee dfdt[],
         void *params)
{
  static int  n=  0;
...
int
 rhs_stiff(MpIeee t, const MpIeee y[], MpIeee f[], void *params)
{
  f[0] = MpIeee( "998.0" ) * y[0] + MpIeee( "1998.0" ) * y[1];
  f[1] = -MpIeee( "999.0" ) * y[0] - MpIeee( "1999.0" ) * y[1];

  return GSL_SUCCESS;
}

int
 jac_stiff(MpIeee t, const MpIeee y[], MpIeee *dfdy, MpIeee dfdt[],
           void *params)
{
  dfdy[0] = MpIeee( "998.0" );
  dfdy[1] = MpIeee( "1998.0" );
  dfdy[2] = -MpIeee( "999.0" );
  dfdy[3] = -MpIeee( "1999.0" );
...
int
 rhs_vanderpol(MpIeee t, const MpIeee y[], MpIeee f[], void *params)
{
  const MpIeee mu=  10.0;

  f[0] = y[1];
  f[1] = -y[0] + mu * y[1] * (MpIeee( "1.0" ) - y[0]*y[0]); 

  return GSL_SUCCESS;
}

int
 jac_vanderpol(MpIeee t, const MpIeee y[], MpIeee *dfdy, MpIeee dfdt[],
	       void *params)
{
  const MpIeee mu=  10.0;
  ...
  dfdy[0] = MpIeee( "0.0" );
  dfdy[1] = MpIeee( "1.0" );
  dfdy[2] = -MpIeee( "2.0" ) * mu * y[0] * y[1] - MpIeee( "1.0" );
  dfdy[3] = mu * (MpIeee( "1.0" ) - y[0] * y[0]);
...
int
 rhs_oregonator(MpIeee t, const MpIeee y[], MpIeee f[], void *params)
{
  const MpIeee c1= 77.27;
  const MpIeee c2= 8.375e-6;
  const MpIeee c3= 0.161;

  f[0] = c1 * (y[1] + y[0] * (MpIeee( "1" ) - c2 * y[0] - y[1]));
  f[1] = MpIeee( "1" )/c1 * (y[2] - y[1] * (MpIeee( "1" ) + y[0]));
  f[2] = c3 * (y[0] - y[2]);
...
int
 jac_oregonator(MpIeee t, const MpIeee y[], MpIeee *dfdy, MpIeee dfdt[],
		void *params)
{
  const MpIeee c1= 77.27;
  const MpIeee c2= 8.375e-6;
  ...
  dfdy[0] = c1 * (MpIeee( "1" ) - MpIeee( "2" ) * c2 * y[0] - y[1]);
  dfdy[1] = c1 * (MpIeee( "1" ) - y[0]);
  dfdy[2] = MpIeee( "0.0" );

  dfdy[3] = MpIeee( "1" )/c1 * (-y[1]);
  dfdy[4] = MpIeee( "1" )/c1 * (-MpIeee( "1" ) - y[0]);
  dfdy[5] = MpIeee( "1" )/c1;
...
int
 rhs_vl(MpIeee t, const MpIeee y[], MpIeee f[], void *params)
{
  const MpIeee a=  1.0;
  const MpIeee b=  1.0;
  const MpIeee c=  1.0;
...
int
 jac_vl(MpIeee t, const MpIeee y[], MpIeee *dfdy, MpIeee dfdt[],
            void *params)
{
  const MpIeee a=  1.0;
  const MpIeee b=  1.0;
...
int
 rhs_stifftrig(MpIeee t, const MpIeee y[], MpIeee f[], void *params)
{
  f[0] = -MpIeee( "50" ) * (y[0] - cos(t));

  return GSL_SUCCESS;
}

int
 jac_stifftrig(MpIeee t, const MpIeee y[], MpIeee *dfdy, MpIeee dfdt[],
            void *params)
{
  dfdy[0] = -MpIeee( "50" );

  dfdt[0] = -MpIeee( "50" ) * sin(t);
...
int
 rhs_e5(MpIeee t, const MpIeee y[], MpIeee f[], void *params)
{
  const MpIeee a=  7.89e-10;
  const MpIeee b=  1.1e7;
  const MpIeee c=  1.13e3;
  const MpIeee m=  1.0e6;
...
int
 jac_e5(MpIeee t, const MpIeee y[], MpIeee *dfdy, MpIeee dfdt[],
            void *params)
{
  const MpIeee a=  7.89e-10;
  const MpIeee b=  1.1e7;
...
test_odeiv_stepper (const gsl_odeiv_step_type *T, const gsl_odeiv_system *sys,
		    const MpIeee h, const MpIeee t, const char desc[],
		    const MpIeee y0[], const MpIeee yfin[], 
		    const MpIeee relerr)
{
  ...
  */
  ...
  MpIeee y[MAXEQ] =  {MpIeee( "0.0" )};
  MpIeee yerr[MAXEQ] =  {MpIeee( "0.0" )};
...
test_stepper (const gsl_odeiv_step_type *T) 
{
  ...
  /* linear */
  h = MpIeee( "1" )e-MpIeee( "1" );
  err_target = MpIeee( "1" )e-MpIeee( "10" );
  y[0] = MpIeee( "0.58" );
  yfin[0] = y[0] + MpIeee( "2" ) * h;
  test_odeiv_stepper (T, &rhs_func_lin, h, 0.0, "linear",
		      y, yfin, err_target);
  
  /* exponential */
  h = MpIeee( "1" )e-MpIeee( "4" );
  err_target = MpIeee( "1" )e-MpIeee( "8" );
  y[0] = exp(MpIeee( "2.7" ));
  yfin[0] = exp(MpIeee( "2.7" ) + h);
  test_odeiv_stepper (T, &rhs_func_exp, h, 2.7, "exponential",
		      y, yfin, err_target);
  /* cosine-sine */
  h = MpIeee( "1" )e-MpIeee( "3" );
  err_target = MpIeee( "1" )e-MpIeee( "6" );
  y[0] = cos(MpIeee( "1.2" ));
  y[1] = sin(MpIeee( "1.2" ));
  yfin[0] = cos(MpIeee( "1.2" ) + h);
  yfin[1] = sin(MpIeee( "1.2" ) + h);
  test_odeiv_stepper (T, &rhs_func_sin, h, 1.2, "cosine-sine",
		      y, yfin, err_target);

  /* classic stiff */
  h = MpIeee( "1" )e-MpIeee( "7" );
  err_target = MpIeee( "1" )e-MpIeee( "4" );
  y[0] = MpIeee( "1.0" );
  y[1] = MpIeee( "0.0" );

  {
    const MpIeee e1=  exp (-h);
    const MpIeee e2=  exp (-1000.0 * h);
    yfin[0] = MpIeee( "2.0" ) * e1 - e2;
    yfin[1] = -e1 + e2;
  }
...
test_evolve_system (const gsl_odeiv_step_type * T,
                    ...
                    MpIeee t0, MpIeee t1, MpIeee hstart,
                    MpIeee y[], MpIeee yfin[],
                    MpIeee err_target, const char *desc)
{
...
int
 sys_driver(const gsl_odeiv_step_type * T,
	    const gsl_odeiv_system * sys,
	    MpIeee t0, MpIeee t1, MpIeee hstart,
	    MpIeee y[], MpIeee epsabs, MpIeee epsrel,
	    const char desc[])
{
...
test_compare_vanderpol (void)
{
  ...
  MpIeee y[11][2];
  MpIeee *yp=  &y[0][MpIeee( "0" )];

  size_t i, j, k;
  int  status=  0;

  /* Parameters for the problem and stepper  */
  const MpIeee start=  0.0;
  const MpIeee end=  100.0;
  const MpIeee epsabs=  1e-8;
  const MpIeee epsrel=  1e-8;
  const MpIeee initstepsize=  1e-5;

  /* Initialize */

  steppers[0] = gsl_odeiv_step_rk2;
  err_target[0] = MpIeee( "1" )e-MpIeee( "6" );
  steppers[1] = gsl_odeiv_step_rk4;
  err_target[1] = MpIeee( "1" )e-MpIeee( "6" );
  steppers[2] = gsl_odeiv_step_rkf45;
  err_target[2] = MpIeee( "1" )e-MpIeee( "6" );
  steppers[3] = gsl_odeiv_step_rkck;
  err_target[3] = MpIeee( "1" )e-MpIeee( "6" );
  steppers[4] = gsl_odeiv_step_rk8pd;
  err_target[4] = MpIeee( "1" )e-MpIeee( "6" );
  steppers[5] = gsl_odeiv_step_rk2imp;
  err_target[5] = MpIeee( "1" )e-MpIeee( "5" );
  steppers[6] = gsl_odeiv_step_rk2simp;
  err_target[6] = MpIeee( "1" )e-MpIeee( "5" );
  steppers[7] = gsl_odeiv_step_rk4imp;
  err_target[7] = MpIeee( "1" )e-MpIeee( "6" );
  steppers[8] = gsl_odeiv_step_bsimp;
  err_target[8] = MpIeee( "1" )e-MpIeee( "7" );
  steppers[9] = gsl_odeiv_step_gear1;
  err_target[9] = MpIeee( "1" )e-MpIeee( "2" );
  steppers[10] = gsl_odeiv_step_gear2;
  err_target[10] = MpIeee( "1" )e-MpIeee( "6" );
  steppers[11] = 0;
  ...
  for (i = 0; i < ns; i++)
    for (j = i+1; j < ns; j++)
      for (k = 0; k < sd; k++)
	{
	  const MpIeee val1=  yp[sd * i + k];
	  const MpIeee val2=  yp[sd * j + k];
...
void
test_evolve_linear (const gsl_odeiv_step_type * T, MpIeee h, MpIeee err)
{
  MpIeee y[1];
  MpIeee yfin[1];
...
void
test_evolve_exp (const gsl_odeiv_step_type * T, MpIeee h, MpIeee err)
{
  MpIeee y[1];
  MpIeee yfin[1];

  y[0] = MpIeee( "1.0" );
  yfin[0] = exp (MpIeee( "2.0" ));
  test_evolve_system (T, &rhs_func_exp, 0.0, 2.0, h, y, yfin, err,
		      "exp[0,2]");
}

void
test_evolve_sin (const gsl_odeiv_step_type * T, MpIeee h, MpIeee err)
{
  MpIeee y[2];
  MpIeee yfin[2];
  ...
  y[0] = MpIeee( "1.0" );
  y[1] = MpIeee( "0.0" );
  yfin[0] = cos (MpIeee( "2.0" ));
  yfin[1] = sin (MpIeee( "2.0" ));
  test_evolve_system (T, &rhs_func_sin, 0.0, 2.0, h, y, yfin, err,
		      "sine[0,2]");
}

void
test_evolve_xsin (const gsl_odeiv_step_type * T, MpIeee h, MpIeee err)
{
  MpIeee y[2];
  MpIeee yfin[2];
...
void
test_evolve_stiff1 (const gsl_odeiv_step_type * T, MpIeee h, MpIeee err)
{
  MpIeee y[2];
  MpIeee yfin[2];
  ...
  y[1] = MpIeee( "0.0" );
  {
    MpIeee arg=  MpIeee( "1.0" );
    MpIeee e1=  exp (-arg);
    MpIeee e2=  exp (-MpIeee( "1000.0" ) * arg);
    yfin[0] = MpIeee( "2.0" ) * e1 - e2;
    yfin[1] = -e1 + e2;
  }
...
void
test_evolve_stiff5 (const gsl_odeiv_step_type * T, MpIeee h, MpIeee err)
{
  MpIeee y[2];
  MpIeee yfin[2];
