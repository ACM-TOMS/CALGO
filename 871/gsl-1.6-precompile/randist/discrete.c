...
gsl_ran_discrete_t *
gsl_ran_discrete_preproc(size_t Kevents, const MpIeee *ProbArray)
{
    size_t k,b,s;
    ...
    gsl_stack_t *Smalls;
    MpIeee *E;
    MpIeee pTotal=  MpIeee( "0.0" );MpIeee  mean;MpIeee  d;
    ...
    g->K = Kevents;
    g->F = (MpIeee*)malloc(sizeof(MpIeee)*Kevents);
    g->A = (size_t *)malloc(sizeof(size_t)*Kevents);

    E = (MpIeee *)malloc(sizeof(MpIeee)*Kevents);
...
{
    size_t i,K;
    MpIeee f;MpIeee p= MpIeee( "0" );
    K= g->K;
    if (k>K) return MpIeee( "0" );
    for (i=0; i<K; ++i) {
        f = (g->F)[i];
#if KNUTH_CONVENTION
        f = K*f-i;
