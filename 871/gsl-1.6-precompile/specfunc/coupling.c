                      ...
                      gsl_sf_result * result)
{
  ...
  if(two_ja < 0 || two_jb < 0 || two_jc < 0) {
    ...
    int  jca=  (-two_ja + two_jb + two_jc) / 2;int  
        jcb=  ( two_ja - two_jb + two_jc) / 2;int  
        jcc=  ( two_ja + two_jb - two_jc) / 2;int  
        jmma=  ( two_ja - two_ma) / 2;int  
        jmmb=  ( two_jb - two_mb) / 2;int  
        jmmc=  ( two_jc - two_mc) / 2;int  
        jpma=  ( two_ja + two_ma) / 2;int  
        jpmb=  ( two_jb + two_mb) / 2;int  
        jpmc=  ( two_jc + two_mc) / 2;int  
        jsum=  ( two_ja + two_jb + two_jc) / 2;int  
        kmin=  locMax3 (0, jpmb - jmmc, jmma - jpmc);int  
        kmax=  locMin3 (jcc, jmma, jpmb);int  
        k;int   sign=  GSL_IS_ODD (kmin - jpma + jmmb) ? -1 : 1;int  
        status=  0;
    MpIeee sum_pos=  MpIeee( "0.0" );MpIeee  sum_neg=  MpIeee( "0.0" );MpIeee  norm;MpIeee  term;
    ...
    norm = sqrt (bcn1.val * bcn2.val)
           / sqrt (bcd1.val * bcd2.val * bcd3.val * bcd4.val * ((MpIeee) two_jc + MpIeee( "1.0" )));
                     ...
                     gsl_sf_result * result)
{
  ...
  if(   two_ja < 0 || two_jb < 0 || two_jc < 0
    ...
    MpIeee norm;
    int  tk;int   tkmin;int   tkmax;
    MpIeee phase;
    MpIeee sum_pos=  MpIeee( "0.0" );
    MpIeee sum_neg=  MpIeee( "0.0" );
    MpIeee sumsq_err=  MpIeee( "0.0" );
    int  status=  0;
    ...
    phase = GSL_IS_ODD((two_ja + two_jb + two_je + two_jd + tkmin)/MpIeee( "2" ))
            ? -MpIeee( "1.0" )
            :  MpIeee( "1.0" );
                     ...
                     gsl_sf_result * result)
{
  ...
  if(   two_ja < 0 || two_jb < 0 || two_jc < 0
    ...
    for(tk=tkmin; tk<=tkmax; tk += 2) {
      ...
      if(term >= MpIeee( "0.0" )) {
        sum_pos += (tk + MpIeee( "1" )) * term;
      }
      else {
        sum_neg -= (tk + MpIeee( "1" )) * term;
      }

      sumsq_err += ((tk+MpIeee( "1" )) * term_err) * ((tk+MpIeee( "1" )) * term_err);
    }

    phase = GSL_IS_ODD(tkmin) ? -MpIeee( "1.0" ) : MpIeee( "1.0" );
...
MpIeee gsl_sf_coupling_3j(int  two_ja, int  two_jb, int  two_jc,
                          int  two_ma, int  two_mb, int  two_mc)
{
...
MpIeee gsl_sf_coupling_6j_INCORRECT(int  two_ja, int  two_jb, int  two_jc,
                                    int  two_jd, int  two_je, int  two_jf)
{
...
MpIeee gsl_sf_coupling_6j(int  two_ja, int  two_jb, int  two_jc,
                          int  two_jd, int  two_je, int  two_jf)
{
...
MpIeee gsl_sf_coupling_RacahW(int  two_ja, int  two_jb, int  two_jc,
                          int  two_jd, int  two_je, int  two_jf)
{
...
MpIeee gsl_sf_coupling_9j(int  two_ja, int  two_jb, int  two_jc,
                          int  two_jd, int  two_je, int  two_jf,
                          int  two_jg, int  two_jh, int  two_ji)
{
