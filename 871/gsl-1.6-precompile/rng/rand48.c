...
static MpIeee rand48_get_double(void *vstate);
...
typedef struct
  {
    unsigned short int  x0;int   x1;int   x2;
  }
rand48_state_t;
...
static MpIeee rand48_get_double(void * vstate)
{
  ...
  return (ldexp((MpIeee) state->x2, -MpIeee( "16" ))
          + ldexp((MpIeee) state->x1, -MpIeee( "32" )) 
          + ldexp((MpIeee) state->x0, -MpIeee( "48" ))) ;
}
