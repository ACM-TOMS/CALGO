                                    ...
                                    MpIeee fft[])
{
  ...
  for (j = 0; j < n; j++)
    {
      const MpIeee arg=  -2 * M_PI * ((const MpIeee) ((j * k) % n)) / ((const MpIeee) n);
                                       ...
                                       MpIeee fft[])
{
  ...
  REAL(fft,stride,0) = ((MpIeee) n) * z_real;
  IMAG(fft,stride,0) = ((MpIeee) n) * z_imag;
                                  ...
                                  MpIeee fft[])
{
...

  {
    ...
    REAL(fft,stride,freq) = ((MpIeee) n) * z_real;
    IMAG(fft,stride,freq) = ((MpIeee) n) * z_imag;
  }
                                      ...
                                      MpIeee fft[])
{
  ...
  for (j = 0; j < n; j++)
    {
      const MpIeee arg1=  2 * M_PI * ((const MpIeee) ((j * k1) % n)) / ((const MpIeee) n);
      const BASE w1_real = (BASE)cos (arg1);
      const BASE w1_imag = (BASE)sin (arg1);
      const MpIeee arg2=  2 * M_PI * ((const MpIeee) ((j * k2) % n)) / ((const MpIeee) n);
...

  {
    ...
    REAL(fft,stride,freq1) += ((MpIeee) n) * z1_real;
    IMAG(fft,stride,freq1) += ((MpIeee) n) * z1_imag;
    REAL(fft,stride,freq2) += ((MpIeee) n) * z2_real;
    IMAG(fft,stride,freq2) += ((MpIeee) n) * z2_imag;
  }
                                    ...
                                    MpIeee fft[])
{
  ...
  for (i = 0; i < n; i++)
    {
      REAL(data,stride,i) = (MpIeee)urand();
      IMAG(data,stride,i) = (MpIeee)urand();
    }
