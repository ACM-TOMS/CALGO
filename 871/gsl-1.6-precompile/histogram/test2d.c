...
{
  MpIeee xr[MR + 1] = 
    { MpIeee( "0.0" ), MpIeee( "1.0" ), MpIeee( "2.0" ), MpIeee( "3.0" ), MpIeee( "4.0" ), MpIeee( "5.0" ), MpIeee( "6.0" ), MpIeee( "7.0" ), MpIeee( "8.0" ), MpIeee( "9.0" ), MpIeee( "10.0" ) };

  MpIeee yr[NR + 1] =  { MpIeee( "90.0" ), MpIeee( "91.0" ), MpIeee( "92.0" ), MpIeee( "93.0" ), MpIeee( "94.0" ), MpIeee( "95.0" ) };
  ...
  for (i = 0; i < M; i++)
    {
      for (j = 0; j < N; j++)
        {
          k++;
          gsl_histogram2d_accumulate (h, (MpIeee) i, (MpIeee) j, (MpIeee) k);
        };
    }
    ...
    for (i = 0; i < M; i++)
      {
        for (j = 0; j < N; j++)
          {
            k++;
            if (gsl_histogram2d_get (h, i, j) != (MpIeee) k)
              status = 1;
          };
      }
    ...
    for (i = 0; i < M1; i++)
      {
        for (j = 0; j < N1; j++)
          {
            gsl_histogram2d_increment (h, (MpIeee) i, (MpIeee) j);
...

  {
    ...
    for (i = 0; i < M; i++)
      {
        MpIeee x0=  MpIeee( "0" );MpIeee  x1=  MpIeee( "0" );
        gsl_histogram2d_get_xrange (h, i, &x0, &x1);

        if (x0 != i || x1 != i + MpIeee( "1" ))
          {
            status = 1;
          }
      }
...

  {
    ...
    for (i = 0; i < N; i++)
      {
        MpIeee y0=  MpIeee( "0" );MpIeee  y1=  MpIeee( "0" );
        gsl_histogram2d_get_yrange (h, i, &y0, &y1);

        if (y0 != i || y1 != i + MpIeee( "1" ))
          {
            status = 1;
          }
      }
  ...
  {
    MpIeee max=  gsl_histogram2d_max_val (h);
    gsl_test (max != MpIeee( "123456.0" ), "gsl_histogram2d_max_val finds maximum value");
  }

  {
    MpIeee min=  gsl_histogram2d_min_val (h);
    gsl_test (min != -MpIeee( "654321.0" ),
  ...
  {
    MpIeee sum=  gsl_histogram2d_sum (h);
    gsl_test (sum != N * M * MpIeee( "27" ) + ((N * M - MpIeee( "1" )) * N * M) / MpIeee( "2" ),
              "gsl_histogram2d_sum sums all bin values");
  }

  {
    /* first test... */
    const MpIeee xpos=  0.6;
    const MpIeee ypos=  0.85;
    MpIeee xmean;
    MpIeee ymean;
    size_t xbin;
    size_t ybin;
    ...
    {
      MpIeee expected_xmean=  (h3->xrange[xbin] + h3->xrange[xbin + 1]) / MpIeee( "2.0" );
      MpIeee expected_ymean=  (h3->yrange[ybin] + h3->yrange[ybin + 1]) / MpIeee( "2.0" );
...

  {
    ...
    const MpIeee xmean=  0.7;
    const MpIeee ymean=  0.7;
    const MpIeee xsigma=  0.1;
    const MpIeee ysigma=  0.1;
    const MpIeee correl=  0.5;
    const MpIeee norm= 
    ...
    for (xbin = 0; xbin < M; xbin++)
      {
        MpIeee xi= 
          ((h3->xrange[xbin] + h3->xrange[xbin + 1]) / MpIeee( "2.0" ) - xmean) / xsigma;
        for (ybin = 0; ybin < N; ybin++)
          {
            MpIeee yi= 
              ((h3->yrange[ybin] + h3->yrange[ybin + 1]) / MpIeee( "2.0" ) -
               ymean) / ysigma;
            MpIeee prob= 
              norm * exp (-(xi * xi - MpIeee( "2.0" ) * correl * xi * yi + yi * yi) /
                          MpIeee( "2.0" ) / (MpIeee( "1" ) - correl * correl));
            h3->bin[xbin * N + ybin] = prob;
          }
      }
    {
      MpIeee xs=  gsl_histogram2d_xsigma (h3);
      MpIeee ys=  gsl_histogram2d_ysigma (h3);
    ...
    for (i = 0; i <= M1; i++)
      {
        if (h->xrange[i] != hh->xrange[i])
          {
            {cout<<"xrange["<<
                    (int)<<"] : "<<setiosflags((ios::floatfield))<< i;
cout.precision(6);cout.fill(' ');cout.width(0);cout.setf(ios::dec,ios::basefield);
cout<<resetiosflags((ios::floatfield))<<" orig vs "<<setiosflags((ios::floatfield))<< h->xrange[i];
cout.precision(6);cout.fill(' ');cout.width(0);cout.setf(ios::dec,ios::basefield);
    ...
    for (j = 0; j <= N1; j++)
      {
        if (h->yrange[j] != hh->yrange[j])
          {
            {cout<<"yrange["<<
                    (int)<<"] : "<<setiosflags((ios::floatfield))<< j;
cout.precision(6);cout.fill(' ');cout.width(0);cout.setf(ios::dec,ios::basefield);
cout<<resetiosflags((ios::floatfield))<<" orig vs "<<setiosflags((ios::floatfield))<< h->yrange[j];
cout.precision(6);cout.fill(' ');cout.width(0);cout.setf(ios::dec,ios::basefield);
cout<<resetiosflags((ios::floatfield))<<"\n";}
            status = 1;
          }
        ...
        if (h->bin[i] != hh->bin[i])
          {
            {cout<<"bin["<<
                    (int)<<"] : "<<setiosflags((ios::floatfield))<< i;
cout.precision(6);cout.fill(' ');cout.width(0);cout.setf(ios::dec,ios::basefield);
cout<<resetiosflags((ios::floatfield))<<" orig vs "<<setiosflags((ios::floatfield))<< h->bin[i];
cout.precision(6);cout.fill(' ');cout.width(0);cout.setf(ios::dec,ios::basefield);
cout<<resetiosflags((ios::floatfield))<<"\n";}
            status = 1;
          }
