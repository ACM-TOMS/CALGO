...
static int  find(const size_t n, const MpIeee range[], 
                 const MpIeee x, size_t * i);

static int
 find(const size_t n, const MpIeee range[], const MpIeee x, size_t * i)
{
...
#ifdef LINEAR_OPT
  {
    MpIeee u=   (x - range[0]) / (range[n] - range[0]);
    i_linear = (size_t) (u * n).toUnsignedInt();
  }
