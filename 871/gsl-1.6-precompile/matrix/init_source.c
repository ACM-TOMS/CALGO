  ...
  for (i = 0; i < p; i++)
    {
      for (j = 0; j < q; j++)
        {
          *(MpIeee*) (data + MULTIPLICITY * (i * tda + j)) = ((i == j) ? one : zero);
        }
    }
}
...
//  cout << "in set_zero function MULTIPLICITY=" << MULTIPLICITY << endl;
//  cout << "in set_zero function tda=" << tda << endl;
  ...
  for (i = 0; i < p; i++)
    {
      for (j = 0; j < q; j++)
        {
          *(MpIeee*) (data + MULTIPLICITY * (i * tda + j)) = MpIeee("0"); //was ZERO
        }
    }
}

void
FUNCTION (gsl_matrix, set_all) (TYPE (gsl_matrix) * m, MpIeee x)
{
  size_t i, j;
  ATOMIC * const data = m->data;
  const size_t p = m->size1 ;
  ...
  for (i = 0; i < p; i++)
    {
      for (j = 0; j < q; j++)
        {
          *(MpIeee*) (data + MULTIPLICITY * (i * tda + j)) = x;
        }
    }
}
