...
*
* D1MACH (FOX P.A., HALL A.D., SCHRYER N.L., FRAMEWORK FOR A PORTABLE
...
*
      INTEGER          N,MSG,IPR
      DOUBLE PRECISION X(N),F,G(N),TYPX(N),RNF,ANALTL,WRK1(N)
*
*  LOCAL VARIABLES
*
      INTEGER          KER,I
      DOUBLE PRECISION GS,ONE
      EXTERNAL         FCN
      INTRINSIC        ABS,MAX
      DATA             ONE/1.0D0/
*
...
*
      ...
      DOUBLE PRECISION X(N),TYPX(N),F,G(N),RNF,ANALTL,FHESD(N)
      DOUBLE PRECISION XD(N),ETA(N),HESS(LICN)
      LOGICAL          SCALE,FCALL,CHECKH
*
*  LOCAL VARIABLES
*
      INTEGER          IST,IEND,KER,I,IR,J,JP,K,MAXGRP,MINGRP
      DOUBLE PRECISION HS,ONE
      EXTERNAL         FCN,UGR
...
*
      INTEGER          I,IR,J,JP,K
      DOUBLE PRECISION STPSIZ,TEMP,EPS,ZERO,ONE,THREE,TEN,THOUS
      INTRINSIC        LOG10,MAX,MIN,SQRT
      DATA             ZERO,ONE,THREE,TEN,THOUS/0.0D+0,1.0D+0,3.0D+0,
     +                 10.0D+0,1000.0D+0/
*
...
*
      ...
      DOUBLE PRECISION XPLS(N),FPLS,GPLS(N),X(N),GRADTL,STEPTL
      DOUBLE PRECISION FSCALE,RGX
*
*  LOCAL VARIABLES
*
      INTEGER          I
      DOUBLE PRECISION D,RELGRD,RSX,RELSTP,ZERO,ONE
      LOGICAL          MXTAKE
      INTRINSIC        ABS,MAX
...
*
      INTEGER          NZEROS
      DOUBLE PRECISION A1,A2,A3,XP(3)
*
*  LOCAL VARIABLES
*
      DOUBLE PRECISION CST1,CST2,Q,R,D,S,T,THETA,PI,CONST
      DOUBLE PRECISION ZERO,HALF,ONE,TWO,THREE,FOUR,NINE
      DOUBLE PRECISION TSEVEN,FFOUR
      INTRINSIC        ACOS,COS,SQRT
      PARAMETER        (PI=3.141592653589793D+0)
      DATA             ZERO,HALF,ONE,TWO,THREE,FOUR,NINE,TSEVEN,
     +                 FFOUR/0.0D+0,5.0D-1,1.0D+0,2.0D+0,3.0D+0,4.0D+0,
     +                 9.0D+0,27.0D+0,54.0D+0/
*
      NZEROS = 0
      CST1 = ONE/THREE
      CST2 = CST1*A1
...
*
      ...
      DOUBLE PRECISION TYPX(N),FSCALE,GRADTL,STEPTL,STEPMX
*
*  LOCAL VARIABLES
*
      INTEGER          I
      DOUBLE PRECISION EPS,TEMP,ZERO,ONE,THREE
      DOUBLE PRECISION D1MACH
      INTRINSIC        LOG10
      DATA             ZERO,ONE,THREE/0.0D+0,1.0D+0,3.0D+0/
*
      EPS = D1MACH(3)
      METHOD = 1
      FSCALE = ONE
...
*
      ...
      DOUBLE PRECISION X(N),TYPX(N),FSCALE,GRADTL,STEPTL,STEPMX
      DOUBLE PRECISION XPLS(N),FPLS,GPLS(N),HESS(LICN),G(N)
      DOUBLE PRECISION S(N),D(N),DN(N),E(N),XD(N),BV(N),VECTOR(N)
      LOGICAL          SCALE
*
*  LOCAL VARIABLES
*
      ...
      DOUBLE PRECISION RNF,ANALTL,F,GNORM,FP,SUM1,SUM2
      DOUBLE PRECISION ALMBDA,OCENT,ONE,TWO,THREE,TEN
      DOUBLE PRECISION EPS,GAMMA,ALPHA,BETA,FN,RGX
      DOUBLE PRECISION DDOT,DNRM2,D1MACH
      LOGICAL          MXTAKE,DSCENT,FCALL,CHECKH
      COMMON           /ALMB/ALMBDA
      COMMON           /COUNT/IFCNT,IGCNT,IHCNT
      EXTERNAL         FCN,UGR,USH
      EXTERNAL         DCOPY,DNRM2,DDOT
      DATA             OCENT,ONE,TWO,THREE,TEN/1.0D-2,1.0D+0,2.0D+0,
     +                 3.0D+0,10.0D+0/
*
...
*
      SUM1 = DDOT(N,S(1),1,S(1),1)
      SUM2 = DDOT(N,S(1),1,D(1),1)
*
      DO 180 I = 1,N
...
*
      INTEGER          N
      DOUBLE PRECISION XPLS(N),FPLS,RNOISE,FDG(N)
*
*  LOCAL VARIABLES
*
      INTEGER          J
      DOUBLE PRECISION XTMPJ,STEPSZ,FHAT,ONE
      EXTERNAL         FCN
...
*
      ...
      INTEGER          JPNTR(N+1),IWA(LIWA)
      DOUBLE PRECISION X(N),G(N),TYPX(N),RNF,FHESD(N)
      DOUBLE PRECISION XD(N),ETA(N),HESS(NZ)
      LOGICAL          SCALE
*
*  LOCAL VARIABLES
*
      INTEGER          JP,I,INFO,J,METHOD,NUMGRP
      DOUBLE PRECISION FD,ONE
      LOGICAL          FCALL
      EXTERNAL         FCN,UGR
      INTRINSIC        ABS,MAX,SQRT
      DATA             ONE/1.0D+0/
...
*
      INTEGER          N
      DOUBLE PRECISION F,FP,G(N),GP(N),S(N),ALPHA,BETA,SH(N),A(N)
*
*  LOCAL VARIABLES
*
      INTEGER          I
      DOUBLE PRECISION GS,GPS,SHS,B1,B2,STS
      DOUBLE PRECISION HALF,TWO,SIX,TFOUR,STWO
      DOUBLE PRECISION DDOT
      EXTERNAL         DDOT
      DATA             HALF,TWO,SIX,TFOUR,STWO/5.0D-1,2.0D+0,6.0D+0,
     +                 24.0D+0,72.0D+0/
*
...
*
      ...
      DOUBLE PRECISION HESS(NZ),S(N),SH(N)
*
*  LOCAL VARIABLES
*
      INTEGER          I,II,JJ
...
*
      INTEGER          N,IRETCD
      DOUBLE PRECISION X(N),F,G(N),P(N),XPLS(N),FPLS,STEPMX
      DOUBLE PRECISION STEPTL,TYPX(N),W2(N)
      LOGICAL          SCALE
*
*  LOCAL VARIABLES
*
      ...
      DOUBLE PRECISION TMP,SLN,SCL,SLP,RLN
      DOUBLE PRECISION TEMP,TEMP1,TEMP2,ALMBMN,ALMBDA
      DOUBLE PRECISION TLMBDA,T1,T2,T3,PLMBDA
      DOUBLE PRECISION A,B,DISC,PFPLS,ZERO,TENTH
      DOUBLE PRECISION HALF,Z99,ONE,TWO,THREE,ALPHA
      DOUBLE PRECISION DDOT
      LOGICAL          MXTAKE
      COMMON           /COUNT/IFCNT,IGCNT,IHCNT
      COMMON           /ALMB/ALMBDA
      ...
      INTRINSIC        ABS,MAX,SQRT
      DATA             ZERO,TENTH,HALF,Z99,ONE,TWO,THREE,ALPHA/0.0D+0,
     +                 1.0D-1,5.0D-1,0.99D+0,1.0D+0,2.0D+0,3.0D+0,
     +                 1.0D-4/
*
      MXTAKE = .FALSE.
      IRETCD = 2
...
*
      ...
      DOUBLE PRECISION A(MAXA),RHS(N),W(N),EPS
      LOGICAL          NEWTON
*
*  LOCAL VARIABLES
*
      INTEGER          NEG1,NEG2,LDIAG,LP,MP,ISTIW,LIW,NSTEPS,MAXFRT
      DOUBLE PRECISION U,TENTH
...
*
      CALL MA27AD(N,NZ,IRN,ICN,IW(ISTIW),LIW,IW,IW(3*N+1),NSTEPS,IFLAG)
*
*  FACTORIZE MATRIX
*
      CALL MA27BD(N,NZ,IRN,ICN,A,MAXA,IW(ISTIW),LIW,IW,NSTEPS,MAXFRT,
     +            IW(3*N+1),IFLAG)
*
...
* SOLVE THE EQUATIONS
*
      CALL MA27CD(N,A,MAXA,IW(ISTIW),LIW,W,MAXFRT,RHS,IW(3*N+1),NSTEPS)
   30 RETURN
...
*
      ...
      DOUBLE PRECISION DT(N),S(N),G(N),W1(N),W2(N),W3(N)
      DOUBLE PRECISION BV(N),GAMMA,HESS(LICN),WK(N),EPS
      LOGICAL          DSCENT
*
*  LOCAL VARIABLES
*
      INTEGER          NZEROS,I
      DOUBLE PRECISION BETA(3),THETA(3),UU,WW,VV,YY,ZZ
      DOUBLE PRECISION AA,A1,A2,A3,AMINB,AMINT,GD
      DOUBLE PRECISION DDOT,ZERO,HALF,ONE,OHALF,TWO,SIX
      DATA             ZERO,HALF,ONE,OHALF,TWO,SIX/0.0D0,0.5D+0,1.0D+0,
     +                 1.5D+0,2.0D+0,6.0D+0/
*
      DSCENT = .TRUE.
...
*
      UU = DDOT(N,S(1),1,W1(1),1)
      WW = DDOT(N,S(1),1,W2(1),1)
      VV = DDOT(N,S(1),1,W3(1),1)
      YY = DDOT(N,BV(1),1,W1(1),1)
      ZZ = DDOT(N,BV(1),1,W3(1),1)
*
...
*
      ...
      DOUBLE PRECISION XPLS(N),FVAL,GPLS(N),RGX
*
*  LOCAL VARIABLES
*
      INTEGER          I,IFCNT,IGCNT,IHCNT
      DOUBLE PRECISION D,RELGRD,ZERO,ONE
      COMMON           /COUNT/IFCNT,IGCNT,IHCNT
...
*
      ...
      SUBROUTINE STUMCD(N,X,NPAIRS,IRN,LIRN,ICN,LICN,FCN,UGR,USH,TYPX,
     +                  FSCALE,GRADTL,STEPTL,ILIM,STEPMX,IPR,METHOD,
...
*
      ...
      DOUBLE PRECISION X(N),TYPX(N),FSCALE,GRADTL,STEPTL,STEPMX
      DOUBLE PRECISION XPLS(N),FPLS,GPLS(N),HESS(LICN)
...
*
     ...
     +            IWRK(4*N+3),LIWRK,LWRK,TERMCD,VECTOR,INFORM)
      ...
      SUBROUTINE STUMSD(N,X,NPAIRS,IRN,LIRN,ICN,LICN,FCN,TYPX,MSG,XPLS,
...
*
      ...
      INTEGER          IRN(LIRN),ICN(LICN),IWRK(LIWRK)
      DOUBLE PRECISION X(N),TYPX(N),XPLS(N),FPLS,GPLS(N)
      DOUBLE PRECISION HESS(LICN),WRK(LWRK)
*
*  LOCAL VARIABLES
*
...
*
      ...
      DOUBLE PRECISION ZERO,ONE,TWO,ALPHA,BETA,GAMMA,TAU,T,C,S,E1,E2
      LOGICAL          SINGLE
      INTRINSIC        ABS,SQRT
*
*  SET DATA.
